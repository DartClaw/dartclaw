...
- **Subscription credential ingestion** – `dartclaw auth claude` stores a `claude setup-token` (read from stdin only, never an argument) in DartClaw's own credential store, and `dartclaw auth codex` runs `codex login` against DartClaw's dedicated `CODEX_HOME`. Both stores live under `<data_dir>/credentials/` with owner-only permissions and are refused if they resolve onto your personal `~/.claude` or `~/.codex` login.
- **`providers.<id>.auth`** – selects the credential to present per provider: `auto` (default; subscription when stored, otherwise API key), `subscription`, or `api_key`. A forced value never silently falls back to the other kind – the execution is refused at admission with the command that fixes it. An alias inherits its family's setting unless it sets its own.
- **Credential health visibility** – per-provider credential mode, best-effort renewal countdown, health state, last-checked time, and remediation command on `/settings` and `GET /api/providers`, probed hourly and at startup. Degradation raises a routed alert and is logged to `serve` stderr independently of alert routing, so turning alerts off does not silence it.

### Changed

- **Provider authentication defaults to the operator's subscription (breaking)** – when a subscription credential is stored, both Claude and Codex now present it on **both** execution boundaries, replacing the 0.24 API-key-only posture. Containerized Claude is mediated by a host-injected `Authorization: Bearer` under the `anthropic-beta: oauth-2025-04-20` beta, and containerized Codex is re-pinned to the ChatGPT backend – so an OAuth/setup-token-only host is no longer refused for container execution. The container boundary is unchanged: still `network:none`, still no credential in container environment, filesystem, arguments, or generated configuration. A subscription Bearer authenticates as your whole account, is long-lived, and is harder to revoke than a scoped API key, and in host mode it is readable by the agent's own subprocess – an API key remains the recommended choice for host-mode deployments running less-trusted agents, and the ToS-conservative choice for container mode. **Migration:** deployments with only an API key configured keep working unchanged under the default `auto` resolution and need no action. To adopt subscription auth, run `claude setup-token` followed by `dartclaw auth claude`, and `dartclaw auth codex` for Codex. To stay on keys where a subscription credential is also present, set `providers.<id>.auth: api_key`. See [Security § Authentication Modes](docs/guide/security.md#authentication-modes).
  - *Note, container-mode Claude on a subscription:* the raw-`Authorization: Bearer` `setup-token` path is verified against a real token — Anthropic accepted a stored `setup-token` presented as a raw Bearer under the `oauth-2025-04-20` beta at the pre-ship gate. There is still no runtime `x-api-key` fallback – the Anthropic adapter selects its header from the resolved credential mode alone, so an upstream refusal fails the turn rather than silently degrading to a key. Host-mode Claude is unaffected.
- **ACP agents are credential-isolated (breaking)** – a `harness.acp.agents.<id>` registration no longer inherits any DartClaw-managed provider credential. The implicit `model_provider` mapping (`anthropic` → `providers.claude` + `credentials.anthropic`, `openai` → `providers.codex` + `credentials.openai`) is removed: `model_provider` now selects validation and routing only. No subscription token is ever presented to an ACP agent – it is a third-party client, and a subscription credential authenticates as your whole account – and no dedicated `CODEX_HOME` is prepared for one. An ACP agent is still never refused on credential grounds. **Migration:** to keep presenting a key, add `harness.acp.agents.<id>.credential: <credentials name>`, referencing an API-key entry sourced from a `${VAR}` reference (the secret is injected under that variable name); otherwise configure the agent's own authentication (its config file, keyring, or login), as with other ACP hosts. See [Configuration § Full Config Reference](docs/guide/configuration.md#full-config-reference) and [Security § Authentication Modes](docs/guide/security.md#authentication-modes).
- **Provider `health` folds in credential health (API)** – `GET /api/providers` reports `"health": "degraded"` (and counts it as degraded in the summary) for a provider whose credential needs operator attention, even when its binary was found and its credential is present; the `/settings` card badge follows. Credential presence is not credential validity, so a provider past its renewal deadline previously reported `healthy` in the same object that reported `reauth-required`. `credentialStatus` is unchanged – it still reports what the deployment holds. Consumers gating on `health == 'healthy'` will see providers flip to `degraded` where they previously did not.
- **`credentialStatus` follows the selected credential (API/UI)** – `GET /api/providers` and the `/settings` card report `missing` (and `degraded`) for a provider whose `providers.<id>.auth` selection cannot be presented – e.g. `subscription` forced with only an API key configured – matching what execution admission refuses; the error line is the same remediation the startup gate prints and names the store it searched. The vendor-login probe (`oauth`) runs only when nothing is selected. A subscription card shows `Stored subscription credential` as its source instead of an API-key variable name.
- **`dartclaw init` verification recognises stored subscription credentials** – a subscription-only instance now verifies without a vendor login probe; a forced-but-absent selection stays `configured but unverified`.
- **`dartclaw auth claude` masks typed input** – the prompt shows one `*` per character instead of nothing at all, and interrupting it with Ctrl-C restores the terminal and exits `130` without storing anything.
- **`dartclaw auth codex`** honours `providers.codex.executable`, and a re-run that leaves an already-current credential in place reports it and exits `0` (previously exit `1`).
- **Store collision error names `data_dir`** – the refusal for a dedicated store resolving onto a vendor login store now names the operator-settable `data_dir` / `--data-dir` rather than the internal `server.data_dir` path. A Claude token whose recorded issue time is unreadable now reads as a stored credential with an unknown renewal deadline (`unknown` health) rather than as absent.

### Fixed

- **Content classification can no longer be skipped by a marker in the content** – any fetched page containing `ray id:`, `just a moment`, or another Cloudflare-challenge marker short-circuited `ContentGuard` to *pass*, so a prompt-injection payload carrying one of those strings reached the agent unclassified. `CloudflareDetector` is removed; a real challenge page classifies as `safe` on its own.
- **The default content classifier no longer forces fail-open silently** – selecting `claude_binary` (the default) set `failOpen: true` with no config key and no log line, so content the classifier could not score was passed through. Fail behavior is now the explicit `guards.content.fail_open` (default `false`, matching `ContentGuard`'s own default), and enabling it logs a warning at startup. **Migration:** deployments that relied on the implicit pass-through must set `guards.content.fail_open: true`; everyone else now fails closed as documented.
- **A long assistant reply is delivered and stored verbatim** – every completed turn ran its own response text through `ExplorationSummarizer`, which is built for large *tool output*: a JSON-ish answer over ~25K tokens came back to the user as `[Exploration summary — JSON, …]` and anything over `context.max_result_bytes` as a 2KB head plus 2KB tail. The reply is now persisted and returned unchanged; the daily-log copy keeps its byte cap.
- **An unclaimed `*_source` output no longer asserts a provenance** – a blank or missing `*_source` was back-filled with the literal `synthesized`, so a step that never claimed provenance still satisfied `entryGate: "<x>_source == synthesized"` and, in `spec-and-implement`, re-authored a specification that should have been reused. The value now stays blank, like any other unclaimed output.
- **Config hot-reload reaches alert watchers** – `ConfigNotifier.reload()` never diffed the `alerts` section, so the three `Reconfigurable`s watching `alerts.*` were never notified and an alert-config change took effect only on restart. (The other undiffed sections — `harness`, `knowledge`, `workflow`, `mcpServers` — are restart-tier with no watchers and stay undiffed.)
- **`container.mounts` is the mount key, and an unknown `container.*` key is reported** – `examples/production.yaml` and the configuration guide both documented `container.mount_allowlist`, which the parser never read, so the shipped example's allowlist was a no-op. Both now use `mounts`, and `ContainerConfig.fromYaml` warns on any key it does not recognize instead of ignoring it.
- **Task-accept git runs with `GIT_CONFIG_NOSYSTEM=1`** – accepting a task stages and commits in the task worktree, but unlike `WorktreeManager` it did not neutralize system-level git config, so system hooks, filters, and `core.sshCommand` ran in-band for that child process.
- **Memory status reads a canonical `MEMORY.md`** – `/api/memory` status parsed `MEMORY.md` and `MEMORY.archive.md` with the legacy line parser unconditionally. A canonical document has no lines it matches, so a populated corpus reported `entryCount: 0` with no categories and stamped it `coverage: exact`. Both now branch on the canonical header, as the daily-log reader already did.

### Security

- **Workflow one-shot steps are documented as outside the guard chain** – a workflow step spawns the provider directly rather than through the long-lived harness, and that path has no hook channel, so `CommandGuard`, `FileGuard`, `NetworkGuard`, `ContentGuard`, and the guard audit log never evaluate it; a step declaring no `allowedTools` outside a container is bounded only by the provider's defaults. No behavior change in this release: the container-disabled startup warning now says so instead of claiming guards are the boundary, and [Security § Guard System](docs/guide/security.md#guard-system) documents the exclusion and the safe configuration. Closing it needs the one-shot and harness execution stacks merged — tracked as TD-122.
...
- **Configurable knowledge-inbox extraction effort** – `knowledge.inbox.effort` (default `medium`, replacing a hardcoded `low`) sets the reasoning effort of the extraction turn. Effort buys deliberation, not output length: the synthesis still has to land in one turn, so a source too large for that has to be split rather than pushed harder. Enabling the inbox on an existing deployment now costs more per file unless `effort: low` is set explicitly.
- **Objective coverage reporting** – every run reports the source's UTF-8 size against the stored wiki page's (`coverage: lossy.md 14.2KB->3.1KB (22%)`) alongside the extraction's own `declared drops`, which is the same comparison an operator makes by hand between `processed/` and `wiki/`. The declaration is the model's account of itself and an empty one is the absence of a claim, not evidence of a complete transfer; the ratio is the half no prompt can talk around.
- **`consolidation-debt` wiki-lint category** – a page carrying ten or more supplement sections is reported as a level rather than as the same merge line every run.
- **Run summaries are logged** – built-in callback jobs (knowledge inbox, wiki lint, memory pruner, session maintenance, auto-task) write their result to the server log before delivery, so a run survives `delivery_mode: none`, an unattended `announce`, and the 30-day `processed/` purge. Prompt-job responses are not logged.

### Changed

- **Wiki frontmatter is parsed as YAML, and an unreadable page is refused rather than rewritten** – flow-style and single-scalar `sources`, single-quoted items, and CRLF pages are read correctly instead of being silently reinterpreted as body text, which destroyed them. A page with an unterminated or unparseable frontmatter block, a leading `---` block whose content is not a YAML mapping, or bytes that are not valid UTF-8 now throws `WikiPageUnreadable` and quarantines the inbox source with an error naming the page.
- **A page the pipeline cannot classify is never promoted** – `hybrid` is recorded only when supplementing a page whose stored provenance this pipeline recognises. A page whose frontmatter records no provenance keeps none, and wiki lint reports it; previously any unrecognised or absent value became `hybrid`, which is the value search ranks as trusted. Frontmatter this pipeline does not recognise (an Obsidian or Jekyll page, say) is merged normally with its keys preserved rather than refused.
- **Stored frontmatter values are escaped on write** – a preserved value or key is emitted plain only when it is a letter-led bare word, and quoted otherwise. Emitted raw, a newline split the block and demoted `sources` into the body, a trailing colon made the page unparseable to its own reader, and a value spelled `null` or `0x1F` came back as a different type.
- **Merge semantics cover the whole page, not two fields** – frontmatter keys the store does not own (`contradicts`, `related`, and anything hand-added) are carried through; `confidence` becomes the weaker of the stored and incoming values, because a page mixing a curated section with a machine supplement is only as strong as its weakest part; a stored confidence outside that vocabulary is written back untouched rather than replaced, the same rule as provenance. Wiki lint reports a provenance or confidence it does not recognise.
- **A zero-contribution write moves no authorship field** – when the page already carries the incoming body, the run reports `(refresh, no new content)` and no content, provenance, confidence, or `last_updated` changes. A source that is new to the page is still added to `sources`, because its knowledge is demonstrably on the page; a repeat of a source already recorded writes nothing at all. The run report distinguishes a supplement (with its count) from a refresh.
- **Only the extraction is retried** – it is the nondeterministic step and it writes nothing. Everything after it is durable, and replaying it is what left duplicate memory findings, duplicate KG facts, and a second supplement section for one source. A durable failure now quarantines the file instead of replaying the writes, and a source that succeeds but cannot be moved to `processed/` is quarantined too – reported once, as a quarantine, so the count matches what is on disk and the next run cannot ingest it again.
- **Run-report API** – `KnowledgeInboxRunReport.coverageGaps` is replaced by `coverage` (one entry per processed file) plus `declaredGaps`; the summary token `coverage-gaps=` is now `declared-gaps=`. `WikiPageWrite.merged` is replaced by `WikiPageOutcome`. `WikiPageStore.lint` moved to the top-level `lintWikiPages`.

### Fixed

- **Knowledge-inbox wiki writes no longer overwrite an existing page** – a slug collision keeps the stored content, appends the new synthesis as a dated supplement section, and unions the frontmatter `sources` so the prior provenance chain survives.
- **Wiki page writes are atomic** – the merge made the stored page the only copy of every prior batch, and a truncating write could destroy it on a crash, a full disk, or a sync-volume I/O error. Writes now stage and rename through the shared `secureWriteFile` seam while keeping the page operator-editable.
- **Extraction is fully validated before any durable write** – the model-controlled wiki confidence is checked up front instead of after the memory findings were already stored.
- **Declared drops are bounded before delivery** – the extraction's dropped-topic list is model-authored text derived from an untrusted source that reaches channel DMs and the SSE stream. It is now capped in count and length and stripped of line breaks at the parse boundary, as are the model-chosen entity, predicate, and value strings in a contradiction detail – in the wiki-lint report as well as the ingestion run's. The run summary additionally collapses every detail line it assembles, so text the pipeline does not author – a provider's turn-failure message, a filename, an OS error string – is reported in full but cannot open a line that reads as a category the run never reported.
- **The search layer reads the same page shapes the writer does** – a CRLF page and a flow-style or single-scalar `sources` list are ranked on their frontmatter instead of being treated as having none, which had demoted exactly the pages the parser fix taught the writer to read.
- **Wiki search snippets no longer bury appended supplements** – the snippet anchored one window at the first match, so on a page grown by supplement sections the oldest section won every query and newly appended knowledge was invisible from search. When the last match falls outside that window, the snippet now splits its budget between a window at the first and a window at the last match.
- **An oversized `memory_read` keeps the newest appended content** – a native read over the response ceiling was cut head-only, which dropped exactly the newest supplement sections of a grown wiki page. It now keeps the head and the tail with a `...[truncated N bytes]...` marker between them.
- **Wiki lint no longer probes outside the wiki directory** – a model-authored link such as `](../../../../etc/hosts.md)` is reported as missing without a filesystem check, so the lint report cannot answer existence questions about the host. Lint also flags a `## Supplement from <source>` heading whose source is absent from the page's recorded `sources`, which is what a forged heading in model-authored body text looks like.
- **Wiki lint reports orphan pages, not leaf pages** – `orphan` read each page's *outbound* links, which is the opposite of what the category claims and of what the docs described. Every page the knowledge inbox writes is a leaf with no links of its own, so a synthesized wiki reported every page it held, every run, and the category carried no signal. It now names pages no other page links to; a page's link to itself does not count.
- **Wiki lint sees a link that carries a section anchor** – `](page.md#section)` matched no link pattern at all, so its target was called unreachable and a break in it was never reported. The path and an optional `#fragment` / `?query` are now parsed separately. Lint also counts supplement sections over the page body rather than the whole file, so its count is the same number the run report carries as `(supplement N)`.
- **Same-session turns are admitted in arrival order** – two turn requests for one session issued back to back could execute in the wrong order (measured 3–20%): the reservation path reads the session file before taking the session lock, and the IO pool does not preserve completion order, so whichever read finished first won the lock. Any two concurrent `reserveTurn`/`startTurn` calls on one session were exposed – an API send racing a scheduled or agent-issued turn, parallel `sessions_send` calls – while channel messages were already serialized upstream by the per-session message queue. `TurnManager` now runs same-session reservations one at a time, in the order they reach `reserveTurn`, at the funnel every `TurnManager` caller goes through; work a caller does before that call is not ordered here, and the session lock still orders only callers that have already reached it.
...
- **Containerized executions lose provider-native web access** – no container keeps the provider's own web search and fetch tools, on any profile including the default `workspace` primary. Native web executes provider-side and cannot be contained under `network:none`, so it is denied uniformly; research turns reach the web only through the guarded, execution-scoped MCP bridge, and the container's content and network guards actually apply to what comes back. Host-mode executions are unaffected.
...
- **ACP security classification renamed to `host_only` (breaking, API)** – the `securityClassification` value reported for a non-guard-mediated ACP target in `GET /api/providers` is now `host_only` instead of `container_isolation_only`, and the public `AcpSecurityClassification` enum value renames to match. DartClaw mediates no container isolation for ACP, so the prior name claimed a boundary that never applied; `guard_mediated` is unchanged.
...
- **Codex app-server approvals use the current wire protocol** – command, file-change, permission, and MCP elicitation requests receive schema-native responses; file batches are evaluated per operation, request IDs retain their JSON type, relative shell paths use the requested working directory, unselected policy-amendment suggestions no longer block one-shot approvals, and unsupported authority fails closed without hanging the turn.
...
- **A malformed configuration file no longer boots with defaults** – a whole-document YAML parse error in `dartclaw.yaml` is now startup-fatal instead of a warning plus a silent fall-back to default (unisolated) configuration. Live reload of a malformed edit still keeps the active configuration.
- **Containerized Claude spawns disable the subprocess env-scrub** – container spawns set `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=0`: the scrub protects host credentials that never enter the container, and the current Claude CLI reads `=1` as requiring host-sandbox tooling (bubblewrap) that cannot run under the container hardening. Host spawns keep the scrub.
- **Containerized long-lived Claude workers can start** – the harness spawned `docker exec` with the untranslated host working directory, so `start()` before any turn died with exit 127 (surfaced only as an initialize timeout). The spawn now resolves the working directory into the container, falling back to the profile's own directory when nothing is mounted. Found by the new mediated-turn conformance suite.
- **Containerized Codex executes tools again** – both Codex lanes (one-shot `exec` and long-lived `app-server`) now run `danger-full-access` inside containers, as the sandbox-interaction matrix already prescribed: the container is the isolation boundary, and Codex's own sandbox tooling (bubblewrap) cannot start under the hardening – every tool call panicked while the turn still reported success. Read-only one-shot steps keep their `read-only` sandbox (failing closed, never open); host-mode Codex keeps its own sandbox. Found by the new mediated-turn conformance suite.
...
- **Windows release builds generate embedded assets on fresh checkouts** – the release workflow now emits the ignored generated Dart libraries before platform compilation, matching local and CI builds.
...
- **Workflow one-shot timeout backstop defaults** – one-shot CLI workflow steps now have an active default stall guard (`governance.turn_progress.stall_timeout: 300s`, `stall_action: cancel`) plus a default wall-clock ceiling (`max_duration: 1800s`). Per-step `timeout_seconds` still wins over `stepDefaults.timeout_seconds`, which wins over the global default. Finished provider turns whose process remains open are grace-killed and still succeed from the parsed terminal result.
- **Workflow CLI live progress spinner** – on an interactive terminal, `dartclaw workflow run` (and standalone `resume`/`retry`) now render a Docker-style animated spinner for the running step(s) showing live elapsed time and ticking per-step token counts, collapsing to an aggregate `N steps running … total` line during parallel/map fan-out, in color. Completed/failed/blocked steps print above the spinner. Live token ticks come from `WorkflowCliTurnProgressEvent` — consumed in-process for standalone runs and forwarded over the per-run SSE stream (`workflow_cli_turn_progress`, scoped via the event's task→run resolution) for connected runs. Non-interactive (piped/CI) and `--json` output is unchanged (append-only, no ANSI). Token emission is currently codex-only; claude parity is specced separately.
- **Workflow runtime-artifacts retention (opt-in)** – a configurable `prune_after_days` setting (default disabled) drives `dartclaw cleanup` to remove — or, in dry-run/warn mode, report — expired per-run `runtime-artifacts/` directories, surfacing applied-delete counts and bytes reclaimed.
- **Workflow run observability and blocked-outcome semantics** – standalone workflow runs now render the failure/blocked reason inline and print a per-story settle-time digest (human + `--json`); completion events carry `outcome`/`reason` (and a map-step blocked count). A foreach child that needs input is treated as a recoverable *blocked* item — the run pauses only when a still-open dependent story depends on it, otherwise it advances — instead of collapsing into an indistinguishable hard failure. The new fields are also forwarded over the server SSE stream for web / connected-mode consumers.
- **Claude provider `approval` / `sandbox` configuration (Codex parity)** – the Claude provider gains two orthogonal config axes — `approval` (permission prompting) and `sandbox` (OS isolation) — mirroring the Codex provider. Full access (`approval: never`) is an explicit opt-in, default OFF; on the host / non-container path it is honored but emits a loud startup warning that it disables all tool gating and grants full filesystem access.
- **`onMaxIterations` loop policy** – workflow loops accept an opt-in `onMaxIterations: continue` so an exhausted remediation loop falls through to the workflow's deterministic verify gate instead of failing the run; the default (`fail`) and nested-loop behavior are unchanged.
- **Workflow UX polish: validate-time skill check, variable-error parity, why-paused parity** – four small operability fixes reusing existing data and surfaces: `dartclaw workflow validate --skills` opt-in-probes each step's provider for its referenced skill and warns on unresolvable refs (probe failures degrade to a note, never a hard failure); the connected CLI `workflow status` table and the web run-detail page now state *why* a run is paused (pending approval step + clearing action / needs-input reason), reaching parity with the standalone status synthesis; and the human `workflow list` output names each workflow's required variables. No new run statuses, API endpoints, or YAML fields.

### Changed

- **Uniform unknown-field validation for workflow YAML** – any unknown field on the workflow root, steps, inline loops, foreach controllers, output configs, `variables` entries, `stepDefaults`, and `gitStrategy` sub-blocks now fails at parse time with a `FormatException` naming the field and block, replacing the retired-name tombstones and silent typo-acceptance. The legacy top-level `loops:` authoring form is dropped. **Breaking (custom workflows only):** a custom YAML carrying a previously-ignored unknown field now fails to load — the error names the field to remove.
- **Workflow authoring plumbing inferred** – `format:` is inferred from `schema:` presets and `resolver: filesystem` from `pathPattern:` + `format: path`; integer presets no longer require `format: json`; explicit author values still win. `gate:` and `entryGate:` share one grammar, with `gate:` retired from the documented surface (still parser-known). The workflow guide is split into a task-oriented authoring guide plus a reference.
- **Workflow registry tiers consolidated** – workflow sources collapse to built-in + one custom dir (`<dataDir>/workflows/custom/`) + project (`<projectDir>/workflows/`). The legacy drop-folder directly under `<dataDir>/workflows/` still loads for one release with a deprecation warning naming the new location.
- **Connected and standalone git promotion/publish reconciled** – both paths now share the standalone core safety semantics (story-branch change commit, dirty integration-worktree sweep, existing-or-temporary integration worktree for promotion, pending-branch commit before publish push), while connected mode keeps project-auth push, PR creation, notes, and artifact persistence. The whole promote + resolver span runs under one per-project transaction lock on both the CLI and server paths, closing a `maxParallel>1` spurious-promote-conflict robustness gap.
- **Merge-resolve exhaustion lets siblings settle** – exhausting merge-resolve attempts no longer cancels-and-requeues in-flight sibling iterations; dispatch stops and siblings settle under a bounded settle-timeout, and a timeout force-fail routes through a run-scoped task cancel that releases the repo lock and slot. The terminal serial-attempt marker is preserved, so a re-conflicting serial retry fails instead of requeuing forever.
- **Canonical workflow event serialization + one shared CLI printer** – workflow lifecycle events define `toJson`/`fromJson` once in `dartclaw_core`; the SSE stream emits `event.toJson()` and both CLI paths (standalone and connected) render through one shared printer mapping, normalizing minor output differences between them.
- **Findings-count derivation follows a defined contract** – `findings_count` and `gating_findings_count` each resolve through an independent three-tier chain (exact key → bare-suffix alias → derive from a structured verdict); the anywhere-in-nested-map integer scan and the gating→total substitution are removed, and an unresolvable gating count with findings present fails honestly instead of guessing.
- **Standalone workflow runs report all missing required variables at once, matching the server rule** – a standalone `workflow run` with several missing required variables now fails with a single aggregated `Missing required variable(s): …` error listing every missing key, instead of an `ArgumentError` naming only the first miss. The variable-validation rule is now shared between standalone and server modes, so a required variable that declares a `defaultValue` no longer errors when omitted (the server's documented behavior) — a small, intentional behavior change for standalone runs.

- **Test-suite speed and log-noise hardening** – marked real-build / real-process cases with the `slow` tag and documented the fast local `-x slow` path, injected test timers into turn wait/stall monitors to remove fixed sleeps from unit coverage, and made expected CLI startup-failure logs capture-only so green runs no longer print `SEVERE` noise while still failing on unexpected severe records.
- **Faster workflow shutdown and cancel** – workflow task lookups during shutdown and cancellation are now run-scoped instead of scanning the full task history, so large histories no longer slow these paths.
- **Workflow-workspace `AGENTS.md` is reconciled, not overwritten** – the managed workspace `AGENTS.md` is tracked against a sidecar marker so template updates still propagate while user edits are preserved (previously every run could clobber local edits); the marker is written before the file so a partial write cannot silently re-arm a clobber.
- **Documented review output-key convention** – the workflow guide documents the canonical per-concept review output keys (review-report path vs. gating count vs. verdict) for custom-workflow authors, locked by a contract test across the built-in and inline workflows.
- **`diff.json` auto-resolution keyed strictly to `diff_summary`** – the workflow context extractor now auto-loads a step's `runtime-artifacts/diff.json` only for the canonical `diff_summary` output key; previously any output key whose name *contained* `diff` or `changes` triggered it. No built-in or shipped workflow used the old substring convention, so built-in behavior is unchanged — but a custom workflow that relied on a `diff`/`changes`-substring output key to auto-resolve the diff must rename it to `diff_summary` (or declare `schema: diff_summary`).
- **Workflow output-declaration hygiene + generic-preset dedup** – the built-in and custom workflow contracts were tightened so every declaration is load-bearing: a single generic `narrative_text` preset backs all free-text narrative outputs (framework semantics stay in per-site inline `description:`), `spec_confidence` reuses the `non_negative_integer` preset, and the name-coupled `prd_path`/`plan_path` presets are removed — `plan-and-implement` now declares those path shapes inline (`resolver: filesystem` + `pathPattern:`) with byte-identical resolved globs. Redundant no-op `inputs:`/`workflowVariables:` declarations (keys already interpolated inline), over-granted `file_edit` on read-only re-review steps, and an orphan `remediation_result` output were pruned, and `code-review`'s re-review now re-reviews the *original* target scope. **Breaking (custom workflows only):** with the `prd_path`/`plan_path` presets and their default path patterns gone, a non-shipped custom workflow declaring a bare `prd:`/`plan:` path output now resolves the generic `**/*` glob instead of `**/*prd.md` / `**/*plan.{json,md}`; declare the pattern inline (`resolver: filesystem` + `pathPattern:`) to restore the narrowed match.
- **Review output-key taxonomy — `review_findings` renamed, parallel sources must prefix** – the review output-key vocabulary was made key-per-concept: a review-report path is `review_report_path` (was `review_findings`), the gate value is `gating_findings_count`, and the structured pass/findings object uses a `verdict`-named key. **Breaking (custom workflows only):** a declared `review_findings` (bare or `<stepId>.review_findings`) output is now rejected at parse time with a `FormatException` naming `review_report_path`; and a review step that feeds an `aggregate-reviews` step must prefix every output key with its step id (`<stepId>.review_report_path`, …) — a bare or mis-prefixed review key on an aggregate source is now a validation error. Single-review workflows with no aggregator keep the bare canonical keys.
- **Workflow YAGNI trim — inert knobs, dead paths, and implicit fallbacks removed** – six edge-accretion items were cut so the authoring surface only carries what the runtime enforces. `onError` is now a typed enum, output resolution for undeclared keys is name-agnostic, and an unenforced budget knob, a legacy mount-location shim, a dead defaulting pass, and an identity wrapper are gone. Behavior of every shipped workflow is unchanged (built-ins declare explicitly whatever they previously received by name-keyed magic). **Breaking:**
  - `maxCostUsd` on a step or in `stepDefaults` is now rejected at parse time with a `FormatException` (no runtime ever enforced a cost budget) — remove it; use `maxTokens` for budget enforcement.
  - The legacy flat `gitStrategy.externalArtifactMount` location is rejected at parse time — nest the block under `gitStrategy.worktree.externalArtifactMount` (unchanged feature, ADR-032).
  - An unknown `onError` value (e.g. `retry`) is now rejected at parse time listing the valid values (`pause`, `continue`); legacy `onError: fail` still parses as `pause`.
  - Undeclared output-key resolution is name-agnostic: the removed `summary`/`findings_summary`/`validation_summary` name magic means a bare undeclared key of that name now resolves as an inline output rather than a narrative one. Declare `schema: narrative_text` to keep narrative resolution.
  - **Custom workflows only:** with the remaining name-keyed default path patterns removed, a bare custom `format: path` output (e.g. `spec_path`, `review_report_path`) now resolves the uniform `**/*` glob instead of a name-derived pattern like `**/*.md` / `**/*review*.md`; declare `resolver: filesystem` + `pathPattern:` (or the `review_report_path` preset) inline to restore the narrowed match. Shipped workflows already declare theirs.
...
- **Workflow start preconditions raise a typed exception** – the 409-vs-500 distinction no longer depends on matching error-message prefixes, so a reworded engine message can't turn a start-conflict into a server error.
- **Merge-resolve / foreach crash-recovery corrections** – two latent-bug fixes folded into the workflow-simplification cleanup of the iteration core: (1) once the merge-resolve serialize-remaining phase reaches `drained`, a persistent serial retry now fails fast instead of re-emitting a serialize sentinel (closing a re-serialize loop); (2) foreach resume no longer marks restored failed/cancelled prerequisites dependency-ready, so a dependent of a failed story stays correctly blocked on resume (aligning restore with the live succeeded-only settle path). Map iteration restore is unchanged.
- **Workflow approval-pause resume guidance is mode-correct** – the message printed when a workflow run pauses for approval no longer hard-codes the misleading "Start `dartclaw serve`" hint. A standalone run now prints `dartclaw workflow resume/cancel <run> --standalone` (it has no server to start); a connected run prints the bare `resume/cancel <run>` against its already-running server. Both paths drop the stale serve prefix.
- **Standalone Claude edits no longer hard-denied** – the Claude one-shot allow-list now grants the edit capability (`Edit`/`MultiEdit`/`NotebookEdit`) on workflow mutation steps, so a standalone run under non-interactive permissions can modify existing files instead of stalling when every edit was denied.
- **`dartclaw cleanup` resilient to an unreadable task database** – a corrupt or write-locked `tasks.db` now degrades to a "retention skipped" warning and non-zero exit instead of crashing the command after other maintenance has already run.
- **Claude full-access workflow steps no longer silently degraded by the env-scrub hardening** – the v0.14.6 spawn hardening sets `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=1` on every Claude spawn, but the current Claude CLI also treats that as a signal to force `--permission-mode default`, which silently overrode the bypass posture a full-access step (`approval: never`) relies on (`--permission-mode bypassPermissions` / `--dangerously-skip-permissions`) — leaving the agent unable to use its tools headlessly. The Claude one-shot provider now sets `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=0` for full-access spawns only, restoring unrestricted execution. Policy-enforced steps keep the scrub — their `--settings permissions.allow` rules are honored under `default` mode, so the hardening still applies there and to the long-lived harness.

## [0.19.0] - 2026-06-26

The **Context Engine** release: DartClaw synthesizes its internal knowledge (LLM-maintained wiki + temporal knowledge graph + memory) into compact, citation-backed packets served to agents over MCP, adds a guard-mediated outbound MCP *client*, and surfaces the knowledge backend through a new read-only web UI on the Afterglow design system.

### Added

- **Context Engine — `context_research` synthesis over MCP** – a single `context_research` MCP tool call fans out across DartClaw's memory, temporal knowledge graph, and LLM-maintained wiki and returns one compact, **citation-backed** packet (synthesized per call, never cached, for freshness) instead of raw ranked rows. Every statement carries resolvable source references; unresolved statements are flagged, and degraded layers / no-source outcomes are reported explicitly. Query and token-budget guards apply; metrics are emitted. (ADR-042.)
- **Outbound MCP client — guard-mediated and audited** – DartClaw can now act as an MCP *client*, consuming external MCP servers / systems-of-record through the runtime's trust boundary: a configurable external-server registry with connection lifecycle; an egress allowlist plus per-call trust-boundary audit; per-server governance (rate limits, token budgets, selective tool surfacing); and runtime composition that builds one outbound pool from the enabled registry and registers surfaced namespaced tools on the live `/mcp` handler, while un-surfaced allowed tools stay dispatchable via explicit `allow_tools` policy. The pool is closed on shutdown.
- **Read-only Knowledge UI (on the new Afterglow design system)** – new web surfaces over the 0.17 knowledge backend: a cross-layer **Knowledge Hub** (`/knowledge`) with wiki/KG/memory/inbox fan-out, layer filters, and source links; a **research packet** view (`/knowledge/research`) rendering `context_research` answers with cross-layer **source attribution** (layer badges, citation markers, provenance popovers, and explicit unverified/unattributed flags); and a **temporal KG timeline** (`/knowledge/timeline`) with as-of/category filtering and superseded/conflict/future-state rendering. Empty, partial-failure, and pagination states included.
...
- **Framework-agnostic workflow engine (ADR-041)** – `packages/dartclaw_workflow` engine `.dart` code no longer carries hardcoded knowledge of AndThen. The engine validates step output using only declared `schema:` (`outputMode: structured`) and generic `format: path` trust-boundary validation, applied uniformly to every path output rather than gated on skill name. The four bespoke AndThen validators, the skill-name dispatch gates, the engine status enum, and the `andthen:*` allowlists are deleted; all framework-specific semantics (status vocabulary, `plan.json` parsing, resume filtering, dependency pruning) now live in the bundled DC-native discovery skills and workflow-YAML gates. A CI fitness gate (`dev/tools/fitness/check_no_framework_coupling.sh`) asserts zero framework-coupling literals in `lib/src/`. The bundled `definitions/*.yaml` workflows and `skills/` payloads remain the only AndThen-specific assets in the package; no behavior change for the three built-in workflows.
- **Standalone workflow config consolidated under `.dartclaw/`** – `dartclaw init --workflow` now scaffolds a single hidden `.dartclaw/` directory holding the config (`.dartclaw/dartclaw.yaml`) plus all standalone runtime state (local DB, `.dartclaw/worktrees/`), mirroring the `.git/`/`.vscode/` convention. A bare `dartclaw workflow run --standalone <name>` (and `resume`/`cancel`/`pause`/`retry`/`status --standalone`) discovers it via cwd-local auto-discovery with no `--config`. `init` also emits a non-destructive `.dartclaw/.gitignore` allowlist that git-tracks the config and custom `workflows/` definitions while ignoring the local DB, `worktrees/`, and the regenerated `workflows/built-in/`. `--config`/`DARTCLAW_CONFIG` precedence and the `~/.dartclaw` fallback are unchanged.
- **`dartclaw init` writes readable block-style config** – generated `dartclaw.yaml` files now serialize as multi-line block YAML instead of a single-line flow map (`{agent: {...}}`), so a freshly initialized config is easy to read and hand-edit. Applies to every `init` path; existing configs are still updated non-destructively.
- **Afterglow ambient ground on all web pages** – the live server CSS is synced to the canonical **Afterglow** design system: an ambient radial-glow ground + film-grain overlay on the app body, plus the extended palette, chart ramp, display type scale, and glass-tier tokens. Existing pages (dashboard, chat, sessions, settings, guards) render on Afterglow with no functional regression.
- **Portable project `localPath`** – a project's relative `localPath` now resolves against the config-file directory (mirroring `data_dir`), so a committed `dartclaw.yaml` can register a repo with no machine-specific absolute path; `localPathAllowlist` entries remain absolute-only and the trust boundary runs on the resolved path.

### Fixed

- **Scheduling API identifiers with special characters** – `GET`/`PUT`/`DELETE` on `/api/scheduling/jobs/<name>` and `/api/scheduling/tasks/<id>` now percent-decode the captured path segment before matching stored identifiers, so names containing `&`, `'`, `<`, `>`, or spaces (as produced by the UI's `encodeURIComponent`) resolve correctly instead of returning 404.
- **Workflow launch-form errors stay visible** – the HTMX workflow run-form returns its inline validation/precondition error fragment with HTTP 200 so HTMX swaps it into the form's target, instead of a 4xx whose body HTMX drops (making the launch error silently vanish). The JSON API path still surfaces the real status code.
- **Standalone active-workspace-root cwd fallback** – a standalone/inline workflow that registers no project and runs with an empty `PROJECT` (the maintainer `.dartclaw/` dogfood harness) now resolves the active workspace root to the runtime cwd and runs its first discovery step, instead of failing with "Workflow requires an active workspace root for output validation". An explicit `-p <project>` and bound worktree paths still take precedence; discovery-emitted paths are still containment-validated against the resolved root.
- **`serve` honors explicit/dev asset paths over cached bundles** – `serve` now resolves assets once and prefers an explicit `--source-dir`/`--templates-dir`/`--static-dir` or `--dev` source tree over a cached release bundle (`~/.dartclaw/assets/v<version>`), so a stale bundle can no longer shadow a fresh checkout (previously surfaced as stale templates/CSS or a missing-template abort). Startup logs the resolved asset source, a missing-template error names the source and remedy, and templates + static assets always resolve from the same root.
- **Quieter workflow skill introspection** – the Claude skill-introspection probe declares its explicit (empty) allowed-tools instead of a now-ignored `--permission-mode plan` flag, suppressing the benign `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB … Permission mode forced to default` warning printed on every workflow probe; the introspected skill list is unchanged.

### Security

- **`allowApiLocalPath` fails closed without an allowlist** – `projects.allowApiLocalPath: true` combined with an empty `localPathAllowlist` is now forced to `false` with a warning, closing a runtime-API arbitrary-path project-registration escalation.

### Breaking

- **Legacy non-dot `./dartclaw/dartclaw.yaml` is no longer auto-discovered** – standalone workflow config resolution now reads only the cwd-local `.dartclaw/dartclaw.yaml` (clean break, no migration). A project carrying just the old `./dartclaw/dartclaw.yaml` falls back to `~/.dartclaw/dartclaw.yaml`. Move the file into `.dartclaw/`, or re-run `dartclaw init --workflow`.

---

## [0.18.0] - 2026-06-11

Universal Agent Harness – DartClaw becomes a universal agent orchestrator. A new first-party ACP (Agent Client Protocol) harness spawns any ACP-compliant agent through one adapter over JSON-RPC/stdio, with capability-gated reverse-calls routed through the existing `FileGuard`/`CommandGuard` chain; Goose (direct-provider) and Mistral Vibe are the two verified targets, and a new agent becomes usable via config alone with zero code changes. Claude Code can delegate work to allowlisted ACP/Codex agents through a `delegate_to_agent` MCP tool with explicit security modes and token budgets. The milestone also hardens runtime operations (per-provider harness pools, stuck-turn visibility and early cancel) and ships behind a cleaner distribution surface (versioned release assets + Homebrew-first install). Nine planned stories: S01 stuck-turn status & early cancel, S02 provider-scoped pools, S03 ACP config & lifecycle, S04 ACP event adapter, S05 ACP reverse-call guard mediation, S06 Goose & Vibe validation, S07 delegation MCP tool, S08 release assets & Homebrew, S09 architecture & guide updates. See `docs/specs/0.18/prd.md` in the private repo for the canonical PRD.

### Added

- **ACP harness** (S03) – first-party from-scratch `json_rpc_2` ACP subprocess client/lifecycle in `dartclaw_core`: binary validation, capability-advertising `initialize`, non-interactive auth, session create/prompt/cancel/close, clean shutdown, provider-identity registration, and structured harness errors. Configured ACP agents share the same harness, routing, structured-error, and pool-release boundary as built-in providers. The `dartclaw_core/lib` LOC ratchet was raised 12800 → 13500 for this core-owned runtime surface.
- **ACP event adapter** (S04) – ACP `session/update` variants and cancellation outcomes map into DartClaw bridge/protocol messages and telemetry, so ACP turns stream and complete through the same consumer contracts as existing harnesses; unsupported optional variants are logged and skipped gracefully.
- **ACP reverse-call guard mediation** (S05) – guard-backed handlers for `fs/read_text_file` → `file_read`, `fs/write_text_file` → `file_write`, and `terminal/create` → `shell` (plus terminal lifecycle and `session/request_permission`), each routed through `GuardChain.evaluateBeforeToolCall(...)`. Includes canonical tool mapping with raw ACP audit names, workspace-jailed `cwd`, `EnvPolicy`-sanitized env overlays, host-side output caps, and fail-closed handling of malformed/unknown terminal IDs.
- **Verified ACP targets** (S06) – Goose and Mistral Vibe registered as the two verified ACP agents with validation evidence/probes classifying prompt/response, file read/write, terminal, permission, and read-blocking behavior as guard-mediated or container-isolation-only. Enforces Goose `developer` extension + non-relay provider constraints and Vibe direct-provider proof; a missing optional binary surfaces a structured validation outcome without blocking the other agent.
- **`delegate_to_agent` MCP tool** (S07) – opt-in delegation configuration and an internal MCP tool that lets Claude Code delegate to allowlisted ACP/Codex agents, validating work directories, task text, security modes (`guard_mediated` / `provider_approval`), governance rate limits, and strict vs. post-run token budgets before returning structured `completed` / `cancelled` / `budget_exceeded` / `error` results. Codex delegation is permitted only as `provider_approval` and never with approval bypass.
- **Stuck-turn status and early cancel** (S01) – operator-visible wait/stuck state for active session turns, a matching task SSE signal, and an authorized early-cancel endpoint plus task/session detail affordance that releases a wedged per-session turn before the global timeout (FR00, closes TD-062).
- **Versioned distribution & Homebrew install** (S08) – published versioned platform assets and checksums for macOS arm64/x64 and Linux x64/arm64, runtime version-drift prevention against release inputs, and a Homebrew-first install path that verifies `dartclaw --version`. On every `v*` tag the `Release Binaries` workflow renders the canonical formula template (`package/homebrew/dartclaw.rb`, via `dev/tools/render_homebrew_formula.dart`) with the verified per-platform digests and pushes `Formula/dartclaw.rb` to the `DartClaw/homebrew-dartclaw` tap, so `brew tap DartClaw/dartclaw && brew install dartclaw` tracks releases with no manual formula edits. Provider CLIs (`claude`, `codex`, Goose, …) remain explicit prerequisites, not implicit dependencies.

### Changed

- **Provider-scoped harness pools** (S02) – the single shared mixed-provider task pool is replaced by provider-scoped pool coordination so one provider's busy workers cannot block another; each configured provider consumes only its own `providers.<id>.pool_size`, unknown providers fail clearly, and the legacy magic capacity floor is removed without changing current Claude/Codex behavior.
- **Architecture and user-guide updates** (S09) – canonical architecture deep-dives and public guides updated for 0.18 (ACP harness, topology-scoped security, ACP agent registration, delegation, production API-key and data-retention guidance, architecture overview, Homebrew-first install), with current-through markers bumped and documentation claims kept traceable to implemented security/distribution behavior.
- **Cross-package shared-util consolidation** – removed high-multiplicity code duplication that the no-third-party-dependency policy had spread across packages. Pulled duplicated per-package copies up into shared first-party code: new `dartclaw_core` utilities `formatLocalDateTime`, `humanizeDuration`/`humanizeSpan`, and a one-shot `httpRequest` (+ unified `HttpClientFactory`); a `ConnectedCommand` base collapsing ~32 CLI command scaffolds; a `SearchMcpTool` base for the Brave/Tavily tools; centralized SSE frame/header plumbing and config-write/guard-admin helpers in `dartclaw_server`; and reuse of existing helpers that callers had bypassed (`yaml_type_safe_reader`, `killWithEscalation`, `asStringKeyedMap`, `chmodOwnerOnly`). All changes are behavior-preserving; no new dependencies. The `dartclaw_core/lib` LOC ratchet (`dev/tools/arch_check.dart`) was raised 12500 → 12800 to absorb the pulled-up runtime primitives — net monorepo LOC decreased.

### Removed

- **Dead-code and no-op cleanup** – removed confirmed-unused code paths together with their full reference sets (call sites, public barrel exports, registrations, and tests), with no change to retained behavior: the `dc-hello` example Stimulus controller, the deprecated `deploy setup` CLI command (use `dartclaw init` instead), the unread `canvas.share.auto_share` config knob, the no-op `knownSystems` storage constant and the `FakeWorkflowTaskBindingCoordinator` testing helper (both dropped from their package public barrels), the legacy parameterless `ContextMonitor.shouldFlush` getter, and the advisor's unconsumed `agent_state_changed` sliding-window entries.
- **Workflow `regex` / `jsonpath` context-extraction types** – both strategies were unimplemented stubs (always threw at execution) and are removed from `ExtractionType`; only `artifact` extraction remains. **Upgrade note**: a workflow definition that specifies `type: regex` or `type: jsonpath` is now rejected when it is parsed or loaded — switch those extraction steps to `artifact`.
- **MVP→0.2 workspace auto-migration** – `WorkspaceService.migrate` and its boot-time invocations are removed; fresh-workspace scaffolding is unchanged. **Upgrade note**: a workspace still on the pre-0.2 layout is no longer auto-migrated on boot — move its root identity/memory files (e.g. `SOUL.md`, `MEMORY.md`, `memory/`) into the `workspace/` subdirectory manually before upgrading.
- Removed non-functional `sessions_spawn` MCP tool (async delegation had no result-retrieval, bypassed the content-guard); use synchronous `sessions_send`. Async/parallel sub-agent delegation may return as a deliberate OpenClaw-parity feature.
...
- **Level-1 governance fitness suite** (S10): six fitness test files at `packages/dartclaw_testing/test/fitness/` – `barrel_show_clauses_test.dart`, `max_file_loc_test.dart`, `package_cycles_test.dart`, `constructor_param_count_test.dart`, `no_cross_package_env_plan_duplicates_test.dart`, `safe_process_usage_test.dart` – plus a `dart format --set-exit-if-changed` CI gate running as a separate step. Allowlists committed under `test/fitness/allowlist/<test-name>.txt` with mandatory rationale comments; every intentional exception is auditable at code-review time. Run locally with `bash dev/tools/run-fitness.sh`. See `packages/dartclaw_testing/test/fitness/README.md` for per-test "How to resolve a failure" guidance.
- **Level-2 governance fitness suite** (S25): six additional fitness test files at `packages/dartclaw_testing/test/fitness/` covering dependency direction, cross-package `src/` import hygiene, testing-package dependency shape, barrel export ceilings, cross-consumer enum/event exhaustiveness, and per-file method-count ceilings. Added `packages/dartclaw_server/test/integration/crash_recovery_smoke_test.dart` for the TD-046 crash-recovery smoke.
...
- **DRY helpers + parser hardening** (S13): `dartclaw_config` – `YamlTypeSafeReader` (`readString`, `readInt`, `readBool`, `readMap`) replaces inline `is`/`is!` type-check blocks across `config_parser.dart` and `config_parser_governance.dart` (407-line reduction from 1603 to 1196 LOC in `config_parser.dart`). `dartclaw_workflow` – `mergeContextPreservingPrivate` and `unwrapCollectionValue` free functions extract duplicated `contextJson` merge and auto-unwrap patterns from the executor; `_fireStepCompleted` extension method consolidates `WorkflowStepCompletedEvent` constructions; `workflow_definition_parser.dart` TD-086 mechanical slice replaces ~15 unguarded `as String?`/`as bool?`/`as int?` casts and lazy `cast<String>()` calls with `_optionalStringValue`/`_optionalBool`/`_optionalInt` helpers that surface `FormatException` instead of `TypeError`.
- **Model grab-bag migration** (S22): domain-specific models moved from `dartclaw_models` and `dartclaw_core` to their canonical owning packages; `dartclaw_models` is now the true shared kernel (`Session`, `Message`, `SessionKey`, `ChannelType`, `AgentDefinition`, `MemoryChunk`). `dartclaw_core` LOC ratchet lowered from 13 000 → 12 500 (closes TD-102). See **Breaking API Changes** below.
- **Schema preset names use underscores**: workflow YAML schema preset identifiers renamed to align with workflow variable naming (e.g. `project-index` → `project_index`, `story-specs` → `story_specs`, `non-negative-integer` → `non_negative_integer`, `diff-summary` → `diff_summary`). All eleven dashed presets now use snake_case; `verdict` and `checklist` are unchanged. Update any custom workflow YAML that references the old names.
- **Breaking – workflow discovery skill rename and plan resume behavior**. Operators upgrading from 0.16.4 must:
  - **Rename downstream `skill:` references** – `dartclaw-detect-spec-input` → `dartclaw-discover-andthen-spec`; `dartclaw-discover-plan-state` → `dartclaw-discover-andthen-plan`.
  - **Rename the `plan-and-implement` entry variable** – switch `-v REQUIREMENTS=…` to `-v FEATURE=…` in CLI invocations, pinned env vars, and wrapper scripts. The previous name is rejected as undeclared.
  - **Reprovision stale data dirs** – `SkillProvisioner.ensureCacheCurrent` now removes `<dataDir>/.agents/skills/dartclaw-{detect-spec-input,discover-plan-state}/` and their `<dataDir>/.claude/skills/` siblings on the next workflow start. The next time the server boots after upgrade, the cleanup runs automatically; no manual delete is required.
  - **Expect resumed plans to skip closed stories** – `dartclaw-discover-andthen-plan` excludes `done`/`skipped` stories from `story_specs.items[]`; an all-closed plan iterates the per-story foreach zero times and proceeds to plan-level review.
  - **Replace `andthen:refactor` in custom plan YAMLs** with per-story `andthen:simplify-code` (built-in `plan-and-implement` now runs it inside the foreach after `quick-review`; `spec-and-implement` runs one top-level pass after `implement`).
  - Output-shape examples for the renamed DC-native discovery skills live in their `SKILL.md ## Output Contract` (single source); `outputExamples:` on the workflow YAML remains available for custom workflows that need to extend or override a non-DC-native skill's output examples.
- **Workflow discovery split**: retired `dartclaw-discover-project`, removed the `project_index` schema preset, and removed `projectIndexPreset` from the workflow package API. `spec-and-implement` now uses `dartclaw-discover-andthen-spec` to guard existing FIS reuse, while `plan-and-implement` uses `dartclaw-discover-andthen-plan` and requires an existing PRD before planning. Custom workflow YAML should replace `project_index` handoffs with explicit `prd`, `plan`, `story_specs`, or `spec_path` outputs.
- **Workflow review path preset consolidation**: removed the short-lived `architecture_review_report_path` schema preset and `architectureReviewReportPathPreset` Dart export. Use `review_report_path` for both `andthen:review` and `andthen:architecture --mode review` outputs.
- **Workflow task policy**: `CliTurnRequest` now carries `allowedTools` and `readOnly`; Claude maps them to one-shot permission settings, while Codex maps `readOnly` to `--sandbox read-only` and treats `allowedTools` as advisory because the Codex CLI has no native tool allowlist.
- **Bash workflow YAML**: bash workflow steps may now use `script:` as a YAML alias for `prompt:`.
- **Workflow flag typing**: `WorkflowStep.type`, `WorkflowGitWorktreeStrategy.mode`, `WorkflowGitExternalArtifactMount.mode`, and `IdentifierPreservationMode` are now backed by enums with strict `fromJsonString` parsing. JSON wire format unchanged; no consumer migration required.
- **Readability pack** (S38): `_logRun` helper with per-step workflow progression at `fine`; typed `Task.worktree` getter; `WorkflowStep.type` deprecated in favour of `taskType`; directory-naming conventions documented; `TaskExecutor.workspaceDir` renamed to `workspaceRoot`; `TaskExecutorLimits` extracted for task-executor policy limits.
- **Release archive dry-run**: `dev/tools/build.sh` was re-run locally and the generated platform/assets archives were inspected for embedded templates, static assets, DartClaw-native skill sources, and workflow definitions; no path drift surfaced.

### Housekeeping & tech debt

S23 bundled the 0.16.5 mop-up into one sweep – no behaviour change beyond the R-L2 breaking deletions listed above.

- **Format gate + pubspec dep alignment** (HK-A/HK-B): `dart format --set-exit-if-changed packages apps` is green and runs in CI on every PR. `yaml: ^3.1.3` and `path: ^1.9.1` are aligned across all `packages/*` and `apps/*` pubspecs; `dev/tools/check-deps.sh` asserts the baseline and exits non-zero on drift.
- **Verification note**: workspace tests pass; remaining skipped cases are intentional integration/capability-gated tests rather than acceptance exclusions.
- **`catch (_)` audit** (HK-C): every production `catch (_)` site under `packages/`+`apps/` now carries either a sibling `_log.fine/_log.warning` call or a one-line rationale comment on the catch line. `workflow_executor.dart`'s artifact-commit / git-init / git-cleanup broad-catch sites delegate to typed handlers in `workflow_artifact_committer.dart` and `workflow_git_lifecycle.dart` that surface `on FormatException` / `on WorkflowGitException` paths and let unexpected types translate into error strings consumed by `_failRun`.
- **`pumpEventQueue` adoption** (HK-D): every `await Future.delayed(Duration.zero)` in `packages/**/test/` was replaced with `await pumpEventQueue()`. `dev/guidelines/TESTING-STRATEGY.md` carries a new "Microtask yields – use `pumpEventQueue()`" section explaining the choice.
- **Typed exceptions** (HK-E): `ScheduleTurnFailureException` (`packages/dartclaw_server/lib/src/scheduling/schedule_service.dart`) and `GitFetchException` (`packages/dartclaw_server/lib/src/project/project_service_impl.dart`) replace the two `throw Exception(...)` sites; both carry `message` + `cause`.
- **Super-parameters** (HK-F): `claude_code_harness.dart` and `codex_harness.dart` constructors use `super.` parameters; the `// ignore: use_super_parameters` comments are removed.
- **`expandHome` test coverage** (HK-G): `packages/dartclaw_security/test/path_utils_test.dart` covers happy-path, `HOME`-unset, `~` alone, `~/` prefix, and non-tilde paths.
- **Test consolidation**: `packages/dartclaw_workflow/test/workflow/_support/workflow_test_paths.dart` exposes `workflowFixturesRoot()`, `workflowDefinitionsDir()`, `codexAvailable()`, and `findAncestorDir(List<String>)` (R-M7 – all inline copies removed). `packages/dartclaw_server/test/task/task_executor_test_support.dart` (R-M8) collapses the duplicated `setUp`/`tearDown` topology in `task_executor_test`, `retry_enforcement_test`, and `budget_enforcement_test`; the harness stays in the server test tree because it depends on server-owned task runtime types. `FakeProcess` redeclarations across 9 test files (DR-M3) now import the canonical helper from `package:dartclaw_testing/dartclaw_testing.dart`.
- **`WorkflowStep.copyWith`** (R-L6): field-exhaustive `copyWith`; `WorkflowDefinitionResolver` consumes it instead of the ~30-arg positional ctor call.
- **Step-outcome warning log** (R-L1): `WorkflowExecutor` now logs a `WARNING` naming run id + step id when a non-`emitsOwnOutcome` step finishes without a `<step-outcome>` marker, alongside the existing `workflow.outcome.fallback` increment (closes the ADR-022 commitment).
- **Git ref argument-injection defence** (SP-1/SP-2 remediation): the originally specified `SkillProvisioner` git-source surface is retired; the current provisioner copies only bundled DartClaw-native skills and has no `config.ref`, `git_url`, cached source, or git subprocess path. Active workflow/project/worktree git ref operands now pass through shared shape validation and reject option-shaped refs before command construction; `branch` / `switch` calls use `--` separators where supported.
- **TD-054**: `_badgeVariantFromClass()` deleted; settings-page callers pass `ChannelStatus` variant strings directly.
- **TD-055**: `_readSessionUsage` default record consolidated to a single file-private constant; net file LOC reduced.
- **TD-056**: `cleanupWorktree(WorktreeManager?, TaskFileGuard?, String taskId, {Project?})` shared utility at `packages/dartclaw_server/lib/src/task/worktree_cleanup.dart` replaces the three near-identical `_cleanupWorktree` impls in `task_routes.dart`, `project_routes.dart`, and `task_review_service.dart`.
- **TD-060**: `dev/tools/sync_version.dart` regenerates `packages/dartclaw_server/lib/src/version.dart` from `packages/dartclaw_server/pubspec.yaml`'s `version:` field; invoked by `dev/tools/build.sh` before AOT compile. Path A chosen (~26 LOC under 30-LOC budget); the manual-bump miss class is closed structurally.
- **TD-061**: Codex stderr now pipes through `utf8.decoder.bind(...).transform(LineSplitter())` and surfaces each line via `_log.warning` (per-line), mirroring `claude_code_harness.dart`'s pattern.
- **TD-073**: `externalArtifactMount` collisions reject at validator load (reachable-step-pair scoped, so mutually-exclusive `parallel`/`gate` branches still legitimately share a destination); regression coverage in `workflow_definition_validator_test.dart`.
- **TD-085**: `SchemaValidator.checkUnsupportedKeywords()` rejects unsupported JSON-Schema keywords (`oneOf`, `anyOf`, `allOf`, `not`, `if`/`then`/`else`, `$ref`, `pattern`, `minLength`, `maxLength`, `minItems`, `maxItems`, `uniqueItems`) at validator-load time with a "supported subset" diagnostic. Silent green is no longer possible for unsupported keywords.
- **DR-M2**: `resolveGitCredentialEnv` dead wrapper at `git_credential_env.dart:88-102` and its `show` entry in `task_exports.dart:12` removed; `resolveGitCredentialPlan` + `GitCredentialPlan` retained.
- **DR-L2**: dead-private-method sweep on `foreach_iteration_runner.dart`, `context_extractor.dart`, `workflow_executor_helpers.dart`, `task_executor.dart`, `workflow_cli_runner.dart` – every uncalled private method deleted; `dart analyze --fatal-infos` clean.
- **S30 residue**: `_err`/`_refErr`/`_contextErr` helpers in `packages/dartclaw_workflow/lib/src/workflow/validation/workflow_validation_helpers.dart` consolidate the validator-rule error-builder boilerplate; 99 call sites migrated across all 6 validation rule files.
- **TD triage decisions recorded**: inline in the S23 FIS' Implementation Observations – TD-069 (H1, H2, M6, M11, M12, M13), TD-090 (closed; existing `approval && parallel` validation covers the multi-approval case), TD-089 (deferred to 0.17 stabilization), TD-086 (H21 narrowed to a dartdoc note; H22/H23/H17/M25 accepted as-is or deferred). TD-029 carries forward with a sharper Trigger pointer (next templates/server-boot wiring refactor).

### Breaking API Changes

Types moved to their canonical owning packages. Update import statements after upgrading – only consumers with explicit `show` imports on the old package are affected.

Public API renames are batched with the model migration in this single migration note. The Dart identifiers changed, but JSON wire formats and package import paths did not change for these rows.

| Old symbol(s) | New symbol(s) | Package / note |
|---------------|---------------|----------------|
| `kDefaultBashStepEnvAllowlist` | `defaultBashStepEnvAllowlist` | `dartclaw_security` |
| `kDefaultGitEnvAllowlist` | `defaultGitEnvAllowlist` | `dartclaw_security` |
| `kDefaultSensitivePatterns` | `defaultSensitivePatterns` | `dartclaw_security` |
| `kWorkflowContextTag`, `kWorkflowContextOpen`, `kWorkflowContextClose` | `workflowContextTag`, `workflowContextOpen`, `workflowContextClose` | `dartclaw_workflow` |
| `kStepOutcomeTag`, `kStepOutcomeOpen`, `kStepOutcomeClose` | `stepOutcomeTag`, `stepOutcomeOpen`, `stepOutcomeClose` | `dartclaw_workflow` |
| `ProjectService.getDefaultProject()` | `ProjectService.defaultProject` | Converted to `Future<Project>` getter |
| `ProjectService.getLocalProject()` | `ProjectService.localProject` | Converted to `Project` getter |
| `SessionService.getOrCreateMain()` | `SessionService.getOrCreateMainSession()` | `getOrCreate` prefix retained for the side-effecting factory |
| `ProviderStatusService.getAll()` | `ProviderStatusService.all` | Converted to getter |
| `ProviderStatusService.getSummary()` | `ProviderStatusService.summary` | Converted to getter |
| `GowaManager.getLoginQr()` | `GowaManager.loginQr()` | Remains a `Future`-returning HTTP method |
| `GowaManager.getStatus()` | `GowaManager.status()` | Remains a `Future`-returning HTTP method |
| `PubSubHealthReporter.getStatus()` | `PubSubHealthReporter.status` | Converted to getter |

**R-L2 deprecation removals** (0.16.4 shims deleted in 0.16.5):

- `WorkflowRegistry.listBuiltIn()` – use `listMaterialized()` instead
- Top-level `deliverResult()` function (`scheduling/delivery.dart`) – use `DeliveryService.deliver()` instead
- `ChannelManager` constructor parameters `taskCreator`, `taskLister`, `reviewCommandParser`, `reviewHandler`, `triggerParser`, `eventBus`, `taskTriggerConfigs` – pass a `ChannelTaskBridge` via `taskBridge:` instead
- `SlashCommandHandler` constructor parameter `eventBus` – events are centralized in `TaskService`; pass `eventBus` to `TaskService` instead
- `taskRoutes` function parameter `eventBus` – same as above
- `ScheduledTaskRunner` constructor parameter `eventBus` – same as above

| Symbol(s) | Old package (0.16.4) | New package (0.16.5) |
|-----------|----------------------|----------------------|
| `Project`, `ProjectStatus`, `ProjectAuthStatus`, `CloneStrategy`, `PrStrategy`, `PrConfig` | `dartclaw_models` | `dartclaw_config` |
| `WorkflowRunStatus` | `dartclaw_models` | `dartclaw_config` |
| `WorkflowDefinition`, `WorkflowStep`, `WorkflowNode`, `ActionNode`, `LoopNode`, `ForeachNode`, `MapNode`, `ParallelGroupNode`, `WorkflowLoop`, `WorkflowVariable`, `WorkflowGitStrategy` (+ git sub-types) | `dartclaw_models` | `dartclaw_workflow` |
| `WorkflowRun`, `WorkflowExecutionCursor`, `WorkflowExecutionCursorNodeType`, `WorkflowWorktreeBinding` | `dartclaw_models` | `dartclaw_workflow` |
| `SkillInfo`, `SkillSource` | `dartclaw_models` | `dartclaw_workflow` |
| `TaskEvent`, `TaskEventKind` (now `enum`) | `dartclaw_models` | `dartclaw_core` |
| `TurnTrace`, `TurnTraceSummary`, `ToolCallRecord` | `dartclaw_models` | `dartclaw_core` |
| `AgentExecution`, `AgentExecutionRepository`, `ExecutionRepositoryTransactor` | `dartclaw_core` | `dartclaw_config` |
| `WorkflowStepExecution`, `WorkflowStepExecutionRepository` | `dartclaw_core` | `dartclaw_config` |
| `LoopDetection`, `LoopDetectedException`, `LoopMechanism`, `LoopDetector` | `dartclaw_core` | `dartclaw_config` |
| `SlidingWindowRateLimiter`, `PromptScope` | `dartclaw_core` | `dartclaw_config` |
| `truncate`, `normalizeDynamicMap`, `canonicalizePathWithExistingAncestors`, `SearchBackend` | `dartclaw_core` | `dartclaw_config` |

`TaskEventKind` additionally changed shape: was a sealed class with 9 empty subclasses, now an `enum`. Pattern-matching `switch` expressions compile unchanged (closes TD-053).
...
- **Workflow execution unification**: workflow-authored agent steps now execute through the coding-task path and express non-mutating intent through `allowedTools`-derived read-only checks instead of a separate workflow streaming/restricted branch
...
- **Workflow read-only enforcement follows the worktree**: read-only workflow steps now fail on file mutations inside their linked worktree instead of only checking the primary project checkout
...
- **Git credential-helper suppression**: DartClaw-orchestrated git operations (clone, fetch, push) now inject `GIT_CONFIG_COUNT=1`, `GIT_CONFIG_KEY_0=credential.helper`, and `GIT_CONFIG_VALUE_0=` to neutralize OS credential helpers (osxkeychain, manager-core, libsecret) for those operations, preventing accidental reuse of ambient credentials. Operator-configured credential plans continue to supply the intended helper
- **Per-package `AGENTS.md` convention**: each Dart package and CLI app in the workspace now carries an `AGENTS.md` symlink to its sibling `CLAUDE.md`, giving non-Claude-Code agents (e.g. Codex) the same package-scoped rules that Claude Code reads. macOS/Linux only – Windows requires `core.symlinks=true`
- **Standalone workflow remote push**: `workflow run --standalone` can now push the workflow branch to the configured remote when `projects.<id>.remoteUrl` is set. The push uses the operator's project credential plan (same credential resolution as server-side publish). CI or scripts that drive `--standalone` and need an upstream branch can now rely on `DartClaw` to perform the push rather than wrapping the command in a manual `git push`
- **AndThen as runtime prerequisite (ADR-025, 2026-04-27 amendment)**: DartClaw no longer ships in-tree ports of AndThen workflow skills. AndThen-derived skills (`dartclaw-spec`, `-plan`, `-prd`, `-exec-spec`, `-review`, `-remediate-findings`, `-quick-review`, `-ops`, etc.) are provisioned at `dartclaw serve` startup under DartClaw's own `dartclaw-` namespace by `SkillProvisioner` (see the runtime-provisioning entry above). The DC-native skills (`dartclaw-detect-spec-input`, `dartclaw-discover-plan-state`, `dartclaw-validate-workflow`, `dartclaw-merge-resolve`) remain in-tree and are copied alongside the provisioned set. Missing `dartclaw-*` skills surface a clear validation error at workflow load time pointing operators at the provisioner config (`andthen.network` / `andthen.git_url` / `andthen.ref`) and `dartclaw serve` restart, not a manual upstream install
- **Workflow contract hardening (S53-S55)**: 30+ focused tests added for step outcome protocol, fallback telemetry, `emitsOwnOutcome`, approval/needsInput resume, credential preflight, token accounting, ADR-024 step-type binding, read-only opt-out, `BRANCH=` bypass regression, foreach cursor fidelity, operator lifecycle precedence matrix, restart idempotency, approval-hold preservation, concurrent checkout serialization, and local human-edit boundary
- **Breaking**: `dartclaw-spec-plan` skill removed – its responsibilities merged into `dartclaw-plan` (Option A merge); PRD creation split out into `dartclaw-prd` (altitude split mirroring AndThen 0.13.0); both skills subsequently removed by ADR-025 above
...
- **Workflow defaults cleanup**: workflow-owned coding tasks auto-advance by default; explicit human checkpoints are modeled with review/approval steps. Omitted `gitStrategy.promotion` is inferred from worktree shape; the default Review queue hides workflow-owned parked artifacts with an on-demand workflow-artifact view
- **Workflow step semantics (ADR-024)**: workflows can now declare a single top-level `project`, built-in workflow YAMLs no longer repeat `project: '{{PROJECT}}'` on every mutating step, and workflow-created tasks persist uniformly as `TaskType.coding`
...
- **`dartclaw_workflow` package**: bumped 0.9.0 → 0.16.0 with migration notes for file-based artifact contract, generalized `entryGate`, `gitStrategy.artifacts`, `externalArtifactMount`, the `inputs:`/`outputs:` rename, the closed `agent|bash|approval|foreach|loop` step-type vocabulary, AndThen runtime provisioning under the `dartclaw-` namespace, the agent-resolved-merge `gitStrategy.merge_resolve` block, and the `{{workflow.runtime_artifacts_dir}}` review-output convention
...
- **`dartclaw_core/lib` LOC ceiling raised 12000 → 13000**: 0.16.4 grew core from 11561 to 12437 lines (harness, channel, task, and workflow-event additions). Ceiling bumped in `dev/tools/arch_check.dart` with a ratchet comment so the next breach forces another conversation. Trimming the new mass back into `dartclaw_models` / `dartclaw_config` is tracked as TD-102 against 0.16.5 S22.
- **`_workflow*` task-config fitness allowlist widened**: `dev/tools/fitness/check_no_workflow_private_config.sh` now enumerates the legitimate consumer sites – the workflow-package internals (`workflow_task_factory.dart` builds the task config map, `workflow_task_config.dart` defines the token-key constants, and a handful of executor helpers / dispatchers / iteration runners read or write workflow context keys) plus two server-side persisted-key reads – instead of the original single-file allowlist that named only `workflow_executor.dart` (which never actually contained any `'_workflow*'` string literals). Reflects the actual storage contract: `_workflowNeedsWorktree` and `_workflowMergeResolveEnv` are intentionally persisted in `task.configJson`. Moving the two server reads behind a typed accessor on `TaskConfigView` is tracked as TD-103 against 0.16.5 S34.
- **Repository layout: top-level `dev/` split**: agent and contributor material moved out of `docs/` into a sibling `dev/` tree. `docs/dev/` → `dev/state/`; `docs/guidelines/` → `dev/guidelines/`; `docs/development/` → `dev/`; `docs/testing/` (sans the user-facing entries) → `dev/testing/`; `tool/` → `dev/tools/`. `docs/` now hosts user-facing material only (`guide/`, `sdk/`). New `dev/testing/profiles/smoke-test/` profile drives the UI smoke test via `bash dev/testing/profiles/smoke-test/run.sh`. CI workflow paths, `.gitignore`, and inbound doc references updated in lockstep.
- **Workspace versions lockstepped to 0.16.4**: every publishable workspace package and `apps/dartclaw_cli` now share `version: 0.16.4`. The previous mix (most packages at `0.9.0`; `dartclaw_core` / `dartclaw_storage` / `dartclaw_server` at `0.10.0`; `dartclaw_workflow` at `0.16.0`) is gone – workspace-wide releases now bump in lockstep per `dev/guidelines/DART-PACKAGE-GUIDELINES.md` § Workspace-Wide Versioning Policy. `bash dev/tools/check_versions.sh` is the gate.

### Fixed

- **Clean-review artifact fallback**: when a workflow review step completes but no saved report is found at the expected path, the engine materializes a diagnostic markdown file under `runtime-artifacts/` that records what the agent returned, preventing silent artifact loss and giving downstream steps a concrete file to parse or report on
...
- **Codex workflow token accounting**: one-shot workflow usage now treats Codex `turn.completed` token counts as cumulative-per-invocation values instead of summing them, and accepts the current CLI `cached_input_tokens` field while keeping the persisted key name `cache_read_tokens`
...
- **Parallel-group failure persists post-merge token totals and post-merge context**: when any step in a parallel group fails, `WorkflowExecutor` now persists `WorkflowContext` and the post-`_updateParallelBudget` `totalTokens` to the run row before emitting `WorkflowStepCompletedEvent` / `ParallelGroupCompletedEvent`. Previously the failure branch overwrote the run row with the pre-merge `refreshedRun.totalTokens` and emitted step-completed events before the merged context had been persisted, so listeners that re-read `run.contextJson` saw stale state and budget accounting under-counted parallel siblings on failure
