...
- **Workflow step semantics (ADR-024)**: workflows can now declare a single top-level `project`, built-in workflow YAMLs no longer repeat `project: '{{PROJECT}}'` on every mutating step, workflow-created tasks persist uniformly as `TaskType.coding`, and semantic step `type:` values are deprecated for engine decisions while remaining available as observability labels
