...
- **AndThen as runtime prerequisite (ADR-025)**: DartClaw no longer ships ported `dartclaw-*` workflow skills (`dartclaw-spec`, `-plan`, `-prd`, `-exec-spec`, `-review`, `-remediate-findings`, `-quick-review`, `-testing`, `-update-state`). Workflow YAMLs now reference the AndThen-installed skills directly (`skill: andthen-spec` etc.). `dartclaw-discover-project` is the only retained DC-native skill. Install AndThen `>= 0.14.3` via `<andthen-repo>/scripts/install-skills.sh` (the standalone-install path with `andthen-` prefix — the Claude Code plugin form of AndThen is not picked up by DartClaw); missing `andthen-*` skills now surface a clear install-path error at workflow validate time
