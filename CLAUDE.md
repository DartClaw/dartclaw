...
<!-- AndThen-style index – workflow commands and DC-native discovery skills read this table to determine where to find and write project documents. -->
...
Formatting is a hard CI gate. Before committing, pushing, or declaring a CI fix done, run
`dart format --line-length=120 --output=none --set-exit-if-changed .`. If it reports changed files, run
`dart format --line-length=120` on those files, include the formatting diff, then rerun the check. CI stops at this gate
before analyze/tests, so format drift hides later failures.
