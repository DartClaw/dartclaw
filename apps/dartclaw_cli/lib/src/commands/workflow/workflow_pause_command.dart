...
class WorkflowPauseCommand extends Command<void> {
  final DartclawConfig? _config;
  final DartclawApiClient? _apiClient;
  final WriteLine _writeLine;
  final ExitFn _exitFn;

  WorkflowPauseCommand({DartclawConfig? config, DartclawApiClient? apiClient, WriteLine? writeLine, ExitFn? exitFn})
    : _config = config,
  ...
  @override
  String get invocation => '${runner!.executableName} workflow pause <runId>';
  ...
  Future<void> run() async {
    ...
    try {
      final result = await apiClient.postObject('/api/workflows/runs/$runId/pause');
      if (argResults!['json'] as bool) {
        _writeLine(const JsonEncoder.withIndent('  ').convert(result));
      } else {
        _writeLine('Workflow ${result['id']} paused (${result['status']}).');
      }
