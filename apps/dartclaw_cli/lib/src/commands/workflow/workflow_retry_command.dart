...
class WorkflowRetryCommand extends Command<void> {
  final DartclawConfig? _config;
  final DartclawApiClient? _apiClient;
  final WriteLine _writeLine;
  final ExitFn _exitFn;

  WorkflowRetryCommand({DartclawConfig? config, DartclawApiClient? apiClient, WriteLine? writeLine, ExitFn? exitFn})
    : _config = config,
  ...
  @override
  String get invocation => '${runner!.executableName} workflow retry <runId>';
  ...
  Future<void> run() async {
    ...
    try {
      final result = await apiClient.postObject('/api/workflows/runs/$runId/retry');
      if (argResults!['json'] as bool) {
        _writeLine(const JsonEncoder.withIndent('  ').convert(result));
      } else {
        _writeLine('Workflow ${result['id']} retried (${result['status']}).');
      }
