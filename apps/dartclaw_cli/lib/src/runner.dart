...
import 'commands/workflow/api_workflow_connection.dart';
...
DartclawRunner buildDartclawRunner() => DartclawRunner()
  ...
  ..addCommand(WorkflowCommand(connection: ApiWorkflowConnection()));

/// Builds the standalone workflow CLI with workflow verbs mounted at its root.
DartclawRunner buildDartclawWorkflowRunner() {
  final runner =
      DartclawRunner(
          executableName: 'dartclaw-workflow',
          description: 'DartClaw standalone workflow runner',
          connectedGlobalOptions: false,
        )
        ..addCommand(InitCommand(workflowOnly: true))
        ..addCommand(RebuildIndexCommand());
  for (final command in workflowSubcommands(standaloneOnly: true)) {
...
class DartclawRunner extends CommandRunner<void> {
  ...
  new({
    ...
    String executableName = 'dartclaw',
    String description = 'DartClaw — security-conscious AI agent runtime',
    bool connectedGlobalOptions = true,
