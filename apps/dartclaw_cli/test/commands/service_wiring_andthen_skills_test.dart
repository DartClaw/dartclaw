...
void main() {
  ...
  test('install_scope: data_dir + outside localPath throws documented error', () async {
    ...
    await expectLater(
      ...
      throwsA(
        isA<SkillProvisionException>().having(
          ...
          'message',
          allOf(
            contains('andthen.install_scope=data_dir'),
            contains('install_scope: user'),
            contains('install_scope: both'),
          ),
        ),
      ),
    );
  ...
  test('ServiceWiring andthen skills bootstrap (positive)', () async {
    ...
    final wiring = ServiceWiring(
      ...
      skillProvisionerSpawnTargetCwds: [dataDir],
    );
    ...
    expect(File(p.join(dataDir, '.agents', 'skills', '.dartclaw-andthen-sha')).readAsStringSync(), 'fake-head');
    expect(Directory(p.join(fakeHome.path, '.agents', 'skills')).existsSync(), isFalse);
    expect(Directory(p.join(fakeHome.path, '.claude', 'skills')).existsSync(), isFalse);
    ...
    for (final name in _shippedDartclawSkillRefs) {
      expect(
        ...
        isNotNull,
        reason: '$name must resolve through the registry after data-dir provisioning',
      );
