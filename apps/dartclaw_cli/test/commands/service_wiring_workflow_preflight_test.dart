...
import 'package:dartclaw_core/dartclaw_core.dart' hide GoogleJwtVerifier, HarnessConfig, TurnManager, TurnRunner;
import 'package:dartclaw_server/dartclaw_server.dart' hide GoogleJwtVerifier, TurnManager, TurnRunner;
...
void main() {
  ...
  Future<({ServiceWiring wiring, WiringResult result})> wire(DartclawConfig config) async {
    ...
    return (wiring: wiring, result: result);
  }
  ...
  test('a workflow provider with no usable credential is refused, naming the store that was searched', () async {
    ...
    final preflight = (await wire(config)).result.workflowService.providerAuthPreflight;
  ...
  test('a probe-lane credential refusal reaches the credential-health monitor', () async {
    ...
    final subscription = wired.result.eventBus.on<CredentialHealthChangedEvent>().listen(transitions.add);
    ...
    await expectLater(wired.wiring.providerProbeEnvironment('codex'), throwsA(isA<StateError>()));
