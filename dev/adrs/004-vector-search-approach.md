...
**Status:** Superseded by [ADR-050](050-native-hybrid-search.md) (2026-07-25). QMD is deprecated in 0.26 but remains functional through this milestone; the following milestone removes it. Originally accepted and fully implemented. The retained `QmdManager` and `QmdSearchBackend` live in `dartclaw_core`; the `SearchBackend` contract lives in `dartclaw_kernel`.
