...
**Consequence for generated assets ([ADR-047](047-embedded-binary-assets.md))**: ADR-047 justifies committing `embedded_assets.g.dart` with "pub.dev doesn't run generators; SDK consumers need them present." That rationale is load-bearing only for packages that ship. It currently holds for `dartclaw_workflow` and `dartclaw_server` **because their status is undecided** — a package cannot be published from a checkout whose generated sources are absent. Keep both files committed while the question is open. If `dartclaw_server` is settled as repo-only, its 1.7 MB generated library becomes a candidate for build-time generation (`dartclaw_workflow`'s is 70 KB, so the cost is concentrated in the server file); revisit ADR-047 at that point rather than assuming it still applies.
