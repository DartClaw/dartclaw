...
Accepted — 2026-06-05 (targets 0.18; security model validated by the A0/A0.1 verification spike)
...
DartClaw advertises `fs.readTextFile`/`fs.writeTextFile`/`terminal` capabilities in `initialize` and implements the reverse-call handlers natively, routing through the existing `GuardChain` (`FileGuard`/`CommandGuard`) and the tool-approval chain (`session/request_permission`). Guard-by-design is **agent- and topology-dependent**, made explicit (per the ADR-035 precedent):

- **Direct-provider topology** (the ACP agent is the sole agent, honoring advertised capabilities — e.g. Goose with a direct model provider + the `developer` extension): file/terminal operations arrive as reverse-calls and are mediated by the guard chain. **Guard-by-design holds.** Verified in A0.1: read/write/shell all routed through the guards; `CommandGuard` blocked a `terminal/create`.
...
- Guard-by-design (handler-layer, type-safe, unbypassable) for direct-provider ACP agents — a stronger model than hook interception, since the agent never has direct filesystem/terminal access.
...
- DartClaw owns the ACP DTOs and transport adapter; extending coverage (e.g. terminal lifecycle, future stable methods) is incremental in-house work.
...
- Minimal client subset only: stdio NDJSON adapter, `initialize` with capability advertisement, `session/new`, `session/prompt`, `session/update`, the four reverse-call handlers, `session/cancel`. Exclude unstable methods (`session/fork`, Elicitation, NES); `session/resume` is available but not built on in 0.18.
...
- **Security in depth** — handler-layer guard mediation for direct topologies; container isolation retained as the boundary elsewhere; default-deny for unverified agents.
