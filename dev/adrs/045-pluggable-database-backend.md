...
3. **Webhook delivery ledger – amended 2026-09-08 (owner 2026-08-07, TD-115).** Dedup remains instance-local, using one SHA-256-named file per delivery ID under `webhook_deliveries/`. An exclusive file create reserves the ID; synchronous atomic rewrites commit it and refresh the seven-day retention anchor. Pending and malformed claims are retained for stale-claim recovery. The abstraction scope remains `tasks.db` + `search.db`.
...
2. **Exact released 0.25 SQLite store** – verify the required-object manifest, then add only the current schema marker in one transaction. The manifest tolerates the nullable orphan `workflow_step_executions.external_artifact_mount` left by stores upgraded from 0.24 because extra columns do not participate in compatibility. There is no second adoption path. Fault injection proves safe rollback. This marker-only adoption is a bounded transition, not a general upgrade framework.
...
- Database layout is decided (see Resolved Q3/Q4): `tasks.db` + `search.db` collapse into one PostgreSQL database with one pool; `turn_state.json` and `webhook_deliveries/` remain instance-local filesystem stores outside the abstraction; SQLite keeps its authoritative and derived files.
...
4. **Instance-local recovery – mechanism amended 2026-09-08 (owner 2026-08-07, TD-115).** Active turns live in `turn_state.json`, keyed by session ID with `turnId` and `startedAt`. Mutations synchronously read, update and atomically replace the file through `secureWriteFileSync`, so an unawaited `set()` is durable at return before turn execution can crash. Reads re-open the file; startup recovery removes orphan records and preserves the one-time session notice. File locality keeps recovery independent of PostgreSQL availability. A shared PostgreSQL table with an `instance_id` remains a rejected alternative, not a technical impossibility; active-active operation is outside this decision. The database abstraction still covers only `tasks.db` + `search.db`.
