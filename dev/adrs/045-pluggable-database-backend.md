...
3. **`postgres` dependency exclusion mechanism — resolved (owner, 2026-07-25): compile-in accepted, no build flavor.** Decided together with the equivalent native-asset question for the planned hybrid-search package (private research: `dartclaw-private/docs/research/dart-native-hybrid-search/research.md` §8): one default binary, all-in. Revisit only if release size becomes a real problem.
