...
| [`merge_resolve_coordinator.dart`](../../packages/dartclaw_workflow/lib/src/workflow/merge_resolve_coordinator.dart) + [`dartclaw-merge-resolve/SKILL.md`](../../packages/dartclaw_workflow/skills/dartclaw-merge-resolve/SKILL.md) | 1 | The plumbing/judgment split: host owns sha capture, cleanliness, locking, bounded attempts, audit and escalation; the model owns semantic resolution and is barred from the destructive git verbs. Its outcome vocabulary is a declared enum schema on `merge_resolve.outcome` and its error message a declared string-or-null, each validated once host-side; a value outside its contract fails the attempt with the key named and is re-asked by the bounded attempt loop, never defaulted or coerced. The conflicted file set is asked of no one: git's promotion result is its single authority. |
