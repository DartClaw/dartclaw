...
When `--permission-prompt-tool stdio` is set, the binary sends a `can_use_tool` control request before each tool execution. DartClaw's current `ToolApprovalPolicy` is `allowAll` — all tools are approved at this stage. The approval mechanism exists as a seam for future fine-grained policies.
...
claude binary
  │
  ├─► can_use_tool ────► allowAll ────► allow response
  │
...
See the [Codex CLI Harness Research](../research/codex-cli-harness/research.md) for the protocol analysis that informed this section.
