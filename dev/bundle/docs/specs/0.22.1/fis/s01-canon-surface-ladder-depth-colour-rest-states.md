...
- [OC05] Every surface-and-chrome canon rule a later story consumes is already in canon when that story starts: the featured-card gradient border paints, `<hr>` renders as a designed rule, a short chat thread bottom-anchors, and table headers band. Canon closes with this story's family complete – no P3 sweep authors a surface rule.
...
- `docs/specs/0.22.1/canon-hoist-manifest.md` – why four rules S08/S09/S12 discovered are authored here instead, what the canon closure covers (the three drift-checked files only – `DESIGN.md` and `showcase.html` stay writable), and which stories consume each rule.
...
- [ ] **S01 [OC01] [TI01,TI02,TI15] Cards, ground and chrome sample as three planes on a card-dense dark surface**
  - **Given** the `visual` profile serving `/health-dashboard` in dark theme at 1440px, where the card interior today samples `#181825`, the ground between the card columns `rgb(35,34,50)` and the sidebar the same `#181825` as the card
...
- [ ] **S02 [OC01,OC02] [TI01,TI02,TI03] Light theme separates chrome, ground and card on its own terms**
  - **Given** the `visual` profile serving `/tasks` with `data-theme="light"`, where the sidebar, the topbar and the card interior all measure exactly `#e6e9ef` today
  - **When** those three regions and a resting card's bottom edge are sampled
  - **Then** the card is ΔE(oklab) ≥ 0.02 from the chrome the sidebar and topbar share, card-vs-ground contrast is ≥ 1.15:1, and the card's shadow spans ≥3 pixel rows instead of one
...
- [ ] **S06 [OC01,OC04] [TI04,TI16] No re-toned surface leaves text below AA, and the surface token is what moves**
  ...
  - **When** the 23-surface contrast pass runs in both themes at 1440px and 768px
...
- [ ] **S08 [OC05] [TI09,TI10,TI11,TI12] The four hoisted surface rules are live in canon before any sweep story consumes them**
  - **Given** the `visual` profile after this story's re-sync, on a healthy `/health-dashboard`, `/sessions/<id>` with a two-turn conversation, and `/tasks` – where today the featured card's gradient border never paints, the six sidebar `<hr>` render at the UA `inset` bevel, the thread stacks from the top with the last message ~558px above the composer, and `<thead>` samples identically to the first body row
  - **When** each of those four regions is sampled and read with `getComputedStyle` in both themes
  - **Then** the hero card's border differs from its own fill by ΔE(oklab) ≥ 0.02, every `hr.sidebar-divider` reports `borderTopStyle: "solid"` with `borderBottomWidth: "0px"`, the last message's bottom edge sits within one `--sp-4` of the `.input-area` top border, and the `<thead>` band differs from the first body row by ΔE(oklab) ≥ 0.02
  - **And** all four are served from `dev/design-system/components.css` with `check_design_system_sync.sh` green, so S08, S09 and S12 consume them without touching canon
...
- [ ] `packages/dartclaw_server/lib/src/static/app.css` carries no edit from this story; the only app-side changes are the two re-synced CSS files, and `icons.css` is untouched. `lib/src/generated/embedded_assets.g.dart` is deliberately left stale – see Constraints & Gotchas.
...
- [ ] The four rules hoisted to this story by `docs/specs/0.22.1/canon-hoist-manifest.md` are present in `dev/design-system/components.css` and in the re-synced `design-system.css`: the eight `.card-featured-*` background-layer fixes, the `hr` element reset, the `.messages` anchoring pair (with `.messages:empty::after` deleted), and the `.data-table thead` band. Canon's surface-and-chrome family is complete at this story's boundary; S08, S09 and S12 add nothing to it.
...
- `../dartclaw-public/dev/design-system/components.css` – `body` ground gradient and glow placement, `.card`, `.sidebar` / `.topbar`, `.card-metric--*` / `.card-tint-*` rest **and** `:hover` states (nine hover fills are pinned to `--bg-mantle`), sidebar text tiers, `.composer-send` / `.composer:focus-within`, plus the four rules hoisted here per `canon-hoist-manifest.md`: § 10's eight `.card-featured-*` rules, a § 1 `hr` element reset, § 4's `.messages` (anchoring, and deleting the dead `:empty::after`), and a `.data-table thead` band in § 14.
...
- Visual validation of all 23 surfaces on the `visual` profile (port 3338), both themes, 1440px and 768px.
...
- Adopting the four hoisted rules on any page -- authoring them in canon is TI09–TI12; `.card-featured-*` adoption is S09's, the `hr` and `.messages` surface checks are S12's, and the `.data-table` sweep is S08's. This story changes no template, no controller and no `app.css` line.
...
- **Constraint**: canon **closes** for `tokens.css` / `components.css` / `icons.css` when P1 completes, so this story is the last chance to author a surface-or-chrome rule. That is why TI09–TI12 exist: a rule left out here cannot be added by the story that needs it, and a P3 story that discovers a gap must stop and report rather than work around it in `app.css`. The closure does **not** cover `DESIGN.md` or `showcase.html` – neither is drift-checked, both stay writable, and S14 reconciles them at release close.
...
- **Constraint**: changing the served CSS drifts `packages/dartclaw_server/lib/src/generated/embedded_assets.g.dart`, which is tracked and gated by `dev/tools/release_check.sh` § 5. Do **not** regenerate it here – `dart run dev/tools/embed_assets.dart` rewrites a large generated file that all fourteen stories touch, and running it per story guarantees conflicts across the parallel P3 waves. Regeneration is a release-close action.
...
- [ ] **TI01** Surface tokens place chrome, page ground and cards on three distinct planes in both themes
  ...
  - **Verify**: on the `visual` profile, the card interior sampled against the ground directly above and beside it gives contrast ≥ 1.15:1 on `/health-dashboard`, `/tasks` and `/memory` in both themes.

- [ ] **TI02** `.card`, `.sidebar` / `.topbar` and the body ground resolve to three different surface tokens
  ...
  - **Verify**: in both themes, `.card`'s computed fill differs from `.sidebar`'s by ΔE(oklab) ≥ 0.02, and no `body` gradient stop equals the card fill. Chrome may legitimately coincide with the gradient's 0% stop once chrome and ground share the crust end – only the *card* fill is required to be absent from the ramp.
...
- [ ] **TI10** No `<hr>` renders at the browser default bevel
...
- [ ] **TI14** showcase.html demonstrates the revised ladder, the resting card colour and the composer's contrast floor
  - `dev/design-system/showcase.html` § Surface Ladder swatches, § Card Taxonomy (metric and tint panels), § Composer. Showcase is the reference for what the system can do, so the rest-state treatments must be visible without hovering. Existing markup already covers three of the four hoisted rules – `.card-featured-accent` / `-error` (`:341, :349, :947`), `hr.divider-*` (`:281, :308, :319, :336, :356, :520, :522`) and a `.messages` block (`:1065`) – so they need no new panel, only re-checking. The `.data-table` band has **no** showcase demo because showcase carries no table markup at all; leave it that way and record the gap for S14's doc reconciliation rather than adding a panel here.
  - **Verify**: opening `showcase.html` in both themes shows the ladder stepping monotonically, the metric and tint cards tinted with no pointer over them, and each `.card-featured-*` panel's border distinct from its fill; `rg -o 'style="' dev/design-system/showcase.html | wc -l` returns ≤ 218, the pre-story occurrence count (`rg -c` counts matching *lines*, 212, and would miss a second `style=` added to an existing line).

- [ ] **TI15** The served CSS matches canon with regenerated provenance headers
  - `packages/dartclaw_server/lib/src/static/tokens.css` and `design-system.css` each carry the two-line `/* Synced from dev/design-system/<file> on YYYY-MM-DD.` + `   sha256: <hash> */` header followed by the canon file verbatim from line 3. `icons.css` is not part of this story. Re-run after any value re-tuned during TI16.
  - **Verify**: `bash dev/tools/fitness/check_design_system_sync.sh` exits 0 and prints no `design-system drift: design-system.css`; `git status --porcelain packages/dartclaw_server/lib/src/static/` lists only `tokens.css` and `design-system.css`; `rg -n '#[0-9a-fA-F]{3,8}\b' dev/design-system/components.css` returns no matches (exit 1) – the pre-story count is zero, so every intermediate surface derives via `color-mix(in oklab, …)`.

- [ ] **TI16** All 23 surfaces are re-validated in both themes at desktop and 768px against the audit baseline
  ...
  - **Verify**: every surface captured in both themes at 1440px and 768px; no text pairing below 4.5:1 (3:1 for ≥18px bold) and no card-vs-ground pairing below 1.15:1 in the capture set.
...
### Execution Contract
- TI15 is not a one-shot step: any value re-tuned during TI16's validation loop must be re-synced and `check_design_system_sync.sh` re-run before the story closes. Canon and served CSS may not diverge at the story boundary.
...
- TI09–TI12 are the hoisted canon rules and must land **before** TI13/TI14 document them and before TI15's sync – they are ordered where they are for that reason, not appended as an afterthought. TI09 and TI12 additionally consume TI01's settled card and surface tokens, so neither can be authored before the ladder is fixed.
