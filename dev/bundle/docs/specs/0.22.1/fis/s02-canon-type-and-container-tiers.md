...
- [ ] **S04 [OC03] [TI06,TI07,TI13] Both container selectors exist and the canon-governed header rule is available**
...
- [ ] `dev/design-system/components.css` contains zero `var(--text-sm)` references: every canon consumer migrates while the P1 type family is open. The alias remains only in `tokens.css` for app-side migration, and no new `--text-*` size token is added beyond `--container-wide` / `--measure`.
- [ ] After the final served-copy sync, `dart run dev/tools/embed_assets.dart` refreshes `packages/dartclaw_server/lib/src/generated/embedded_assets.g.dart`, and `dart test packages/dartclaw_server/test/generated/embedded_assets_test.dart` passes before the story closes.
...
- [ ] **TI03** Existing canon consumers leave `--text-sm`, and heading components bind the complete heading tier
...
- [ ] **TI07** Data-table headers have the canon rule and the app-local container family mirrors the canonical modifier
...
- [ ] **TI14** The served CSS and tracked embedded bundle match final canon
...
- [ ] **TI15** S02's own visual deltas hold in both themes at desktop and 768px
