...
- [OC05] The type-and-icon canon obligations the P3 sweeps discovered are shipped here, so no sweep story has to open canon: every mask name the task-event code path emits resolves to a glyph instead of a blank square, the collapse toggles have both chevrons, a server-composed task prompt keeps its line structure, and `.section-title` carries the complete heading-tier binding before canon closes.
...
>
> Two-step so the app never breaks mid-migration: S02 aliases `--text-sm` to `--text-base` in canon `tokens.css`, stops treating it as a tier in DESIGN.md, defines all seven composite classes and migrates every canon `components.css` consumer while the P1 type family is open. S07 then migrates the remaining app-side and non-drift-checked demo usages, deletes the alias from canon `tokens.css` under its one token-only exception, re-syncs only served `tokens.css`, regenerates embedded assets and closes generated parity green. No other story introduces a new `--text-sm` usage or reopens a post-P1 canon family.
...
- [x] **S01 [OC01] [TI02,TI13] A named tier is applied as one class, not four declarations**
...
- [x] **S02 [OC02] [TI01,TI02,TI13] The ladder steps are perceptible**
...
- [x] **S03 [OC02,OC04] [TI01] An un-migrated `--text-sm` call site still renders at a real size**
  - **Given** `app.css` still contains its 79 `var(--text-sm)` references behind the alias, while canon `components.css` has migrated all of its consumers in TI03
...
- [x] **S04 [OC03] [TI06,TI07,TI13] Both container selectors exist and the canon-governed header rule is available**
  - **Given** the showcase's `.content-inner--wide` specimen and source containing the app-owned `.page-inner--wide` mirror
  - **When** their computed and declared `max-width` values are inspected
  - **Then** both resolve to `var(--container-wide)` / 1280px, bare `.content-inner` stays 900px, and canon `.data-table th` carries `white-space: nowrap`
  - **And** no rendered scheduling-wide outcome is claimed here: S08 applies `.page-inner--wide` to tasks, task detail and scheduling and proves the scheduling page renders wider than 900px with unclipped headers

- [x] **S05 [OC03] [TI06] The wide tier is opt-in – an unmodified container keeps the 900px measure**
...
- [x] **S06 [OC02] [TI04] Generated markdown keeps its heading structure**
...
- [x] **S07 [OC02] [TI03,TI05] Card titles and the topbar title sit above body size**
...
- [x] **S08 [OC05] [TI09,TI10] Every mask name the app already emits resolves to a glyph**
...
- [x] **S09 [OC05] [TI08] A server-composed task prompt keeps its line structure**
...
- [x] `bash dev/tools/fitness/check_design_system_sync.sh` exits 0: served `tokens.css`, `design-system.css` **and `icons.css`** are byte-identical to canon `tokens.css` / `components.css` / `icons.css` from line 3 onward, with a regenerated two-line `/* Synced from … sha256: … */` provenance header whose hash matches the source.
- [x] `dart test packages/dartclaw_server/test/static/design_system_icons_sync_test.dart` passes: every `--icon-*`, `.icon-*` and `[data-icon="…"]` in the served copy is present in canon.
- [x] DESIGN.md § Typography has exactly seven tier rows, one per shipped composite class, with no `body-sm` row; the frontmatter `typography:` block matches it (no `body-sm` key, `heading-md.fontSize: 18px`, `heading-md.letterSpacing: -0.02em`).
- [x] DESIGN.md § Layout primitives documents `container-wide` and `measure` beside `container-max`, and states which surface classes take the wide tier and which keep 900px – matching `plan.json#sharedDecisions` "Wide-container assignment" exactly, including workflow **detail** on the wide list and **projects** on the 900px list.
- [x] DESIGN.md § Icon vocabulary has a row for every `--icon-*` canon defines, including the four TI09/TI10 add, and the `check` / `circle-x` rows name their new `data-icon` semantic instead of `—`.
- [x] `--font-sans` is settled: zero references remain in canon or served CSS/HTML/JS, and DESIGN.md § Typography states the mono-only decision explicitly.
- [x] `showcase.html` demonstrates all seven composite classes and both container tiers, and its § Icons gallery carries a specimen for every name TI09/TI10 add; the seven bare inline `font-size:` rows in its type-scale block are gone.
- [x] `dev/design-system/components.css` contains zero `var(--text-sm)` references: every canon consumer migrates while the P1 type family is open. The alias remains only in `tokens.css` for app-side migration, and no new `--text-*` size token is added beyond `--container-wide` / `--measure`.
- [x] After the final served-copy sync, `dart run dev/tools/embed_assets.dart` refreshes `packages/dartclaw_server/lib/src/generated/embedded_assets.g.dart`, and `dart test packages/dartclaw_server/test/generated/embedded_assets_test.dart` passes before the story closes.
- [x] WCAG AA text contrast holds in both themes on every surface whose text re-scaled.
...
- `dev/design-system/components.css` – the `.t-*` composite layer, migration of every canon `var(--text-sm)` consumer, `.card-header` / `--sm` and `.section-title` heading-tier bindings, `.msg-content` heading split + prose-scoped measure, `.msg-user .msg-content p` line structure, `.topbar` title tier, `.content-inner--wide`, `.data-table th` nowrap
...
- Migrating app-side `--text-sm` call sites and deleting the alias – S07 owns that second step. S02 migrates every canon `components.css` call site now; S07's one post-P1 canon exception is limited to deleting the alias from `tokens.css`, syncing served `tokens.css`, and regenerating embedded parity.
- Per-page assignment of `.content-inner--wide` / `.page-inner--wide` – the modifier ships opt-in here; the P3 surface stories apply it per the plan's fixed wide-container assignment. S08 owns the first rendered scheduling-wide proof; S02 proves selector availability in canon, the app mirror and showcase only.
- Adding `.t-metric` to the shared `metricCard` fragment – S16 owns `components.html` / `components.dart` and applies the already-defined class once for every KPI consumer before the surface sweeps. S02 proves `.t-metric` exists and renders in showcase.
...
- Hand-editing `packages/dartclaw_server/lib/src/generated/embedded_assets.g.dart`. This story re-syncs three served static files, so after the final embed-root change it regenerates the bundle with `dart run dev/tools/embed_assets.dart` and closes with generated parity green, per `plan.json#executionNotes`.
...
**Approach**: Land the whole type-and-layout revision in canon (`tokens.css` / `components.css` / `icons.css` / `DESIGN.md` / `showcase.html`) and re-sync the served copies in the same story; migrate every canon `components.css` consumer now, but alias `--text-sm` to `--text-base` rather than deleting it so the remaining app-side call sites keep resolving while S07 migrates them.
...
Four further canon obligations arrive by hoist because canon closes after P1: `.section-title` joins TI03, while TI08–TI10 carry the message and icon rules. They are small and independent of the type-and-measure work, but the icon work widens the story's canon footprint to a third drift-checked file, `icons.css`, which carries its own strict-sync test on top of the byte-identity check.
...
- **Critical**: the alias is a live re-scale, not a no-op – `--text-sm: var(--text-base)` moves the 79 `app.css` call sites from 13px to 14px at once. Every canon `components.css` consumer migrates explicitly in this story, including `.card-header`, `.card-header-gradient` and `.section-title`; none is left for S07 to reopen after P1. Must handle by: re-validating the dense app surfaces those alias sites drive (data tables, sidebar items, banners, card bodies) in both themes at desktop and 768px before declaring done.
...
Before TI01, capture fresh visual baselines for `showcase.html`, `/projects`, the seeded `/sessions/<id>` (the chat surface; `/` redirects there – `/chat` is not a route), `/settings` and `/scheduling` in both themes at 1440×900 and 768px. These are S02's story-local comparators for the type/token changes it actually makes. The audit's 92-shot set is not a per-story comparator here — it serves S01's audit-baseline gate and S14's release-level re-validation.

Also capture the accumulating checkout exactly as this FIS receives it. This is the delta baseline for every close-out proof below; it requires no clean tree or commit:

```sh
BASE=.agent_temp/0.22.1-s02-entry
rm -rf "$BASE"
...
- [x] **TI01** The type scale carries a perceptible ladder and `--text-sm` is no longer a distinct tier
...
- [x] **TI03** Existing canon consumers leave `--text-sm`, and heading components bind the complete heading tier
  - Migrate every remaining `var(--text-sm)` in `dev/design-system/components.css` while the P1 type family is open. A component whose markup can compose a `.t-*` class does so in showcase and downstream templates; a canon rule that must carry typography itself mirrors the matching tier's complete four-property binding rather than only swapping its size token.
  - `.card-header`, `.card-header-gradient` and `.section-title` move to the `.t-heading` values (`var(--text-lg)` / `var(--weight-bold)` / `var(--leading-tight)` / `var(--tracking-tight)`); `.section-title` is the S07 handoff hoisted here by `canon-hoist-manifest.md`, so it may not remain for post-P1 repair. Add `.card-header--sm` at `var(--text-base)` / `var(--weight-bold)` for dense list rows. It ships with no in-story consumer by design – templates opt in during P3 (see What We're NOT Doing). Consumes TI02's tier values.
  - **Verify**: `rg -n 'var\(--text-sm\)' dev/design-system/components.css` returns no matches; a `.card` header and `.section-title` compute to 18px/600 with 1.3 leading and tight tracking; `.card-header-gradient` matches; `.card-header--sm` computes to 14px/600

- [x] **TI04** Markdown headings in messages render as distinct steps
...
- [x] **TI05** The topbar page title sits on the page-title tier
...
- [x] **TI06** Canon carries a second container tier and a stated reading measure
...
- [x] **TI07** Data-table headers have the canon rule and the app-local container family mirrors the canonical modifier
  - `dev/design-system/components.css#.data-table th` gains `white-space: nowrap`; `packages/dartclaw_server/lib/src/static/app.css` gains `.page-inner--wide { max-width: var(--container-wide); }` beside `.page-inner` (app.css:600) – app-local because `.page-inner` is, per DESIGN.md § Layout's Migration note. Consumes TI06's token. This story proves both selectors are available; S08 applies the app modifier to scheduling and proves the rendered width/header outcome. The tasks table remains overridden app-side until S08 (see Constraints & Gotchas and What We're NOT Doing).
  - **Verify**: `rg -nU --multiline-dotall '^\.page-inner--wide\s*\{[^}]*max-width:\s*var\(--container-wide\)' packages/dartclaw_server/lib/src/static/app.css` matches the mirror; `rg -nU --multiline-dotall '^\.data-table th[^{]*\{[^}]*white-space:\s*nowrap' dev/design-system/components.css` matches the canon rule; showcase's `.content-inner--wide` specimen computes to 1280px while bare `.content-inner` stays 900px

- [x] **TI08** A server-composed user message keeps its line structure *(hoisted from S08 TI16 — `canon-hoist-manifest.md`)*
...
- [x] **TI10** Every task-event mask name resolves, and the two attribute mappings the app already calls exist *(hoisted from S08 TI05 — `canon-hoist-manifest.md`)*
...
- [x] **TI11** The dead `--font-sans` token is settled
...
- [x] **TI13** showcase.html demonstrates every tier, both container tiers and the new glyphs
...
- [x] **TI14** The served CSS and tracked embedded bundle match final canon
  - Copy canon `tokens.css` → `packages/dartclaw_server/lib/src/static/tokens.css`, canon `components.css` → `packages/dartclaw_server/lib/src/static/design-system.css`, and canon `icons.css` → `packages/dartclaw_server/lib/src/static/icons.css`, each prefixed by the regenerated two-line `/* Synced from <source> on <YYYY-MM-DD>.\n   sha256: <sha256 of source> */` header. All three files are in scope – `icons.css` because TI09 and TI10 edit it. After the final copy, run `dart run dev/tools/embed_assets.dart` before objective verification.
  - **Verify**: `bash dev/tools/fitness/check_design_system_sync.sh` exits 0 (it checks all three pairs; dry-run today exits 0, so re-run it *after* the canon edits and before the copy to confirm it goes red first – a check that was already green proves nothing about the re-sync); `dart test packages/dartclaw_server/test/static/design_system_icons_sync_test.dart packages/dartclaw_server/test/generated/embedded_assets_test.dart` passes; both generated asset files remain tracked

- [x] **TI15** S02's own visual deltas hold in both themes at desktop and 768px
  - Re-capture the five story-start targets (`showcase.html`, `/projects`, the seeded `/sessions/<id>`, `/settings`, `/scheduling`) in both themes at 1440×900 and 768px and compare only to the fresh captures taken before TI01. S02 owns the revised token values, all seven selector definitions, canon component bindings, showcase specimens and selector availability. It does **not** own downstream adoption defects: S08 closes tasks-table clipping and proves rendered scheduling-wide adoption; S16 adds `.t-metric` to shared `metricCard`; S07 and S08–S12/S15 apply `.t-*` classes to app templates and opt dense card headers down; each owning story records any dense-table overflow rather than reverting to 13px. The audit's 92-shot set is not used here — it serves only S01's audit-baseline gate and S14's release-level re-validation.
  - Re-check WCAG AA text contrast on every text run whose size changed (13→14px alias sites and 16→18px `--text-lg` sites), and verify the S02-owned outcomes: showcase tiers compute to their seven bindings; project card headers and canon `.section-title` compute to the heading tier; topbar title and markdown heading steps render as specified; scheduling's existing canon-governed header does not regress. A known downstream defect named above is a handoff, not an S02 failure, unless S02's selector or binding is absent.
  - **Verify**: no S02-owned target regresses against its story-start capture in either theme or viewport; every re-scaled text run measures ≥ 4.5:1 against its background; each downstream defect observed is recorded with its named owner, never absorbed into this story
...
- TI15 is the story's gate, not optional polish: compare the bounded S02 target set to the fresh captures taken before TI01. The 92-shot audit serves only S01's audit-baseline gate and S14's release re-validation, and downstream adoption failures route to the named owner rather than widening S02.
...
- Capture the five story-start visual targets before TI01. TI01 must complete before TI02–TI07: they all consume the revised token values. TI08–TI10 are independent of the token values and may run at any point before TI14.
- TI14 (re-sync + embed regeneration) must run after every canon and served-copy edit is final – a partial re-sync or stale embedded bundle leaves the story red. It now covers three CSS files, not two: TI09 and TI10 edit `icons.css`. Re-run TI14 after remediation that changes any embed root.
...
- [x] `rg -n 'var\(--text-sm\)' dev/design-system/components.css packages/dartclaw_server/lib/src/static/design-system.css` returns no matches, while `dev/design-system/tokens.css` and served `tokens.css` still carry the one migration alias S07 later deletes. With `BASE=.agent_temp/0.22.1-s02-entry`, the app-side snapshot confirms S02 did not absorb S07's migration.
- [x] No new `--text-*` size token was added; only `--container-wide` and `--measure` are new tokens.
- [x] `git diff --no-index --name-only --no-renames "$BASE/dev/design-system" dev/design-system` names exactly five changed canon files – `tokens.css`, `components.css`, `icons.css`, `DESIGN.md`, `showcase.html` – and `git diff --no-index -U0 "$BASE/dev/design-system/icons.css" dev/design-system/icons.css | rg '^-[^-]'` produces no output. The entry snapshot already contains S01 and every other accumulated checkout edit, so this proof isolates S02 without a commit or branch base.
- [x] The four hoisted type/icon obligations are present and no more: `components.css` gained the `.msg-user .msg-content p` rule and the complete `.section-title` heading-tier binding; `icons.css` gained four `--icon-*` tokens with their `.icon-*` mappings and exactly two `[data-icon]` mappings; nothing from another canon family landed here (`canon-hoist-manifest.md` routes surfaces/chrome to S01 and form/control/tab/state to S03).


## Implementation Observations

### Run: 2026-07-29 11:54 UTC – observations

#### NOTICED BUT NOT TOUCHING

- `dev/design-system/icons.css` — three `--icon-*` tokens are defined twice (`--icon-workflow` :149/:155, `--icon-clock` :69/:161, `--icon-search` :113/:167). Last declaration wins, so rendering is correct, but the duplicates are confusing. Pre-existing; needs an icon-inventory pass, not a type-story edit.
- `dev/design-system/icons.css` — `--icon-database` has a `[data-icon="database"]` mapping but no `.icon-database` class mapping (50 tokens vs 49 class mappings). Pre-existing.
- `dev/design-system/showcase.html:16,22` — two `var(--text-sm)` uses remain in showcase-only styles. `plan.json#sharedDecisions` assigns "the remaining app-side **and non-drift-checked demo usages**" to S07; left for S07 rather than absorbed here.
- `dev/design-system/components.css` `.text-sm` utility now resolves to the body size, so its name is misleading. ~20 consumers across `packages/dartclaw_server/lib/src/templates/` plus showcase; retiring the class name is an app-side change owned by S07/P3.
- `.card-metric .metric-value` (components.css) duplicates `.t-metric`'s four-property binding. S16 owns adopting `.t-metric` in the shared `metricCard` fragment; the duplicate rule should be deleted at that point or `.t-metric` adoption is decorative.
- `.card-header--sm` declares only `font-size` + `font-weight` as the FIS specifies, so it inherits `.card-header`'s `--tracking-tight` at 14px. Deliberate per spec; flagged in case the dense tier should carry `letter-spacing: normal`.
- `--measure` is `72ch`, which resolves against each element's own font-size. On `.msg-content h1`/`h2` that yields ~864px/~778px rather than the 605px prose measure. Headings are short so it is not visually load-bearing, and the selector list was fixed by the FIS.

#### DEVIATIONS / VERIFY RE-TARGETS

Both taken under the FIS's own Constraints & Gotchas rule: *"dry-run the cascade on the actual surface before asserting a per-surface outcome, and re-target the Verify to a surface the canon rule actually governs when it does not."*

- **Acceptance Scenario S07 / TI03 named `/projects`** for the `.card-header` proof. `/projects` renders app-local `.project-card-header`, not canon `.card-header`, so the canon rule does not govern that surface. Re-targeted to `/settings` ("Agent Configuration") and `/memory`, which do carry canon `.card-header` — both compute 18px/600, line-height 23.4px, letter-spacing -0.36px. The topbar half of S07 was verified on `/projects` as written (20px/600).
- **Canon `.section-title` has zero app consumers today.** The pairing templates use an app-local `pairing-section-title`, not the canon class. Its complete heading-tier binding was therefore verified directly against canon in `showcase.html` (18px/600/23.4px/-0.36px) rather than on a rendered app surface. The binding is what S07 consumes per `canon-hoist-manifest.md`.

#### OPEN QUESTION — DECIDE IN THE P1 WINDOW (S03/S04), NOT P3 — `.t-caption` tracking composition

TI02 prescribes `letter-spacing: normal` on every `.t-*` class, `.t-caption` included, while DESIGN.md § Typography (as TI12 also prescribes) documents that uppercase micro-labels *"compose `--tracking-caps` on top of `.t-caption`"*. Because the composite layer is declared last, a same-specificity rule declared earlier (a bare `.section-label`, for example) loses its caps tracking the moment `.t-caption` is added alongside it. Rules that out-specify the tier — `.data-table th` at (0,1,1) — are unaffected.

No consumer applies `.t-*` yet (P3 owns adoption), so this is latent rather than a live defect. Surfaced rather than silently deviating from an explicit FIS prescription.

**The decision window is S03/S04, not P3.** `plan.json#sharedDecisions` closes the drift-checked canon families after P1, so dropping `letter-spacing` from `.t-caption` is a legal one-line canon edit today and contract-banned once P1 ends. Routing it to P3 would guarantee it is never actioned: by the time a sweep adds `.t-caption` beside a bare `.section-label` and the caps tracking silently disappears, the fix is out of contract.

#### ORPHANS FIXED (caused by this story's own changes)

- `components.css` `.msg { width: 100% }` — `.messages` is a flex column and `.msg`'s auto side margins cancel the default stretch, so the bubble shrink-wraps its children. Once TI06's `--measure` gave prose a max-width, prose-heavy bubbles collapsed to the measure and dragged full-width children down with them: a table in a mixed message rendered 605px instead of the full bubble width, inverting OC03 and Acceptance Scenario S05. Fixed by giving `.msg` an explicit width basis; re-verified live (bubble 900px, prose 605px, table 866px, `pre` 866px). Found by the Step 4b review, not by the per-task Verify — S05's literal Verify passed while the outcome it encodes was broken.
- `showcase.html` "Size Scaling" row labelled its specimens 13px/16px, stale after TI01 moved the tokens; rebuilt on the real ladder (12/14/18/20/24).
- `tokens.css` `--tracking-tight` comment said "Display sizes"; the heading tier now uses it too.
- `DESIGN.md` § Code highlighting claimed code inside wells renders at `text-sm`, "a step below body" — both halves false after the alias re-scale, and its CSS twin was deleted in this same change.
- `icons.css` `--icon-file-warning` initially used `M12 12v4` / `M12 17h.01`-adjacent geometry that pushed the alert dot into the file frame; corrected to Lucide's `M12 9v4` / `M12 17h.01` and re-verified visually at 28px.

### Run: 2026-07-29 12:28 UTC – observations

#### CLOSE-OUT REMEDIATION — `.t-caption` caps-tracking composition, RESOLVED

The open question recorded in the previous run is settled. Orchestrator decision: fix inside S02 while the type family is open, because S03/S04 do not own type and post-P1 the change is contract-banned.

**The defect, measured before the fix.** Applying `.t-caption` to any single-class uppercase micro-label collapsed its caps tracking from 0.96px to `normal` — confirmed live on `.section-label`, `.msg-role` and `.sidebar-section-label`, and by inspection on `.tool-call-io-label`, `.notif-group` and `.palette-section`. All six declare `letter-spacing: var(--tracking-caps)` at specificity (0,1,0) and sit earlier in the file than the composite type layer, so `.t-caption`'s own `letter-spacing: normal` won on source order. `.data-table th` (0,1,1) and `.card-metric .metric-label` (0,2,0) were never at risk.

**The fix — one rule in the type layer, beside `.t-caption`:**

```css
.tracking-caps,
.t-caption.tracking-caps {
  letter-spacing: var(--tracking-caps);
}
```

The compound selector is what makes the composition order-independent: at (0,2,0) it beats both `.t-caption` and any equal-specificity component rule regardless of declaration order. The bare selector keeps the utility meaningful on its own so the class is not a silent no-op when used outside the caption tier (it is em-relative, so it yields 1.12px at the 14px body size and 0.96px at the 12px caption size).

**Constraints honoured.** All seven tiers still declare all four properties — `.t-caption` keeps `letter-spacing: normal`, so the composite-class shared decision is intact and no tier was weakened. The mechanism is a composition class, not a variant tier, which is what DESIGN.md documents.

**Consumer contract**, now explicit in DESIGN.md § Typography with a markup example and in the showcase type panel (two specimens: caption tier alone vs composed):

```html
<div class="section-label t-caption tracking-caps">SERVICES</div>
```

**Live impact: none.** No template or controller carries `class="tracking-caps"` today, and `.t-*` has no app consumers yet, so the rule is purely additive — every app surface renders byte-identically. It exists so the P3 sweeps that adopt `.t-caption` have a working composition path instead of silently dropping tracking on six components.

**Verification**: composition proven for all six at-risk components plus the bare tier and bare utility; all seven tiers re-checked at 4/4 properties; served CSS re-synced and embedded assets regenerated; drift check exit 0; icons-sync and embedded-parity tests pass; showcase specimen visually confirms the tracking difference; full CI-equivalent gate green.
