...
- [OC04] List, card and neutral-state surfaces have canonical search/toolbar, pagination, empty-state, absent-value, muted status-badge and header-actions treatments to adopt, instead of each inventing one.
...
> Only P1 stories S01-S04 author rules in the drift-checked `tokens.css`, `components.css` and `icons.css`; those rule families close after P1. S07 has one serialized, deletion-only exception: after app-side migration it deletes the `--text-sm` alias from `tokens.css`, re-syncs only served `tokens.css`, regenerates embedded assets and closes generated parity green. It may not edit `components.css`, `icons.css` or any other canon family. S05 authors and re-syncs nothing; every other P3 story consumes canon and reports a missing rule for hoisting to S01-S04. `DESIGN.md` and `showcase.html` remain writable because they are not drift-checked.

_This story is the hoist target for the **form / control / tab / state** family. Four obligations the P3 wave would otherwise discover after canon closes land here: the field-width scale and canonical invalid-state hook (both Forms, **TI02**), the `.tabs--sticky` material and overflow-affordance correction (**TI06**), the neutral `.status-badge-muted` treatment (**TI09**), and `.card-header-actions` (**TI10**). Consumers: S11 (settings), S15 (projects), S10 (disabled channel status), and S05 (the invalid-state swap). Each consumer arrives to a complete canon contract; a rule omitted here has no later author._
...
<!-- source: docs/specs/0.22.1/plan.json#sharedDecisions -->
<!-- extracted: e18cf85 -->
> S02 aliases `--text-sm` to `--text-base`, defines the seven composite classes and migrates every canon `components.css` consumer while the P1 type family is open. S07 later migrates app-side and non-drift-checked demo usages, then uses its one token-only exception to delete the alias from `tokens.css`, sync served `tokens.css`, regenerate embedded assets and close parity green. No other story introduces `--text-sm` or reopens a canon family.
...
- `docs/specs/0.22.1/canon-hoist-manifest.md` – why the field/invalid, tab and card-header rules discovered by the sweep land in this story rather than in their consumers. Read the "Consequences for the P3 stories" section together with S10's exact-selector arrival gate: S10, S11 and S15 have no post-P1 canon-edit right, so every known state-family gap must close here.
...
- `../dartclaw-public/packages/dartclaw_server/lib/src/web/channel_status.dart#ChannelStatus.disabled` – the existing `status-badge-muted` consumer whose class has no canon rule; S10 must find the exact badge selector in canon and served CSS on arrival.


## Acceptance Scenarios

- [x] **S01 [OC01] [TI01,TI02,TI03,TI04,TI12] The canonical form family renders a complete DartClaw form from canon alone**
...
- [x] **S02 [OC02] [TI05,TI06,TI12] One tab component survives the ten-tab settings case**
...
- [x] **S03 [OC03] [TI07,TI12] A resting ghost button reads as a control**
...
- [x] **S04 [OC04] [TI08,TI09,TI10,TI12] List, card and neutral-state surfaces have canonical toolbar, pager, empty, absent-value, muted badge and header-actions treatments**
  ...
  - **And** an empty `<span class="value-absent"></span>` renders the em dash as generated content in `--fg-sub0`, a `.meter` at 0% carries the `.meter--empty` treatment rather than reading as a solid rule, and `.status-badge.status-badge-muted` renders a legible neutral disabled/unavailable state without borrowing a semantic success, warning or error hue
...
- [x] **S05 [OC01,OC02] [TI01,TI02,TI03,TI04,TI05,TI06,TI13] Live surfaces change only where a canonical name already exists in the app, and change coherently**
  ...
  - **And** fresh story-start captures of `/settings`, `/scheduling`, `/knowledge`, `/projects` and the seeded `/sessions/<id>` (the chat surface; `/` redirects there – `/chat` is not a route) were taken before the first canon edit (the audit's 92-shot set is stale as a per-story comparator once S01 re-tones and S02 re-scales – it stays the release-level baseline S14 uses)
  - **When** the `visual` profile (port 3338) renders those surfaces in both themes at 1440×900 and 768px after the re-sync
...
- [x] `dev/tools/fitness/check_design_system_sync.sh` exits 0 – the served `design-system.css` is byte-identical to canon below its regenerated two-line `sha256:` provenance header.
- [x] `dev/design-system/components.css` remains at zero `var(--text-sm)` occurrences, preserving S02's completed canon migration.
- [x] No *unqualified* `input` / `select` / `textarea` / `label` / `fieldset` element selector is added to `components.css`; every new form and control rule is class-bound (element-*qualified* selectors such as `select.form-select` are the intended shape – see Constraints).
- [x] Every class this story adds has backing CSS, a DESIGN.md entry and a `showcase.html` demonstration – and nothing is documented that has no backing CSS.
- [x] `.status-badge-muted` exists in canon and the re-synced served CSS, is documented and demonstrated, and S10's disabled-channel arrival gate can resolve the exact selector without a post-P1 canon request. This does not add `.status-dot--muted`; S10 continues to pair the neutral badge with the existing `--idle` dot.
- [x] The only `z-index` literal this story adds is `.tabs--sticky`'s, so before S04 begins `rg -c 'z-index:\s*(1[0-9]|[2-9][0-9]|[0-9]{3,})' dev/design-system/components.css` returns exactly `5`: the three pre-existing canon literals (`20`, `15`, `100`), S01's temporary `.skip-link` `30`, and `.tabs--sticky` `10`. S04 TI01 converts all five; fewer means an upstream handoff is missing, not that this story is cleaner.
...
- `dev/design-system/components.css` – new Forms, Tabs, and Toolbar/Pager sections; revised Buttons, Cards (the `.card-header-actions` slot beside `#.card-header`), Status Badges (`.status-badge-muted`) and Empty-state/Meter rules; one named exemption added to the existing § 24 Reduced Motion block for the tab overflow indicator; the numbered category list in the file header.
...
- A `.status-dot--muted` variant -- `.status-badge-muted` is the missing neutral badge treatment and lands here; a muted dot is a separate motion/state vocabulary decision. S10 pairs disabled with the existing static `.status-dot--idle` and must not infer dot variants from badge suffixes.
...
**Why four obligations arrive before the sweep stories**: canon closes after P1, so a rule S10, S11 or S15 discovers mid-sweep has nowhere to land – later stories re-syncing the same served files would collide on the sha256 provenance line, and authoring canon during adoption inverts canon-before-adoption. The field/invalid, tab, muted-badge and card-header obligations all belong to this story's form/control/tab/state family. They land as first-class tasks (TI02, TI06, TI09, TI10), documented and demonstrated like every other primitive, so each consumer can prove the exact selector on arrival rather than defer a known missing rule.
...
file   | dev/design-system/components.css#.status-badge                        | Existing semantic badge variants omit the neutral `.status-badge-muted` class already emitted for disabled channel and guard states
...
- **Constraint**: `.status-badge-muted` is a neutral, static badge variant, not a synonym for warning and not a dot-family addition -- Must handle by: deriving its text, surface and border from neutral foreground/surface tokens, adding no pulse or semantic hue, and keeping S10's independent disabled dot suffix on `idle`.
...
- **Constraint (L3 resolution — stacking)**: S04 defines the `--z-*` ladder one wave later, so `var(--z-sticky)` does not exist while this story runs and a fallback (`var(--z-sticky, 10)`) would leave dead syntax in canon that no sweep catches. **Decision: `.tabs--sticky` declares the literal `z-index: 10`** (the value `app.css#.settings-tabs` uses today, below canon's existing 15/20/100) and S04 converts it. S01 also hands S04 the temporary `.skip-link` `30`, so the pre-S04 blanket count is exactly five. The handoff is a gate, not a coordination note: S04 TI01 must convert both deliberate literals plus the three pre-existing ones and end with no app-layer literal match. Record the tab literal in Implementation Observations at story close so S04's executor reads it rather than rediscovering it.
...
Before TI01, capture the story-start visual targets named under Validation in both themes at 1440×900 and 768px. Also snapshot the accumulating no-commit checkout exactly as S03 receives it; every Git-style own-delta assertion below compares to this entry, never to `main`, `HEAD` or a merge base:

```sh
BASE=.agent_temp/0.22.1-s03-entry
rm -rf "$BASE"
...
- [x] **TI01** Canon carries a Forms section covering the field, label, text-input, select, textarea, error and hint shapes the app invented
  - New numbered section in `dev/design-system/components.css`: `.form-field` (column stack), `.form-field--inline` (label left, control trailing – absorbs `app.css#.form-group-toggle`), `.form-field--checkbox` (control first, label after, `--sp-2` gap – absorbs `app.css#.form-group-checkbox`), `.form-row` (multi-field horizontal group – absorbs the app's `.form-row`, 10 rules / 13 uses), plus `.form-label`, `input.form-input`, `select.form-select`, `textarea.form-textarea`, `.form-error`, `.form-hint`. Derive surface, focus, `:focus-visible`, `:disabled` and `::placeholder` handling – plus `.form-error`'s empty/non-empty `min-height` reservation (app.css:841-842) that prevents validation layout shift – from `app.css#.form-input`, but add the `var(--inset-sm)` depth § Native selects requires. Controls set `font: inherit`; their base rules declare no `font-size`.
  - At `max-width: 768px`, canon retains `font-size: 16px` on **exactly** `input.form-input` and `select.form-select` (the native-control iOS focus-zoom floor). It belongs with the canonical controls, not in `app.css`; `textarea.form-textarea` is not added to this selector unless a separately evidenced native-zoom case requires it.
  - **Verify**: `rg -n '\.form-(field|label|input|select|textarea|error|hint|row)' dev/design-system/components.css` lists every class above; `rg -n '^(input|select|textarea|label|fieldset)[^.a-zA-Z0-9_-]' dev/design-system/components.css` prints nothing, while `printf 'input[type=checkbox] { margin: 0 }\n' | rg -n '^(input|select|textarea|label|fieldset)[^.a-zA-Z0-9_-]'` prints its line and `printf 'input.form-input { }\n' | <same pattern>` prints nothing – the two piped controls prove the pattern actually fires on an unqualified selector and spares the intended element-qualified one, so the empty result over `components.css` is a finding rather than a broken regex; at a 768px viewport, `getComputedStyle` reports at least `16px` for both `input.form-input` and `select.form-select`.

- [x] **TI02** The Forms section carries the canonical invalid state and the field-width scale — the two Forms rules hoisted out of the sweep wave
...
- [x] **TI03** Checkbox, radio and toggle controls exist in canon as CSS-only rules on native inputs
...
- [x] **TI04** DESIGN.md § Native selects has real backing CSS
...
- [x] **TI05** The system has exactly one tab component
...
- [x] **TI06** `.tabs` signals its own overflow, and `.tabs--sticky` sits on the ground rather than over it — the tab-family corrections hoisted out of the sweep wave
  ...
  - **Verify (rules exist)**: `awk '/^\.tabs--sticky/,/}/' dev/design-system/components.css | rg -n 'position: sticky|z-index: 10'` prints both lines (`rg -n 'tabs--sticky' dev/design-system/components.css` returns nothing today, so the awk range is empty until this task lands); `rg -c 'z-index:\s*(1[0-9]|[2-9][0-9]|[0-9]{3,})' dev/design-system/components.css` returns exactly `5` – the pre-existing `20`, `15`, `100`, S01's temporary `.skip-link` `30`, and this task's `.tabs--sticky` `10`. S04 TI01 must see and convert all five; a count of four means the skip-link handoff never arrived.
  ...
  - **Verify (sticky material)**: a `.tabs--sticky` bar pinned over scrolled content shows no hard-edged seam against the body gradient in either theme, at 768px and 1440×900

- [x] **TI07** The button vocabulary covers size and gives ghost buttons a rest-state affordance
...
- [x] **TI08** List surfaces have a canonical search toolbar and pager to adopt
...
- [x] **TI09** The state family covers empty, absent, zero-value and muted-status cases
  ...
  - Add `.status-badge-muted` beside the existing status-badge variants. It is the neutral static treatment for disabled/unavailable states already emitted by `ChannelStatus.disabled` and the guards-disabled row: neutral foreground, neutral surface tint and neutral border only, with no success/warning/error hue and no animation. Do not add `.status-dot--muted`; S10's disabled presentation keeps the independent `idle` dot suffix.
  - **Verify**: an empty `<span class="value-absent"></span>` renders `–` in the showcase; a `.meter` at 0% is visibly distinct from the same meter with no `--empty` modifier; `.status-badge.status-badge-muted` is legible and visibly neutral in both themes; `rg -n 'empty-state-title|value-absent|meter--empty|^\.status-badge-muted' dev/design-system/components.css` returns all four rules, while `rg -n '^\.status-dot--muted' dev/design-system/components.css` returns no matches

- [x] **TI10** A card header has a right-aligned actions slot — the Cards rule hoisted out of the sweep wave
  ...
  - **Verify**: `rg -n '^\.card-header-actions' dev/design-system/components.css` returns the rule (nothing today – exit 1); with `BASE=.agent_temp/0.22.1-s03-entry`, `git diff --no-index -U0 "$BASE/dev/design-system/components.css" dev/design-system/components.css | rg -n '^-.*card-header\b'` prints nothing, while the same entry-snapshot diff piped to `rg -n '^\+.*card-header-actions'` prints the addition – together proving the slot was *added* beside `.card-header` rather than by rewriting it, even though earlier stories are already present in the checkout; in the showcase, a card header with the slot and one without render identically apart from the trailing buttons, in both themes

- [x] **TI11** DESIGN.md documents every family this story adds, and documents nothing without backing CSS
  - Frontmatter `components:` entries in the existing `backgroundColor` / `textColor` / `typography` / `rounded` / `padding` shape for the new families; § Components gains Forms, Tabs and Toolbar/Pager subsections in the table-plus-prose style; § Native selects states the prohibition is now backed; § Buttons gains the size tier and the ghost rest-state rule; the empty-state guidance gains the title/action/absent-value rules; and § Status badges names `.status-badge-muted` as the neutral disabled/unavailable treatment, distinct from every semantic hue and from the dot vocabulary. The four hoisted obligations are documented like any other primitive, not as footnotes: the Forms subsection states the invalid-state contract (`[aria-invalid="true"]` / `:user-invalid` are equivalent hooks, and an invalid control always renders its `.form-error` message – the non-colour signal the NFR requires) and the field-width scale with its two values and when to reach for each; the Tabs subsection states the sticky material and the overflow contract — that a `.tabs` bar scrolls rather than wraps, signals its own overflow with a right-edge indicator, and degrades to a hidden indicator on engines without scroll-driven animations, so a consumer knows what it is adopting and does not re-invent a wrap; `.status-badge-muted` closes S10's known disabled-state selector gap; and `.card-header-actions` joins the § Cards *card sub-elements* table beside `.card-header` / `-body` / `-footer`. Depends on the final class names settled in TI01-TI10.
  - **Verify**: every class introduced in TI01-TI10 appears in DESIGN.md, and every class DESIGN.md's new prose names resolves to a rule in `components.css` (cross-grep both directions, zero misses); `rg -n 'card-header-actions|form-input--num|form-input--short|aria-invalid|user-invalid|tabs--sticky|status-badge-muted' dev/design-system/DESIGN.md` returns all seven (nothing today – exit 1), so a hoisted rule cannot ship undocumented and leave its consumer story with no contract to read

- [x] **TI12** showcase.html demonstrates every new class
  - Panels for Forms (all controls, both states, plus the invalid state and the two width modifiers), Tabs (a ten-tab bar that genuinely overflows at 768px — give the demo bars stable hooks such as `.tabs-demo-ten` / `.tabs-demo-four` so TI06's overflow assertions can address them — a four-tab bar that fits, and a sticky bar over enough scrolled content to pin), the extended Buttons panel, list toolbar + pager, the expanded empty state, a neutral `.status-badge.status-badge-muted` specimen, and a card header with and without the actions slot – following the existing `<h2>` + `.well-content` panel structure. Delete the `style="color:var(--fg)"` inline override on `<strong>No sessions yet</strong>` that the canon's own showcase currently needs, since `.empty-state-title` from TI09 replaces it.
  - **Verify**: `rg -n 'style="color:var\(--fg\)">No sessions yet' dev/design-system/showcase.html` prints nothing while `rg -c 'No sessions yet' dev/design-system/showcase.html` still returns 1 – the override is gone and the panel is not (the unrelated Approval Gates override at showcase.html:601 stays); `rg -n 'card-header-actions|form-input--num|form-input--short|aria-invalid|tabs--sticky|status-badge-muted' dev/design-system/showcase.html` returns all six (nothing today – exit 1); and each class from TI01-TI10 appears at least once in `showcase.html`

- [x] **TI13** The served CSS and tracked embedded bundle match canon, and the story adds no `--text-sm` usage
  - Regenerate `packages/dartclaw_server/lib/src/static/design-system.css` as the two-line `/* Synced from dev/design-system/components.css on <date>. sha256: <hash> */` header plus the canon body; re-run after any remediation that touches canon. Run `date +%Y-%m-%d` for the header date. `icons.css` also needs re-syncing because TI04 adds the chevron token pair; `tokens.css` only if a task introduced a token. After the final served-copy change, run `dart run dev/tools/embed_assets.dart` before objective verification; re-run after any remediation that changes an embed root.
  - **Verify**: `bash dev/tools/fitness/check_design_system_sync.sh` exits 0; `dart test packages/dartclaw_server/test/generated/embedded_assets_test.dart` passes; `rg -n 'var\(--text-sm\)' dev/design-system/components.css packages/dartclaw_server/lib/src/static/design-system.css` returns no matches, preserving S02's zero-reference boundary; `rg -n '^\.status-badge-muted\s*\{' dev/design-system/components.css packages/dartclaw_server/lib/src/static/design-system.css` returns one exact selector in each file, closing S10's arrival gate; and both generated asset files remain tracked

### Testing Strategy

### Validation

- Visual validation runs against two targets, not one: `showcase.html` opened directly for the new primitives (it links only canon CSS), and the `visual` testing profile on port 3338 for the interaction set on live surfaces – `/settings` and `/scheduling` (forms and selects), `/knowledge` (pager, search field), `/projects` and the seeded `/sessions/<id>` (ghost buttons), plus `/tasks` (small buttons) and a channel-detail allowlist form (the `:user-invalid` delta: submit the required entry field empty after touching it, and confirm the treatment appears only after interaction, never on page load).
...
- Both themes at 1440×900 and 768px for each target.

### Execution Contract

- TI13 must be the last canon- or embed-root-touching action. If review or remediation edits `components.css`, `tokens.css`, `icons.css` or any served static asset after TI13, re-run the re-sync, `dart run dev/tools/embed_assets.dart`, the generated parity test and the drift check before declaring the story done – every gate closes green.
...
- [x] With `BASE=.agent_temp/0.22.1-s03-entry`, `git diff --no-index --name-only --no-renames "$BASE/dev/design-system" dev/design-system` names exactly `components.css`, `icons.css`, `DESIGN.md` and `showcase.html`; `cmp -s "$BASE/dev/design-system/tokens.css" dev/design-system/tokens.css` exits 0. The entry already contains every earlier accumulated edit, so this is S03's own canon delta rather than a branch-base diff.
- [x] The equivalent entry-snapshot diff over `packages/dartclaw_server/lib/src/static` names exactly `design-system.css` and `icons.css`; no controller, template or `app.css` change is attributed to S03.
- [x] `rg -n '^\.status-badge-muted\s*\{' dev/design-system/components.css packages/dartclaw_server/lib/src/static/design-system.css` returns one exact selector per file, and `rg -n 'status-badge-muted' dev/design-system/DESIGN.md dev/design-system/showcase.html` proves the documented and rendered contract S10 must find on arrival. `.status-dot--muted` remains absent by design; disabled pairs with `status-dot--idle`.


## Implementation Observations

### Run: 2026-07-29 14:33 UTC – observations

#### HANDOFF: `.tabs--sticky` z-index literal — for S04 executor

`components.css` `.tabs--sticky` declares the literal `z-index: 10` (the value `app.css#.settings-tabs` uses today), per the L3 decision in this FIS Constraints: `var(--z-sticky)` does not exist until S04, and a `var(--z-sticky, 10)` fallback would leave dead syntax no sweep catches.

`rg -c "z-index:\s*(1[0-9]|[2-9][0-9]|[0-9]{3,})" dev/design-system/components.css` returns exactly **5**: `30` (S01 temporary `.skip-link`), `20`, `15`, `100` (all three pre-existing), and `10` (`.tabs--sticky`). S04 TI01 must convert all five and end with no app-layer literal match.

#### HANDOFF: tab overflow affordance — validated behaviour, for S11

Validated on **HeadlessChrome 146.0.0.0 (Chromium)**, which supports scroll-driven animations.

- Ten-tab bar at 768px genuinely overflows (scrollWidth 929 > clientWidth 720). At scrollLeft 0 the right-edge cue is at opacity 1 and the last tab is not fully visible; scrolled to the end the cue is at opacity 0 and the last tab is fully readable. The four-tab bar (scrollWidth === clientWidth) shows opacity 0 at every scroll position. maskImage is `none` on both, and the border-bottom runs unbroken to the right edge.
- **Holds under `prefers-reduced-motion: reduce`** — the section 28 exemption (`.tabs::after { animation-duration: auto !important; }`) survives the blanket `0.01ms !important`, and the cue still tracks scroll position. Verified with reduced motion emulated both on and off.
- No engine tested degraded the cue. On an engine without scroll-driven animations `animation-timeline` is dropped, the duration resolves to 0 and `both` pins the end keyframe, so the cue is simply hidden — today behaviour, never a stuck-visible cue.
- Scroll timelines settle on the next animation frame: reading getComputedStyle in the same task as a scripted scrollLeft assignment returns a stale value. Assert in a separate evaluation after a frame, or the check passes vacuously.

Two corrections to the mechanism as written in this FIS, both required to make it work at all:

1. **`scroll(self inline)` became `scroll(nearest inline)`.** The animation runs on `.tabs::after`, and a pseudo-element is not a scroll container, so `self` resolves to an *inactive* timeline and the cue never appears — it silently held its hidden base state while every computed assertion still passed. `nearest` resolves to `.tabs`, the bar's own scroller. Overflow-conditionality is unchanged: a scroll container with no scrollable range in the axis drives an inactive timeline either way (confirmed on the four-tab bar).
2. **The cue tints with `--fg`, not a surface token.** The FIS "darkens rather than covers" reads naturally as a crust-toned scrim, but `--bg-crust` is only below the ground in dark theme; in light it sits *above* the ground, so a crust tint resolved to roughly the ground's own colour and measured ~1.03:1 — invisible, while passing every opacity assertion. `--fg` is the one token guaranteed to contrast with the ground in both themes. The decisive property the FIS names (surface-independence: it tints rather than covering, so it never has to match a varying backdrop and never bands) is preserved.

#### `.tabs-actions` at 768px and below

`app.css#.tab-header-actions` goes full-width below the tabs at <=768px. That shape is **not** carried into canon: `.tabs` is `flex-wrap: nowrap` by the Architecture Decision (scroll, never wrap), so a full-width wrap inside the scroller is not expressible. The absorbed idiom is the one TI10 restates — `margin-left: auto` + flex + `--sp-2` gap — which is what shipped. The auto margin claims free space while the bar fits and collapses to zero once it overflows, leaving the actions directly after the last tab and reachable by the same scroll (verified clickable at scroll end at 768px; the cue carries `pointer-events: none` so it never blocks them).

#### NOTICED BUT NOT TOUCHING

- `dev/design-system/icons.css` declares `--icon-search` twice inside the same `:root` with byte-identical values. Pre-existing and harmless (later wins, same value); deduping is an icons-family edit this story has no mandate for.
- `dev/design-system/DESIGN.md` names four classes with no canon backing: `.chip--file`, `.page-content`, `.page-inner`, `.page-inner--wide`. All predate this story (present in the S03 entry snapshot); the `.page-*` trio are app-owned layout primitives DESIGN.md documents deliberately.
- DESIGN.md frontmatter `colors:` has no `bg-ground-edge` entry although tokens.css defines the token (S01). The `tabs-sticky` component entry therefore references `{colors.bg-base}` with a clarifying comment.
- `components.css#.btn`'s own border (`--border` = `bg-surface0`) sits far below 3:1 against every plane, so a governed `.btn-ghost` now carries a *more* visible resting boundary than the default button. Raising `.btn` border is a Buttons-family decision with release-wide blast radius, not an S03 fix.
- `packages/dartclaw_server/lib/src/templates/channel_detail.html` ships `class="form-checkbox"` (3 occurrences) with **no** app.css rule behind it, so canon checkbox treatment reaches those controls on re-sync. `.form-checkbox` appears in neither of Scenario S05's lists, so the enumeration is incomplete rather than wrong. `/channels` is not routable on the `visual` profile, so this could not be validated live here — S10 should confirm on the channels profile.
- Two further live deltas trace to names Scenario S05 does not enumerate: `.status-badge-muted` (already emitted by `ChannelStatus.disabled` and the guards-disabled row; visible on `/scheduling`) and `.empty-state .btn` spacing. Both are canon rules this story was required to add; the interaction-set list simply predates them.
- `.form-error` min-height pair was promoted verbatim from app.css as TI01 directed, but it does **not** reserve the line in advance, so raising and clearing an error still reflows the fields below. The FIS TI01 wording ("prevents validation layout shift") overstates what the rule does; the shipped comment and DESIGN.md row describe the real behaviour.
- Live `.btn-sm` still renders a 12px label because app.css's own `.btn-sm` loads later and wins on font-size. Canon side is clean (no font-size); S05 deletion closes it.

### Run: 2026-07-29 14:49 UTC – observations

#### Post-review notes (quick-review pass)

**`.btn-icon-sm` touch-target bump — considered and deliberately NOT shipped, for S05.**
A review pass proposed carrying `app.css`'s `@media (max-width: 1024px)` 44px bump for `.btn-icon-sm` into canon, on the grounds that S05's purge of the app rule would otherwise shrink five live icon buttons from 44px to 28px on tablets. It was implemented, then reverted: it contradicts Acceptance Scenario S03 (`.btn-sm` differs from `.btn` by compact padding and a smaller `min-height` **only**) and inverted the size tier, leaving the *small* icon button (44px) larger than `.btn-icon` (32px, never bumped) below 1024px. Canon now has no 1024px breakpoint and the tier is monotonically smaller again.

**S05 must decide this when it deletes `app.css#.btn-icon-sm`.** Options: accept 28px (still clears WCAG 2.5.8 AA at 24px; 44px is 2.5.5 AAA), or reinstate a touch bump for the whole icon-button family rather than the `-sm` tier alone — and prefer `@media (pointer: coarse)` over a viewport-width proxy, since 1024px misses in both directions (a 1000px desktop mouse window gets big targets, a 1366px tablet keeps small ones).

**`.list-toolbar .icon-search` orphans below ~270px of container width.** The glyph is a separate flex item whose negative margin gives it near-zero size, so once `.form-input--search` (`flex: 1 1 16rem`) wraps to the next flex line the glyph stays behind on the previous one. Below roughly 270px of toolbar content width the search icon floats over nothing. Not fixed here: the robust fix is a positioned glyph+field sub-container, which is new markup contract this story has no mandate for. No shipped surface reaches that width today.

**`type="search"` vs `type="text"` for `.form-input--search` is undecided.** The showcase demo uses `type="search"`, and WebKit/Blink render a native `::-webkit-search-cancel-button` inside the otherwise fully-branded control once text is entered — inconsistent with the section's whole premise of removing native chrome. Canon neither resets that pseudo-element nor states which input type it sanctions. Whichever story adopts the toolbar should settle it: reset the cancel button, or document `type="text"` as the canonical markup.

### Run: 2026-07-29 17:07 UTC – observations

#### POST-CLOSE HOIST: two mobile touch-target floors (orchestrator-directed, during S05)

S05s purge of `app.css` exposed two <=768px floors the app carried and canon never absorbed. Both land in S03s families. Canon-only change: `components.css` + `DESIGN.md`, served copies re-synced, embedded assets regenerated, drift + parity green. No `app.css`, template or controller edit (S05 was concurrently mutating those). plan.json stays `done`.

**1. `.form-toggle` <=768px 48px box — WCAG 2.5.8 AA regression, mandatory.**
Measured 36x20 post-purge on live `/scheduling` (20px tall, under the 24px minimum); was 48x48 while `app.css#.toggle-switch` still existed. Canon now takes a 48px `min-width`/`min-height` on the wrapper at <=768px and re-centres the slider at its unchanged 36x20 via `inset: 50% auto auto 50%` + `translate(-50%,-50%)`. The switch does not get bigger; its target does.

**The first implementation was wrong and is worth not repeating.** Growing only the `input` (negative insets: `inset: -14px -6px`) buys the 48px target for free because the input is out of flow — no layout cost, slider untouched. It measured perfectly in isolation. But stacked toggle rows sit on a ~28px pitch, so 48px hit areas reached 14px into the rows above and below: measured **20px of vertical overlap between every adjacent pair**, i.e. a tap near a row boundary flips the wrong setting. Mis-activating an adjacent toggle is worse than a small target. The box has to *reserve* the space, which is what every other 48px floor in the file (`.btn`, `.sidebar-nav-item`, `.input-area textarea`) already does. Cost: toggle rows are taller on mobile — documented in DESIGN.md § Forms.

Verified: showcase at 768px, hit area 48x48, slider still 36x20, knob still 14x14, **zero overlapping pairs** (pitch 28 -> 56), both themes; live `/scheduling` at 768x900 hit area 48x48, WCAG 2.5.8 pass, no console errors.

**2. `.tab` <=768px 48px `min-height` — TI04(a) gate preservation.**
Anchor tabs measured 91x36 post-purge (clears 2.5.8, but meaner than every other control on the same screen; button tabs previously floored via a generic app rule). Canon now floors both forms uniformly.

`.tab` also gained `display: inline-flex; align-items: center` in its base rule — not cosmetic: a `<button>` centres its own content and an `<a>` does not, so once the floor raises the box past the text height the two tab forms would sit differently. Without it the anchor bar renders its label top-aligned.

Verified live after S05s swap: `/settings` 10 anchor tabs all exactly 48px with zero legacy `.settings-tab` remaining; `/memory` 4 button tabs >=48px, zero legacy `.tab-btn`. The overflow cue still fires on the real settings bar (scrollWidth 878 > clientWidth 736, cue opacity 1 at scrollLeft 0) — the audits "ten tabs reading as nine" is closed on the live surface, not just in the showcase.

**Measurement trap for whoever re-checks this.** A `.tabs` bar containing a `.tabs-actions` slot measures taller than the floor because the slot holds a `.btn`, which takes the pre-existing 48px mobile floor and stretches the flex line (showcase four-tab bar reads 53px, ten-tab likewise). Measure a bar with **no** actions slot to see the tab floor itself — the showcase sticky demo is the clean specimen.

**Note for S04/S07:** `.tabs--sticky` no longer carries the `z-index: 10` literal this story shipped — S04 already converted it to `var(--z-sticky)`. The earlier handoff record above is satisfied; the "exactly 5 literals" count it cites no longer applies.

### Run: 2026-07-29 18:19 UTC – observations

#### POST-CLOSE HOIST 2: `textarea.form-textarea` joins the <=768px 16px floor

TI01 shipped the iOS focus-zoom floor on `input.form-input` and `select.form-select` only, with the rule comment holding textarea out until "a separately evidenced native case" arrived. It has arrived, so the third text-entry control joins the group. Canon-only: `components.css` + `DESIGN.md`, served copies re-synced, embedded assets regenerated, drift + parity green, my two assets verified byte-identical in the bundle (`design-system.css` 120770B, `icons.css` 36842B).

**The evidence, measured on the running visual profile at 768px before the change** (not taken on report):
- `/settings` compact instructions (`textarea.form-textarea`) computed **14px** while `input.form-input` and `select.form-select` in the same form computed 16px.
- `/tasks` new-task description and acceptance criteria: **14px** each.
- Two more are dynamic rather than in the static DOM: the task-detail pushback box and the workflow-variable inputs the workflows controller generates.
- These five regressed *by being adopted*: they previously carried an input class and took the app stylesheet mobile floor, and dropped to 14px the moment they moved onto the canonical textarea class.
- Two textarea families were also disagreeing on the same device — the app keeps its own 16px floor for bespoke `.well-content .form-row` textareas, so one textarea zoomed on focus and the one beside it did not.

**After**, same viewport: `/settings` 16px, `/tasks` both 16px, each matching the input beside it. Desktop at 1440 still computes 14px, so the floor stays viewport-scoped and no desktop type regressed. Rule confirmed live over HTTP on the shared server.

**Specificity note for anyone extending this block.** `textarea.form-textarea` is (0,1,1), the same as the base control rule that sets `font: inherit`, so the media block wins on source order, not weight — keep it after the base group. The app-side `.well-content .form-row textarea` at (0,2,1) still out-specifies canon here, but it sets 16px too, so the two agree rather than fight. Unrelated: the app carries a *separate* comment about textareas being absent from a dialog `min-height: 48px` group — that one is about height, not font-size, and is untouched by this change.

### Run: 2026-07-30 04:17 UTC – observations

#### POST-CLOSE HOIST 3: empty-state glyph rendered as a solid accent square

Canon-only fix in the empty-state family. `components.css` + `showcase.html` + `DESIGN.md`; served copies re-synced, embedded assets regenerated, drift + parity + analyze + format green. `icons.css` untouched (frozen family). No template, controller or `app.css` edit.

**Mechanism, confirmed by reading both rules and then reproducing it.** The shared empty-state fragment emits `<div class="icon" aria-hidden="true">&#10095;_</div>` when the mascot is off. The base `.icon` is mask machinery: `background-color: currentColor` painting a 1em box that a mask-image is expected to cut out. Canon`s `.empty-state .icon` is a *text* treatment (font-size, accent colour, glow) written for that typed glyph, and it never neutralised the base fill. With no mask there is nothing to cut, so the box stayed solid — and because the fill and the text both resolve to `currentColor`, the glyph was invisible inside its own accent square. Measured before the fix: `background-color` and `color` both `rgb(166, 227, 161)`, `mask-image: none`, 24x24 box.

**Fix.** `.empty-state .icon:not([class*="icon-"])` drops `background-color` AND the 1em `width`/`height`. Both halves are needed: dropping only the fill leaves the two-character mark pinned to 1em, so the underscore wraps and lands orphaned under the chevron — which I only caught by looking at the rendered result after the first fix, not at the computed values, which already looked correct.

**Why the `:not()` rather than a blanket reset.** An `.icon` carrying an `.icon-<name>` modifier is a genuine masked icon and still needs the fill and the square to paint through its mask; a blanket `background-color: transparent` would silently blank every masked icon anyone later puts in an empty state. No empty state uses one today, so a blanket rule would have passed every check I could run and shipped a trap into canon instead. Both paths are now demonstrated side by side in the showcase so the distinction is visible rather than asserted.

**The real cure is upstream and not available to canon:** a text glyph should not wear a mask-system class at all. That name is in the shared fragment, not in `components.css`. Whoever owns that fragment should give the typed mark its own class; this rule can then be deleted.

**Verification.** Showcase, both themes: bare glyph renders the accent prompt mark on one line with its glow, `background-color` transparent; `.icon.icon-file-text` beside it still masks and fills correctly (dark `rgb(166,227,161)`, light `rgb(36,102,28)`).

**Could NOT verify on the named live surface, and it is worth knowing why.** `/health-dashboard` guard audit does not currently render the shared fragment at all — its empty state is an inline `<div class="empty-state" style="padding:...">No guard events recorded yet</div>` with no `.icon`. Sweeping `/health-dashboard`, `/memory`, `/knowledge`, `/projects`, `/tasks`, `/workflows`, `/scheduling` and a task timeline found zero `.empty-state .icon` on the running profile: the only two fragment callers (tasks list, task timeline) both have seeded content, so the empty branch never renders. The defect is real in canon regardless of which template currently reaches it, and it was reproduced with the exact markup the fragment emits — but nobody should record this as "verified on the guard-audit empty state", because that surface no longer has the shape.

#### NOTICED BUT NOT TOUCHING (icons family, frozen)

`icons.css` defines the token `--icon-database` but ships **no matching `.icon-database` class rule**, so `<span class="icon icon-database">` renders as a bare filled square with no mask — the same visual failure this hoist fixes, from a different cause. Found while building the masked-icon specimen (I had to switch it to `.icon-file-text`). Out of scope here: the icons family is frozen and this is not the empty-state family. Worth a check for other tokens without class rules.

### Run: 2026-07-30 04:21 UTC – observations

#### CORRECTION to Hoist 3 — the guard-audit surface DOES have the fragment shape

Retracting one claim from the hoist-3 block above: "`/health-dashboard` guard audit does not currently render the shared fragment ... that surface no longer has the shape" is **wrong**, and anyone reading this for release verification should ignore it.

The surface does compose the fragment — `packages/dartclaw_server/lib/src/templates/audit_table.dart:54` calls `emptyStateTemplate(...)`. My sweep queried the **running** 3338 profile, and a `.dart` template is compiled into that server VM rather than read from disk per request, so the process was still serving markup from before that adoption landed. The stale-VM trap bit the verification sweep rather than the edit path.

What that changes: nothing about the fix, and nothing about the evidence. The defect and the repair were both reproduced against the exact markup the fragment emits, and the canon rule is theme-verified. What it changes is the *reachability* claim — the guard-audit empty state is a real consumer, so re-verification on a freshly started instance is meaningful rather than pointless.

Generalised, because it will catch the next sweep too: a live query against a long-running profile only proves what that VM was built with. `.html` templates and the served CSS reload from disk; `.dart` templates do not. When a sweep finds "no consumers", confirm against source before concluding the shape is gone.
