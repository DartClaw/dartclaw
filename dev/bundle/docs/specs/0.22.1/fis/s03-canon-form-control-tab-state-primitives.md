...
- [ ] **S04 [OC04] [TI08,TI09,TI10,TI12] List, card and neutral-state surfaces have canonical toolbar, pager, empty, absent-value, muted badge and header-actions treatments**
...
- [ ] `dev/design-system/components.css` remains at zero `var(--text-sm)` occurrences, preserving S02's completed canon migration.
...
- [ ] `.status-badge-muted` exists in canon and the re-synced served CSS, is documented and demonstrated, and S10's disabled-channel arrival gate can resolve the exact selector without a post-P1 canon request. This does not add `.status-dot--muted`; S10 continues to pair the neutral badge with the existing `--idle` dot.
- [ ] The only `z-index` literal this story adds is `.tabs--sticky`'s, so before S04 begins `rg -c 'z-index:\s*(1[0-9]|[2-9][0-9]|[0-9]{3,})' dev/design-system/components.css` returns exactly `5`: the three pre-existing canon literals (`20`, `15`, `100`), S01's temporary `.skip-link` `30`, and `.tabs--sticky` `10`. S04 TI01 converts all five; fewer means an upstream handoff is missing, not that this story is cleaner.
...
- [ ] **TI09** The state family covers empty, absent, zero-value and muted-status cases
...
- [ ] **TI13** The served CSS and tracked embedded bundle match canon, and the story adds no `--text-sm` usage
