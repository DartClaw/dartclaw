...
- [ ] After the final template/static change, `dart run dev/tools/embed_assets.dart` refreshes the tracked generated bundle before objective verification.
- [ ] The full `dartclaw_server` test suite is green, including `test/generated/embedded_assets_test.dart`; no allowed-red generated test and no narrowed parity coverage.
...
- [ ] **TI06** Canon, served CSS and the tracked embedded bundle are provably current, and no canon name keeps an `app.css` rule head
...
- [ ] `bash dev/tools/fitness/check_design_system_sync.sh` exits 0, with `BASE=.agent_temp/0.22.1-s05-entry`, `git diff --no-index --quiet "$BASE/dev/design-system" dev/design-system` exits 0, and both generated asset files remain tracked
- [ ] `dart test packages/dartclaw_server` is fully green, including `test/generated/embedded_assets_test.dart`
