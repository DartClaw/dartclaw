...
- [OC01] No native browser dialog appears anywhere in the Web UI – every confirmation renders as the canonical in-app dialog in both themes, and any `hx-confirm` attribute added in future converts automatically without touching JavaScript.
- [OC02] Every destructive confirmation this story ships names the object it will destroy by the same label the user just read on screen – the chat's title, the project's name, the guard rule's displayed value. For the scheduled-task row the same guarantee is carried by S08's inline bar; this story's part is the `data-task-title` attribute that feeds it, because the call site passes the id today.
...
- [ ] **S01 [OC01] [TI01,TI04] Deleting a chat confirms in the app's own dialog**
  ...
  - **Then** a `<dialog>` carrying `dialog--confirm` is present in the DOM and open, its body names the session's title as shown in the sidebar, the page behind it is dimmed by the canonical scrim, no OS dialog appears, and the JS event loop keeps running (an in-flight SSE update still lands); pressing Escape closes it and issues no `DELETE /api/sessions/…` request
...
- [ ] **S02 [OC01] [TI02] `hx-confirm` markup routes through the same dialog with the attribute untouched**
  ...
  - **Then** the same `dialog--confirm` frame renders the attribute's text as its body with a `btn btn-danger-fill btn-sm` confirm control, and on confirm the original htmx request is issued exactly once with no second confirmation prompt
...
- [ ] **S04 [OC02] [TI05] The scheduled-task delete drops its native dialog and hands the title to S08's inline bar**
  ...
  - **Then** the control carries `data-task-title="Visual review seed task"` alongside its existing `data-task-id="visual-review-seed"`, and clicking it raises no OS dialog and creates no `dialog.dialog--confirm` — the row-scoped replacement is S08 TI13's inline `.delete-confirm-bar`, which this attribute feeds, and the title-not-id assertion travels with the pattern (`plan.json#sharedDecisions`); with a task titled `Deploy "prod" <now> & wait` the attribute carries that text `tl:attr`-escaped, so S08's `textContent` render shows it literally
...
- `packages/dartclaw_server/lib/src/static/controllers/dc_scheduling_controller.js` – deletion of the native `confirm()` from `deleteScheduledTask` and nothing else; the replacement pattern is S08's
- `packages/dartclaw_server/lib/src/templates/scheduling.html` – `data-task-title` on the scheduled-task delete control, the one artefact this story hands to S08 TI13's inline bar
...
- Building the scheduled-*task* delete's replacement confirmation – `plan.json#sharedDecisions` rules that row-scoped destructive actions resolve to the inline `.delete-confirm-bar`, and S08 TI13 generalises `confirmDeleteJob`'s construction to cover both scheduling rows. This story removes the native `confirm()` at that call site (TI05) and emits the `data-task-title` attribute S08's bar reads; it adds no modal there.
...
**Boundary of the "one implementation" rule** — read before TI07. `confirmDialog()` is the single **confirmation** API: every yes/no destructive gate in the product goes through it, and no story writes a second one. It is deliberately not a dialog *framework* — its signature (`{title, body, confirmLabel, danger}`) has no field slot, and widening it into a generic form-builder would trade one small helper for a mini UI library, which the release's no-new-capabilities constraint forbids. So the guard-edit dialog (TI07) is built inline in `dc_settings_controller.js`, composing S04's canon `.dialog` frame and S03's `.form-*` controls directly. That is **not** a second dialog implementation: it defines no frame, no scrim and no width rule of its own, and it is exactly the mechanism S04's feedback decision table assigns to the *structured input* row (`.dialog` with real form controls). The rule the criteria enforce is therefore: one confirmation API, one canon frame, zero forked frames — not one dialog element in the codebase.
...
- **Avoid**: adding `confirmDialog` to the `window.dartclaw.ui` compatibility namespace – Instead: import it from `./shared.js` in each controller that calls it (`dc_shell_controller.js`, `dc_projects_controller.js`, `dc_settings_controller.js`). `dc_scheduling_controller.js` is **not** one of them: TI05 only deletes a call there, so the file keeps reaching feedback helpers through `this.ui` and gains no import.
- **Critical**: TI05 opens a confirmation gap that only S08 closes. Between this story (P3/W1) and S08 TI13 (P3/W2) the scheduled-task delete fires on a single click with no confirmation of any kind — an interim regression of the same class as the plan's documented ones, deliberate under `sharedDecisions`, and closed inside P3 so it never reaches a release boundary. Must handle by: recording it in Implementation Observations at story close (per `executionNotes`, that is the only block S14 reads) rather than patching it with a modal, which would be the second confirmation API S08 forbids. If S08 is descoped, this reopens as a real defect — say so in the observation.
...
- [ ] **TI02** Every `hx-confirm` attribute in the app renders as the canonical dialog, with no template edits
  ...
  - **Verify**: `Test: rg -n "htmx:confirm" packages/dartclaw_server/lib/src/static/controllers/ | wc -l` is 2 (one add, one remove) and `rg -n "issueRequest\(true\)" packages/dartclaw_server/lib/src/static/controllers/dc_shell_controller.js` matches; `git diff --stat -- packages/dartclaw_server/lib/src/templates/topbar.html packages/dartclaw_server/lib/src/templates/workflow_detail.html` is empty; browser: each of the three `hx-confirm` sites (topbar session reset, workflow Cancel, workflow Reject on `/workflows/<id>`) opens the dialog and confirming issues the request exactly once (Network panel), while an ordinary `hx-get` sidebar refresh creates no dialog
...
- [ ] **TI05** The scheduled-task delete loses its native `confirm()` and gains the title S08's inline bar needs
  - Two edits, no replacement UI. (1) `deleteScheduledTask()` in `dc_scheduling_controller.js`: drop the `window.confirm(...)` clause from the guard, leaving `if (!taskId) return;`. Add no `confirmDialog()` call and no import — `sharedDecisions` rules this a *row-scoped* destructive action, so its confirmation is S08 TI13's inline `.delete-confirm-bar`, generalised from this file's own `confirmDeleteJob`, and a modal here would be the second confirmation API S08 forbids. (2) `templates/scheduling.html`: emit `data-task-title` via `tl:attr` alongside the existing `data-task-id` on the delete button (line ~200; the row already renders `${task.title}` at ~181), mirroring how `confirmDeleteJob` reads `dataset.jobName`. That attribute is the whole handoff; S08 owns the bar, the naming assertion and any id fallback.
  - **Verify**: `rg -n 'window\.confirm' packages/dartclaw_server/lib/src/static/controllers/dc_scheduling_controller.js` prints nothing (it currently prints `:403`); `rg -n 'confirmDialog|delete-confirm-bar' packages/dartclaw_server/lib/src/static/controllers/dc_scheduling_controller.js` returns only the pre-existing `confirmDeleteJob` construction and no `confirmDialog` — this story adds neither; `rg -n 'data-task-title' packages/dartclaw_server/lib/src/templates/scheduling.html` matches on the `dc-scheduling#deleteScheduledTask` button (currently no match); browser on the `visual` profile: `document.querySelector('[data-action*="deleteScheduledTask"]').dataset.taskTitle === 'Visual review seed task'`
...
- [ ] **TI07** Editing a guard extension is a single atomic form in a dialog
  - This is the story's one **structured-input** dialog and it is built inline, not through `confirmDialog()` — permitted and bounded by the Architecture Decision's "Boundary of the one-implementation rule": compose the canon frame, never fork it, and add no second confirmation API. The `[data-guard-editor-edit]` handler in `dc_settings_controller.js` opens one `.dialog` (S04 frame, S03 `.form-field` / `.form-input` / `.form-select` controls) containing the value input pre-filled from the current entry and, for the `file` guard's `extra_rules`, a select offering exactly `no_access`, `read_only` and `no_delete` – mirroring `templates/settings.html:499-507`. The dialog's controls carry their own hooks (not the singleton `data-guard-editor-*` attributes the panel reads via `root.querySelector`) and the dialog lives outside `[data-guard-editor]`. Both values submit together in the existing single `PUT`; cancelling submits nothing. The dialog closes before the request's toast fires.
...
- [ ] **TI09** The forbidden call forms and the single-implementation rule are guarded by a test
  - Extend `packages/dartclaw_server/test/static/app_js_test.dart` (follow its existing read-the-source assertion style) with a case that scans every file under `lib/src/static/controllers/` for the six forbidden call forms and asserts exactly one `confirmDialog` definition and one `htmx:confirm` binding pair.
  - **Verify**: `Test: dart test packages/dartclaw_server/test/static/app_js_test.dart` passes, and the new case fails when a `window.confirm(` call is temporarily re-introduced into any controller
...
- Both themes at desktop (1440×900) and 768px for each new dialog state, on the `visual` profile (port 3338): chat delete, restart confirm, workflow reject (`hx-confirm` path), project remove, guard edit form, guard delete, and both allowlist removals. The scheduled-task delete is **not** on this list — it renders no dialog in this story (TI05); its state is S08's to capture.
...
- TI01 must complete before TI02–TI04, TI06–TI08 and TI10 — every one of those consumes `confirmDialog()` from it. TI05 is independent: it deletes a call and adds a template attribute, and adds no confirmation. TI09's source-level guard runs last, after every other task has landed, or it fails on call sites not yet converted.
- Depends on S05 having landed: the canonical `.dialog` classes must already be in the served CSS, and the call-site line numbers quoted in the PRD will have moved – locate by call shape (`rg -n 'window\.(alert|confirm|prompt)|(^|[^.\w])(alert|confirm|prompt)\('`), not by line.
...
- [ ] `bash dev/tools/fitness/check_design_system_sync.sh` is green and `git diff -- packages/dartclaw_server/lib/src/static/app.css` shows no `.dialog` rule added.
...
- [ ] The scheduled-task delete's confirmation gap between this story and S08 TI13 is written into Implementation Observations **in the canonical private FIS** (`dartclaw-private/docs/specs/0.22.1/fis/`), not only the bundle copy — per `plan.json#executionNotes`, that is the only block S14's ledger reads.
- [ ] `git diff --name-only` for this story shows changes only under `packages/dartclaw_server/lib/src/static/`, `packages/dartclaw_server/lib/src/templates/` and `packages/dartclaw_server/test/static/` – proves the no-backend-surface criterion.
