...
- [x] **S01 [OC01] [TI01,TI02] Tasks table headers hold one line at the wide container tier**
...
- [x] **S02 [OC01] [TI04,TI11] Tables and the agent pool read as designed surfaces, not outlines**
...
- [x] **S03 [OC02] [TI06,TI07,TI08,TI14] Task detail reads as a hierarchy on shared baselines**
...
- [x] **S04 [OC02] [TI09,TI16] Server-generated content renders in its intended structure**
...
- [x] **S05 [OC03] [TI05,TI15] Controls and statuses carry their state in the glyph, not only in an attribute**
...
- [x] **S06 [OC03] [TI03] At 768px the surfaces keep their state signals, geometry and borders**
...
- [x] **S07 [OC04] [TI10,TI12,TI13] Absent values, empty lists and destructive confirmations have one treatment each**
...
- [x] **S08 [OC04] [TI10] A task with events still renders its timeline and its filters**
...
- [x] **S09 [OC03] [TI17] The retained `.custom-select` family still works beside the new canonical form controls**
...
- [x] **S10 [OC02,OC04] [TI18] Shared page scaffolding renders on every owned surface**
...
- [x] **This story changes no canon file and re-syncs nothing.** With `BASE=.agent_temp/0.22.1-s08-entry`, `rsync -ainc --delete "$BASE/dev/design-system/" dev/design-system/` prints nothing and `cmp -s "$BASE/packages/dartclaw_server/lib/src/static/tokens.css" packages/dartclaw_server/lib/src/static/tokens.css && cmp -s "$BASE/packages/dartclaw_server/lib/src/static/design-system.css" packages/dartclaw_server/lib/src/static/design-system.css && cmp -s "$BASE/packages/dartclaw_server/lib/src/static/icons.css" packages/dartclaw_server/lib/src/static/icons.css` exits 0. Canon closes after P1 (plan `sharedDecisions`, *Canon-first, and canon closes after P1*); the three rules this story previously proposed to author are hoisted — `.data-table thead` to S01, `.msg-user .msg-content p` and the task-event icon tokens/mappings to S02 (`canon-hoist-manifest.md`). Any further canon rule this story discovers it needs is **stopped and reported for hoisting**, never added here.
- [x] `dc_tasks_controller.js` task-detail polling still starts and stops correctly. The invariant is that the badge's trimmed `textContent` remains **exactly the status label and nothing else** – `dc_tasks_controller.js` lowercases before comparing (`:385`, `:411`), so `Queued` / `Running` are fine and `task_detail.dart` already title-cases; what breaks polling is any added `.status-dot` child contributing text of its own.
...
- [x] **TI01** Tasks, task detail and scheduling render at the wide container tier
...
- [x] **TI02** Tasks table headers hold one line and column widths are keyed to their semantic classes
...
- [x] **TI03** At 768px the three surfaces keep their state signals, share one table geometry and keep their borders
...
- [x] **TI04** Every table on these surfaces sits on a visible surface
...
- [x] **TI05** Every icon on these surfaces resolves to a mask glyph, and the scheduled-task toggle carries its state
...
- [x] **TI06** Task-detail metadata rows share one label baseline and one value baseline
...
- [x] **TI07** Task-detail panels complete their later restructuring on S07's canon-card baseline
...
- [x] **TI08** The task token summary renders as metric cards, not metadata rows
...
- [x] **TI09** Server-rendered diffs carry the canonical add / delete / hunk treatment
...
- [x] **TI10** Empty, absent and loading cases have one treatment each across the three surfaces
...
- [x] **TI11** The agent pool and its runner cards use the canonical meter and card anatomy
...
- [x] **TI12** The scheduling heartbeat block uses the tier its content deserves
...
- [x] **TI13** The two scheduling row-delete affordances converge on one confirmation pattern
...
- [x] **TI14** Type tiers, metadata colour and badge sizing follow canon on all three surfaces
...
- [x] **TI15** One total lifecycle presentation map drives every task label, pill and dot
...
- [x] **TI16** The task session's composed prompt keeps its line structure
...
- [x] **TI17** The three surfaces validate clean against the story-start baseline, and the retained `.custom-select` still works
...
- [x] **TI18** Task pages and empty cases consume S16's shared fragments with no fallback
...
### Run: 2026-07-30 01:56 UTC – observations

#### DEFERRALS (for the release glitch ledger)

- **SSE `task_event.iconChar` and `task_event_display.dart#compactEventIconChar` — deferred, blocked by the no-backend-work constraint.** Every *consumer* moved to the mask path (`tasks.dart`, `tasks.html`, `dc_tasks_controller.js`); the field stays on the wire unread because `task_sse_routes.dart` lives under `lib/src/api/`. `rg -l compactEventIconChar` returns exactly the deferred pair. The next story that legitimately owns `lib/src/api/` deletes both.
- **`--icon-fallback` question-mark treatment for unmapped icon names — deferred, not built.** It changes canon's base `.icon` / `[data-icon]::before` rules and is a new capability. This story instead makes the unknown branch emit *neither* `icon` nor a mask class, which removes the solid square without inventing a glyph.
- **`.run-card-step` and `.run-card--attention` on pool runner cards — deferred, no data.** No step counter or blocked state is modelled for a pool runner; adopting them would mean inventing data.
- **Task IA overhaul — deferred to Cross-Surface UX.** Out of scope by the plan's no-new-UX constraint.
- **Tasks filter-bar `.custom-select-trigger` is 44px at ≤768px, below the 48px touch floor — deferred, pre-existing and outside this story's edit surface.** The 48px floor is id-keyed to `#new-task-dialog` (S05's recorded shape), so the dialog instance measures 48px and the filter-bar instance 44px. TI17's premise that "the 48px floor still applies to `.custom-select-trigger`" holds only for the dialog instance. `.custom-select` is byte-identical to the story-entry snapshot; fixing this needs an owner decision about whether the floor becomes global.

#### MECHANISM NOTES (Verify met, recipe adapted)

- **TI06 used row `subgrid`, not the literal declaration list in the task.** The prescribed `grid-auto-flow: column; grid-template-rows: auto auto` places whole `.task-meta-item` boxes into alternating cells, which does not put labels on one row and values on another, and `grid-auto-flow: column` cannot wrap — at 768px eight items would overflow. `.task-meta-grid` is `repeat(auto-fit, minmax(7rem, max-content))` with `align-items: baseline`, and `.task-meta-item` takes the parent's two row tracks via `grid-template-rows: subgrid`. Measured on `/tasks/{id}`: label text-baseline spread **0px**, value text-baseline spread **0.6px** (Verify allows ±1px), and the grid still wraps. Box-top alignment alone would not have met it — a pill value and a bare-text value differ by the pill's padding, which only `baseline` absorbs.
- **TI12's "largest type is a real heading" reads against the disabled case.** The interval is a numeric KPI, and DESIGN.md's container decision tree (§ Containers, "Is a numeric KPI?") routes those to `.card .card-metric--{color}` at the 32px metric tier — so an *enabled* heartbeat legitimately shows a 32px numeral. Disabled emits no metric card at all, and the page's largest type then measures 18px (a `.section-header`). Verified live at both states.
- **TI18's page heads carry a description line, because none of the three surfaces had an existing subtitle to pass through.** DESIGN.md § Layout → Page title and skip link forbids a second `<h1>` and directs a surface needing a visible heading to "a subtitle or a description head"; `pageHeaderTemplate`'s empty-title mode exists for exactly this. `knowledge_hub.html` is the in-repo precedent (topbar title + `.pagehead` subtitle). Task detail's line is derived from task data (`Goal: …`, else `<Type> task – <Status>`), not static copy; `/tasks` and `/scheduling` take one static line each.

#### BOUNDARY CROSSINGS

- **`templates/components.{html,dart}` gained a `statusPill` fragment and `statusPillTemplate`.** TI15 requires the tasks-table STATUS column to render `.status-pill` with the map's variant, and no pill emitter existed. `statusBadgeTemplate` also gained an optional `dot` parameter (default null), so its five existing callers are unchanged. Both fragments keep the dot empty, which is what preserves `dc_tasks_controller.js`'s `textContent`-trimming poll gate.
- **`app.css#.session-item-link > .provider-badge` is a new rule on the shell's surface (S12's).** Contract-forced: TI14 removes `.provider-badge`'s baked-in `margin-left`, and `.session-item-link` is a gapless flex row that relied on it. Without the replacement the sidebar badge collapses against the session title. Verified live: 8px gap retained. `.task-card-meta` gained the same ownership as a flex `gap` (it had no rule at all before).
- **`app.css#.task-chat-embed:hover, .task-review-bar:hover` neutralises canon's `.card:hover`.** Routed here by S07: neither element is an affordance (one is a scroll container, one is pinned), and canon ships no `.card--flat`. Chose the local opt-out over hoisting a canon variant, since canon closed after P1. Verified: the hover declarations resolve to exactly the resting appearance (`translate: none`, resting shadow, `--bg-card`, `--border-highlight` top edge).

#### NOTICED BUT NOT TOUCHING

- **Canon's `.data-table thead` band measures 1.138:1 against the body in dark and 1.101:1 in light.** Visibly a band, and Scenario S02 only requires "distinct from the body" (the ≥1.15 floor is card-vs-ground, which measures 1.200 dark / 1.237 light and passes). The rule is S01's; recorded as a consumed-canon observation, not re-toned locally.
- **`.page-subtitle` measures 4.52:1 in light theme** — the same 0.02-above-AA-floor family S07 already flagged for S01/S14 (`.meter-label`, `.tab-meta-label`, `.detail-label`). This story's `pageHeader` adoption newly exposes that tone on three more surfaces. No new defect, wider blast radius; the fix belongs to the token owner.
- **`app.css#.task-meta-warning` and the `.metric-card` family remain** — S07 and S15 respectively recorded them as theirs.

#### SITES CLOSED THAT PRIOR STORIES ROUTED HERE

- S05 #1 `.well-content .form-row` bespoke input recipe: swapped. `scheduling.html`'s two forms now use canon `.form-field` / `.form-label` / `.form-input` / `.form-textarea` / `.form-select` / `.form-field--checkbox`; the app-local control recipe and its `.form-row` override are deleted (canon's `.form-row` means a *horizontal* group, so the override was fighting canon's meaning). Only the section title and field stack stay app-owned. A mono-face rule for the cron inputs was added and then removed as a no-op — the product's body font is already JetBrains Mono, so canon's `font: inherit` supplies it.
- S05 #4 `.custom-select`: verification only, and byte-identical to the entry snapshot. Trigger/caret/menu/option/check all paint; open, close, ArrowDown and Escape all work; the trigger's fill, 1px border and 4px radius match the canonical `.form-input` beside it in the new-task dialog.
- S06 N2 `executeDeleteJob` double-click guard: already present at story entry (`disabled = true` on both execute paths). The remaining unification was renaming the shared bar's cancel handler `cancelDeleteJob` → `cancelDelete`, since one construction serves both tables.
- S07 four heading rules → `.t-heading` with the app-local `font-size` deleted; all four compute 18px live.
- S16 `scheduling.html:32,118` literal `"+ "` labels → verb+noun with `data-icon="plus"`.
- S16 container-swapped empties: the timeline, task-list, task-detail and both scheduling-table empties now carry the shared fragment's icon + `.empty-state-title` + body.

#### DEFECTS FOUND AND CLOSED ON THESE SURFACES (not named by a task)

- `dc_tasks_controller.js` wrote a literal `—` for an absent turn-status reason, contradicting the `.value-absent` the server had rendered into the same element. Now toggles `.value-absent` and lets canon supply the dash.
- The tasks-table Project cell rendered blank for a task with no project. Now routes through `absentValue`.
- Task detail's three status bars hardcoded their own label + badge variant (`Queued` / `Interrupted` / `Running`), a second presentation source TI15 forbids. All three now render `${statusBadgeHtml}`.

### Run: 2026-07-30 02:21 UTC – observations

#### QUICK-REVIEW REMEDIATION (4 findings fixed, 3 recorded as Note)

Fixed:

- **The scheduled-task delete kept an `|| taskId` fallback, which TI13 bans outright** (`dc_scheduling_controller.js#deleteScheduledTask`). The title is now required alongside the id (`if (!taskId || !taskTitle) return;`), matching the guard idiom one line above. Without this, any render path that lost `data-task-title` would silently ask the operator to confirm deletion against a UUID they never read — the exact defect the title contract exists to prevent, and one the happy-path Verify could not falsify. `app_js_test.dart` now pins both the required-title guard and the absence of the fallback.
- **`.task-col-project` and `.task-col-created-by` were the two metadata columns left wrappable** while `word-break: break-word` still applied to every `td`. At the table's 840px min-width floor with the Project column shown, a long project name could still break mid-word — which Scenario S01's Then clause names explicitly. Both joined the nowrap set; only `.task-col-title` stays wrappable, since it is the column that absorbs the remaining width. Re-measured live: 28 of 31 rows are exactly 42px (one text line); the three taller rows are two 2-line *titles* and one goal sub-line, all intended.
- **`status-badge-unknown` was emitted with no backing rule.** TI15 adds a tenth lifecycle row, so the badge variant it emits was an orphan this story created — it fell back to the bare `.status-badge` treatment by accident. Added beside its eight siblings with a neutral `--fg-sub0` recipe; measured distinct from both the base badge and `status-badge-draft`.
- **`task_event_icon_map_test.dart`'s no-default-glyph assertions were unanchored `contains` over the whole 900-line controller** — any `? x : null` elsewhere would have satisfied them. Scoped to the resolver's function body and added an assertion that the body names no `icon-*` literal, so a default glyph cannot be reintroduced silently.

Recorded, not fixed:

- **`task_event_display.dart#statusBadgeClass` still resolves a null/empty `newStatus` to `status-badge-draft`** — a second status-presentation source, one seam over from TI15's map. A `statusChanged` event with no recorded target renders a draft-tinted badge on a timeline whose task badge reads Unknown. Not fixed here: the FIS declares `task_event_display.dart` read-only, so closing it needs its own story. This narrows TI15's "one presentation source" claim to the task list and task detail.
- **The scheduling tables' 768px column-hiding stays `:nth-child`-keyed.** The Constraints gotcha prescribes semantic re-keying, but its stated hazard is the tasks table's *conditional* Project column; neither scheduling table has a conditional column, and re-keying would mean adding semantic classes to markup no task covers. TI03's own instruction names the `:nth-child` positions as the mechanism.
- **`task_timeline.dart#timelineEventItemHtml` has no production caller** (only its own test). Pre-existing dead seam; this story's zero-event suppression additionally makes its `#timeline-events` target absent for a task with no events, so anyone reviving it must re-check that precondition.

Dismissed on inspection: a reported crash risk at `tasks.dart`'s `taskStatusPresentations[status]!` — the `continue` guard above it runs first, and every key that reaches the assertion came from `grouped`, whose keys are all `taskStatusKey` outputs, so the map lookup cannot be null.

Gates re-run after remediation: 3056 server tests pass (3 skipped), `dart analyze --fatal-infos` clean workspace-wide, format clean, embedded assets regenerated and parity green.
