...
- [x] **S01 [OC01] [TI01,TI02] The knowledge hub header is built from canonical components and its KPI strip states what it counts**
...
- [x] **S02 [OC01] [TI03,TI04] Knowledge results, empty state and pager are the canonical ones and the result title is the link**
...
- [x] **S03 [OC01] [TI05] The KG timeline shares the hub's tab bar and empty state, and its reset control says what it does**
...
- [x] **S04 [OC02] [TI07] Navigable routes render inside the app shell without turning wiki failures into an oracle**
...
- [x] **S05 [OC03] [TI09,TI10] Channel detail exposes one control per mode setting, and the badge, banner and pairing block follow the selection**
...
- [x] **S06 [OC03] [TI08,TI11,TI12] A not-running channel says so, an empty allowlist renders identically on both paths, and Google Chat shows no dead CTA**
...
- [x] **S07 [OC04] [TI06,TI13] These surfaces hold at 768px and stop conveying state by colour alone**
...
- [x] **S08 [OC01,OC02,OC03,OC04] [TI15] Every finding this story does not close is recorded as an explicit deferral S14 can read**
...
- [x] **S09 [OC01,OC03] [TI14] Each swept page exposes exactly one `<h1>`, and it comes from the topbar**
...
- [x] `dev/tools/fitness/check_design_system_sync.sh` exits 0 at story end; with `BASE=.agent_temp/0.22.1-s10-entry`, the three canon and three served-CSS `cmp -s` checks exit 0. This story edits none of the closed canon or served CSS files.
- [x] No `window.alert` / `window.confirm` / `window.prompt` / bare `alert(` / `confirm(` / `prompt(` is introduced in `lib/src/static/controllers/`.
- [x] No service, schema or API change: with `BASE=.agent_temp/0.22.1-s10-entry`, the `rsync -ainc --delete` comparisons for `packages/dartclaw_storage/` and `packages/dartclaw_server/lib/src/knowledge/` print nothing.
- [x] No new runtime JS dependency and no new external origin in `layout.html` or the CSP; with `BASE=.agent_temp/0.22.1-s10-entry`, the `layout.html` `cmp -s` check exits 0.
- [x] `--container-wide` / `.content-inner--wide` / `.page-inner--wide` is applied to no surface in this story – knowledge results keep the 900px measure and channel detail is not on S02's wide list.
- [x] No new `var(--text-sm)` usage is introduced (assert on this story's diff, not on `app.css` tree-wide — retiring the 79 existing uses is S07's gate).
- [x] Each of the 15 glitches and 20 adoption gaps this story claims is closed, or recorded in Implementation Observations as a deferral with a reason. "Listed" means the findings under the audit headings named in Deeper Context; multi-surface findings that merely touch these pages are **not** claimed here, and where one is partly closed (the not-running channel entry's Start/Connect half) the uncovered half is recorded as a deferral rather than dropped.
- [x] `packages/dartclaw_server/lib/src/web/web_routes.dart`, `web/channel_status.dart`, `web/pages/health_page.dart` and `templates/loader.dart` are the only non-page-template `.dart` files this story edits. `health_page.dart` threads the requested audit page into the inline full-page render; `loader.dart` adds the wiki template basename to `expectedTemplates`; none is a service, schema or API change.
- [x] The new wiki page pair uses basename `wiki_document`; a Dart assertion counts exactly one equal entry in `loader.dart#expectedTemplates`, and the route succeeds through non-dev filesystem initialization (`initTemplates(..., devMode: false)`) as well as the real embedded bundle.
...
- [x] **TI01** The knowledge hub header uses the canonical tab, toolbar and chip components
...
- [x] **TI02** The knowledge layer strip reads as a KPI row and distinguishes a failed layer from an empty one
...
- [x] **TI03** Knowledge results are cards whose title is the link, with one badge and a clamped snippet
...
- [x] **TI04** The knowledge hub's empty state and pager are the canonical ones
...
- [x] **TI05** The KG timeline shares the hub's tab bar and empty state, and its controls are honestly labelled
...
- [x] **TI06** Knowledge and KG surfaces collapse on the app's breakpoint, carry canon caps tracking, and give layer identity the categorical ramp
...
- [x] **TI07** Wiki documents and the audit table render inside the app shell in filesystem and embedded modes
...
- [x] **TI08** One exhaustive typed `ChannelStatus` presentation record drives every status consumer
...
- [x] **TI09** The channel mode picker exposes exactly one control per setting to assistive tech
...
- [x] **TI10** The ACTIVE badge, restart banner and pairing block follow the selected mode without a reload
...
- [x] **TI11** An empty allowlist renders the same row on the server and client paths
...
- [x] **TI12** The shared status presentation states not-running truthfully, and Google Chat shows no dead pairing CTA
...
- [x] **TI13** The attribution popover dismisses on pointer-out and on Escape
...
- [x] **TI14** The topbar owns the only `<h1>` on each of this story's three surfaces
...
- [x] **TI15** The story's deferrals are recorded and the surfaces are validated in both themes
...
- [x] None of the three closed canon files (`dev/design-system/tokens.css` / `components.css` / `icons.css`) and none of the served `design-system.css` / `tokens.css` / `icons.css` is modified by this story. Any `DESIGN.md` / `showcase.html` edit is intentional and outside the drift check.
- [x] Dart counts exactly one `wiki_document` entry in `loader.dart#expectedTemplates`; the route passes under non-dev filesystem and real embedded initialization; all seven wiki rejection paths return one opaque response.
- [x] One no-default typed `ChannelStatus` presentation record covers all seven values and is the sole source for settings/detail label, badge, dot, banner, policy hint and connected-only action fields; null WhatsApp/Signal routes consume `ChannelStatus.disabled.presentation`; S03's `status-badge-muted` is present and used; every field is table-tested for every value, including `connectionError` and `connected`, and every emitted badge/dot/banner suffix has an exact served-CSS selector.
- [x] After source-tree targeted checks, run `dart run dev/tools/embed_assets.dart`, require `dart test packages/dartclaw_server/test/generated/embedded_assets_test.dart` green, run the real embedded wiki-route regression through default `initEmbeddedTemplates()`, then run the full `dart test packages/dartclaw_server/test` suite. The story closes only with every stage green.


## Implementation Observations

### Run: 2026-07-30 05:31 UTC – observations

#### DEFERRALS

- **Knowledge layer counts report the filtered result set, not the corpus.** `KnowledgeHubService.search` returns early for non-selected layers (`knowledge_hub_service.dart:108`), so `layerCounts` can only ever count collected items. Recomputing from an unfiltered corpus query is a service change, which the PRD puts out of scope. *In-scope remedy shipped:* the strip is now titled "Layers in these results" and named through `aria-labelledby`, so a `0` under a non-selected layer is truthful rather than misleading.
- **Snippet markdown-stripping and word-boundary snapping.** `WikiSearchSource._snippet` (`packages/dartclaw_storage`) takes a fixed 240-character substring with no markdown strip and no word-boundary snap — a service change, out of scope. *In-scope remedy shipped:* `.knowledge-result-snippet` clamps to three lines with a trailing ellipsis, so an overflowing snippet no longer cuts hard; a snippet that fits within three lines still shows the upstream mid-word cut (visible as "…curated summaries, gui" on the seeded README).
- **The KG timeline has no time axis, and canon has no chronology component.** This is the audit's `kg-timeline` canon-change entry: it needs a new canon component family (canon closed after P1) plus a new capability (the release adds none). Deferred rather than closed — the timeline keeps category-first grouping with `.kg-window-pill` validity windows as its only temporal cue.
- **No Start / Connect action for a not-running channel.** No such endpoint exists, so adding one is a new UX capability. This story states the condition truthfully (state banner plus an apply-when-started policy hint) but does not make it actionable. This is the uncovered half of the not-running-channel finding; the stated-condition half is closed.
- **Canon has no `a.chip[aria-current]` selected treatment.** Canon's pressed-chip rule is element-qualified — `button.chip[aria-pressed="true"]` (`components.css:1956`) — and the layer filters must stay GET-navigation anchors, so `aria-pressed` there would be both invalid ARIA and visually inert. Canon is frozen for this story, so the active chip carries `aria-current="page"` and canonical focus only, with no page-local selected styling added. Verified live: the active chip is distinguishable programmatically but not visually.

#### IMPLEMENTATION NOTES

- **The categorical ramp lands on the badge tint and border, not on the badge text.** TI06 requires `.layer-badge--{wiki,kg,memory,inbox}` to resolve to `--chart-3`…`--chart-6` by fixed layer index *and* each badge to measure ≥ 4.5:1 in both themes. Those two clauses cannot both hold with hue-coloured text: in light theme `--chart-5` (`--pink` `#ea76cb`) measures **1.98:1** and `--chart-6` (`--sky` `#04a5e5`) **2.06:1** on their own 10% tint, and no recipe available on this surface fixes it — on plain `--bg-base` they are still 2.13:1 and 2.26:1, and mixing toward `--fg` only reaches 3.2:1 before the four layers stop being distinguishable. DESIGN.md:428 independently scopes the extended palette to "decorative/categorical only … data-viz category colors", never text. Resolution: each variant sets `background`/`border-color` from its `--chart-N`, and `.layer-badge` text moves from `--fg-sub1` to `--fg`. The hue assignment by fixed index is unchanged. Measured in-browser via `getComputedStyle` + canvas resolution, both themes at the badge's real 12px: dark 8.93–9.31:1, light 5.62–6.00:1. Re-basing `--chart-5`/`--chart-6` would be the token-side alternative and is a canon change this story cannot make.
- **`type="search"` settled as the canonical markup, with an app-side reset.** S03 recorded the open question. Confirmed in Chromium: with text entered *and* focus, Blink paints a native `::-webkit-search-cancel-button` (a white ✕) inside the branded control. `type="search"` is kept — it carries the `searchbox` role and platform Escape-to-clear — and the UA affordance is reset via `input.form-input--search::-webkit-search-cancel-button { appearance: none }` in `app.css`. The contract is written into `dev/design-system/DESIGN.md` § List toolbar and pager (prose, not drift-checked). **The reset is a hoist candidate for `components.css`**, since it is keyed on a canon class; it lives app-side only because canon was frozen. S14 should reconcile.
- **`.channel-mode-badge` contrast fixed on this surface, as a recipe defect.** Confirmed the 3.69:1 light-theme measurement. The cause is the 16% accent self-tint lifting the ground toward the text; `--accent` on plain `--bg-surface0` measures 4.55:1 light / 8.46:1 dark. Only the `background` changed. No token moved.
- **Live audit page 2 could not be exercised on the `visual` profile.** That profile ships no `audit.ndjson`, so the reader returns `AuditPage.empty` (`totalPages: 0`) and any `page` clamps to 1. The inline-versus-fragment branch, the absence of a redirect, and history-restore handling were all verified live; page-2 row content plus `page`/`verdict`/`guard` survival into the poll URL are proven by `web_routes_test.dart` against 60 seeded entries ("Page 2 of 2", "Showing 26–30 of 30 events").
- **The wiki document page renders two `<h1>`s.** The topbar emits one, and the rendered markdown emits another when the document itself opens with `# Heading` (the seeded README does). TI14's single-`<h1>` requirement is scoped to `/knowledge`, `/knowledge/timeline` and `/settings/channels/*`, all three of which verify at exactly one; the wiki document's second heading comes from user content through the shared `data-markdown` pipeline and was left alone rather than post-processing user markdown.

#### NOTICED BUT NOT TOUCHING

- `web_routes.dart` — the wiki route's `Uri.decodeComponent` can throw on a malformed percent-escape (e.g. `%zz`), which escapes as an unhandled 500 rather than joining the seven opaque 404 paths. Pre-existing; outside the enumerated seven, and converting it would be an unrequested behaviour change.
- `channel_detail.html` — the hero meta pills ("DM mode: pairing") are server-rendered and are not updated by `syncModeCards`, so after a client-side mode change they show the previous value until reload. Pre-existing; TI10 scopes the sync to badge, banner and pairing block.
- `app.css:637` `.pagehead` — carries `margin-bottom: var(--sp-4)` on top of `.page-inner`'s `gap: var(--sp-6)`, which is the ~40px double-spacing the audit measured. It is a global rule shared by every page, not a knowledge-block rule, so TI06's "knowledge section blocks" mandate does not reach it. The three knowledge/kg blocks it does reach were dropped.
- `s10` FIS TI11 Verify names `.allowlist-empty`; S16 retired that class and the empty row is now canon `.text-muted` on both the server and client paths. The substance — an identical "No entries" row on first render and after a client round-trip — is met and verified; only the class name in the Verify line is stale.
- `packages/dartclaw_server/test/templates/entry_motion_template_test.dart` — the hard-coded template census moved 17 → 18 for the new `wiki_document.html`, which does carry `print-in`. Re-pointed, not weakened.

### Run: 2026-07-30 06:00 UTC – observations

#### QUICK-REVIEW REMEDIATION

A fresh-context adversarial critic reviewed the S10 change set. Seven findings; four accepted and fixed, one dismissed on a falsifier, two accepted as Notes.

**Fixed**

- **TOCTOU in the wiki route (MEDIUM).** `web_routes.dart` canonicalised the resolved path, verified containment, then read through the *original* handle — so a symlink swapped in after the check could serve content from outside the wiki root, defeating the guard the seventh rejection path exists to provide. The workspace is agent-writable, so the race has a real actor. Now reads `File(canonicalFilePath)` — the path that was actually verified.
- **Click-to-pin was unreachable for pointer and touch users (MEDIUM).** TI13's Verify requires "opening by click and moving the pointer away leaves it open". A pointer is always hovering the marker when its click lands, so the popover was already open-and-unpinned and `toggle()` *closed* it; on touch, the synthesised mouseenter+click made a single tap open-then-close. `toggle()` now closes only when already pinned, so a click on a hover-opened popover pins it. The Node harness case that "proved" pinning was exercising an unreachable state (toggle from closed) and has been re-pointed to the real hover→click sequence, plus a second case proving a click on a pinned popover still closes it.
- **No regression pin on server-side escaping of wiki content (MEDIUM).** Wiki documents are workspace-writable, so the body is untrusted. The mechanism was correct (`tl:text` into `data-markdown`, rendered client-side by marked + DOMPurify) but nothing failed if a later edit switched the slot to `tl:utext` — every other wiki test would have stayed green. Added a test seeding `<script>` and `<img onerror=>` payloads and asserting they arrive angle-bracket-escaped inside the container, with the negatives scoped to the document container (the layout legitimately carries its own `<script>` tags). Mutation-checked: flipping the slot to `tl:utext` fails it.
- **Dead enum members (LOW).** `ChannelStatus.label` and `.badgeClass` had no consumer left once every surface moved to the presentation record; removed. `badgeVariant` stays — it is the badge's own naming, used by `settings.dart`.

**Dismissed**

- The reviewer attributed the `'unknown'` → `''` fallbacks at `health_page.dart:54,58` to S10 as an out-of-seam edit. Falsifier: `health_page.dart` was already modified in the working tree at S10's entry (it is in the story-start dirty baseline, and its last commit is 0.16.5), so the whole-file `git diff` mixes a sibling story's uncommitted work with S10's. S10's only edits to that file are the `auditPageNumber` parse and threading `page:` into the audit read.

**Accepted as Notes (not fixed here)**

- **Arrow-key roving persists every intermediate access mode (MEDIUM).** Arrows call `next.click()`, which routes through the existing save path, so arrowing from `disabled` to `pairing` also PATCHes `open` and `allowlist` en route; abandoning mid-traversal leaves an unchosen DM access policy persisted, and rapid arrowing puts unordered PATCHes in flight against a single shared `previousValue` closure. This is a **security-policy field** (who may DM the agent). Not fixed because the FIS pins the behaviour — scenario S05 states "Arrow keys move focus and selection within the radiogroup" — so moving to focus-only-with-Space-to-commit (the WAI-ARIA APG pattern recommended precisely when activation has a server round-trip) would contradict a stated Acceptance Scenario and needs the design-change path, not a review-time edit. **Escalated to the orchestrator for a decision.**
- **`runChannelDetailInitializers` double-fetches pairings on load (LOW).** It calls `initPairingPolling()` after `initChannelDetail()` already did, so a page whose pairing block is visible issues two immediate `GET /dm-pairing`. Both call sites predate S10; a single interval survives, so there is no leak — only a duplicate request.

### Run: 2026-07-30 06:11 UTC – observations

#### ORCHESTRATOR RULING — arrow-key mode commits

The escalated MEDIUM (arrow-key roving persisting intermediate access modes) was ruled on by the orchestrator: **do not** take the WAI-ARIA focus-only-with-Space-to-commit route. It would contradict pinned Acceptance Scenario S05 and need a design-change amendment mid-sweep, and the PRD bars new UX capabilities. Arrow-selection semantics stay exactly as specced — arrows still move focus *and* selection, immediately.

The two underlying defects were fixed instead, both compatible with the scenario's literal wording:

- **Debounced commit.** `createModeCommitScheduler` (exported from `dc_settings_controller.js`) holds the selected value for `MODE_COMMIT_DELAY_MS` (350ms) of quiet before writing. Selection, `aria-checked`, roving tabindex and the ACTIVE badge all still move on the keystroke, so the interaction is unchanged to the operator; only the PATCH waits. Transit modes are therefore never persisted — the write carries the mode the operator rests on, which is exactly what a click would have written. This closes the security concern: no unchosen DM access policy reaches config.
- **Serialized requests with per-request rollback.** At most one PATCH is in flight per field; a selection made during a request is queued and drained on completion. Each request captures its rollback target **at dispatch** rather than reading a shared `previousValue` closure, so an overlapping failure restores the value that was committed when that request went out, not whatever the newest selection happened to be. A rejection rolls back and drops anything queued behind it — the queued value was chosen against a state that never existed.
- `cleanupChannelDetail()` now cancels pending commits, so a navigation cannot let a debounced write land after the page is gone.

**Verification.** `test/static/mode_commit_scheduler_test.dart` drives the shipped factory with injected timers (no clock, no DOM, no network) across seven cases: rapid traversal writes once with the settled value and no transit mode; abandonment writes only the resting mode; a round trip back to the committed value writes nothing; a selection during an in-flight request is serialized behind it; a failure rolls back to the dispatch-time value; a failure with a queued value rolls back and drops the queue; `cancel()` disarms a pending write. Mutation-checked — removing the debounce, sharing the rollback value, or dropping the in-flight guard each fails a specific assertion.

Confirmed live on the `channels` profile: arrowing three times through the DM radiogroup moved selection immediately with **0** PATCHes in flight, then issued **exactly one** `{"channels.whatsapp.dm_access":"disabled"}` after the settle — `allowlist` and `open` were never written. A single click still commits once and drives the ACTIVE badge, pairing block and restart banner as before.

#### DOUBLE-FETCH FIXED

The recorded LOW (two immediate `GET /dm-pairing` per load) was a one-line removal after all. `initChannelDetail()` ended with its own `initPairingPolling()` while both of its callers already call it. The **inner** call was removed rather than the outer ones: `initChannelDetail` early-returns on an already-initialised page, so only the caller-side call covers both the fresh-init and the controller-reconnect path.
