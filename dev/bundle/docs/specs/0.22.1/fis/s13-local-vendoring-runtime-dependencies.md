...
- `../dartclaw-public/dev/adrs/047-embedded-binary-assets.md#decision` – the embed pipeline this story extends (base64 text map + separate binary map, checked-in generated library, CI drift gate); read before touching `embed_assets.dart`.
...
- [ ] **S01 [OC01,OC02] [TI01,TI02,TI05,TI06] The UI renders and behaves identically with every external origin blocked**
  - **Given** the `visual` profile is running on `http://localhost:3338` and the browser blocks every request to a non-`localhost` origin (e.g. Chromium started with `--host-resolver-rules="MAP * ~NOTFOUND, EXCLUDE localhost"`)
  - **When** an authenticated operator loads `/`, navigates to `/tasks` via the sidebar, and opens a seeded chat session
  - **Then** `document.fonts.check('16px "JetBrains Mono"')` returns `true` (the face actually loaded – computed `font-family` alone cannot distinguish a failed load, since it returns the declared stack either way), the sidebar navigation swaps `#main-content` without a full page load, the seeded assistant message renders as HTML rather than literal `**markdown**`, the network log records zero requests to non-`localhost` origins, and the console records zero `error`-level entries
...
- [ ] **S04 [OC03] [TI08] Re-introducing a CDN subresource fails the fitness check**
  - **Given** a working tree in which `<script defer="defer" src="https://cdn.jsdelivr.net/npm/marked@15/marked.min.js"></script>` has been pasted back into `layout.html`
  - **When** `bash dev/tools/fitness/check_no_external_origins.sh` runs
  - **Then** it exits non-zero and names `layout.html` and the offending line; removing the line makes it exit `0`
...
- [ ] `dev/testing/UI-SMOKE-TEST.md` carries Regression Checks row `R-13` (offline render with all external origins blocked, font proven via `document.fonts.check`), and the highest `TC-NN` id in the document remains `TC-31`.
...
- **No regeneration of `packages/dartclaw_server/lib/src/generated/embedded_assets.g.dart`** -- S14 owns the single end-of-release regeneration; every canon re-sync in this release drifts it the same way.
...
- **Constraint**: `embedded_assets.g.dart` is intentionally left stale by this story, so `test/generated/embedded_assets_test.dart` stays red until S14 regenerates -- this is the plan's release-wide condition (every canon re-sync drifts it), not a defect introduced here. Do not regenerate; do not "fix" the test by narrowing it.
...
- [ ] **TI08** A fitness check fails on any external subresource in the served surfaces
  - New `dev/tools/fitness/check_no_external_origins.sh`, following the shape of `check_design_system_sync.sh`, asserting three things: no `<link|script|img|iframe|source>` tag under `packages/dartclaw_server/lib/src/templates/` carries an external `src`/`href` – quote- and scheme-tolerant, matching `(src|href)\s*=\s*["']?(https?:)?//` so single-quoted attributes and protocol-relative `//cdn…` forms are caught; no external `@import` and no `url(` referencing an external origin in any CSS under `packages/dartclaw_server/lib/src/static/` (recursive, `**/*.css`), matching `url\(\s*["']?(https?:)?//` so the quoted `url('https://…')` form TI05's own house style would produce is caught; no `https://` in `packages/dartclaw_server/lib/src/auth/security_headers.dart`. Outbound `<a href>` navigation, code comments and `VENDORS.md` upgrade commands are out of the scan by construction, not by allowlist. Register it in `dev/tools/fitness/run_all.sh` beside the other checks.
  - **Verify**: `Shell: bash dev/tools/fitness/check_no_external_origins.sh exits 0 on the clean tree (proves S05); after pasting <script src="https://cdn.jsdelivr.net/npm/marked@15/marked.min.js"> into layout.html it exits non-zero naming layout.html (proves S04); after adding src: url('https://fonts.gstatic.com/x.woff2') to app-tokens.css it exits non-zero (quoted-url form); rg -n check_no_external_origins dev/tools/fitness/run_all.sh matches`

- [ ] **TI09** The offline render check is a recorded, repeatable regression check
  - Add row `R-13` to the Regression Checks table in `dev/testing/UI-SMOKE-TEST.md`: load the `visual` profile on 3338 in a browser with all non-`localhost` origins unreachable – the row names the mechanism (e.g. Chromium `--host-resolver-rules="MAP * ~NOTFOUND, EXCLUDE localhost"`) so every future run blocks uniformly rather than improvising a per-domain blocklist – and confirm `document.fonts.check('16px "JetBrains Mono"')` returns `true` (computed `font-family` cannot prove the face loaded), an `hx-get` navigation swaps, a seeded chat message renders as HTML, and the network log shows zero non-`localhost` requests. Use the `R-NN` table rather than a new `TC-NN` so S14's stated `TC-01…TC-31` release gate stays accurate.
  - **Verify**: `Shell: rg -n "R-13" dev/testing/UI-SMOKE-TEST.md matches a Regression Checks row naming JetBrains Mono (via document.fonts.check), the blocking mechanism, and external origins blocked; the highest TC id in the document is still TC-31 (proves the Structural Criterion on R-13)`
...
<!-- Per-task Verify lines + scenario tests scaffolded from the Acceptance Scenarios are sufficient. -->

### Validation

- Capture one representative surface (`/tasks`, which mixes table, card and label type) in both themes at desktop and 768px, online and with external origins blocked, and confirm the two captures are pixel-identical – the typeface swap is the only visual delta this story can produce, and it is global.

### Execution Contract

<!-- Standard exec-spec discipline is sufficient; document order carries the only cross-task dependencies (TI01/TI02 produce the files TI03–TI06 consume). -->


## Final Validation Checklist

- [ ] `git status` shows `packages/dartclaw_server/lib/src/generated/embedded_assets.g.dart` unmodified – S14 owns the single release-wide regeneration.
