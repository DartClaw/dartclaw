...
- [OC03] All containerized Claude and Codex executions reach host MCP only through the execution-scoped bridge with a deny-by-default tool inventory; every container profile can use its approved DartClaw search/fetch tools while provider-native web access, unapproved MCP tools, and arbitrary Internet access remain unavailable, and workspace-profile containers share the same scoped-MCP and no-direct-egress boundary. (Reconciled 2026-08-12: provider-native web is denied for **all** container profiles, not retained for `workspace` – native web executes provider-side and cannot be contained under `network:none`; see [ADR-052](../../../../adrs/052-native-provider-web-tools-denied-all-container-profiles.md).)
