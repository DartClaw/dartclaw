...
- Reusable harnesses are an opportunistic cache, not capacity. There are no cache-size, TTL, prewarm, or reuse-policy knobs. Harness-construction inputs are fixed for a coordinator's lifetime, so provider and security profile fully identify compatible workers within it. Lookup prefers the exact session, then any healthy worker with the same provider/profile; a mismatch or unknown health requires a fresh worker.
...
- Gate and coordinator tests prove FIFO queueing, prompt failure of queued/future waiters after final-slot quarantine, fail-fast acquisition, release after success/error/cancellation, hard per-provider capacity, capacity-only one-shots without unused harness construction or warm-cache eviction, exact provider/profile isolation, exact-session affinity before compatible reuse, fresh construction for incompatibility, cache scavenging, and truthful lease events/snapshots.
...
- Observability tests prove busy/free/current-work state comes only from leases; terminal outcomes update primary and worker metrics exactly once at the shared runner-settlement seam; and disposed workers leave current runner registries without accumulating stale rows or references.
...
- The post-implementation simplification removed 202 net lines from the completed worker-capacity change: 150 production lines and 55 test lines, with 3 documentation lines added to state the final contract. It removed the caller-built fingerprint/configuration identity, dead system surface, redundant cached event, exposed lease internals, duplicated admission branches, the unused standalone worker builder, and dead standalone behavior state. Coordinator-owned startup and lifecycle cleanup replaced duplicated factory cleanup. Relative to the pre-change baseline, the final architecture adds 553 production lines for hard capacity, safe reuse, teardown quarantine, outcome retention, and truthful observability; the overall increase is dominated by 1,746 net test lines and 164 documentation lines proving and explaining the new contract.
- The simplify review rejected two superficially smaller but unsafe changes: direct provider-map lookup still accepts public directly-constructed maps with unnormalized keys, and blank provider IDs fail closed instead of silently becoming Claude.
