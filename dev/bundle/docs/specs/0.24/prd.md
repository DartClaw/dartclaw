...
> **Related Assets**: `memory-architecture-recommendations.md`; `0.24-gap-review-codex-2026-08-12.md`; `0.24-memory-model-mixed-review-codex-2026-08-12.md`; `s05-atomic-memory-apply.reconciliation-ledger.md`; ADR-002, ADR-007, ADR-029, ADR-033, ADR-042, ADR-045, ADR-050; `dev/architecture/data-model.md`; `dev/architecture/system-architecture.md`; `dev/architecture/observability-operations-architecture.md`.
...
- Host-bound provenance for observation and mutation, including bounded pre-compaction observation capture where the provider exposes a deterministic lifecycle hook.
...
**Acceptance Criteria**:
- [x] `MEMORY.md` is a bounded prompt index, not an append-only chronological authority.
- [x] Detailed curated personal/experiential memory lives in canonical topic pages reachable from the index.
- [x] Existing dated activity records are provenance-labelled observations and are not prompt-authoritative by storage location alone.
- [x] `MEMORY.archive.md` remains searchable cold memory with an explicit relationship to active entries and migration.
- [x] `learnings.md` is a canonical learning document with stable entry identity/revisions, validation, fingerprint participation, newest-50 retention, search/read projection, and the shared collection revision; it remains distinct from personal memory.
- [x] `MEMORY.audit.md` is a canonical, fingerprinted role committed transactionally with retirement, but it is never indexed or model-readable.
- [x] `wiki/` remains source-backed synthesized knowledge and the temporal KG remains the source-linked time-aware fact store; sourced facts are not duplicated into personal topic memory as a second authority.
- [x] Recognized legacy entries receive stable identity without changing their semantic text; opaque legacy content is preserved and reported rather than dropped.
- [x] Canonical identity survives index deletion/rebuild and is preserved by both 0.25 derived-search implementations; canonical memory remains file-based and outside ADR-045's `DatabaseBackend`.
...
**Acceptance Criteria**:
- [x] `memory_observe` appends bounded provenance-labelled content under a closed observation or learning role; observations are not injected as authoritative instructions, learnings retain the existing bounded self-improvement role rather than becoming personal topic memory, and a successful write advances the shared collection revision.
- [x] `memory_search` returns bounded role-discriminated matches with provenance and a resolvable locator; personal-memory matches also carry canonical entry ID and revision, while wiki matches retain native source identity rather than fabricated memory-entry metadata.
- [x] `memory_read` supports bounded reads by stable entry ID and topic/store scope without requiring a whole-file read.
- [x] `memory_apply` accepts an expected collection revision and an atomic structured change set whose only operation kinds are add, revise, merge, and remove; archive and re-topic changes are revisions, not additional operation kinds. Operations never accept caller-authored provenance, actor, or source fields: the host binds complete provenance from its active trusted context.
- [x] All operations use one validation/mutation authority and the existing shared workspace lock and atomic-write discipline.
- [x] Every successful canonical write advances the same collection revision; read/conflict results expose the current revision needed for compare-and-swap.
- [x] `memory_save` is removed from the model-facing contract rather than retained as a duplicate alias.
- [x] Every production caller, prompt, recipe, provider mapping, and allowlist using `memory_save` is migrated: observation producers use `memory_observe`, curated personal-memory producers use `memory_apply`, and learning producers use `memory_observe`'s bounded learning role without routing learnings into personal topic memory.
- [x] `memory_observe` and `memory_apply` retain persistent-memory write policy/audit treatment wherever provider interception exists; `memory_search` and `memory_read` remain read-only, renamed tools never fall through to generic MCP identity, and documented provider limits remain truthful.
- [x] Invalid IDs, stale revisions, malformed per-operation fields, caller-supplied provenance, missing host provenance context, and size violations fail before any canonical or index mutation.
- [x] A remove operation removes active content and derived-index rows; the audit retains identity, time, host-derived actor/source, and the caller's verbatim reason under a 1,024-character cap. The host never copies deleted entry content into the audit, although the caller's reason may quote it.
- [x] A supported pre-compaction lifecycle hook attempts to persist a redacted, UTF-8-bounded conversation-tail observation with host session provenance before acknowledging compaction exactly once; capture failure is logged but fails open only after the attempt settles.

**Inputs / Outputs**:
- **Inputs**: Structured observations, queries, IDs/topics, expected revision, structured change set, and trusted host-owned caller/source context.
...
- Provenance, actor, owner, and source identity are host-bound context, never model-authored operation fields.
...
**Acceptance Criteria**:
- [x] An explicit curation action snapshots a bounded index, relevant topic entries, and bounded observations with a collection revision.
- [x] Curation admits at most 50 current entries within 64 KiB and 50 observations within 64 KiB. An observation is uncurated when recorded after the last confirmed successful curation; failed, conflicted, and interrupted runs never advance that anchor.
- [x] The model returns only a structured proposal; it does not directly rewrite files or claim success before host commit.
- [x] The proposal may add, revise (including archive or re-topic changes), merge, or remove entries; every revise/remove target and merge source must be an entry included in the bounded snapshot, and the host attributes resulting mutations to the curation run.
- [x] The host validates and atomically applies the proposal through the same mutation service used by `memory_apply`.
- [x] A concurrent valid memory write causes the stale curation proposal to fail without partial effects.
- [x] Curation reports either changed and exact-no-op IDs after a wholly valid atomic commit, or proposal rejection with per-operation reasons and no changed IDs; invalid operations are never skipped into a partial commit.
- [x] The existing threshold-triggered `MemoryConsolidator` and all automatic dispatch wiring are removed or disabled.
- [x] No autonomous background curation is introduced in 0.24; explicit execution registers one host callback as a system action on the existing run-now surface. Its descriptor is read-only through scheduling configuration surfaces, and it is excluded from timers, automatic retries, and YAML mutation while sharing overlap protection and list/show visibility.
- [x] Registered system-action IDs are reserved; a configured YAML job whose ID collides with any system action is rejected before scheduling starts or list/show/run publishes an ambiguous identity.
...
- Startup settles an interrupted running lifecycle to failed and never resumes it automatically; when commit outcome is indeterminate, status discloses the snapshot and current revisions rather than claiming no change.
...
**Acceptance Criteria**:
- [x] The effective context of each primary human-facing turn contains the memory index state current at turn start.
- [x] The bounded index supplied to each primary turn includes the current collection revision required by `memory_apply`.
- [x] The default loaded representation is capped at both 150 rendered lines and `memory.max_bytes` (default 32 KiB), whichever is reached first.
- [x] Topic detail, archive, observations, learnings, wiki, and KG are not bulk-injected; they are retrieved through their appropriate tools/context layer.
- [x] Index selection uses semantic index entries and explicit priority/recency metadata, never a physical head/tail slice of a fully read file.
- [x] Loading does not read or UTF-8 encode an oversized whole canonical file before applying the budget.
- [x] Prompt memory is delimited and described as potentially stale/untrusted contextual data, not executable instructions; the existing safety/instruction precedence remains intact.
- [x] Primary-turn freshness and restart behavior are truthful and tested for every supported prompt strategy/provider.
...
**Acceptance Criteria**:
- [x] Search callers pass plain natural language; each backend exclusively owns its query encoding/escaping.
- [x] Punctuation, quotes, operators, and empty/whitespace queries have one documented outcome through MCP, Hub, Context Research, and fallback paths.
- [x] Every result exposes a stable unique source locator that resolves to its canonical source; personal-memory locators resolve to canonical entries, wiki locators retain native source identity, and generic `memory_save` or `archive` locators are invalid.
- [x] Wiki synthesis continues to rank above raw personal memory for the same topic and retains its provenance label, as required by ADR-029.
- [x] Wiki composition has one owner per search request; Context Research does not trigger a duplicate recursive scan.
- [x] Canonical commit success and derived-index success are separate observable outcomes.
- [x] Live mutation, pruning, migration, and full rebuild use one normalization/chunking/identity contract and converge on equivalent indexed rows.
- [x] Status/count failures are never converted into healthy zero values.
- [x] Search remains derived and rebuildable, `userId` scoped, and compatible with ADR-045 set-membership parity.
...
- A valid canonical corpus remains available for prompt context, observation capture, and apply when the derived index cannot open or rebuild; search reports unavailable/degraded rather than healthy empty. Only unreadable canonical state blocks startup.
...
**Acceptance Criteria**:
- [x] Prune-time exact-replay deduplication compares role/topic, case-sensitive whitespace-normalized content, and complete provenance/source-event identity; the earliest-created record survives and later duplicates are retired atomically. `memory_apply` performs no add-time deduplication and every add receives a new host-generated changed ID; equal text with distinct or incomplete provenance remains distinct, and semantic deduplication remains model curation.
- [x] Archive writes are rejected before exceeding the canonical readable ceiling and never leave half-applied active/archive/index state.
- [x] A canonical Markdown source is accepted through 64 MiB inclusive; an operation that would create or read 64 MiB plus one byte is rejected with no partial canonical mutation.
- [x] A dated observation partition is accepted through 8 MiB inclusive; a limit-plus-one append is rejected instead of trimming prior observations. Recursive traversal processes at most 1,000 regular files and 64 MiB aggregate per request, and search returns the best 50 results after ranking every admitted candidate, with explicit degradation signals when a processing ceiling is reached. These request ceilings are not whole-corpus caps.
- [x] Ordinary reads and mutations authenticate and open only the selected records or documents within one request budget, preserving unopened canonical members. Startup, missing-state adoption, recovery, and full rebuild may process successive independently bounded batches, but publish no reconciled revision, fingerprint, or healthy index evidence until the complete canonical union authenticates.
- [x] Migration processes at most 256 parsed records per in-memory batch while preserving one final atomic corpus commit; its operator report contains at most 100 diagnostics and 64 KiB UTF-8, with total and omitted counts when truncated.
- [x] Raw observations are not automatically deleted or hard-capped in aggregate in 0.24; aggregate disk use, oldest/newest coverage, and a warning at or above 64 MiB are visible, and user-initiated deletion is documented.
- [x] `memory.max_bytes` and archive-age configuration reject zero, negative, or out-of-range values through every load/write path. Fixed file, partition, traversal, result, and warning safety ceilings remain non-configurable constants and have exact-limit/limit-plus-one tests.
- [x] Rebuild creates and validates a fresh sibling index, then atomically replaces the target; corrupt target bytes are not opened as a prerequisite.
- [x] Failure before replacement preserves the previous target and canonical files; orphaned temporary files are safely recognizable/recoverable.
- [x] Deleted-index startup does not settle into a silently empty healthy index when canonical sources contain data.
- [x] Cooperating runtime maintenance and mutation paths share one lock/revision authority and do not overlap incompatible phases.
...
**Acceptance Criteria**:
- [x] Existing status/Memory/Knowledge surfaces distinguish curated entries, topics, archive, observations, learnings, wiki sources, and derived chunks.
- [x] Surfaces show collection revision, prompt-index budget usage, aggregate observation usage/coverage, last successful curation/reconciliation, and current index health.
- [x] Explicit curation is available through the existing on-demand job/operator execution path with clear running/succeeded/conflicted/failed outcomes.
- [x] A saved-but-index-degraded response names the durable outcome and repair action.
- [x] Users can inspect and manually edit/delete canonical Markdown, including raw observations, while the runtime is stopped; on restart, supported edits are validated and collection-revision/index reconciliation completes or reports degraded before health is reported.
- [x] Recovery guidance covers migration backup, deleted/corrupt index rebuild, legacy opaque content, and failed curation.
...
**Acceptance Criteria**:
- [x] Remove `MemoryConsolidator`, its prompts, threshold dispatch wiring, overlap risk, and tests that only assert dispatch.
- [x] Remove obsolete `searchVector`, legacy `MemoryChunk`, and unused identity helpers unless an accepted 0.25 contract has a concrete production consumer.
- [x] Remove duplicate wiki-search composition and retain one tested owner.
- [x] Do not add a new package, database, daemon, scheduler, approval framework, or speculative provider abstraction for this work.
- [x] QMD receives no new memory semantics and follows ADR-050 deprecation/removal timing.
- [x] 0.25 owns database abstraction, PostgreSQL, multilingual FTS, embeddings, hybrid/vector indexing, and QMD deprecation at Phase B GA; QMD implementation removal remains one milestone later per ADR-050.
- [x] 0.27 owns autonomous scheduling/policy, governed idle-time stewardship, richer approval workflows, validation/dogfooding loops beyond 0.24 contract tests, and OKF interoperability.
- [x] 0.27 retains guarded wiki/KG knowledge-write policy and semantics; 0.24 memory mutation tools do not write wiki/KG or broaden their existing write contracts.
...
**Acceptance Criteria**:
- [x] Add contract/fault tests for mutation CAS, prompt bounds/freshness, query ownership, locator resolution, index convergence/degradation, resource ceilings, and corrupt-index recovery.
- [x] Add narrow fitness checks where a cheap structural invariant prevents regression, including no generic locator, no caller-side FTS encoding, no automatic consolidator, and no unconsumed exported memory API.
- [x] Record the memory corpus, identity, provenance, mutation, prompt-context, and 0.25/0.27 boundaries in a new or amended accepted ADR; amend ADR-002's `MEMORY.md`-only source wording for the bounded index plus topic corpus.
- [x] Amend ADR-042 for stable memory-entry locators and reconcile ADR-007 status/content with actual provider prompt behavior.
- [x] Preserve ADR-029 wiki-over-memory ranking, ADR-045's file-canonical/database-index boundary, and ADR-050's 0.25 Phase B deprecation plus one-milestone-later removal timing in code-backed acceptance tests and documentation.
- [x] Update data-model, system, security/semantic-trust, observability/operations, package AGENTS, configuration, workspace, search, CLI/API, and recovery documentation in the same changes as behavior.
- [x] All architecture-review findings below map to an implemented acceptance test or an explicit later-release boundary.
...
2. **Observation capture**: A journal, ordinary turn, or supported pre-compaction lifecycle hook calls the same observation authority → host stores bounded content with host-bound provenance → observation remains outside prompt-authoritative memory but is available as bounded curation context. Pre-compaction acknowledgement waits for the capture attempt, then fails open exactly once if persistence fails.
...
- **Observation**: host-generated ID, recorded time, host-bound origin/caller/session reference when available, content, truncation markers, trust/provenance label, and optional references to resulting curated entries.
...
- **Memory snapshot/change set**: collection revision, bounded included entries/observations with their identities, revisions, and provenance, plus structured operations and reasons. Existing targets and merge sources must come from the included entry set; operation input carries no caller-authored provenance, and commit is all-or-nothing.
- **Memory locator**: stable source identity that resolves independently of a derived chunk row ID or database rebuild.
- **Corpus manifest**: non-authoritative coordination state covering codec-validated canonical documents and byte-verbatim `memory/legacy/` members with authenticated paths, roles, lengths, digests, and record ownership. It permits bounded selected access, preserves unopened members, detects legacy-byte drift, and is rebuilt from canonical files when absent.
...
- **Deletion audit**: entry ID, time, host-derived provenance, and a bounded retirement reason – caller-authored for apply/merge/remove and host-authored for pruning. The host copies no retired entry content into the audit; a caller-authored reason may independently quote it.
...
| Caller-authored apply provenance would make attribution forgeable | Host binds provenance from active trusted context; operation schemas reject provenance/actor/source fields in FR2. |
| Per-request traversal ceilings could be misapplied as whole-corpus caps | Selected access stays within one budget; complete-union authentication and rebuild proceed in bounded batches before healthy publication in FR6. |
| Claude pre-compaction capture was declared but not wired | Persist a bounded redacted observation with host session provenance before exactly-once acknowledgement in FR2. |
...
| Retrieval efficiency | Recursive/file retrieval is bounded and duplicate composition removed. | One wiki composition per request; explicit file/count/byte/result ceilings; no request ceiling becomes a total-corpus cap. |
...
| Same normalized content, store/topic, and provenance/source event is added again | The add commits with a new host-generated ID; a later prune collapses only a complete exact-replay tuple to the earliest-created entry. Distinct or incomplete source-event identity remains separate. | Run pruning or explicit semantic curation. |
...
| Corpus exceeds one traversal budget | Ordinary operations authenticate only selected members; startup/rebuild processes bounded batches and withholds healthy publication until complete-union authentication. | Repair the reported canonical member or resource condition and rerun reconciliation. |
| Pre-compaction observation capture fails | Log the failed persistence attempt, acknowledge compaction exactly once after it settles, and retain no fabricated success. | Inspect memory/index health; later turns or explicit curation continue normally. |
...
| Index deleted or unusable while canonical data exists | Startup/status reports missing/degraded and rebuilds or directs repair; never healthy empty. Valid canonical prompt context, capture, and apply remain available while search is degraded. | Run documented reconciliation/rebuild. |
...
| User requests forgetting | Remove active canonical content and index rows; the audit retains metadata plus the caller's bounded verbatim reason, but the host copies no deleted entry content. | Restore only from user backup if deletion was intentional. |
...
The implemented branch preserves the PRD's product scope and milestone boundaries. Implementation and gap review made these contracts more explicit:

- **Closed design correction – host-bound apply provenance**: `memory_apply` operations do not supply provenance, actor, owner, or source fields. The host derives attribution from its active trusted context and rejects dispatch without that context. This resolves contradictory operation wording without changing the trusted-host threat model.
- **Bounded corpus clarification**: the 1,000-file/64 MiB aggregate ceiling bounds one request, not total retained memory. Ordinary operations authenticate selected members; startup, recovery, and rebuild authenticate the complete union through successive bounded batches before publishing reconciled identity or healthy index state. The authenticated manifest remains derived coordination state, never canonical memory.
- **Review-discovered lifecycle wiring**: where Claude exposes `PreCompact`, DartClaw attempts bounded redacted observation capture with host session provenance before acknowledging the hook. The observation remains non-authoritative, and capture failure cannot deadlock compaction.
- **Canonical role clarification**: learnings and the deletion audit participate in the canonical corpus and shared revision/fingerprint authority. Learnings remain bounded and model-readable; deletion audit metadata remains outside search/read.
- **Curation lifecycle clarification**: curation uses separately bounded entry and observation candidate sets, advances its observation anchor only after confirmed success, and settles interrupted evidence truthfully without automatic resume.
- **Derived-index availability clarification**: a derived-index startup/rebuild failure degrades search but does not make valid canonical prompt, capture, or mutation paths unavailable.

The S05 reconciliation entry is closed, the final plan-level gap review reports no remaining Fix or Note findings, and no deferred PRD/code divergence remains.
...
| Bind provenance in the trusted host | Model/tool input is semantically untrusted and cannot author the attribution retained in canonical entries or deletion audits. | Caller-supplied provenance, actor, or source fields. |
| Authenticate a non-authoritative corpus manifest | Bounded selected reads and sparse writes must preserve unopened members while startup/rebuild still prove the complete canonical union. | Treat request ceilings as corpus caps; materialize the whole corpus for every operation; make the manifest authoritative. |
| Capture supported pre-compaction context as an observation | The lifecycle boundary can retain a bounded redacted tail without making it prompt-authoritative or blocking compaction indefinitely. | Ignore the hook; inject the tail as curated memory; acknowledge before persistence is attempted. |
