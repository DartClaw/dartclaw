...
- [ ] **S01 [OC01] [TI02,TI04] Enabled job distills today's turn log into MEMORY.md**
...
- [ ] **S02 [OC02] [TI01,TI04] Default off – zero behavior change**
...
- [ ] **S03 [OC01] [TI02] Empty day – no turn log, nothing recorded**
...
- [ ] **S04 [OC03] [TI04] Journal composes with the existing size-cap consolidation, never replaces it**
...
- [ ] **S05 [OC02] [TI05] Invalid schedule fails loudly, matching `memory.pruning` semantics**
...
- [ ] **S06 [OC04] [TI04] Injected log instructions cannot widen journal capabilities**
...
- [ ] **S07 [OC02] [TI04] Enabled built-in rejects a colliding user job ID**
...
- [ ] No `memory.consolidation` config key is introduced, and the new job/config/class names never use "consolidation" – that term remains reserved for describing `MemoryConsolidator`'s existing size-cap dedup and for TI06's explicit journal-vs-consolidation disambiguation. (Proved by Final Validation gate + TI06 Verify.)
- [ ] `MemoryConsolidator`, `MemoryPruner`, and `knowledge.inbox` behavior, prompts, and registration are byte-for-byte unchanged; existing `dartclaw_config`, `dartclaw_server`, and `dartclaw_cli` suites pass. (Proved by TI04 Verify + CI gate.)
- [ ] The job is registered at the production composition root (`SchedulingWiring.wire()` in `apps/dartclaw_cli`), proven by a wiring-layer test – not only by `dartclaw_server`-level unit tests. (Proved by TI04 Verify.)
- [ ] The config API projection includes a `journal` block with `enabled` and `schedule` under `memory`, alongside the existing `pruning` block. (Proved by TI03 Verify.)
- [ ] Recipe 02's config block and `examples/personal-assistant.yaml` agree on the built-in `memory.journal` toggle, with no hand-written `id: daily-journal` job remaining in the example. (Proved by TI07 Verify.)
- [ ] The journal job reuses the existing session-local tool-policy seam; it introduces no journal-specific guard framework and does not claim enforcement beyond the active provider boundary. (Proved by TI04 Verify.)
- [ ] `ScheduledJob.allowedTools` is an internal runtime policy field only: `ScheduledJob.fromConfig` leaves it null for every user-defined job, and `ScheduleService` forwards it unchanged to `TurnManager.startTurn`; no `scheduling.jobs[].allowed_tools` config surface is introduced. (Proved by TI04 Verify.)
- [ ] When the built-in is enabled, the assembled scheduled-job list has exactly one `memory-journal` ID; a user collision fails before `ScheduleService.start()` and cannot produce duplicate display/system rows or timer replacement. (Proved by S07 + TI04 Verify.)
- [ ] Completion evidence includes one production-path live journal run with an available configured primary provider; deterministic fake-harness evidence alone cannot close S01/S03. The observed tool call and resulting categorized `MEMORY.md` entry are recorded in Implementation Observations. (Proved by Final Validation gate.)
...
- [ ] **TI01** `MemoryConfig` carries `journalEnabled` (default `false`) and `journalSchedule` (default `'0 22 * * *'`), parsed from `memory.journal.{enabled, schedule}` with `FieldMeta` rows `memory.journal.enabled` / `memory.journal.schedule` (`ConfigMutability.restart`, matching `memory.pruning.*`), equality/hashCode extended
...
- [ ] **TI03** The config API/UI projection surfaces `memory.journal.*` alongside `memory.pruning.*`
...
- [ ] **TI04** The scheduler carries and applies the built-in journal's closed tool policy
...
- [ ] **TI05** An invalid `memory.journal.schedule` fails scheduling wiring with the same error surface as an invalid `memory.pruning.schedule` (`CronExpression.parse`), never a silently missing job
...
- [ ] **TI07** `examples/personal-assistant.yaml` opts into `memory.journal` instead of the hand-written `daily-journal` job (the `weekly-review` job stays as the `scheduling.jobs` example)
...
- [ ] `rg -n "memory\.consolidation" packages/ apps/ docs/ examples/` in `dartclaw-public` produces no matches (the feedback's example key must not leak in).
- [ ] Required live smoke passes through the production service path with an available configured primary provider; Implementation Observations record provider, trigger, seeded fact, `memory_save` evidence, and resulting categorized entry. No unsigned skip is accepted.
