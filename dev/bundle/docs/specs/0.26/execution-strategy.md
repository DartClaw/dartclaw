...
- Keep `plan.json`'s declared package-ceiling owners. S01 owns kernel changes, S05 the workflow re-cut, S07 the core raise; other affected stories report measured deltas. Resolve conflicting re-cut instructions before editing the same constant. Never hide growth by silently raising a ceiling.
