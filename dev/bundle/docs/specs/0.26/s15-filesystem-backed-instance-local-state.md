...
- **TI05** Both store suites pass on Windows through a repeatable runner
  ...
  - **Verify**: `cmd: bash dev/tools/parallels_windows.sh powershell dev/tools/run_windows_tests.ps1 packages/dartclaw_core/test/storage/turn_state_store_test.dart packages/dartclaw_core/test/storage/webhook_delivery_store_test.dart` – exit 0 with both suites passing on Windows, including the purge-under-open-handle case in its Windows branch (S07)
  - **SATISFIES**: S07
