...
version: alpha
name: DartClaw
description: Terminal-aesthetic design system for a developer-focused AI agent runtime. Catppuccin Mocha (dark) / Latte (light) palette, monospace typography, terminal-green accent.
colors:
...
# DartClaw Design System

Terminal-aesthetic design system for a developer-focused AI agent runtime. Catppuccin Mocha/Latte palette, monospace typography, terminal-green accent.
...
- **Aesthetic primitives** — monospace type throughout, a layered surface ladder for depth, hue-aware shadows that match the palette's blue-violet tint, a single terminal-green accent, semantic colors only for state (success/error/warning/info).
- **Density** — information-dense by default; whitespace is earned, not assumed. The reading rhythm is set by tight, rectangular cards on a calm gradient base.
...
- **Base size** — 14px (`body-md`). Larger sizes are reserved for page title (20px) and section headings (16px). Smaller sizes carry metadata and pill text (12–13px).
- **Weights** — three only: `400` (normal body), `500` (medium — session titles, UI labels), `600` (bold — headings, role labels).
- **Line height** — `1.6` for body and code; `1.3` for headings and tight UI like the input textarea.
...
Fixed 3-stop gradient: `linear-gradient(170deg, crust → base → mantle)`. Prevents a flat-panel look and provides a calm, slightly atmospheric ground for the cards.
...
Cards and similar surfaces also carry a 1px **luminous top-edge highlight** (`rgba(255, 255, 255, 0.08)` in dark, `rgba(0, 0, 0, 0.06)` in light). This catches the eye and reinforces that the surface sits on top of, rather than inside, the background.
...
- `.btn` — default (surface bg + border)
- `.btn-primary` — accent bg, high-contrast text, glow on hover
- `.btn-ghost` — transparent bg, no border
- `.btn-danger` — transparent bg, error border
- `.btn-icon` — square, icon-only
- `.btn-full` — full-width
- States: hover (lighter bg + border highlight), active (darker), disabled (0.4 opacity)
...
Toasts auto-dismiss after 4s, slide in from the right.

### Messages

- `.msg` — base message with left border accent
- `.msg-user` — green left border
- `.msg-assistant` — blue left border
...
- **Don't** introduce a second typeface "for headings" or "for body". One family, three weights, five sizes.
