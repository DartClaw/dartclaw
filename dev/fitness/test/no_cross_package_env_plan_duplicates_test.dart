// Fitness function: ProcessEnvironmentPlan implementations must live in dartclaw_kernel.
//
// What this enforces:
//   Any class that implements `ProcessEnvironmentPlan` must live inside
//   `packages/dartclaw_kernel/`. Implementations in other packages duplicate
//   the canonical kernel type and create drift risk.
...
//   all non-kernel impls to the kernel; this fitness test prevents
//   re-introduction.
//
// How to resolve a failure:
//   Option A (preferred): Delete the cross-package impl and use
//   `InlineProcessEnvironmentPlan` from `dartclaw_kernel` instead.
//   Option B (genuine credential-carrying impl): Add an entry to
//   `allowlist/no_cross_package_env_plan_duplicates.txt`
//   with format `<ClassName>@<relative-file-path>  # <rationale>`. The
//   rationale must explain why this impl cannot live in dartclaw_kernel.
...
void main() {
  ...
  test('allowlist entries have required rationale format', () {
    assertAllowlistFormat(
      allowlistFile(repoRoot, 'no_cross_package_env_plan_duplicates.txt'),
      entryFormat: '<ClassName>@<relative-path>',
    );
  });

  test('ProcessEnvironmentPlan implementations only in dartclaw_kernel or allowlisted', () {
    final violations = <String>[];
    final implPattern = RegExp(r'class\s+(\w+)[^{]*\bimplements\b[^{]*\bProcessEnvironmentPlan\b');
    final kernelPrefix = 'packages/dartclaw_kernel/';
    ...
    for (final baseDir in ['packages', 'apps']) {
      ...
      for (final pkg in dir.listSync().whereType<Directory>()) {
        ...
        for (final file in libDir.listSync(recursive: true).whereType<File>()) {
          ...
          if (relativePath.startsWith(kernelPrefix)) continue;
          ...
          for (final match in implPattern.allMatches(source)) {
            ...
            if (!allowlist.containsKey(key)) {
              ...
              violations.add(
                '$relativePath:$line: $className implements ProcessEnvironmentPlan outside dartclaw_kernel — '
    ...
    if (violations.isNotEmpty) {
      fail(
        'Cross-package ProcessEnvironmentPlan violations (see $fitnessReadmePath):\n'
        '  ${violations.join('\n  ')}',
      );
    }
  });
