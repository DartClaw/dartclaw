...
- release asset audit: confirm GitHub Releases has `dartclaw-v{VERSION}-macos-arm64.tar.gz`, `dartclaw-v{VERSION}-macos-x64.tar.gz`, `dartclaw-v{VERSION}-linux-x64.tar.gz`, and `dartclaw-v{VERSION}-linux-arm64.tar.gz`, each matching `.sha256`, with `SHA256SUMS.txt` covering every platform archive. Each platform archive must contain `bin/dartclaw` **and** `lib/libsqlite3.*` (the bundled SQLite library — `libsqlite3.dylib` on macOS, `libsqlite3.so` on Linux); an archive missing `lib/` was built without the native asset and will crash at the first SQLite call
...
3. **Tag** annotated `v<version>` from the squash commit; push tag.
   The release workflow must publish `macos-arm64`, `macos-x64`, `linux-x64`, and `linux-arm64` platform archives, per-archive `.sha256` files, and aggregate `SHA256SUMS.txt`, and (via the `homebrew` job) push the rendered `Formula/dartclaw.rb` to the `DartClaw/homebrew-dartclaw` tap.
