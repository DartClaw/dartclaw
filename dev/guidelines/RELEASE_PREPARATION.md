...
- Container conformance on both engines: run `dart test --run-skipped -t integration packages/dartclaw_server/test/integration/` on **Linux Docker** *and* on **Docker Desktop / OrbStack**, and record both non-skipped results (engine, host OS, date, pass counts) in the release PR. A container-execution claim in the docs is release-ready only with evidence from both engines; one platform passing is not a release gate pass. The default-suite matrix (`packages/dartclaw_server/test/execution_conformance_matrix_test.dart`) additionally fails if any advertised provider/execution/surface combination has lost its runtime evidence — it does not substitute for running the fixtures it names.
