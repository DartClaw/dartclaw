...
Use **agent-browser** as the primary tool for visual validation — it's faster and simpler for the snapshot/interact loop that makes up most validation work.

Core agent-browser workflow:
1. `agent-browser open <url>` — navigate to page
2. `agent-browser snapshot` — capture page state (use `-i` for interactive element refs)
3. `agent-browser click @e1` / `agent-browser fill @e2 "text"` — interact using refs
4. Re-snapshot after page changes

Use **chrome-devtools MCP** when deeper inspection is needed (computed styles, DOM debugging, console errors, JS execution, network requests). Key operations:
- `evaluate_script` — inspect DOM state, computed styles, localStorage
- `list_console_messages` — check for JS errors
- `take_screenshot` — capture visual state when agent-browser snapshots are insufficient
- `resize_page` / `emulate` — test responsive layouts at specific pixel widths
