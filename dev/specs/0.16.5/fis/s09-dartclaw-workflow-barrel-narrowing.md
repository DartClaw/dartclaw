...
Tighten the `dartclaw_workflow` public API by replacing 34 wholesale `export 'src/...'` lines in `packages/dartclaw_workflow/lib/dartclaw_workflow.dart` with `show`-clause exports, then curate the surface down to ≤35 intentional public symbols by removing internal-only types. This is a load-bearing prerequisite for S10's `barrel_show_clauses_test.dart` fitness gate and locks in Shared Decision #20 (per-pkg soft caps).
...
> - Barrel exposes ≤35 public symbols
...
- [ ] The barrel exposes ≤35 distinct public symbols from `src/...` exports (the existing two re-exports — `package:dartclaw_core` and `package:dartclaw_models` — stay as bulk re-exports and are out of scope for the cap, mirroring how the cap is phrased in plan + Shared Decision #20)
...
### Barrel exports go from 34 wholesale → ≤35 with `show`
- **Given** `packages/dartclaw_workflow/lib/dartclaw_workflow.dart` currently has 34 `export 'src/...'` lines without `show` clauses
- **When** the story is complete
- **Then** `rg "^export 'src/" packages/dartclaw_workflow/lib/dartclaw_workflow.dart | rg -v ' show '` returns zero lines, AND counting distinct identifiers in the `show` clauses across all `export 'src/...'` lines yields ≤35
...
- **Escalate**: If audit reveals the curated total cannot fit ≤35 without removing a symbol that has confirmed downstream consumers, stop and surface the conflict — do not silently breach the cap or remove an in-use symbol.
...
2. **Defer curation to S25** (this story does only the `show`-clause mechanical pass) — rejected: plan explicitly names ≤35 in S09 acceptance and lists internal-only candidates; deferring would re-litigate scope and S10 would freeze a still-bloated allowlist.
...
file   | packages/dartclaw_workflow/lib/dartclaw_workflow.dart        | Target file — current state has 4 `show`-clause exports + 34 wholesale; end state ≤35 curated `show`-clause exports plus the two existing `package:` re-exports
...
- [ ] **TI01** Audit produces a per-file inventory: every `lib/src/...` currently wholesale-exported has its public top-level declarations enumerated, paired with a downstream-consumers check (`rg`-grepped against `packages/dartclaw_server`, `apps/dartclaw_cli`, `packages/dartclaw_*` test trees). Output: an in-spec working note (held in the agent's scratch context, not committed) that classifies each declaration as `keep` / `internal-only-remove` / `retain-with-rationale`. Curated `keep` total ≤35.
  ...
  - **Verify**: Working note enumerates classification for every line currently in the barrel; the `keep` count is ≤35; every plan-named likely-internal candidate has a remove-or-retain decision with one-line evidence (downstream consumer file:line, or "no downstream consumer found").
...
- [ ] **TI06** Final cap and self-check: distinct symbol count across all `show` clauses in `dartclaw_workflow.dart` (excluding the two `package:` re-exports) ≤35; `umbrella_exports_test.dart` in `packages/dartclaw/test/` still passes.
  - Count via: read each `show` clause; collect unique identifiers; assert size ≤35. The plan's "≤35" target is a hard cap aligned with Shared Decision #20.
  - **Verify**: `dart test packages/dartclaw` passes (umbrella contract); a manual count of `show`-clause identifiers from `packages/dartclaw_workflow/lib/dartclaw_workflow.dart` yields ≤35.
...
- [TI02] Scenario "Barrel exports go from 34 wholesale → ≤35 with `show`" → Add `show` clauses preserving all symbols; assert via `rg` count + workspace test pass
...
- [TI03,TI06] Scenario "Retained borderline candidate carries rationale" → Manual review of the resulting barrel: every retained candidate from the plan list has a rationale comment
...
- Feature-specific: count assertion of distinct `show`-clause identifiers ≤35 (manual or grep-based; not a test file — that's S10's domain).

### Execution Contract

- Implement tasks in listed order. Each **Verify** line must pass before proceeding.
- Prescriptive details (file paths, plan-named candidates, the 35 cap) are exact.
