...
- `packages/dartclaw_models/lib/src/workflow_definition.dart:475-477` — actual location of `WorkflowStep.type` (the field lives in `dartclaw_models`, not `dartclaw_workflow` as the plan brief states; the plan brief's `:471` line ref is stale).
...
- [ ] **Pre-flight**: S35 has landed — `WorkflowStep.type` is typed `WorkflowTaskType` (not `String`) and `WorkflowTaskType.fromJsonString` exists. If S35 has not landed, this story stops with `BLOCKED: S35 dependency not met`
- [ ] `WorkflowStep` in `packages/dartclaw_models/lib/src/workflow_definition.dart` exposes a new `taskType` field (typed `WorkflowTaskType`) and the old `type` field carries `@Deprecated('Use taskType')` — both fields have identical values populated from a single ctor source of truth (canonical: ctor accepts `taskType`, defaults to `WorkflowTaskType.agent`; `type` is a `@Deprecated` getter that returns `taskType` so JSON round-trip remains driven by one field)
...
- `WorkflowStep.taskType` field + `@Deprecated('Use taskType')` on the existing `type` field in `packages/dartclaw_models/lib/src/workflow_definition.dart`; in-tree call-site migration under `packages/dartclaw_workflow/lib/src/workflow/`
...
- **`packages/dartclaw_models/lib/src/workflow_definition.dart`** — Part (c). Add `taskType` field, deprecate `type`, update ctor + `toJson` + `fromJson`. **Note**: file lives in `dartclaw_models`, not `dartclaw_workflow` (plan brief is stale on this point).
...
file   | packages/dartclaw_models/lib/src/workflow_definition.dart                                | Part (c) — WorkflowStep.type at :475-477; ctor :646-680; fromJson :714-735
...
- **S35 dependency strict**: `WorkflowTaskType` enum + `WorkflowTaskType.fromJsonString` must exist before part (c) starts. -- Verify: `grep "enum WorkflowTaskType\|class WorkflowTaskType" packages/dartclaw_models/lib/` returns ≥ 1 line. If not, STOP with `BLOCKED: S35 dependency not met`.
...
- **Plan-brief line numbers are stale**: `workflow_definition.dart:471`, `workflow_executor.dart:577,588,…`, `task_executor.dart:35-62` cited in the plan brief do not match current `e670c47` line numbers. -- Workaround: locate by symbol/pattern. Actual locations: `WorkflowStep.type` is at `dartclaw_models/lib/src/workflow_definition.dart:475-477`; `TaskExecutor` ctor is at `task_executor.dart:40-94`; log sites are at lines listed in the severity-mapping table above.
- **`WorkflowStep` lives in `dartclaw_models`, not `dartclaw_workflow`**: plan brief and technical research both reference `packages/dartclaw_workflow/lib/src/workflow/workflow_definition.dart` but the file actually lives at `packages/dartclaw_models/lib/src/workflow_definition.dart`. -- Workaround: edit the correct file. The `WorkflowStep` import on the consumer side is already routed via the `dartclaw_models` re-export through `dartclaw_core`.
...
- [ ] **TI01** Verify pre-flight dependencies.
  - Confirm S35 has landed: `rg "enum WorkflowTaskType|class WorkflowTaskType" packages/dartclaw_models/lib/` returns ≥1 line; `rg "WorkflowTaskType.fromJsonString" packages/dartclaw_models/lib/` returns ≥1 line. Confirm `TaskExecutorLimits` does **not** already exist (S16 has not landed first): `rg "class TaskExecutorLimits" packages/dartclaw_server/lib/src/task/` returns 0 lines. Record findings under Implementation Observations.
  - **Verify**: both checks return expected results. If S35 not landed, STOP with `BLOCKED: S35 dependency not met`. If `TaskExecutorLimits` already exists, switch to "import-only" mode for part (e) and record under Implementation Observations.
...
- [ ] **TI06** Part (c) — add `WorkflowStep.taskType` field; deprecate `type`.
  - Open `packages/dartclaw_models/lib/src/workflow_definition.dart` at `:475-477`. Replace the `final String type` field with: `final WorkflowTaskType taskType;` (typed via S35's enum). Add a deprecated getter: `@Deprecated('Use taskType') WorkflowTaskType get type => taskType;`. Update ctor at `:646-680`: replace `this.type = 'agent'` with `this.taskType = WorkflowTaskType.agent`. Update `toJson` at `:680-712`: replace `if (type != 'agent') 'type': type` with `if (taskType != WorkflowTaskType.agent) 'type': taskType.name` (or `taskType.toJson()` if S35 provides). Update `fromJson` at `:714-735`: replace `type: (json['type'] as String?) ?? 'agent'` with `taskType: json['type'] != null ? WorkflowTaskType.fromJsonString(json['type'] as String) : WorkflowTaskType.agent`.
  - **Verify**: `dart analyze packages/dartclaw_models` reports zero issues; `dart test packages/dartclaw_models/test/workflow_definition_test.dart` passes — JSON round-trip is byte-identical for the existing fixtures; a manual round-trip on `{'type': 'bash'}` produces `{'type': 'bash'}` with `step.taskType == WorkflowTaskType.bash`.
...
- [ ] **TI13** Workspace verification.
  - Run `dart format --set-exit-if-changed packages apps`, `dart analyze --fatal-warnings --fatal-infos`, `dart test packages/dartclaw_server/test/task`, `dart test packages/dartclaw_workflow/test/workflow`, `dart test packages/dartclaw_models/test`, `dart test packages/dartclaw_core/test`, and `dart test` workspace-wide. Scan `rg 'TODO|FIXME|placeholder|not.implemented' packages/dartclaw_workflow/lib/src/workflow/workflow_executor.dart packages/dartclaw_core/lib/src/task/task.dart packages/dartclaw_models/lib/src/workflow_definition.dart packages/dartclaw_server/lib/src/task/task_executor.dart packages/dartclaw_server/lib/src/task/task_executor_limits.dart packages/dartclaw_core/lib/dartclaw_core.dart` for stragglers.
...
- [TI06,TI07] Scenario _`WorkflowStep` JSON round-trip is byte-identical post-rename_ → `dart test packages/dartclaw_models/test/workflow_definition_test.dart` proves round-trip; `dart test packages/dartclaw_workflow/test/workflow/` proves dispatcher logic still routes by kind.
