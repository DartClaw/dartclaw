...
| [012](../adrs/012-per-type-container-isolation.md) | Per-Type Container Isolation | Accepted (amended 2026-08-11 — profiles are filesystem/capability templates; each live container authority owns a dedicated container, and container harnesses are never cached across release) | Security / isolation |
...
| [044](../adrs/044-workflow-orchestration-agent-architecture.md) | Workflow Orchestration Agent — In-Engine Decision-Object Seam, Dedicated Scope, Hybrid Anti-Thrash, Per-Hold Lifecycle | Proposed (targets 0.29 after the 2026-08-07 release renumber) | Workflow / agent |
| [045](../adrs/045-pluggable-database-backend.md) | Pluggable Database Backend — SQLite Default, PostgreSQL Opt-In | Accepted 2026-07-24; schema strategy amended 2026-07-30 (current-schema bootstrap + compatibility gate replaces the planned migration runner; targets **0.25**) | Storage |
...
- **Logical-agent orchestration and execution allocation are host-owned and provider-independent**: `sessions_spawn` and `sessions_send` are the only logical-agent conversation tools. After global governance, one execution coordinator owns the serialized primary-interactive lane and all per-provider worker leases. Main user/channel turns use the fixed primary lane; cron/system jobs, advisor turns, tasks, and logical agents use worker capacity; workflow one-shots are capacity-only. `providers.<id>.pool_size` is the hard worker-execution ceiling and excludes the primary lane. Harness reuse is opportunistic and knob-free: within one immutable coordinator composition, prefer the exact session, then a healthy *host* worker with the same provider and identical effective execution policy, otherwise create fresh; unknown or unhealthy state is never reused. Container workers are never cached — each live container authority owns a dedicated container destroyed on release (ADR-012, 2026-08-11 amendment). Replacement requires confirmed managed-root teardown or the slot is quarantined. Observability derives from leases. Provider branching stays inside adapters and composition/wiring. The SDK single-harness task fallback is compatibility-only and is not a server logical-agent path. This supersedes the earlier idle-runner pool interpretation and the 2026-08-05 provider-native-subagent decision; the ADR-016 execution-routing amendment was implemented in 0.24.
...
- **LOC fitness ceilings get headroom, not allowlist churn**: when a size ceiling (workflow-package LOC, test-file lines) trips on ordinary story growth, bump the ceiling with a dated rationale instead of churning allowlists; structural gates (method counts, barrel exports, core LOC ratchet) stay strict. Owner decision, 2026-07-05.
