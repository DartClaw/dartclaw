...
- **Process termination confirmation ownership**: killWithEscalation returns a typed ProcessTerminationResult and logs unconfirmed termination locally through its existing label and Logger; AgentHarness and HarnessPool lifecycle APIs remain Future<void>. This meets the observable warning contract without threading shutdown results across public harness interfaces.
...
- **Delegation enforcement boundary: host-enforced delegated turns; provider-enforced native subagents**: Two-tier tool-policy enforcement posture (0.24 agent-tool-policy-enforcement FIS): DartClaw-dispatched delegated turns (sessions_send → SessionDelegate → harness turn) are host-enforced by ToolPolicyGuard with threaded agent identity on all harnesses (Claude, Codex, ACP). Provider-native subagent execution inside a main turn – today only Claude Code's agents-handshake subagents, whose PreToolUse hooks carry no subagent identity (agentId null) – is provider-enforced via the per-agent tools key forwarded in the initialize payload; the host does not attempt identity recovery on that path. Codex has no provider-native subagent registration path in DartClaw, so all Codex delegation falls in the host-enforced tier; any future provider-native mechanism on another harness inherits the provider-enforced posture unless revisited. Rationale: the provider-side gate already ships (TI09), hook payloads do not identify the executing subagent, and a host-side recovery mechanism would add speculative complexity to a path that has a real gate – KISS. Owner-decided via andthen:preflight interview, 2026-08-05.
- **Codex host guard enforcement is approval-routed (inactive under approval: never)**: The Codex harness's only beforeToolCall guard site is the approval handler, so host-side tool-policy/network guard enforcement on Codex operates only under approval: on-request/granular; under the recommended deadlock workaround (approval: never + full sandbox, control-protocol.md), no Codex tool call reaches the host guards – enforcement there is provider/sandbox-side only. Consequences accepted 2026-08-05 (0.24 agent-tool-policy-enforcement FIS, preflight round 2): OC-level guard-block claims are unconditional only on Claude; DartClaw's own-MCP semantic canonical mapping is Claude-scoped (Codex MCP calls carry the generic mcp_tool_call raw name), so Codex+MCP delegated search is a documented limitation; serve warns at startup when sandboxed agents + a Codex harness + approval: never coexist; control-protocol.md's 'guard chain remains active' claim is corrected to message-level hooks only. Revisit if the upstream Codex approval deadlock (codex#11816) is fixed or MCP calls gain approval routing.
