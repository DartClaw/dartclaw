...
| [ADR-003](../adrs/003-coding-task-support-and-agent-extensibility.md) §programmatic provider-native agents (partial) | Logical-agent orchestration decision (2026-08-09) | Partial. Durable DartClaw sessions, host policy, and the shared worker pool replace the Claude-native agent payload as the orchestration contract; provider ecosystems and adapter capabilities remain in force. |
...
- **Logical-agent orchestration is host-owned and provider-independent**: `sessions_spawn` and `sessions_send` are the only logical-agent conversation tools. Both route through `LogicalAgentSessionService`, durable DartClaw sessions, host guards, and the shared bounded `HarnessPool`; `AgentDefinition.provider` and `securityProfile` select state-free execution capacity without changing orchestration semantics. Provider-native agent registration is not an orchestration path. Harness adapters may implement continuity differently only where their external protocols require it, while preserving the same persisted-session contract and isolation. `providers.<id>.pool_size` is the sole worker-capacity setting. This supersedes the 2026-08-05 two-tier provider-native-subagent decision, whose separate enforcement path added provider-specific policy and duplicated host orchestration. Owner decision, 2026-08-09.
