...
- **Windows bash qualification evidence ownership**: S06 owns a committed manual scenario at dev/testing/scenarios/windows-bash-step.md and a stable latest-run record at dev/testing/evidence/windows-bash-step.md; git history preserves prior evidence. S09 remains the core runtime smoke path and does not absorb bash qualification.
