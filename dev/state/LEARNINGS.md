...
- **Codex constrains the reply to the schema and returns it as ordinary text.** Verified at codex-cli 0.153.4: `turn/start` takes the unmodified strict envelope schema and the final assistant message conforms even when the prompt never mentions JSON, yet `turn/completed.params` is `threadId`/`turn` and the agentMessage item carries only `text`. So `supportsStructuredOutput` stays `false`, `supportsOutputSchemaConstraint` is `true`, and the host reads and validates the constrained body (ADR-031 amendment 2026-09-13).
- **A same-thread re-ask that names no defect reproduces it.** The model rereads its own bad reply as context, so state the parser's message, offset and excerpt or expect the identical output twice.
