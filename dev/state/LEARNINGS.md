...
- **`gating_findings_count` is a DartClaw-owned LLM-prompt schema preset, and it must track AndThen's fix-character routing, not severity.** The count is *not* an AndThen-emitted field; it is the prompt description in `schema_presets.dart` (`gatingFindingsCountPreset`) that tells the review agent which integer to emit, and remediation loops key their exit gate on `gating_findings_count == 0`. **Why:** AndThen's review skill routes a finding by *fix-character* – a bounded/mechanical fix → **Fix** (auto-appliable by `andthen:remediate-findings --auto`); a design-judgment / decision / reconciliation gap stays **Note** at any severity and is never auto-applied. If the preset counts by severity (the old "MEDIUM-or-higher" wording), a Note-routed MEDIUM+ inflates the count, the exit gate can never be satisfied while only Note findings remain, and the loop churns to `maxIterations` → `failLoop` (deadlock on a Note-only stall). **How to apply:** define the count as *still-unresolved Fix-routed findings only*; 0 means nothing remains for automated remediation. Don't touch the deterministic `isGatingFinding` severity fallback in `review_finding_derivations.dart` – it operates on inline `verdict`-shaped payloads that carry only `severity` (no routing), and is a fallback, not the AUTO_MODE happy path. `gating_findings_count` is used *only* as a loop entry/exit measure in the built-in YAMLs (never a terminal pass/fail quality gate), so realigning the definition is a safe global change with no behavior-masking risk.
