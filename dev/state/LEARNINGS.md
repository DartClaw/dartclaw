...
- **A bare `exit(...)` inside a `Process` fake kills the test runner and `dart test` still reports success.** In a subclass method an unqualified `exit(0)` binds to `dart:io`'s top-level `exit`, NOT the inherited `FakeProcess.exit` — the runner dies mid-suite with status 0, so remaining tests never run and CI is green. `dartclaw_core` lost 57 tests this way for several releases. Symptom to watch for: a package's reported test count silently shrinking, or no "All tests passed!" line despite exit 0. Always call `this.exit(...)` on a fake, and treat a dropping test count as a failure.
- **`dart test` runs suites as isolates in ONE process, so `Directory.current` is shared.** A suite that assigns it moves the cwd for every suite running beside it; anything on a relative path then resolves elsewhere. Inject the working directory (`WorktreeManager.currentDirectoryProvider`) instead. This is what forced `-j 1` on the three biggest packages, costing 3-5x wall time until it was fixed.
