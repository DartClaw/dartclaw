...
| Surface | Model is asked to produce | Output contract | Validating / consuming component |
|---------|---------------------------|-----------------|----------------------------------|
| `packages/dartclaw_runtime/lib/src/behavior/behavior_file_service.dart#BehaviorFileService.composeSystemPrompt` (plus `#composeStaticPrompt` / `#composeAppendPrompt` for append-mode harnesses) | The agent's persona and standing behaviour – composed from SOUL.md / USER.md / TOOLS.md / MEMORY.md plus the built-in defaults, and – for a primary-scope turn carrying a `TurnOrigin` – one host-rendered `## Channel` section naming the channel, direct/group and the JSON-encoded contact label. The compaction and identifier-preservation instructions, which steer the provider's own compaction pass, ride the composed system prompt; the append composition carries AGENTS.md only | `free text` (system prompt; no machine-read return) | `packages/dartclaw_runtime/lib/src/turn_runner.dart` and `turn_runner_execution.dart` drive the harness turn and pass `TurnContext.origin` through; a non-blank `systemPromptOverride` replaces the composition verbatim |
...
| `packages/dartclaw_workflow/lib/src/workflow/merge_resolve_coordinator.dart` | Semantic resolution of merge conflicts, plus the outcome, resolution summary and error message. Authors both the step prompt and the three output descriptions | `structured envelope` step outcome; `merge_resolve.outcome` is an `enum` over `mergeResolveOutcomeValues` (`resolved`/`failed`/`cancelled`) and `merge_resolve.error_message` a `{'type': ['string', 'null']}` schema, both declared as the output's schema and so provider-enforced where structured output is supported; `merge_resolve.resolution_summary` is `text` | The coordinator itself – it owns sha capture, cleanliness, locking, bounded attempts, audit and escalation. It validates each declared value once: an absent or out-of-vocabulary outcome, or an error message that is neither a string nor null, is a contract failure recorded in the attempt artifact's `errorMessage` naming the key, re-asked by the bounded attempt loop, never defaulted or coerced. An unstated or null `error_message` stays null. The conflicted file set is not asked for at all – it is the promotion result's own list, refreshed by each re-conflict |
...
| `packages/dartclaw_workflow/skills/dartclaw-merge-resolve/SKILL.md` | The resolution procedure and its terminal outcome, committed all-or-nothing | `structured envelope` step outcome; the outcome vocabulary and the string-or-null error message are the schemas the coordinator declares on `merge_resolve.outcome` and `merge_resolve.error_message` | `merge_resolve_coordinator.dart` – see its row above |
