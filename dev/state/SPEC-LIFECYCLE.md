...
- **The milestone PRD is consolidated into the complete record of the cycle**: each numbered story's outcome (what shipped, deviations from plan, commit refs where useful) and any `plan.json` or FIS content worth keeping – `## Implementation Observations` (deviation records, NOTICED-BUT-NOT-TOUCHING items, assumption/claim inventories that discharge validation items), `## Deferred Decisions`, reconciliation-ledger entries, and legacy `## Learnings` sections – are folded into the private canonical PRD before the plan and FIS files are deleted anywhere.
