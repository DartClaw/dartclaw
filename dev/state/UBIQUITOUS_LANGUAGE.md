...
| Operator Login Store | The operator's own vendor login – `~/.claude` / `~/.codex`, or wherever `CLAUDE_CONFIG_DIR` / `CODEX_HOME` points. A Dedicated Credential Store creates and refreshes its Subscription Credential separately instead of copying this login. DartClaw resolves the operator store for collision refusal, auth-status probing, explicit `use_system_codex_home` seeding, and mirroring Codex plugin capabilities into that separate store | user login, host login, system home |
