...
- **Claude ends a headless turn's `result` while backgrounded subagents still run, then runs a notification turn by itself.** Under 2.1.x `Agent` backgrounds by default; the CLI re-lists its background tasks on `system`/`background_tasks_changed` (agents and background shells alike, `task_type` `local_agent` / `local_bash`) and, once a task reports, injects a turn ending in a second `result` (`origin.kind: task-notification`) with no stdin input. A process restart in between – the finalizer's `--json-schema`, a session switch – kills them: seven AndThen eval cells lost their reviewer and implementer children this way on 2026-09-07 ("stopped (not completed) when the previous process exited"). `ClaudeCodeHarness` holds a successful turn while a non-`local_bash` task is listed (`control-protocol.md` § 4.8); the turn timeout bounds it. No CLI flag, setting or env var makes `Agent` foreground by default or disables background tasks (2.1.263: `--help`, the docs, the binary's strings); `CLAUDE_AUTO_BACKGROUND_TASKS` and frontmatter `background: true` force the opposite. Verified with `claude --print --input-format stream-json` probes, 2026-09-07. `run_in_background: false` in a skill prompt is now only a preference, not a workaround.
