#!/usr/bin/env bash
# Start DartClaw with the channels testing profile (WhatsApp + Signal enabled).
# See dev/testing/channel-e2e-manual.md (private repo) for the hardware
# pairing flow this profile is designed to drive.
#
# Usage: bash dev/testing/profiles/channels/run.sh [extra args...]
# Example: bash dev/testing/profiles/channels/run.sh --port 4000
...
# Unset JAVA_HOME so signal-cli uses its Homebrew-bundled Java
unset JAVA_HOME
...
# DARTCLAW_CHANNELS_DATA_DIR=<path> to debug persistent state across runs.
if [ -n "${DARTCLAW_CHANNELS_DATA_DIR:-}" ]; then
  DATA_DIR="${DARTCLAW_CHANNELS_DATA_DIR}"
  mkdir -p "${DATA_DIR}"
  if [ ! -e "${DATA_DIR}/dartclaw.yaml" ]; then
    cp -R "${SEED_DIR}/." "${DATA_DIR}/"
  fi
else
  DATA_DIR="$(mktemp -d "${TMPDIR:-/tmp}/dartclaw-channels-XXXXXX")"
