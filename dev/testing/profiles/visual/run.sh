...
# DARTCLAW_VISUAL_DATA_DIR=<path> to debug persistent state across runs.
if [ -n "${DARTCLAW_VISUAL_DATA_DIR:-}" ]; then
  DATA_DIR="${DARTCLAW_VISUAL_DATA_DIR}"
  mkdir -p "${DATA_DIR}"
  if [ ! -e "${DATA_DIR}/dartclaw.yaml" ]; then
    cp -R "${SEED_DIR}/." "${DATA_DIR}/"
  fi
else
  DATA_DIR="$(mktemp -d "${TMPDIR:-/tmp}/dartclaw-visual-XXXXXX")"
