...
## Public dataset evaluation (2026-09-13)

The selected settings were also measured locally against the first 50 English and 50 Swedish test rows from
FineWeb2-IR, recorded at dataset revision `859bb94aea8e3b7d99ac1bd630f8b1e288fa579c`. The original paired questions and
complete passages formed a mixed 100-document SQLite memory corpus. The production projection and search pipeline
ran at `2fa2cac3`, whose tree matched the initial, unpublished `v0.26.0` tag. The model, cutoff 0.20, weights, RRF constant and
candidate limit were unchanged; no settings were tuned after scoring.

| Mode | Paired passage first | Paired passage within five | MRR@5 |
|---|---:|---:|---:|
| Keyword | 32/100 | 32/100 | 0.320 |
| Vector | 99/100 | 100/100 | 0.995 |
| Hybrid | 100/100 | 100/100 | 1.000 |

A second phase removed the paired passages for rows 0, 5, 10, …, 45 in each language and searched those 20 questions
against the remaining 80 documents. Before scores were available, a separate reviewer judged 13 as having no
relevant passage, two as related context without the requested answer, three as having useful alternatives and two
as underspecified. The 13 no-match judgments require the actual subject or named entity; another recipe or football
article does not qualify solely by sharing a broad topic.

Keyword returned nothing for all 13 reviewed no-match questions. Vector and hybrid each returned nothing for three
and returned irrelevant results for ten, totaling 126 chunks and 88 document/query pairs per mode. For example,
“Vad är bot-bot?” retrieved a fishing-lure description, and the Romário election query retrieved Roma history.
The other seven withheld questions are not included in this no-match rate. Relevant and irrelevant cosine scores
overlap: an unrelated recipe scored 0.386, while the paired passage for “Vad är en Twizy?” scored 0.316 and had no
keyword match. A single higher cutoff cannot retain every paired hybrid match while rejecting every reviewed
no-match result in this sample.

This supplies additional evidence of ranking quality and weak semantic rejection at the selected settings. It does
not demonstrate an absence of irrelevant results. The sample is much smaller than the source corpus, its questions
are generated, and paired judgments do not establish the relevance of every other returned passage. These are local
measurements, not full FineWeb2-IR benchmark scores, PostgreSQL qualification or additional automatic release gates.

The sample receipt, original rows, temporary driver, full results, score-blind relevance judgments and independently
recounted summary are retained under `.agent_temp/reviews/0.26/public-benchmark/`. The first completed report SHA-256 is
`68d72c6a4d75213f0bcb14736edd2e720b9a16bc9f95b839b505c5e5720d5926`.
...
- [FineWeb2-IR](https://huggingface.co/datasets/hotchpotch/FineWeb2-IR) includes Swedish and English, generated
  passage-based questions and mined hard negatives. The bounded test sample above supplements the 0.26 evaluation
  and provides a baseline for later expansion/reranking work. Its web-domain and synthetic questions differ from
...
FineWeb2-IR was measured for 0.26 as recorded above. BEIR/MS MARCO and the QMD fixture remain researched candidates,
not completed evaluations. Query expansion and reranking remain future work; none of these datasets is a runtime
dependency.
