...
When DartClaw is implementing a planned milestone, the active spec set (PRD, plan, FIS, supporting research) may be **temporarily checked into this repo** under `docs/specs/<version>/`. These are transient working copies on a feature branch — they are removed before the feature branch is squash-merged to `main`.

`main` should never contain `docs/specs/<version>/` directories. If you find one there, the convention below was not followed; remove via `git rm` + PR.

## Convention

- Spec files in `docs/specs/<version>/` are **tracked** (not gitignored) so the implementing workflow can read them as ordinary files and PR reviewers can see them in the diff.
...
- New files appear under `docs/specs/<version>/` (PRD, plan, FIS files, possibly `technical-research.md`).
...
That's a leak — the lifecycle above wasn't followed correctly on the feature branch. Remove via `git rm docs/specs/<version>/` + PR. Nothing in `main` actually depends on those files; their canonical home is elsewhere.
