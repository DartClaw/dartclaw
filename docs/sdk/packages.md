...
| `dartclaw_config` | Typed config loading, metadata, validation, and authoring helpers | `DartclawConfig`, `ConfigMeta`, `ConfigValidator`, `ConfigWriter` | Hosts and tools that need to load, inspect, validate, or rewrite DartClaw config | No | `Repo-only support package`[^adr008] |
