...
| --- | --- | --- | --- | --- | --- |
| `dartclaw` | Umbrella package re-exporting the main SDK surface | `ClaudeCodeHarness`, `HarnessConfig`, `Channel`, `BridgeEvent`, `MemoryService` | Default choice for apps that want the core runtime plus storage and channel integrations | Yes, via `dartclaw_storage` | `0.0.1-dev.1 (placeholder)`[^adr008] |
...
| `dartclaw_core` | sqlite3-free runtime primitives | `AgentHarness`, `ClaudeCodeHarness`, `Channel`, `BridgeEvent`, `EventBus` | Flutter desktop, custom storage, or environments where native sqlite3 is a problem | No | `0.0.1-dev.1 (placeholder)`[^adr008] |
