...
final class AcpClient {
  ...
  ///
  /// [onStreamError] receives an undecodable [stdout] in place of the peer,
  /// which would strand it on a `listen()` future nothing reads until [close].
  factory(
    ...
    return AcpClient._start(
      ndjsonChannel(stdout, stdin, onStreamError: onStreamError),
  ...
  Future<Map<String, dynamic>> initialize() async {
    ...
    final value = await _peer.sendRequest('initialize', {
      'protocolVersion': 1,
      'clientInfo': {'name': 'dartclaw', 'version': '0.18.0'},
      'clientCapabilities': clientCapabilities,
    });
    if (_authRequired(value)) {
  ...
  Future<String> createSession({required String cwd}) async {
    ...
    throw AcpHarnessException(
      ...
      'ACP session/new response is missing required sessionId',
  ...
  Future<AcpPromptResult> prompt({required String sessionId, required String text}) async {
    final value = await _peer.sendRequest('session/prompt', {'sessionId': sessionId, 'prompt': text});
    if (_authRequired(value)) {
      throw const AcpHarnessException(
        AcpHarnessErrorCode.authRequired,
        'ACP agent requires interactive authentication',
  ...
  Map<String, dynamic> _clientCapabilities() {
    return {
      ...
      'terminal': _reverseCallCapabilities['terminal'] == true,
    };
  }

  static bool _authRequired(Object? value) {
    if (value is! Map) return true;
    final result = Map<String, dynamic>.from(value);
    for (final key in const ['authRequired', 'auth_required']) {
      if (!result.containsKey(key)) continue;
      final authRequired = result[key];
      if (authRequired is! bool) return true;
      if (authRequired) return true;
    }
    for (final key in const ['auth', 'authentication']) {
      if (!result.containsKey(key)) continue;
      final auth = result[key];
      if (auth is! Map) return true;
      var readable = false;
      if (auth.containsKey('required')) {
        readable = true;
        final required = auth['required'];
        if (required is! bool || required) return true;
      }
      if (auth.containsKey('authenticated')) {
        readable = true;
      ...
      if (auth.containsKey('status')) {
        ...
        if (status is! String || status.toLowerCase() != 'authenticated') return true;
      }
  ...
  static Map<String, dynamic> _mapResult(Object? value) {
