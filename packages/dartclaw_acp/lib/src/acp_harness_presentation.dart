...
extension _AcpHarnessPresentation on AcpHarness {
  ...
  Future<T> _withinTurnDeadline<T>(Future<T> operation, DateTime? deadline) =>
      deadline == null ? operation : operation.timeout(_remainingUntil(deadline));
  ...
  void _handleSessionUpdate(Map<String, dynamic> update) {
    ...
    try {
      ...
      AcpHarness._log.severe(error.message);
  ...
  Map<String, Object?> _diagnostics({int? exitCode}) {
    ...
    if (_stderrDiagnostics.isNotEmpty) {
      final stderr = utf8.decode(_stderrDiagnostics, allowMalformed: true);
      diagnostics['stderr'] = _stderrDiagnosticsTruncated ? '${AcpHarness._diagnosticsTruncatedMarker}$stderr' : stderr;
    }
    return diagnostics;
  }
