...
///
/// [onStreamError] takes ownership of every input error – notably the
/// `FormatException` `utf8.decoder` throws on a byte it cannot decode – so
/// nothing listening on the channel sees it. The stream is not ended by the
/// error: it continues, and ends only when the input does.
StreamChannel<String> ndjsonChannel(
  Stream<List<int>> input,
  StreamSink<List<int>> output, {
  void Function(Object error)? onStreamError,
}) {
  final lines = input.transform(utf8.decoder).transform(const LineSplitter()).where((line) => line.isNotEmpty);
  return StreamChannel.withCloseGuarantee(
    onStreamError == null ? lines : lines.handleError(onStreamError),
    _NdjsonSink(output),
  );
}
