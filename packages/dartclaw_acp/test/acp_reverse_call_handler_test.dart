...
void main() {
  group('ACP reverse-call handlers', () {
    ...
    test('file calls use the active turn workspace and session authorization', () async {
      ...
      expect(guard.evaluatedContexts.map((context) => context.sessionId), everyElement('task-session'));
      expect(guard.evaluatedContexts.map((context) => context.agentId), everyElement('search'));
      expect(guard.evaluatedContexts.map((context) => context.rawProviderToolName), [
        'fs/read_text_file',
        'fs/write_text_file',
      ]);
    });
