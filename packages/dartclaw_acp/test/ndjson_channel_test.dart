...
void main() {
  group('ndjsonChannel', () {
    ...
    test('onStreamError takes a decode error, and the stream survives it', () async {
      final inCtrl = StreamController<List<int>>();
      final outCtrl = StreamController<List<int>>();
      outCtrl.stream.listen((_) {});
      final errors = <Object>[];
      ...
      final channel = ndjsonChannel(inCtrl.stream, outCtrl.sink, onStreamError: errors.add);
      channel.stream.listen(lines.add, onError: (Object error) => fail('error reached the listener: $error'));

      inCtrl.add([0xff]);
      await pumpEventQueue();
      inCtrl.add(utf8.encode('{"after":true}\n'));
      await pumpEventQueue();

      expect(errors.single, isA<FormatException>());
      expect(lines, ['{"after":true}']);
    });
