...
GitHubWebhookConfig parseGitHubWebhookConfig(Map<String, dynamic> yaml, List<String> warns) {
  ...
  final triggers = (yaml['triggers'] as List?)
      ...
      .map((entry) {
        ...
        return GitHubWorkflowTrigger(
          event: (raw['event'] as String?)?.trim() ?? 'pull_request',
          actions:
              (raw['actions'] as List?)?.map((value) => value.toString()).toList() ?? const ['opened', 'synchronize'],
          labels: (raw['labels'] as List?)?.map((value) => value.toString()).toList() ?? const [],
          workflow: (raw['workflow'] as String?)?.trim() ?? 'code-review',
        );
      })
