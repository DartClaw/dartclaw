...
final class ConversationIndexer implements MessageServiceObserver, SessionServiceObserver {
  ...
  new({required this.index, required this.sessions, required this.messages, this.userId = 'owner'});
  ...
  void onMessageAppended(Message message) {
    _enqueue(() async {
      ...
      if (document != null) await index.upsert([document], userId: userId);
    });
  }

  @override
  void onMessagesCleared(String sessionId, List<String> messageIds) {
    _enqueue(() => index.delete(messageIds, userId: userId));
  }
  ...
  void onSessionTypeChanged(String sessionId, SessionType oldType, SessionType newType) {
    ...
    _enqueue(() async {
      ...
      if (documents.isNotEmpty) await index.upsert(documents, userId: userId);
    });
  }
