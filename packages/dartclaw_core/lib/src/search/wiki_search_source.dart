...
import 'package:dartclaw_core/dartclaw_core.dart'
    show MemoryFileService, MemoryResourceLimits, MemoryRole, splitFrontmatter;
...
class WikiSearchSource {
  ...
  ///
  /// An unterminated fence reads as no frontmatter: search ranks what it can
  /// read, where the page store refuses the same page outright.
  static String? _frontmatter(String text) => splitFrontmatter(text)?.frontmatter;

  static String _stripFrontmatter(String text) {
    final split = splitFrontmatter(text);
    if (split == null || split.frontmatter == null) return text;
    return split.body.trimLeft();
  }
