...
void main() {
  group('DockerValidator', () {
    test('accepts valid config', () {
      final errors = DockerValidator.validate(const ContainerConfig(
        enabled: true,
        extraMounts: ['/data/workspace:/workspace:rw'],
      ));
    ...
    test('rejects sensitive mount: /.ssh', () {
      final errors = DockerValidator.validate(const ContainerConfig(
        enabled: true,
        extraMounts: ['/home/user/.ssh:/root/.ssh:ro'],
      ));
      expect(errors, isNotEmpty);
    ...
    test('rejects docker socket mount', () {
      final errors = DockerValidator.validate(const ContainerConfig(
        enabled: true,
        extraMounts: ['/var/run/docker.sock:/var/run/docker.sock'],
      ));
      expect(errors, isNotEmpty);
    });
