...
void main() {
  ...
  group('event data', () {
    test('GuardBlockEvent carries all fields', () {
      final e = GuardBlockEvent(
        guardName: 'FileGuard',
        guardCategory: 'filesystem',
        verdict: 'block',
        verdictMessage: 'access denied',
      ...
      expect(e.guardName, 'FileGuard');
      expect(e.guardCategory, 'filesystem');
      expect(e.verdict, 'block');
      expect(e.verdictMessage, 'access denied');
      expect(e.hookPoint, 'beforeToolCall');
      expect(e.sessionKey, 'agent:main:web:');
      expect(e.sessionId, 'sess-1');
      expect(e.channel, 'web');
      expect(e.peerId, 'user-1');
      expect(e.timestamp, now);
    });

    test('ConfigChangedEvent carries all fields', () {
      final e = configEvent();
      expect(e.changedKeys, ['agent.model']);
      expect(e.oldValues, {'agent.model': 'old'});
      expect(e.newValues, {'agent.model': 'new'});
      expect(e.requiresRestart, isFalse);
      expect(e.timestamp, now);
    });

    test('SessionCreatedEvent carries all fields', () {
      final e = SessionCreatedEvent(
        sessionId: 's1',
        sessionKey: 'agent:main:web:',
        sessionType: 'web',
        timestamp: now,
      ...
      expect(e.sessionKey, 'agent:main:web:');
      expect(e.sessionType, 'web');
      expect(e.timestamp, now);
    });

    test('SessionEndedEvent carries all fields', () {
      final e = SessionEndedEvent(
        sessionId: 's2',
        sessionKey: 'agent:main:cron:daily',
        sessionType: 'cron',
        timestamp: now,
      );
      expect(e.sessionId, 's2');
      expect(e.sessionKey, 'agent:main:cron:daily');
      expect(e.sessionType, 'cron');
    });

    test('SessionErrorEvent carries error field', () {
      final e = errorEvent(error: 'timeout');
      expect(e.error, 'timeout');
      expect(e.sessionId, 's1');
    });

    test('toString() returns readable representation', () {
      expect(guardEvent().toString(), contains('GuardBlockEvent'));
      expect(configEvent().toString(), contains('ConfigChangedEvent'));
      expect(createdEvent().toString(), contains('SessionCreatedEvent'));
      expect(endedEvent().toString(), contains('SessionEndedEvent'));
      expect(errorEvent().toString(), contains('SessionErrorEvent'));
    });
  });
}
