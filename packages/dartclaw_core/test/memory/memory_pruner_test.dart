...
void main() {
  ...
  group('prune() integration', () {
    ...
    test('canonical prune leaves the error and learning documents byte-identical', () async {
      final old = DateTime.now().toUtc().subtract(const Duration(days: 120));
      final detail = CanonicalMemoryEntry(
        id: '6fda91a3-f1b4-46c2-acd6-5bb4d13c13b9',
        revision: 1,
        topic: 'general',
        summary: 'Prunable fact',
        content: 'Prunable fact',
        created: old,
        updated: old,
        provenance: MemorySourceRef(sourceLocator: 'test'),
      );
      final corpus = CanonicalMemoryCorpus(
        ...
        topics: [
          MemoryTopicDocument(topic: 'general', entries: [detail]),
        ],
        archive: MemoryArchiveDocument(),
      ...
      expect(result.entriesArchived, 1);
      expect(File('${tempDir.path}/errors.md').readAsBytesSync(), errorsBefore);
      expect(File('${tempDir.path}/learnings.md').readAsBytesSync(), learningsBefore);
      final status = await authority.statusSnapshot();
      expect(status.errorEntryCount, 1);
      expect(status.learningEntryCount, 1);
    });
