...
void main() {
  group('SqliteTaskRepository', () {
    ...
    group('tasks', () {
      ...
      test('uses an inert storage placeholder and derives legacy refusals from raw rows', () async {
        for (final id in const [
          ...
          await repository.insert(
            _task(
              id: id,
              configJson: switch (id) {
                'coding-invalid' => const {'needsWorktree': 'yes'},
                'coding-false' => const {'needsWorktree': false},
                'coding-true' => const {'needsWorktree': true},
                _ => const {},
              },
            ),
          );
        ...
        expect(
          db.select('SELECT type FROM tasks WHERE id = ?', ['post-upgrade']).single['type'],
          taskTypeStoragePlaceholder,
        );
        for (final entry in const {
          'research': 'research',
          'coding-absent': 'coding',
          'coding-invalid': 'coding',
          'coding-false': 'coding',
          'coding-true': 'coding',
          'writing': 'writing',
        }.entries) {
          db.execute('UPDATE tasks SET type = ? WHERE id = ?', [entry.value, entry.key]);
        }

        expect((await repository.getById('research'))?.legacyRefusal, TaskLegacyRefusal.securityProfileUndeclared);
        expect((await repository.getById('coding-absent'))?.legacyRefusal, TaskLegacyRefusal.worktreeUndeclared);
        expect((await repository.getById('coding-invalid'))?.legacyRefusal, TaskLegacyRefusal.worktreeUndeclared);
        expect((await repository.getById('coding-false'))?.legacyRefusal, isNull);
        expect((await repository.getById('coding-true'))?.legacyRefusal, isNull);
        expect((await repository.getById('writing'))?.legacyRefusal, isNull);

        final research = (await repository.getById('research'))!;
        await repository.update(research.copyWith(title: 'Still refused'));
        expect(db.select('SELECT type FROM tasks WHERE id = ?', ['research']).single['type'], 'research');
        expect((await repository.getById('research'))?.legacyRefusal, TaskLegacyRefusal.securityProfileUndeclared);
      });
      ...
      test('a step execution round-trips its exact surviving field set on a fresh database', () async {
        final execution = AgentExecution(id: 'ae-mount', provider: 'claude');
        final task = Task(
          id: 'task-mount',
          title: 'Mount-free task',
          description: 'Round-trips a step execution through both repositories',
          createdAt: DateTime.parse('2026-03-10T10:00:00Z'),
          agentExecution: execution,
        ...
        const stepExecution = WorkflowStepExecution(
          taskId: 'task-mount',
          agentExecutionId: 'ae-mount',
          workflowRunId: 'run-mount',
          stepIndex: 1,
          stepId: 'implement',
          stepType: 'agent',
          gitJson: '{"branch":"feature/mount"}',
          providerSessionId: 'thread-mount',
          structuredSchemaJson: '{"type":"object"}',
          structuredOutputJson: '{"result":"done"}',
          followUpPromptsJson: '["verify"]',
          mapIterationIndex: 0,
          mapIterationTotal: 3,
        ...
        final loaded = await stepExecutions.getByTaskId('task-mount');

        expect(loaded, equals(stepExecution));
        // Enumerated, not a negative match: this is the complete persisted
        // field set now that the cross-clone artifact mount is gone.
        expect(loaded!.toJson().keys, [
          'taskId',
          'agentExecutionId',
          'workflowRunId',
          'stepIndex',
      ...
      test('filters by status', () async {
        await repository.insert(_task(id: 'draft-task', status: TaskStatus.draft));
        await repository.insert(_task(id: 'queued-task-a', status: TaskStatus.queued));
        await repository.insert(_task(id: 'queued-task-b', status: TaskStatus.queued));

        final queued = await repository.list(status: TaskStatus.queued);

        expect(queued.map((task) => task.id), ['queued-task-b', 'queued-task-a']);
      });
