  ...
  group('SkillPromptBuilder.build', () {
    test('skill + prompt -> skill line + blank line + prompt', () {
      final result = builder.build(
        ...
        resolvedPrompt: 'Review this file.',
      );
    ...
    test('skill + no prompt + contextSummary -> skill line + Context section', () {
      final result = builder.build(
        skill: 'andthen:review-code',
        contextSummary: '- findings: some findings',
      );
      expect(
        result,
        "Use the 'andthen:review-code' skill.\n\nContext:\n- findings: some findings",
      );
    });
    ...
    test('evaluator flag passes through to augmenter', () {
      final result = builder.build(
        skill: null,
        resolvedPrompt: 'Evaluate this.',
        outputs: {
          'result': const OutputConfig(format: OutputFormat.json),
        },
        evaluator: true,
      );
      // evaluator=true with json output and no schema -> verdict preset applied
      expect(result, contains('## Required Output Format'));
    });
  });
