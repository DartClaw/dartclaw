...
void main() {
  ...
  group('WorkflowTemplateEngine.extractContextReferences', () {
    ...
    test('strips [map.index] suffix from context key', () {
      expect(
        engine.extractContextReferences('{{context.items[map.index]}}'),
        {'items'},
      );
    });
    ...
    test('strips [map.index].field suffix from context key', () {
      expect(
        engine.extractContextReferences('{{context.results[map.index].title}}'),
        {'results'},
      );
    });
    ...
    test('excludes map.* references', () {
      expect(
        engine.extractContextReferences('{{map.item}} {{map.index}}'),
        isEmpty,
      );
    });
  });
  ...
  group('WorkflowTemplateEngine.resolveWithMap', () {
    group('map.item — Map item', () {
      ...
      test('array-typed field renders as bullet list', () {
        final ctx = _ctx();
        final mapCtx = _mapCtx(item: {'tags': ['x', 'y', 'z']}, index: 0, length: 1);
        expect(
          engine.resolveWithMap('{{map.item.tags}}', ctx, mapCtx),
          '- x\n- y\n- z',
        );
      });
      ...
      test('dot notation exceeding 3 levels throws ArgumentError', () {
        final ctx = _ctx();
        final mapCtx = _mapCtx(item: {'a': {'b': {'c': {'d': 'x'}}}}, index: 0, length: 1);
        expect(
          () => engine.resolveWithMap('{{map.item.a.b.c.d}}', ctx, mapCtx),
          throwsA(isA<ArgumentError>()),
        );
      });
      ...
      test('{{map.item.field}} on non-Map item throws ArgumentError', () {
        ...
        expect(
          () => engine.resolveWithMap('{{map.item.field}}', ctx, mapCtx),
          throwsA(isA<ArgumentError>()),
        );
      });
    });
    ...
    group('context.key[map.index] — indexed context lookup', () {
      test('resolves list element at current index', () {
        final ctx = _ctx(data: {'items': ['a', 'b', 'c']});
      ...
      test('explicit dot-access bypasses auto-extraction', () {
        ...
        final mapCtx = _mapCtx(item: 'unused', index: 0, length: 1);
        expect(
          engine.resolveWithMap('{{context.results[map.index].score}}', ctx, mapCtx),
          '0.9',
        );
      });

      test('explicit dot-access on non-Map element resolves to empty string', () {
        final ctx = _ctx(data: {'items': ['scalar']});
        final mapCtx = _mapCtx(item: 'unused', index: 0, length: 1);
    ...
    group('mixed and backward compat', () {
      ...
      test('null MapContext — missing variable still throws', () {
        final ctx = _ctx();
        expect(
          () => engine.resolveWithMap('{{MISSING}}', ctx, null),
          throwsA(isA<ArgumentError>()),
        );
      });
    });
