...
void main() {
  group('GoogleChatJwtVerifier.verify', () {
    ...
    test('OIDC token with accounts.google.com issuer returns true', () async {
      ...
      final token = _signToken(
        audience: 'https://example.com/integrations/googlechat',
        issuer: GoogleChatJwtVerifier.oidcIssuer,
        email: null,
        emailVerified: null,
      );
  ...
  group('GoogleChatJwtVerifier certificate caching', () {
  ...
  group('GoogleChatJwtVerifier audience modes', () {
    test('app-url mode fetches certs from oauth certs endpoint', () async {
      ...
      expect(await verifier.verify('Bearer $token'), isTrue);
      expect(requestedUrl, GoogleChatJwtVerifier.googleCertsUrl);
    });
    ...
    test('OIDC token fetches certs from v3 endpoint and parses JWK', () async {
      ...
      expect(await verifier.verify('Bearer $token'), isTrue);
      expect(requestedUrl, GoogleChatJwtVerifier.googleOidcCertsUrl);
    });
    ...
    test('project-number mode fetches certs from chat service account metadata endpoint', () async {
      ...
      expect(await verifier.verify('Bearer $token'), isTrue);
      expect(requestedUrl, GoogleChatJwtVerifier.chatServiceAccountCertsUrl);
    });
  });
}

GoogleChatJwtVerifier _buildVerifier({
  GoogleChatAudienceConfig? audience,
  http.Client? httpClient,
  Duration cacheTtl = const Duration(minutes: 10),
  DateTime Function()? now,
}) {
  return GoogleChatJwtVerifier(
    audience:
        audience ??
    ...
    httpClient:
        ...
        MockClient((request) async {
          ...
          if (request.url == GoogleChatJwtVerifier.googleOidcCertsUrl) {
            return http.Response(_jwkResponse, 200);
          }
          return http.Response(jsonEncode({_kid: _certificatePem}), 200);
        }),
...
String _signToken({
  ...
  String issuer = GoogleChatJwtVerifier.expectedIssuer,
  String kid = _kid,
  String privateKeyPem = _privateKeyPem,
  DateTime? expiresAt,
  String? email = GoogleChatJwtVerifier.expectedIssuer,
