...
class ConfigDelta {
  ...
  /// (e.g. `'context.*'`, `'security.*'`). Restart-tier sections never appear
  /// here — see `ConfigNotifier.restartRequiredSections`.
  ...
  /// - A glob key like `'context.*'` matches a changed key `'context.*'`.
  /// - A specific key like `'context.reserve_tokens'` also matches a
  ///   changed key `'context.*'` because the changed section contains that
  ///   watched field.
  bool hasChanged(String key) {
    ...
    // A glob watch key (e.g. 'context.*') matches a changed key that starts
