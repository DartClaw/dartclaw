...
/// A valid `approval: never` emits a loud security warning because it disables
/// Claude's native prompt gate. DartClaw's guard chain remains active.
void _validateClaudeApproval(String providerId, Map<String, dynamic> options, List<String> warns) {
  ...
  if (raw == 'never') {
    addConfigAdvisory(
      warns,
      'providers.$providerId.approval is "never" – this disables Claude native tool prompts for every Claude lane. '
      'DartClaw still evaluates its guard chain before tool calls. Host launches also disable subprocess environment '
      'scrubbing so Claude can honor the permission mode. Use only for fully trusted runs.',
    );
  }
}
...
McpServersConfig _parseMcpServers(
  ...
  for (final entry in raw.entries) {
    ...
    // `package:yaml` refuses two equal mapping keys, but it compares by value
    // and type: `1:` and `"1":` are distinct keys that name one server here.
    // Without this the second would shadow the first and its network_class
    // would silently replace the operator's.
    if (entries.containsKey(serverName)) {
      throw FormatException('mcp_servers.$serverName is declared twice — two entry keys name the same server.');
    }
    final value = entry.value;
    if (value is! Map) {
