...
enum FileAccessLevel {
  /// Completely block reads, writes, and deletes.
  noAccess,

  /// Allow reads but block writes and deletes.
  readOnly,

  /// Allow reads and writes but block deletion.
  noDelete,
}
...
class FileGuardRule {
  /// Glob pattern matched against resolved file paths.
  final String pattern;

  /// Access level enforced when [pattern] matches.
...
class FileGuardConfig {
  /// Ordered protection rules evaluated against requested paths.
  ...
  FileGuardConfig withSelfProtection(String configPath) {
    return FileGuardConfig(
      rules: [
        ...rules,
        FileGuardRule(pattern: configPath, level: FileAccessLevel.readOnly),
      ],
    );
  }
...
class FileGuard extends Guard {
  ...
  List<_PathOp> _extractPathsFromBash(String command) {
    ...
    for (final sub in subCommands) {
      ...
      if (fileReadCommands.contains(cmd)) {
        for (final a in args) {
          results.add(_PathOp(a, FileOperation.read));
        }
      } else if (fileWriteCommands.contains(cmd)) {
        for (final a in args) {
          results.add(_PathOp(a, FileOperation.write));
        }
      } else if (fileDeleteCommands.contains(cmd)) {
        for (final a in args) {
      ...
      } else if (copyMoveCommands.contains(cmd)) {
  ...
  GuardVerdict? _checkAccess(String path, FileOperation operation) {
    ...
    return switch (mostRestrictive) {
      FileAccessLevel.noAccess => GuardVerdict.block('File access blocked: no_access on $path'),
  ...
  /// Matches [path] against rule [pattern] under the workspace glob dialect
  /// ([globToRegex]), against the full path, the basename and every path suffix.
  static bool _globMatches(String pattern, String path) {
    ...
    final regex = RegExp(globToRegex(pattern));
