...
class GuardContext {
  ...
  /// Message origin: 'channel', 'web', 'cron', or null.
  final String? source;
...
abstract class Guard {
  /// Maximum time allowed for one evaluation.
  Duration get evaluationBudget => const Duration(seconds: 5);

  /// Optional guard-owned verdict when [evaluationBudget] expires.
  ///
  /// Returning null delegates unexpected timeout policy to the chain.
  GuardVerdict? onEvaluationTimeout(GuardContext context) => null;
...
class GuardChain {
  ...
  Future<GuardVerdict> _evaluate(GuardContext context) async {
    ...
    for (final guard in currentGuards) {
      ...
      try {
        verdict = await guard.evaluate(context).timeout(guard.evaluationBudget);
      } on TimeoutException catch (e, st) {
        _log.severe('Guard ${guard.name} exceeded ${guard.evaluationBudget}: $e', e, st);
        try {
          verdict = guard.onEvaluationTimeout(context) ?? _failureVerdict('timeout', e);
        } catch (hookError, hookStack) {
          _log.severe('Guard ${guard.name} timeout policy threw: $hookError', hookError, hookStack);
          verdict = _failureVerdict('error', hookError);
        }
      } catch (e, st) {
        _log.severe('Guard ${guard.name} threw: $e', e, st);
        verdict = _failureVerdict('error', e);
      }
  ...
  GuardVerdict _failureVerdict(String kind, Object error) =>
      failOpen ? GuardVerdict.warn('Guard $kind (fail-open): $error') : GuardVerdict.block('Guard $kind: $error');
}
