...
final List<Pattern> defaultSensitivePatterns = <Pattern>[
  ...
  // Covers `AWS_SECRET_ACCESS_KEY` and its per-vendor equivalents, which the
  // `_API_KEY` pattern does not reach.
  RegExp(r'.*_ACCESS_KEY$', caseSensitive: false),
  RegExp(r'.*_SECRET$', caseSensitive: false),
  RegExp(r'.*_TOKEN$', caseSensitive: false),
  RegExp(r'.*_CREDENTIALS?$', caseSensitive: false),
...
abstract final class SafeProcess {
  ...
  static Future<ProcessResult> run(
    ...
    return Process.run(
      ...
      environment: resolveEnvironment(env, baseEnvironment: baseEnvironment),
  ...
  ///
  /// Exposed so callers and tests can inspect what a policy admits without
  /// starting a process.
  static Map<String, String> resolveEnvironment(EnvPolicy policy, {Map<String, String>? baseEnvironment}) {
