...
/// A scheduled task template parsed from a `scheduling.jobs` entry of type
/// `task`.
///
...
class ScheduledTaskDefinition {
  ...
  /// Parses one template block: `id`, `schedule`, `enabled` and a `task`
  /// section.
  ///
  ...
  static ScheduledTaskDefinition? fromYaml(Map<dynamic, dynamic> yaml, List<String> warnings) {
    ...
    final refusal = retiredTaskCategoryRefusal([taskMap['task_type'], taskMap['type']]);
