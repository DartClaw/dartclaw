...
void main() {
  ...
  group('AgentExecution', () {
    ...
    test('source file stays task and workflow agnostic', () async {
      // AgentExecution lives in dartclaw_kernel. Resolve its source through the
      // package: URI mechanism so the lookup is independent of CWD and of where
      // `dart test` stages the precompiled kernel.
      final sourceUri = await Isolate.resolvePackageUri(Uri.parse('package:dartclaw_kernel/src/agent_execution.dart'));
      expect(sourceUri, isNotNull, reason: 'could not resolve package:dartclaw_kernel/src/agent_execution.dart');
      final source = File.fromUri(sourceUri!).readAsStringSync();

      expect(source, isNot(contains('/task/')));
      expect(source, isNot(contains('package:dartclaw_core')));
