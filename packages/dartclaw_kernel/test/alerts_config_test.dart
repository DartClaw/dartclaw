...
''');
    ...
    test('both numeric guards keep their exact warnings and section defaults', () {
      final config = _load('''
alerts:
  cooldown_seconds: 0
  burst_threshold: 0
''');
      ...
      expect(config.alerts.cooldownSeconds, 300);
      expect(config.alerts.burstThreshold, 5);
      expect(config.warnings, [
        'alerts.cooldown_seconds must be >= 1 — using default',
        'alerts.burst_threshold must be >= 1 — using default',
      ]);
    });
  });
}
