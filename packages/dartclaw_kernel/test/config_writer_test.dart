...
''';
  ...
  group('nested path creation', () {
    ...
    test('creates deeply nested path alongside existing keys', () async {
      ...
      await writer.updateFields({'scheduling.heartbeat.interval_minutes': 15});
  ...
  group('backup failure aborts write', () {
    test('write aborted when backup target is read-only', () async {
      ...
      await expectLater(writer.updateFields({'port': 3001}), throwsA(isA<StateError>()));
  ...
  group('error handling', () {
    test('throws FileSystemException when config file does not exist', () async {
      await expectLater(writer.updateFields({'port': 3001}), throwsA(isA<FileSystemException>()));
    });
  ...
  group('type round-trips', () {
    test('preserves int, string, bool values', () async {
      ...
      await writer.updateFields({'port': 3001, 'host': 'localhost', 'debug': true});
  ...
  test('rejects every invalid memory integer representation without changing config bytes', () async {
    ...
    // The refusal now comes from the field's declared `min: 1` rather than a
    // path-name special case, so the wording is the standard bounded-integer
    // and type wording every other field uses.
    const refusals = <(Object, String)>[
      (0, 'must be >= 1, got 0'),
      (-1, 'must be >= 1, got -1'),
      (1.5, 'must be an integer, got double'),
      ('1', 'must be an integer, got String'),
      (double.infinity, 'must be an integer, got double'),
    ];
    for (final (value, expected) in refusals) {
      expect(
        ...
        throwsA(
          isA<ArgumentError>().having(
            (error) => '$error',
            'message',
            allOf(contains(expected), isNot(contains('positive integer'))),
          ),
        ),
        reason: '$value',
      );
  ...
  test('a whole-number double is persisted as an integer so the value survives a reload', () async {
    File(configPath).writeAsStringSync('port: 3000\nmemory:\n  max_bytes: 32768\n');

    await writer.updateFields({'port': 4000.0, 'memory.max_bytes': 65536.0});

    final written = File(configPath).readAsStringSync();
    expect(written, contains('port: 4000'));
    expect(written, contains('max_bytes: 65536'));
    expect(written, isNot(contains('.0')));

    // The loader's integer readers accept only int and String, and the memory
    // reader throws rather than falling back — so a persisted double would
    // silently reset the port and refuse the config outright.
    final config = DartclawConfig.load(configPath: configPath, fileReader: (path) => File(path).readAsStringSync());
    expect(config.server.port, 4000);
    expect(config.memory.maxBytes, 65536);
    expect(config.warnings, isNot(anyElement(contains('port'))));
  });
  ...
  group('dispose', () {
    ...
    test('writes after dispose are rejected', () async {
      ...
      expect(() => writer.updateFields({'port': 3001}), throwsA(isA<StateError>()));
