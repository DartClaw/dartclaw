  ...
  group('DartclawConfig', () {
    group('defaults', () {
      test('all fields have expected default values', () {
        ...
        expect(config.server.staticDir, 'packages/dartclaw_runtime/lib/src/static');
...
''', configPath: '/workspace/dartclaw/dartclaw.yaml');
      ...
      test('onboarding expiry below its bound keeps its exact warning and default', () {
        final config = loadYaml('onboarding:\n  expiry_days: 0\n');
      ...
      test('parseWithLoadWarnings surfaces an out-of-package parser warning in warnings', () {
        ...
        final parsed = config.parseWithLoadWarnings((warns) {
          warns.add('Invalid type for demo.port: "String" — using default');
          return 'parsed';
        });

        expect(parsed, 'parsed');
        expect(config.warnings, contains('Invalid type for demo.port: "String" — using default'));
        expect(config.reloadBlockingWarnings, contains('Invalid type for demo.port: "String" — using default'));
      });

      test('parseWithLoadWarnings keeps an advisory out of reloadBlockingWarnings', () {
        final config = loadNoFile();

        config.parseWithLoadWarnings((warns) {
          addConfigAdvisory(warns, 'demo.legacy_key is deprecated');
          return null;
        });

        expect(config.warnings, contains('demo.legacy_key is deprecated'));
        expect(config.reloadBlockingWarnings, isNot(contains('demo.legacy_key is deprecated')));
      });
      ...
      test('a key nothing describes refuses the load instead of being retained', () {
        expect(
          () => loadYaml('port: 3000\nbogus_key: 42\n'),
          throwsA(
            isA<FormatException>().having(
              (e) => e.message,
              'message',
              allOf(contains("Unknown config field: 'bogus_key'"), contains('registerExtensionParser')),
            ),
          ),
        );
      });

      test('every undescribed path is named in one failure, not just the first', () {
        expect(
          () => loadYaml('bogus_key: 42\nguard_audit:\n  made_up: 1\nagent:\n  nonexistent: true\n'),
          throwsA(
            isA<FormatException>().having(
              (e) => e.message,
              'message',
              allOf(
                contains("Unknown config field: 'bogus_key'"),
                contains("Unknown config field: 'guard_audit.made_up'"),
                contains("Unknown config field: 'agent.nonexistent'"),
              ),
            ),
          ),
        );
...
''';
      ...
      test('source_dir resolves default static and templates paths', () {
        ...
        expect(config.server.staticDir, '/opt/dartclaw/packages/dartclaw_runtime/lib/src/static');
        expect(config.server.templatesDir, '/opt/dartclaw/packages/dartclaw_runtime/lib/src/templates');
      });
    ...
    group('automation.scheduled_tasks legacy rewrite', () {
      const legacyYaml = '''
automation:
  scheduled_tasks:
    - id: legacy-task-1
...
''';
      ...
      test(
        'the load names scheduling.jobs with type: task as the replacement, and does not call the key deprecated',
        () {
          final config = loadYaml(legacyYaml);
          final advisory = config.warnings.singleWhere((w) => w.contains('automation.scheduled_tasks'));
          expect(advisory, allOf(contains('scheduling.jobs'), contains('type: task')));
          expect(advisory, isNot(contains('deprecated')));
        },
      );

      test('a bare automation block raises exactly one advisory and creates no extension section', () {
        final config = loadYaml('automation:\n  scheduled_tasks: []\n');
        expect(config.warnings, hasLength(1));
        expect(config.warnings.single, contains('automation.scheduled_tasks'));
        expect(config.extensions, isEmpty);
        expect(config.scheduling.taskDefinitions, isEmpty);
      });

      test('the legacy form schedules exactly what the scheduling.jobs form schedules', () {
        // The mechanical rewrite is the upgrade map: a deployment that has not
        // migrated yet must keep the schedule it has today.
        const modernYaml = '''
scheduling:
  jobs:
    - id: legacy-task-1
      ...
      task:
        ...
        type: analysis
''';
        final legacy = loadYaml(legacyYaml);
        final modern = loadYaml(modernYaml);

        expect(legacy.scheduling.taskDefinitions, modern.scheduling.taskDefinitions);
        // The jobs the scheduler registers from these definitions are pinned in
        // scheduled_task_runner_test.dart, which owns that construction.
      });
...
''');
    ...
    group('features namespace', () {
      ...
      test('old crowd_coding key loads with its accept-set advisory', () {
        final config = loadYaml('crowd_coding:\n  enabled: true\n');
        expect(config.features.threadBinding.enabled, isFalse);
        expect(config.warnings, contains(ConfigMeta.toleratedLegacyKeys['crowd_coding']!.replacement));
        expect(config.reloadBlockingWarnings, isEmpty);
        expect(config.extensions, isNot(contains('crowd_coding')));
      });

      test('removed advisor section loads with its accept-set advisory', () {
        final config = loadYaml('advisor:\n  enabled: true\n  triggers: [periodic]\n');
        expect(config.warnings, contains(ConfigMeta.toleratedLegacyKeys['advisor']!.replacement));
        expect(config.extensions, isNot(contains('advisor')));
      });

      test('old canvas key loads with its accept-set advisory', () {
        final config = loadYaml('canvas:\n  enabled: true\n');
        expect(config.warnings, contains(ConfigMeta.toleratedLegacyKeys['canvas']!.replacement));
        expect(config.extensions, isNot(contains('canvas')));
      });
    });
  });
}
