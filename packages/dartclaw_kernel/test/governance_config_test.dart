...
void main() {
  group('GovernanceConfig', () {
    test('defaults — all features disabled', () {
      ...
      expect(config.turnLimits.stallTimeout, const Duration(minutes: 5));
      expect(config.turnLimits.stallAction, TurnProgressAction.cancel);
      expect(config.turnLimits.turnTimeout, const Duration(minutes: 30));
    });
...
''');
      ...
      test('an unparseable window keeps the existing default rather than collapsing to zero', () {
        final config = loadYaml('''
governance:
  rate_limits:
    per_sender:
      ...
      window: sometimes
''');
        expect(config.governance.rateLimits.perSender.windowMinutes, const PerSenderRateLimitConfig().windowMinutes);
      });
...
''');
      ...
      test('turn_limits parses stall timeout, action, and wall clock', () {
        final config = loadYaml('''
governance:
  turn_limits:
    stall_timeout: 45s
    ...
    turn_timeout: 900s
''');
        expect(config.governance.turnLimits.stallTimeout, const Duration(seconds: 45));
        expect(config.governance.turnLimits.stallAction, TurnProgressAction.cancel);
        expect(config.governance.turnLimits.turnTimeout, const Duration(seconds: 900));
      });

      test('invalid turn_limits action warns and keeps the default', () {
        final config = loadYaml('''
governance:
  turn_limits:
    stall_timeout: 30s
...
''');
        expect(config.governance.turnLimits.stallTimeout, const Duration(seconds: 30));
        expect(config.governance.turnLimits.stallAction, TurnProgressAction.cancel);
        expect(config.warnings, anyElement(contains('governance.turn_limits.stall_action')));
      });
    });
