  ...
  group('MessageRedactor', () {
    ...
    test('redacts complete boundary-aware secret assignments', () {
      final cases = <({String input, String expected})>[
        ...
        (input: 'password=secret and then restart the service.', expected: 'password=***.'),
        (input: 'password: hunter2, then click Save.', expected: 'password: ***.'),
        (input: 'password=secret, status: healthy', expected: 'password=***, status: healthy'),
        (input: 'password: secret, status=healthy', expected: 'password: ***, status=healthy'),
        (input: 'password=secret; status: healthy', expected: 'password=***; status: healthy'),
        (input: 'password: secret status=healthy', expected: 'password: *** status=healthy'),
        ...
        (input: 'password: abc}SECRET_SUFFIX', expected: 'password: ***'),
        (input: 'password=abc. Restart the service.', expected: 'password=***.'),
        (input: 'password=secret, status=healthy', expected: 'password=***, status=healthy'),
    ...
    test('leaves values alone when the key is not credential-shaped', () {
      final cases = [
        'max_tokens=4096',
        'input_tokens=2048',
    ...
    test('redacts the value after a secret-shaped key even when it reads as prose', () {
      // Fail-closed by design: prose that follows a credential key is
      // over-redacted rather than risking a passphrase surviving as prose.
      final cases = <({String input, String expected})>[
        (input: 'Reset your password: use Settings, then sign in again.', expected: 'Reset your password: ***.'),
        (input: 'The token: field describes a parser concept.', expected: 'The token: ***.'),
        (input: 'The password: field is required.', expected: 'The password: ***.'),
        (input: 'Password: correct horse battery.', expected: 'Password: ***.'),
        (input: 'Password: use Settings, then sign in again.', expected: 'Password: ***.'),
        (input: 'Password: use hunter2', expected: 'Password: ***'),
        (input: 'Token: field describes a parser concept.', expected: 'Token: ***.'),
        (input: 'Authorization: Basic authentication is enabled.', expected: 'Authorization: ***.'),
        (input: 'Authorization: use Settings, then sign in again.', expected: 'Authorization: ***.'),
      ];
      ...
      // The passphrase leaves no fragment behind.
      for (final fragment in ['correct', 'horse', 'battery']) {
        expect(redactor.redact('Password: correct horse battery.'), isNot(contains(fragment)));
      }
    });
