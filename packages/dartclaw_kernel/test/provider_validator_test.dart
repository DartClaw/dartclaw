...
void main() {
  group('ProviderValidator', () {
    ...
    test('a container-bound provider does not need its CLI on the host', () async {
      // The image carries the CLI; a CI runner or a thin host has none.
      final providers = ProvidersConfig(entries: {'claude': const ProviderEntry(executable: '/nonexistent/claude')});
      final result = await ProviderValidator.validate(
        providers: providers,
        registry: CredentialRegistry(
          credentials: const CredentialsConfig(entries: {'anthropic': CredentialEntry(apiKey: 'anthropic-key')}),
