...
void main() {
  group('ScheduledTaskDefinition.fromYaml', () {
      ...
      final cases = <({Map<String, dynamic> yaml, void Function(ScheduledTaskDefinition def) verify})>[
        ...
        (
          yaml: {
            'id': 'auto',
            'schedule': '0 0 * * *',
            'task': {'title': 'Task', 'description': 'Desc', 'type': 'analysis'},
          },
    ...
    test('refuses a scheduled research task with explicit-profile remediation', () {
      expect(
        () => ScheduledTaskDefinition.fromYaml({
          'id': 'legacy-research',
          'schedule': '0 0 * * *',
          'task': {'title': 'Research', 'description': 'Investigate', 'type': 'research'},
        }, <String>[]),
        throwsA(
          isA<RetiredTaskTypeException>().having(
            (error) => error.toString(),
            'message',
            allOf(contains('research'), contains('securityProfile')),
          ),
        ),
      );
    });

    test('refuses a scheduled coding task with worktree-declaration remediation', () {
      expect(
        () => ScheduledTaskDefinition.fromYaml({
          'id': 'legacy-coding',
          'schedule': '0 0 * * *',
          'task': {'title': 'Coding', 'description': 'Fix it', 'type': 'coding'},
        }, <String>[]),
        throwsA(
          isA<MissingWorktreeDeclarationException>().having(
            (error) => error.toString(),
            'message',
            contains('needsWorktree'),
          ),
        ),
      );
    });
      ...
      final cases = <({Map<String, dynamic> yaml, void Function(ScheduledTaskDefinition def) verify})>[
        ...
        (
          yaml: {
            'id': 'test-task',
            'schedule': '0 0 * * *',
            'task': {'title': 'Test', 'description': 'Test task', 'type': 'analysis'},
          },
          verify: (def) {
        ...
        (
          yaml: {
            'id': 'alias-test',
            'schedule': '0 0 * * *',
            'task': {'title': 'Alias Test', 'description': 'Uses task_type key', 'task_type': 'writing'},
          },
          verify: (def) => expect(def.title, 'Alias Test'),
        ),
      ];
    ...
    test('neither alias can mask a retired value in the other', () {
      for (final testCase in const [
        (
          task: {'title': 'Legacy', 'description': 'Both keys', 'task_type': 'analysis', 'type': 'research'},
          declaration: 'securityProfile',
        ),
        (
          task: {'title': 'Legacy', 'description': 'Both keys', 'task_type': 'coding', 'type': 'writing'},
          declaration: 'needsWorktree',
        ),
      ]) {
        expect(
          () => ScheduledTaskDefinition.fromYaml({
          ...
          throwsA(isA<Exception>().having((error) => error.toString(), 'message', contains(testCase.declaration))),
          reason: testCase.task.toString(),
        );
      }
    });
  });
          ...
          >[
            (
              ...
              verify: (json, taskMap) {
                ...
                expect(taskMap.containsKey('type'), isFalse);
                expect(taskMap.containsKey('task_type'), isFalse);
