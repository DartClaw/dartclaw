...
const guardEditorFamilies = {'command', 'file', 'network'};
...
final class GuardEditorService {
  ...
  Future<GuardEditorResult> _persist({
    ...
    candidateYaml.putIfAbsent(guard, () => <String, dynamic>{});
    (candidateYaml[guard] as Map<String, dynamic>)[field] = entries;
  ...
  DartclawConfig _freshConfig() => loadDartclawConfig(configPath: writer.configPath);
...
List<Object?> _entriesFor(Map<String, dynamic> yaml, String guard, String field) {
  ...
  yaml.putIfAbsent(guard, () => <String, dynamic>{});
  final sectionYaml = yaml[guard] as Map<String, dynamic>;
...
Object? _normalizeEntry(String guard, String field, Object? value) {
  ...
  if (stringValue.isEmpty) {
    throw GuardEditorValidationException(['$guard.$field: value is required']);
  }
  return stringValue;
}
...
List<Guard> _guardsForTest(GuardChain chain, String guard) {
  return chain.guards.where((candidate) => candidate.name == guard || candidate.category == guard).toList();
}
