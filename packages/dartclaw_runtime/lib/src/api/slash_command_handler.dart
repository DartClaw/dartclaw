...
class SlashCommandHandler implements SlashCommandExecutor {
  ...
  Future<Map<String, dynamic>> _handleNew(
    ...
    try {
      description = _newTaskDescription(arguments);
    } on RetiredTaskTypeException {
      return _cardBuilder.errorNotification(title: 'Task Type Retired', errorSummary: retiredResearchTaskTypeMessage);
    } on MissingWorktreeDeclarationException {
      return _cardBuilder.errorNotification(title: 'Task Type Retired', errorSummary: retiredCodingWorktreeMessage);
    }
    if (description.isEmpty) {
      return _cardBuilder.errorNotification(title: 'Missing Description', errorSummary: 'Usage: /new <description>');
    }
    ...
    } on MissingWorktreeDeclarationException catch (error) {
      return _cardBuilder.errorNotification(title: 'Worktree Declaration Required', errorSummary: error.toString());
  ...
  String _newTaskDescription(String arguments) {
    final description = arguments.trim();
    final separator = description.indexOf(':');
    if (separator < 0) {
      return description;
    }

    final prefix = description.substring(0, separator).trim().toLowerCase();
    if (prefix == retiredResearchTaskType) {
      throw const RetiredTaskTypeException();
    }
    if (prefix == retiredCodingTaskType) {
      throw const MissingWorktreeDeclarationException();
    }
    return description;
  }
