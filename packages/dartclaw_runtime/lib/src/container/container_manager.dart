...
/// Translates host-computed execution variables for a containerized spawn.
...
class ContainerManager implements ContainerExecutor {
  ...
  /// The container CLI this manager drives, as chosen by [detectRuntime].
  String get _binary => config.runtimeBinary;

  /// The supported container CLIs, in probe order.
  static const runtimeBinaries = ['docker', 'podman'];

  /// The first of [runtimeBinaries] that answers a version probe, or `null`
  /// when none does.
  ///
  /// Every verb this class issues is CLI-compatible across both; the engine
  /// architecture probe's format field is the one known divergence, and a
  /// runtime that cannot answer it downgrades rather than failing startup.
  /// How long a probe may take before the runtime counts as unavailable.
  ///
  /// A wedged daemon — socket present, nothing answering, the common Docker
  /// Desktop failure — never returns rather than erroring, and this probe runs
  /// before `serve` binds a port. Without a deadline the posture that exists to
  /// downgrade instead of blocking startup would hang it indefinitely.
  static const runtimeProbeTimeout = Duration(seconds: 5);

  static Future<String?> detectRuntime({RunCommand runCommand = Process.run}) async {
    for (final binary in runtimeBinaries) {
      try {
        final result = await runCommand(binary, ['version']).timeout(runtimeProbeTimeout);
        if (result.exitCode == 0) return binary;
      } on TimeoutException {
        _log.warning('Container runtime $binary did not answer a version probe within $runtimeProbeTimeout');
      } catch (e) {
        _log.fine('Container runtime $binary not available: $e');
      }
    }
    return null;
  }

  /// Whether this manager's container runtime is available and running.
  Future<bool> isRuntimeAvailable() async {
    try {
      final result = await _run(_binary, ['version']);
      return result.exitCode == 0;
    } catch (e) {
      _log.fine('Container runtime $_binary not available: $e');
      return false;
    }
  }
  ...
  Future<void> ensureImage() async {
    ...
    final buildResult = await _run(_binary, ['build', '-t', config.image, dockerContextDir]);
  ...
  Future<void> start() async {
    ...
    final startResult = await _run(_binary, ['start', containerName]);
  ...
  Future<BridgeChannel> startBridge(BridgeSurface surface, int port) async {
    ...
    final process = await _start(_binary, [
      'exec',
      '-i',
      containerName,
  ...
  Future<ContainerHealth> health() async {
    final result = await _run(_binary, ['inspect', '--format', '{{.State.Running}}', containerName]);
    if (result.exitCode == 0) {
