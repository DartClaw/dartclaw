...
/// scheduled and system background — resolves through this type, so
...
final class ExecutionPolicyResolver {
  ...
  ///
  /// [declaredContainerProfileFor] resolves the container profile a
  /// composer-supplied harness registration declares for a provider, or `null`
  /// when it declares none. It is the only source of a provider-declared
  /// profile — the registration reads it straight out of operator YAML, so
  /// there is no second, code-declared default to rank against it. It is read
  /// per resolution rather than snapshotted, because placement is resolved
  /// before the runtime's effective provider entries exist.
  new({
    required DartclawConfig config,
    required Set<String> availableContainerProfiles,
    String? Function(String providerId)? declaredContainerProfileFor,
  }) : _config = config,
       _availableContainerProfiles = Set.unmodifiable(availableContainerProfiles),
       _declaredContainerProfileFor = declaredContainerProfileFor;

  /// Creates the task-policy authority for standalone compositions whose only
  /// placement input is the primary runner's already-resolved policy.
  factory forStandalonePolicy(ExecutionPolicy? primaryPolicy) {
    final policy = primaryPolicy ?? const ExecutionPolicy.host();
    return ExecutionPolicyResolver(
      config: DartclawConfig.defaults().copyWith(
        container: ContainerConfig(enabled: policy.isContainer),
        tasks: TaskConfig(execution: policy.mode),
      ),
      availableContainerProfiles: policy.containerProfile == null ? const {} : {policy.containerProfile!},
    );
  }
  ...
  final String? Function(String providerId)? _declaredContainerProfileFor;
  ...
  /// task execution declaration: scheduled prompts without agent identity and system
  /// background turns.
  ...
  /// provider's harness registration declares. That registration is operator
  /// YAML as much as `security_profile` is, so an inherited host mode must
  /// reject rather than discard it.
  ExecutionPolicy resolveForAgent(AgentDefinition definition, {required String providerId}) {
    final declaredProfile = _providerContainerProfile(providerId);
    return _resolve(
      ...
      containerProfile: definition.securityProfile ?? declaredProfile ?? neutralContainerProfile,
      subject: 'logical agent "${definition.id}"',
      yamlPath: 'agent.agents.${definition.id}.execution',
    );
  }

  /// Resolves the task lane, applying an explicit operator-declared profile.
  ExecutionPolicy resolveForTask({String? securityProfile}) {
    if (securityProfile != null && !containerSecurityProfiles.contains(securityProfile)) {
      throw ExecutionPolicyException(
        'Cannot run the task lane in unknown container profile "$securityProfile". '
        'Allowed profiles: ${containerSecurityProfiles.join(', ')}.',
      );
    }
    final taskMode =
        _config.tasks.execution ?? (_config.container.enabled ? ExecutionMode.container : ExecutionMode.host);
    if (securityProfile != null && taskMode == ExecutionMode.host) {
      throw ExecutionPolicyException(
        'Cannot run the task lane on the host: its declared container profile "$securityProfile" has no host '
        'equivalent. Remove securityProfile, or set tasks.execution: container to keep it.',
      );
    }
    return _resolve(
      explicitMode: _config.tasks.execution,
      containerProfile: securityProfile ?? neutralContainerProfile,
      subject: 'the task lane',
      yamlPath: 'tasks.execution',
      profileIsOperatorConfigured: securityProfile != null,
    );
  }
  ...
  List<String> hostOverrideWarnings() {
    ...
    if (_config.tasks.execution == ExecutionMode.host) warn('tasks.execution');
  ...
  /// outage, but the operator learns which agent or task lane is unrunnable
  ...
  List<String> failClosedWarnings({Iterable<AgentDefinition>? agents}) {
    ...
    try {
      ...
      warnings.add('The task lane cannot run in this deployment: ${error.message}');
    }
    return List.unmodifiable(warnings);
  }
  ...
  /// agents or task requests reach it.
  ///
  /// [agents] is the deployment's effective agent set, which includes built-in
  ...
  List<String> providerCompatibilityWarnings({
    ...
    void check(String providerId, ExecutionPolicy Function() resolve) {
      ...
      final verdict = inventory.verdictFor(providerId: providerId, policy: policy);
    ...
    check(defaultProviderId, resolveForTask);
    return List.unmodifiable(warnings);
  }

  /// The container profile a harness registration declares for [providerId],
  /// or `null` when none does.
  String? _providerContainerProfile(String providerId) => _declaredContainerProfileFor?.call(providerId);
