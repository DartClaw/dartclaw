...
abstract final class CredentialPreflight {
  static final _log = Logger('CredentialPreflight');

  /// Logs every warning and raises [CredentialPreflightException] when a
  /// project credential cannot be used.
  ///
  /// Runs before the runtime is staged: a project whose credential resolves
  /// empty must abort before anything is provisioned for it.
  static void enforce(DartclawConfig config, Map<String, String> env) {
    final result = validate(config, env);
    for (final warning in result.warnings) {
      _log.warning(warning);
    }
    if (result.hasHardErrors) throw CredentialPreflightException(result.hardErrors);
  }
