...
/// Function type for running git commands off the event loop, injectable for
/// testing.
///
/// Returns primitives rather than a `ProcessResult` because the production
/// implementation carries them back across an [Isolate.run] boundary.
typedef ProjectGitRunner = Future<({int exitCode, String stderr, String stdout})> Function(
  List<String> args, {
  Map<String, String>? environment,
  String? workingDirectory,
});

/// Default [ProjectGitRunner] that runs git via [Isolate.run].
Future<({int exitCode, String stderr, String stdout})> _isolateGitRunner(
  ...
  return Isolate.run(() async {
    // Clone/fetch is remote transport: it needs `insteadOf`, proxy and
    // `sshCommand` from user or system git config to work at all.
