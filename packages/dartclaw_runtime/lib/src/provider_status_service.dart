...
class ProviderStatusService {
  ...
  ProviderStatus _buildStatus(MapEntry<String, ProviderEntry> entry) {
    ...
    final registrationValidation = provider.options['registration_validation_owned'] == true
        ? _registrationValidation(provider.options['registration_validation_result'])
        : null;
    final securityClassification = registrationValidation?['securityClassification'] as String?;
    final validationEvidence = _validationEvidence(registrationValidation?['evidence']);
  ...
  Map<String, dynamic>? _registrationValidation(Object? value) {
    if (value is! Map) {
      return null;
    }
  ...
  /// Its text comes from `dartclaw_kernel`'s single remediation author, so the
