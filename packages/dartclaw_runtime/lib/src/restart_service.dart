...
const _restartPendingFileName = 'restart.pending';
final _restartPendingLog = Logger('RestartPending');

/// Writes or merges restart-pending fields atomically.
void writeRestartPending(String dataDir, List<String> fields) {
  final filePath = p.join(dataDir, _restartPendingFileName);
  ...
  if (file.existsSync()) {
    try {
      ...
      if (raw is List) existingFields = raw.whereType<String>().toList();
    } catch (e) {
      _restartPendingLog.fine('Corrupted restart fields file — overwriting: $e');
    }
  }
...
/// Reads restart-pending state, returning null when none exists or it is unreadable.
Map<String, dynamic>? readRestartPending(String dataDir) {
  final file = File(p.join(dataDir, _restartPendingFileName));
  if (!file.existsSync()) return null;
  try {
    return jsonDecode(file.readAsStringSync()) as Map<String, dynamic>;
  } catch (e) {
    _restartPendingLog.fine('Failed to read restart.pending: $e');
    return null;
  }
}

/// Returns the pending field names, or an empty list when none are readable.
List<String> restartPendingFields(String dataDir) {
  final pending = readRestartPending(dataDir);
  return (pending?['fields'] as List<dynamic>?)?.whereType<String>().toList() ?? const <String>[];
}

/// Reads and removes the restart-pending marker after a successful restart.
Map<String, dynamic>? consumeRestartPending(String dataDir) {
  final file = File(p.join(dataDir, _restartPendingFileName));
  if (!file.existsSync()) return null;
  final pending = readRestartPending(dataDir) ?? const <String, dynamic>{};
  file.deleteSync();
  return pending;
}
