...
Future<void> ensureWorkflowLocalPathProjectReady({
  ...
  final status = await runGit(const ['status', '--porcelain=v2', '--branch'], workingDirectory: localPath);
  ...
  final origin = await runGit(const ['remote', 'get-url', 'origin'], workingDirectory: localPath);
