...
import 'package:dartclaw_google_chat/dartclaw_google_chat.dart'
    show GoogleChatSpaceEventsWiring, GoogleChatWebhookHandler, googleChatSubscriptionRoutes;
...
import 'api/emergency_stop_routes.dart';
...
import 'web/settings/settings_surface.dart';
...
class DartclawServer {
  ...
  late final TaskCreationService? _taskCreationService = _tasks.taskService == null
      ? null
      : TaskCreationService(tasks: _tasks.taskService!, projects: _tasks.projectService);
  late final TaskActionService? _taskActionService = _tasks.taskService == null
      ? null
      : TaskActionService(
          tasks: _tasks.taskService!,
          reviewService:
              _tasks.taskReviewService ??
              TaskReviewService(
                tasks: _tasks.taskService!,
  ...
  late final ChannelAccessService? _channelAccessService = _core.configWriter == null || _core.dataDir == null
      ? null
      : ChannelAccessService(
          writer: _core.configWriter!,
          dataDir: _core.dataDir!,
  ...
  late final GuardEditorService? _guardEditorService = _core.configWriter == null || _core.dataDir == null
      ? null
      : GuardEditorService(
          writer: _core.configWriter!,
          dataDir: _core.dataDir!,
  ...
  String get _appName => _core.config?.server.name ?? 'DartClaw';

  /// Internal constructor — prefer `composeServer` to assemble instances.
  new fromDeps({
    ...
    required ServerWebDeps web,
       ...
       _mcpHandler = McpProtocolHandler(
         resultTrimmer: core.resultTrimmer,
         guardChain: _auditedGuardChain(core),
         auditLogger: core.auditLogger,
       );

  /// Returns [ServerCoreDeps.guardChain], refusing it without an audit sink.
  ///
  /// The two fields are independent nullables, so a composer can otherwise get
  /// guard enforcement with no record - the one pairing `docs/guide/customization.md`
  /// rules out. A null sink makes the dispatch seam's audit a silent no-op, so
  /// nothing downstream would report the omission. Supplying neither stays
  /// legal: a host that guards nothing audits nothing.
  static GuardChain? _auditedGuardChain(ServerCoreDeps core) {
    if (core.guardChain != null && core.auditLogger == null) {
      throw ArgumentError.value(
        null,
        'auditLogger',
        'a guard chain requires an audit sink: guard enforcement with no audit trail is not a supported '
            'composition. Pass a GuardAuditLogger, or omit the guard chain.',
      );
    }
    return core.guardChain;
  }
  ...
  Handler _buildHandler() {
    ...
    _mountEmergencyStopRoutes(router);
    ...
    if (gatewayToken != null || unauthenticatedLoopback) {
      router.post(
        ...
        mcpRoute(
          ...
          requireLoopbackHost: unauthenticatedLoopback,
          clients: gatewayToken == null ? const [] : (_core.config?.gateway.mcpClients ?? const []),
          auditLogger: _core.auditLogger,
          toolCanonicals: _core.mcpToolCanonicals,
  ...
  void _mountConfigRoutes(Router router) {
    ...
    if (rc != null) {
      final cfgRouter = configRoutes(
        ...
        heartbeatIntervalMinutes: _core.config?.scheduling.heartbeatIntervalMinutes ?? 30,
        scheduledJobs: _web.schedulingJobs,
      );
  ...
  /// The one project-mutation authority, shared by `/api/projects*` and the
  /// projects page's own mutating routes.
  late final ProjectMutationService? _projectMutations = switch (_tasks.projectService) {
    final ProjectService projects => ProjectMutationService(
      projects: projects,
  ...
  void _mountProjectRoutes(Router router) {
    ...
    final mutations = _projectMutations;
    if (ps != null && mutations != null) {
      final projectRouter = projectRoutes(ps, mutations: mutations);
  ...
  void _mountTaskRoutes(Router router) {
    ...
    if (taskService != null && eventBus != null) {
      ...
      final taskRouter = taskRoutes(
        ...
        mergeStrategy: _tasks.mergeStrategy ?? 'squash',
        baseRef: _tasks.baseRef ?? 'main',
        creationService: _taskCreationService,
        actionService: _taskActionService,
      );
  ...
  void _mountEmergencyStopRoutes(Router router) {
    final taskService = _tasks.taskService;
    final eventBus = _observability.eventBus;
    final sseBroadcast = _observability.sseBroadcast;
    if (taskService == null || eventBus == null || sseBroadcast == null) return;
    registerEmergencyStopRoutes(
      router,
      turnManagerGetter: () => _turn.turns,
  ...
  void _mountSessionRoutes(Router router) {
    ...
    String buildSidebarHtml({required SidebarData sidebarData, List<NavItem> navItems = const []}) {
      ...
      return buildSidebar(sidebarData: sidebarData, navItems: resolvedNavItems, appName: _appName);
    }
  ...
  void _mountWebRoutes(Router router) {
    final webRouter = webRoutes(
      ...
      trustedProxies: _core.config?.auth.trustedProxies ?? const [],
      dataDir: _core.dataDir,
      contentGuardApiKeyConfigured: _web.contentGuardApiKeyConfigured,
      contentGuardFailOpen: _web.contentGuardFailOpen,
      schedulingJobs: _web.schedulingJobs,
      systemJobNames: _web.systemJobNames,
      pageRegistry: _pageRegistry,
      config: _core.config,
      taskService: _tasks.taskService,
      taskCreationService: _taskCreationService,
      taskActionService: _taskActionService,
      goalService: _tasks.goalService,
      projectService: _tasks.projectService,
      projectMutations: _projectMutations,
      eventBus: _observability.eventBus,
    ...
    if (tokenService != null && gatewayToken != null) {
      ...
      pipeline = pipeline.addMiddleware(
        authMiddleware(
          ...
          rateLimiter: _authRateLimiter,
          publicPaths: [
            // `/mcp` authenticates its own callers: a configured client's bearer
            // is not the gateway token, so this middleware would reject it
            // before the route could resolve it. `mcpRoute` carries the same
          ...
          (_) => Response.notFound(
            errorPageTemplate(
              ...
              topbarHtml: pageTopbarTemplate(
                ...
                restartBannerHtml: restartBannerHtml(_core.dataDir),
              ),
            ),
            headers: htmlHeaders,
          ),
