...
class RunnerObserver {
    ...
    if (runnerId != null && runner != null) {
      ...
      switch (event.kind) {
        ...
        case ExecutionEventKind.quarantined:
          _setState(metrics, WorkerState.crashed);
  ...
  void _markTerminal(_MutableMetrics metrics, WorkerState state) {
    _setState(metrics, state == WorkerState.crashed ? WorkerState.crashed : WorkerState.stopped);
  }

  void _setState(_MutableMetrics metrics, WorkerState state) {
    metrics.state = state;
    metrics.currentTaskId = null;
    metrics.currentSessionId = null;
    _fireState(metrics);
