...
final class ReviewInvalidRequest extends ReviewResult {
  ...
  const new(this.message, {required this.field});
}
...
class TaskReviewService {
  ...
  Future<ReviewResult> _reviewUnlocked(String taskId, String action, {String? comment, required String trigger}) async {
    ...
    if (normalizedAction.isEmpty) {
      return const ReviewInvalidRequest('action must not be empty', field: 'action');
    }
    final targetStatus = switch (normalizedAction) {
    ...
    if (targetStatus == null) {
      return const ReviewInvalidRequest('action must be one of: accept, reject, push_back', field: 'action');
    }

    final trimmedComment = comment?.trim();
    if (targetStatus == TaskStatus.running &&
        normalizedAction == 'push_back' &&
        (trimmedComment == null || trimmedComment.isEmpty)) {
      return const ReviewInvalidRequest('comment must not be empty for push_back', field: 'comment');
    }
    ...
    try {
      ...
      final transitionConfig = (targetStatus == TaskStatus.running && normalizedAction == 'push_back')
      ...
      _log.warning(
        'Unexpected failure while attempting to ${_displayAction(normalizedAction)} task $taskId',
        error,
        stackTrace,
      );
  ...
  Future<ProcessResult> _git(String workingDirectory, List<String> arguments, {required String action}) async {
    try {
      return await _runGit(arguments, workingDirectory: workingDirectory);
  ...
  Future<void> _persistPushErrorArtifact(String taskId, String errorDetails) async {
    ...
    await _persistArtifact(
      ...
      filename: 'push-error.txt',
      failureLabel: 'push error artifact',
    );
  }
  ...
  Future<void> _persistPrArtifact(
    ...
    kind: kind,
    content: content,
    filename: isFilePath ? '${name.toLowerCase().replaceAll(RegExp(r'[^a-z0-9]+'), '-')}.txt' : null,
    failureLabel: 'PR artifact "$name"',
  );
  ...
  Future<void> _persistArtifact(
    ...
    try {
      ...
      if (filename != null) {
        ...
        final directory = Directory('$_dataDir/tasks/$taskId/artifacts');
        await directory.create(recursive: true);
        final file = File('${directory.path}/$filename');
        await file.writeAsString(content);
        path = file.path;
      }
      await _tasks.addArtifact(id: const Uuid().v4(), taskId: taskId, name: name, kind: kind, path: path);
    } catch (error) {
      _log.warning('Failed to persist $failureLabel for task $taskId: $error');
    }
  }
  ...
  Future<void> _persistConflictArtifact(String taskId, MergeConflict conflict) async {
    ...
    await _persistArtifact(
      ...
      name: 'conflict.json',
      kind: ArtifactKind.data,
      content: jsonEncode(conflict.toJson()),
      filename: 'conflict.json',
      failureLabel: 'conflict artifact',
    );
  }
