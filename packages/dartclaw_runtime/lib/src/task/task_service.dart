...
class TaskService implements WorkflowTaskService {
  ...
  Future<Task> create({
    ...
    if (configJson.containsKey(TaskConfigView.securityProfileKey)) {
      throw ArgumentError.value(
        configJson[TaskConfigView.securityProfileKey],
        'configJson',
        '${TaskConfigView.securityProfileKey} is host-reserved; declare it through the authenticated task API',
      );
    }
    final normalizedSecurityProfile = _trimmedOrNull(securityProfile);
    if (normalizedSecurityProfile != null && !containerSecurityProfiles.contains(normalizedSecurityProfile)) {
      throw ArgumentError.value(
        securityProfile,
        'securityProfile',
        'must be one of: ${containerSecurityProfiles.join(', ')}',
      );
    }
    ...
    if (normalizedSecurityProfile != null) {
      sanitizedConfig[TaskConfigView.securityProfileKey] = normalizedSecurityProfile;
    }
    ...
    final nextExecution = identical(sessionId, _sentinel)
        ...
        : currentExecution?.copyWith(sessionId: resolvedSessionId) ??
              (resolvedSessionId == null
                  ? null
                  : AgentExecution(id: fallbackExecutionId, sessionId: resolvedSessionId));
