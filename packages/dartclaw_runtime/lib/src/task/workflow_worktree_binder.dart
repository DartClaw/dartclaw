...
final class WorkflowWorktreeBinder implements WorkflowTaskBindingCoordinator {
  ...
  Future<bool> ensureInlineWorkflowBranchCheckedOut(Task task, Project project, String branch) async {
    ...
    if (status.exitCode != 0) {
      await _failTask(
        task,
        kind: TaskFailureReason.worktreeBinding,
    ...
    // Workflow-orchestrated checkouts are automation-owned; a plain task
    // checkout is user-visible git in the operator's own working tree.
    final checkout = await runGit(
      ['checkout', branch],
      workingDirectory: project.localPath,
      noSystemConfig: isWorkflow,
    );
