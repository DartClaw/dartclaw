...
/// Git worktree lifecycle manager for tasks that declare worktree isolation.
///
...
///
/// Creates isolated git worktrees with dedicated branches, giving each caller
/// a sandboxed working directory. On task completion, the worktree and
...
class WorktreeManager {
  ...
  Future<WorktreeInfo> create(
    ...
    if (project != null) {
      ...
      final args = createBranch
          ...
          : await _attachWorktreeArgs(worktreePath, effectiveBaseRef, workingDirectory: effectiveProjectDir);
      final worktreeResult = await _runGit(args, workingDirectory: effectiveProjectDir);
      ...
      if (createBranch) {
        final branchResult = await _runGit(['branch', '--', branch, ref], workingDirectory: effectiveProjectDir);
      ...
      final worktreeArgs = createBranch
          ...
          : await _attachWorktreeArgs(worktreePath, ref, workingDirectory: effectiveProjectDir);
      final worktreeResult = await _runGit(worktreeArgs, workingDirectory: effectiveProjectDir);
      if (worktreeResult.exitCode != 0) {
        if (createBranch) {
          // Clean up the branch if worktree creation fails
          await _runGit(['branch', '--delete', branch], workingDirectory: effectiveProjectDir);
        }
        throw WorktreeException(
          'Failed to create worktree at $worktreePath',
          gitStderr: (worktreeResult.stderr as String).trim(),
  ...
  // Attaching checks the ref out as a branch so commits made in the worktree
  // advance it (the shared workflow worktree on the integration branch). Git
  // refuses that when another worktree already holds the branch — the operator
  // reviewing the branch they are standing on — so only that case detaches,
  // and the branch name then travels as WorktreeInfo metadata only.
  Future<List<String>> _attachWorktreeArgs(String worktreePath, String ref, {required String workingDirectory}) async {
    final list = await _runGit(['worktree', 'list', '--porcelain'], workingDirectory: workingDirectory);
    final held =
        list.exitCode == 0 && const LineSplitter().convert(list.stdout as String).contains('branch refs/heads/$ref');
    return ['worktree', 'add', if (held) '--detach', worktreePath, ref];
  }
  ...
  Future<void> _rollbackCreatedWorktree({
    ...
    final removeResult = await _runGit([
      'worktree',
      'remove',
      '--force',
      worktreePath,
    ...
    final branchResult = await _runGit(['branch', '--delete', '--force', branch], workingDirectory: workingDirectory);
    if (branchResult.exitCode != 0) {
      _log.warning(
  ...
  Future<bool> _localRefExists(String ref, String workingDirectory) async {
    ...
    final result = await _runGit(['rev-parse', '--verify', '--quiet', safeRef], workingDirectory: workingDirectory);
  ...
  Future<void> cleanup(String taskId, {Project? project}) async {
    ...
    // Remove worktree
    final removeResult = await _runGit(['worktree', 'remove', worktreePath], workingDirectory: effectiveProjectDir);
    if (removeResult.exitCode != 0) {
      _log.warning(
    ...
    // Delete branch
    final branchResult = await _runGit(['branch', '--delete', branch], workingDirectory: effectiveProjectDir);
    if (branchResult.exitCode != 0) {
      _log.warning(
  ...
  Future<bool> _branchExists(String branchName, {String? projectDir}) async {
    final result = await _runGit(['branch', '--list', branchName], workingDirectory: projectDir ?? _defaultProjectDir);
  ...
  Future<List<_RegisteredWorktree>> _listRegisteredWorktrees({required String workingDirectory}) async {
    final result = await _runGit(['worktree', 'list', '--porcelain'], workingDirectory: workingDirectory);
    if (result.exitCode != 0) {
      throw WorktreeException(
  ...
  Future<void> _removeRegisteredWorktree(String worktreePath, {required String workingDirectory}) async {
    final result = await _runGit(['worktree', 'remove', '--force', worktreePath], workingDirectory: workingDirectory);
    if (result.exitCode != 0) {
      throw WorktreeException(
  ...
  Future<void> _pruneDanglingWorktrees(String workingDirectory) async {
    final result = await _runGit(['worktree', 'prune'], workingDirectory: workingDirectory);
    if (result.exitCode != 0) {
      throw WorktreeException(
