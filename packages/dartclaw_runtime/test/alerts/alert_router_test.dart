...
CredentialHealthChangedEvent _reauthRequiredEvent() => CredentialHealthChangedEvent(
  providerId: 'claude',
  state: CredentialHealthState.reauthRequired,
  detail: 'The stored setup-token no longer authenticates.',
  timestamp: _now,
);
...
void main() {
  ...
  group('AlertRouter — event classification and routing', () {
    final cases = [
      ...
      (
        ...
        fire: () => bus.fire(CompactionStartingEvent(sessionId: 's1', trigger: 'manual', timestamp: _now)),
        expectedDeliveries: 0,
        assertTargets: false,
      ),
      (
        name: 'a non-alertable lifecycle event produces no deliveries',
        fire: () => bus.fire(SessionEndedEvent(sessionId: 's1', sessionType: 'web', timestamp: _now)),
        expectedDeliveries: 0,
        assertTargets: false,
      ),
    ];
  ...
  group('AlertRouter — throttling integration', () {
    ...
    test('a burst of credential_reauth_required summarises as [CRITICAL], not [WARNING]', () {
      fakeAsync((async) {
        final localBus = EventBus();
        final localAdapter = FakeAlertDeliveryAdapter();
        final localRouter = AlertRouter(
          bus: localBus,
          adapter: localAdapter,
          config: _config(targets: const [_target0], cooldownSeconds: 300, burstThreshold: 5),
        );

        for (var i = 0; i < 6; i++) {
          localBus.fire(_reauthRequiredEvent());
          async.flushMicrotasks();
        }
        ...
        expect(localAdapter.delivered, hasLength(2));
        final summary = localAdapter.delivered.last.$2;
        expect(summary.text, startsWith('[CRITICAL]'));
        expect(summary.text, contains('Re-authentication Required Summary'));
    ...
    test('a burst of compaction events summarises as [INFO]', () {
      fakeAsync((async) {
        final localBus = EventBus();
        final localAdapter = FakeAlertDeliveryAdapter();
        final localRouter = AlertRouter(
        ...
        final event = CompactionCompletedEvent(sessionId: 's1', trigger: 'auto', timestamp: _now);
        for (var i = 0; i < 6; i++) {
          localBus.fire(event);
          async.flushMicrotasks();
        }
        ...
        expect(localAdapter.delivered, hasLength(2));
        expect(localAdapter.delivered.last.$2.text, startsWith('[INFO]'));
