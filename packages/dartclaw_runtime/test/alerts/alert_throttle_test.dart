...
import 'package:dartclaw_runtime/src/alerts/alert_throttle.dart';
...
void main() {
  group('AlertThrottle — basic cooldown', () {
    test('first call to shouldDeliver returns true', () {
      ...
      expect(throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0), isTrue);
      throttle.dispose();
    });
    ...
    test('second call within cooldown returns false', () {
      fakeAsync((async) {
        ...
        expect(throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0), isFalse);
        throttle.dispose();
      });
    });
    ...
    test('call after cooldown expiry starts a new cycle', () {
      fakeAsync((async) {
        ...
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0);
    ...
    test('delivered-only entry expires before the next isolated alert', () {
      fakeAsync((async) {
        ...
        expect(throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0, now: start), isTrue);

        async.elapse(const Duration(minutes: 6));

        expect(
          throttle.shouldDeliver(
            'guard_block',
  ...
  group('AlertThrottle — burst summary', () {
    test('6 events with burstThreshold=5 triggers summary with count=5', () {
      fakeAsync((async) {
        ...
        final throttle = AlertThrottle(
          cooldown: const Duration(minutes: 5),
          burstThreshold: 5,
          onSummary: (type, _, target, count) => summaries.add((type, target, count)),
        );
    ...
    test('the summary carries the severity captured at first suppression', () {
      fakeAsync((async) {
        final severities = <AlertSeverity>[];
        final throttle = AlertThrottle(
          cooldown: const Duration(minutes: 5),
          burstThreshold: 2,
          onSummary: (_, severity, _, _) => severities.add(severity),
        );

        // Production cannot reach this: severity is a function of the alert
        // type, which is part of the key. Varying it anyway is what makes the
        // capture point observable rather than asserted by the test's name.
        throttle.shouldDeliver('credential_reauth_required', AlertSeverity.critical, _target0);
        throttle.shouldDeliver('credential_reauth_required', AlertSeverity.critical, _target0);
        throttle.shouldDeliver('credential_reauth_required', AlertSeverity.info, _target0);

        async.elapse(const Duration(minutes: 5, seconds: 1));

        expect(severities, [AlertSeverity.critical]);
        throttle.dispose();
      });
    });
    ...
    test('exactly burstThreshold suppressed events triggers summary', () {
      fakeAsync((async) {
        ...
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0); // delivered
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0); // suppressed #1
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0); // suppressed #2
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0); // suppressed #3 (= threshold)
  ...
  group('AlertThrottle — per-key independence', () {
    test('different event types to same target throttled independently', () {
      fakeAsync((async) {
        ...
        expect(throttle.shouldDeliver('container_crash', AlertSeverity.warning, _target0), isTrue);
        // guard_block to _target0 again: suppressed
        expect(throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0), isFalse);
        // container_crash to _target0 again: suppressed
        expect(throttle.shouldDeliver('container_crash', AlertSeverity.warning, _target0), isFalse);
    ...
    test('same event type to different targets throttled independently', () {
      fakeAsync((async) {
        ...
        expect(throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0), isTrue);
        expect(throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target1), isTrue);
        // Both suppressed on second
        expect(throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0), isFalse);
        expect(throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target1), isFalse);
    ...
    test('throttle state of one key does not affect another key summary', () {
      fakeAsync((async) {
        ...
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target1);
  ...
  group('AlertThrottle — timer lifecycle', () {
    test('timer not started on first event (no suppression)', () {
      fakeAsync((async) {
        ...
        final throttle = AlertThrottle(
          cooldown: const Duration(minutes: 5),
          burstThreshold: 5,
          onSummary: (_, _, _, _) => summaryCalled = true,
        );
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0);
    ...
    test('entry cleaned up after timer fires', () {
      fakeAsync((async) {
        ...
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0);
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0); // starts timer
    ...
    test('dispose cancels pending timers — no summary delivered', () {
      fakeAsync((async) {
        ...
        final throttle = AlertThrottle(
          cooldown: const Duration(minutes: 5),
          burstThreshold: 2,
          onSummary: (_, _, _, count) => summaries.add(count),
        );

        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0);
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0);
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0); // suppressedCount=2
  ...
  group('AlertThrottle — reconfigure', () {
    test('reconfigure applies new cooldown to subsequent cycles', () {
      fakeAsync((async) {
        ...
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0);
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0); // starts timer (10 min)
        ...
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0);
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0);
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0); // 5 suppressed ≥ threshold
    ...
    test('active entry timers not interrupted by reconfigure', () {
      fakeAsync((async) {
        ...
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0);
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0);
        throttle.shouldDeliver('guard_block', AlertSeverity.warning, _target0); // 5 suppressed, timer running
