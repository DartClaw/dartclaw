...
import 'package:dartclaw_runtime/src/api/guard_editor_service.dart' show validateGuardEditorConfig;
...
''');
  ...
  group('guard extension CRUD', () {
    test('save-time validation uses the production guard builder', () {
      ...
      expect(guards.map((guard) => guard.name), containsAll(['command', 'file', 'network']));
      expect(guards.map((guard) => guard.name), isNot(contains('input-sanitizer')));
    });
...
''');
    ...
    test('PUT reports an unusable body and an unusable index with one message', () async {
      const message = 'Request body must be valid JSON and index must be numeric';
      final router = createRouter();

      final emptyBody = await router.call(
        withAdminAuthContext(
          Request(
            'PUT',
            Uri.parse('http://localhost/api/config/guards/file/extra_rules/0'),
            body: '',
            headers: {'content-type': 'application/json'},
          ),
        ),
      );
      expect(emptyBody.statusCode, 400);
      expect((await readJson(emptyBody))['error'], containsPair('message', message));

      final badIndex = await request(
        router,
        'PUT',
        '/api/config/guards/file/extra_rules/not-a-number',
        body: {'pattern': '**/secrets/**', 'level': 'read_only'},
      );
      expect(badIndex.statusCode, 400);
      expect((await readJson(badIndex))['error'], containsPair('message', message));
    });
  ...
  group('POST /api/config/guards/test', () {
    ...
    test('the retired input-sanitizer family is not a testable guard', () async {
      final response = await request(
        createRouter(),
        'POST',
        '/api/config/guards/test',
