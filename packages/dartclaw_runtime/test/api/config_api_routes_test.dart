...
''');
  ...
  Router createRouter({DartclawConfig? config, RuntimeConfig? runtime, EventBus? eventBus}) {
    ...
    final rc = runtime ?? _runtimeFor(cfg);
...
github:
  ...
  group('GET /api/scheduling/jobs', () {
    // `memory-curation` is a plain job ID; the built-in refuses the collision
    // at config load, not at request time.
    test('memory-curation is an ordinary job with no system-action projection', () async {
      writeJobsToYaml([
        {'name': memoryCurationJobId, 'schedule': '0 1 * * *', 'prompt': 'operator job', 'delivery': 'none'},
      ]);
      final router = createRouter();

      final list = await api(router).expectJsonList('GET', '/api/scheduling/jobs');
      final job = list.single as Map<String, dynamic>;
      expect(job, containsPair('name', memoryCurationJobId));
      expect(job.containsKey('type'), isFalse);
      expect(job.containsKey('lifecycle'), isFalse);
      final shown = await api(router).expectJsonObject('GET', '/api/scheduling/jobs/$memoryCurationJobId');
      expect(shown, containsPair('name', memoryCurationJobId));
      expect(shown.containsKey('lifecycle'), isFalse);

      // Edit and delete were the two verbs the reserved-ID 409 specifically blocked.
      final edited = await api(router).expectJsonObject(
        'PUT',
        '/api/scheduling/jobs/$memoryCurationJobId',
        json: {'prompt': 'edited by the operator'},
      );
      expect((edited['job']! as Map)['prompt'], 'edited by the operator');
      await api(router).expectJsonObject('DELETE', '/api/scheduling/jobs/$memoryCurationJobId');
      await api(router).expectJsonObject(
        'POST',
        '/api/scheduling/jobs',
        json: {'name': memoryCurationJobId, 'schedule': '0 7 * * *', 'prompt': 'recreated', 'delivery': 'none'},
        status: 201,
      );
    });
  ...
  group('PATCH /api/config — validation', () {
    ...
    for (final testCase in const [
      ...
      // Described in the published schema, and still refused here: the YAML
      // file stays the only place guard enforcement can be turned off.
      (name: 'read-only guard switch', patch: {'guards.enabled': false}, field: 'guards.enabled'),
      // An object-valued field is written wholesale, so a settable container
      // would carry the read-only fields inside it past their own refusal.
      (
        name: 'read-only channel container',
        patch: {
          'channels': {
            'google_chat': {'service_account': 'stolen.json'},
          },
        },
        field: 'channels',
      ),
    ...
    test('a value the runtime would clamp away is refused without touching YAML or restart.pending', () async {
      // Before the bound was declared this returned 200, wrote 5000 to
      // dartclaw.yaml and set restart.pending for a value the loader clamps to
      // 1440 at that very restart.
      final before = File(configPath).readAsStringSync();
      final json = await adminApi(createRouter())
          .expectJsonObject('PATCH', '/api/config', json: {'knowledge.inbox.interval_minutes': 5000}, status: 400);

      final errors = json['errors'] as List;
      expect(errors, hasLength(1));
      expect((errors.first as Map)['field'], 'knowledge.inbox.interval_minutes');
      expect((errors.first as Map)['message'], contains('between 1 and 1440'));
      expect(File(configPath).readAsStringSync(), before);
      expect(File(p.join(dataDir, 'restart.pending')).existsSync(), false);
    });

    test('the declared bound itself is accepted and takes the ordinary restart route', () async {
      final json = await adminApi(createRouter())
          .expectJsonObject('PATCH', '/api/config', json: {'knowledge.inbox.interval_minutes': 1440});

      expect(json['pendingRestart'], contains('knowledge.inbox.interval_minutes'));
      expect(File(configPath).readAsStringSync(), contains('1440'));
      expect(File(p.join(dataDir, 'restart.pending')).existsSync(), true);
    });

    test('the fold classifies its keys: enabled flags live, both schedule intervals restart', () async {
      final router = createRouter();
      ...
      final json = await adminApi(router).expectJsonObject(
        ...
        json: {
          ...
          'scheduling.heartbeat.interval_minutes': 45,
          'workspace.git_sync.interval_minutes': 45,
        },
      );
      ...
      expect(
        json['pendingRestart'],
        containsAll(<String>['scheduling.heartbeat.interval_minutes', 'workspace.git_sync.interval_minutes']),
      );
    });
...
channels:
  ...
  group('PATCH /api/config – alerts delivery surface', () {
    // The alert router swaps the whole alerts section at runtime, so both keys
    // are reloadable rather than restart-tier; a notifier-less router would
    // report them pendingRestart instead.
    Router createRouterWithNotifier() => configApiRoutes(
    ...
    test('_meta.fields lists alerts.targets and alerts.routes as reloadable', () async {
      final json = await api(createRouter()).expectJsonObject('GET', '/api/config');

      final fields = (json['_meta'] as Map<String, dynamic>)['fields'] as Map<String, dynamic>;
      for (final key in ['alerts.targets', 'alerts.routes']) {
        expect(fields.containsKey(key), isTrue, reason: '$key must be describable');
        expect((fields[key] as Map<String, dynamic>)['mutable'], 'reloadable', reason: '$key reload tier');
      }
    });

    test('alerts.targets accepts a list of {channel, recipient} objects and applies without restart', () async {
      final json = await adminApi(createRouterWithNotifier()).expectJsonObject(
        'PATCH',
        '/api/config',
        json: {
          'alerts.targets': [
            {'channel': 'signal', 'recipient': '+1000'},
          ],
        },
      );

      expect(json['applied'], ['alerts.targets']);
      expect(json['pendingRestart'], isEmpty);

      final reloaded = DartclawConfig.load(configPath: configPath);
      expect(reloaded.alerts.targets, hasLength(1));
      expect(reloaded.alerts.targets.single.channel, 'signal');
      expect(reloaded.alerts.targets.single.recipient, '+1000');
      expect(File(p.join(dataDir, 'restart.pending')).existsSync(), isFalse);
    });

    test('a non-object element in alerts.targets is rejected, not silently dropped', () async {
      final json = await adminApi(createRouterWithNotifier()).expectJsonObject(
        'PATCH',
        '/api/config',
        json: {
      ...
      final errors = json['errors'] as List;
      expect(errors.map((error) => (error as Map<String, dynamic>)['field']), contains('alerts.targets'));
      expect(File(configPath).readAsStringSync(), isNot(contains('+1000')));
    });

    test('alerts.routes accepts a per-type map of target selectors', () async {
      final json = await adminApi(createRouterWithNotifier()).expectJsonObject(
        'PATCH',
        '/api/config',
        json: {
      ...
      expect(json['applied'], ['alerts.routes']);
      expect(DartclawConfig.load(configPath: configPath).alerts.routes['credential_reauth_required'], ['*']);
    });
  });
  ...
  group('Task Job CRUD via scheduling.jobs', () {
      ...
      writeTaskJobsToYaml([
        {
          'id': 'existing-task',
          'schedule': '0 9 * * *',
          'task': {'title': 'Existing', 'description': 'Desc'},
        },
      ]);
      final router = createRouter();
      await api(router).expectResponse(
        'POST',
        '/api/scheduling/tasks',
        json: {'id': 'existing-task', 'schedule': '0 10 * * *', 'title': 'Another', 'description': 'Desc'},
        status: 409,
      );
    });
      ...
      writeTaskJobsToYaml([
        {
          'id': 'my-task',
          'schedule': '0 9 * * *',
          'task': {'title': 'Old title', 'description': 'Desc'},
        },
      ]);
      final router = createRouter();
      final json = await api(router)
      ...
      writeTaskJobsToYaml([
        {
          'id': 'removable-task',
          'schedule': '0 9 * * *',
          'task': {'title': 'Remove me', 'description': 'Desc'},
        },
      ]);
      final router = createRouter();
    ...
    test('special-character task id round-trips through PUT/DELETE', () async {
      ...
      await api(router).expectResponse(
        'POST',
        '/api/scheduling/tasks',
        json: {'id': taskId, 'schedule': '0 9 * * *', 'title': 'Review', 'description': 'Desc'},
        status: 201,
      );
    ...
    test('POST /api/scheduling/jobs with type: task does not require delivery', () async {
      ...
      final json = await api(router).expectJsonObject(
        ...
        json: {
          ...
          'type': 'task',
          'schedule': '0 10 * * *',
          'task': {'title': 'Maintenance task', 'description': 'Do some maintenance'},
        },
        status: 201,
      );

      expect(json['job']['name'], 'scheduled-maintenance-task');
...
''');
  ...
  group('G4 three-tier PATCH partitioning', () {
    ...
    test('a hand-edited undescribed key answers with the structured error, not an uncaught throw', () async {
      // Someone edits an unknown key into dartclaw.yaml behind a running
      // server. The load sweep refuses that file; the route must answer with
      // the refusal it already has rather than letting it escape.
      final notifier = ConfigNotifier(const DartclawConfig.defaults());
      final router = createRouterWithNotifier(notifier);
      File(configPath).writeAsStringSync('not_a_real_section:\n  x: 1\n');

      final response = await adminApi(router)
          .expectResponse('PATCH', '/api/config', json: {'concurrency.max_parallel_turns': 4}, status: 500);
      final body = jsonDecode(await response.readAsString()) as Map<String, dynamic>;

      expect((body['error'] as Map)['code'], 'INTERNAL_ERROR');
      expect((body['error'] as Map)['message'], contains("Unknown config field: 'not_a_real_section'"));
      expect(notifier.current, const DartclawConfig.defaults(), reason: 'the running config is untouched');
    });
