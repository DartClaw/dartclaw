...
void main() {
    ...
    restClient = FakeGoogleChatRestClient(
      onSend:
          ({
            ...
            return (
              messageName: '$spaceName/messages/$sentMessageCounter',
              threadName: null,
  ...
  group('JWT verification', () {
    ...
    test('records the forwarded client for an invalid JWT received from a trusted proxy', () async {
      jwtVerifier.shouldVerify = false;
      final events = <FailedAuthEvent>[];
      final sub = eventBus.on<FailedAuthEvent>().listen(events.add);
      addTearDown(sub.cancel);
      ...
      final response = await handler.handle(
        Request(
          'POST',
          Uri.parse('http://localhost/integrations/googlechat'),
          headers: {'authorization': 'Bearer invalid', 'x-forwarded-for': '10.0.0.1, 10.0.0.2'},
          body: jsonEncode(_payload()),
          context: {'shelf.io.connection_info': FakeConnectionInfo('192.168.1.100')},
        ),
      );
    ...
    test('ignores forwarded identity for an invalid JWT received from an untrusted proxy', () async {
      jwtVerifier.shouldVerify = false;
      final events = <FailedAuthEvent>[];
      final sub = eventBus.on<FailedAuthEvent>().listen(events.add);
      addTearDown(sub.cancel);
      ...
      final response = await handler.handle(
        Request(
          'POST',
          Uri.parse('http://localhost/integrations/googlechat'),
          headers: {'authorization': 'Bearer invalid', 'x-forwarded-for': '10.0.0.1'},
          body: jsonEncode(_payload()),
          context: {'shelf.io.connection_info': FakeConnectionInfo('192.168.1.50')},
        ),
      );

      expect(response.statusCode, 401);
      await pumpEventQueue();
      expect(events.single.remoteKey, '192.168.1.50');
    });
  ...
  group('payload edge cases', () {
    ...
    test('returns 413 for an oversized streamed payload without Content-Length', () async {
      final response = await handler.handle(
        Request(
          'POST',
          Uri.parse('http://localhost/integrations/googlechat'),
