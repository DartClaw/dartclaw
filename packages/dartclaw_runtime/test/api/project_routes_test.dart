...
void main() {
  ...
  group('malformed bodies keep the JSON error envelope', () {
    // The decoders cannot type-check every nested shape, so one still throws.
    // Both write routes must answer the structured 500 rather than let shelf_io
    // emit a bare text/plain page a client cannot read a code out of.
    test('a create whose pr.labels is not a list returns 500 INTERNAL_ERROR', () async {
      expect(
        await client.expectJsonErrorCode(
          'POST',
          '/api/projects',
    ...
    test('an update whose pr.labels is not a list returns 500 INTERNAL_ERROR', () async {
      projects.seed(makeProject(id: 'my-proj'));
