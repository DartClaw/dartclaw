...
void main() {
  ...
  test('/new creates a task, stores origin, and fires a slash_command event', () async {
    ...
    final response = await handler.handle(
      const SlashCommand(name: 'new', arguments: 'analyze competitor pricing'),
  ...
  test('/new refuses the retired research type with explicit-profile remediation', () async {
    final response = await handler.handle(
    ...
    final header = _singleCard(response)['header'] as Map<String, dynamic>;
    expect(header['title'], 'Task Type Retired');
    expect(header['subtitle'], 'Error');
    expect(response.toString(), allOf(contains('securityProfile'), contains('research')));
  });

  test('/new refuses the retired coding type with worktree-declaration remediation', () async {
    final response = await handler.handle(
      const SlashCommand(name: 'new', arguments: 'coding: fix the login page'),
      spaceName: 'spaces/AAAA',
      senderJid: 'users/123',
      spaceType: 'DM',
    );
    ...
    final header = _singleCard(response)['header'] as Map<String, dynamic>;
    expect(header['title'], 'Task Type Retired');
    expect(response.toString(), allOf(contains('needsWorktree'), contains('coding')));
  });

  test('/new keeps an inert leading token in the task description', () async {
    ...
    expect(created, contains(isA<Task>().having((task) => task.title, 'title', 'migration_plan: prepare cutover')));
    expect(defaultTask.status, TaskStatus.queued);
  });
  ...
  test('/new without a description returns an error card', () async {
    ...
    expect(_sectionText(response), contains('Usage: /new &lt;description&gt;'));
  });
  ...
  test('/status summarizes active tasks and sessions', () async {
    ...
    await tasks.create(id: 'task-done', title: 'Publish notes', description: 'Publish notes', autoStart: false);
