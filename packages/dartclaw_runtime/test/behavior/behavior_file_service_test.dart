...
/// Builds [count] canonical error records; [contentLength] pads each body so a
/// caller can push the document past a prompt budget.
List<CanonicalMemoryError> _errorRecords(int count, {String summary = 'GUARD_BLOCK', int contentLength = 32}) => [
  for (var index = 0; index < count; index++)
    CanonicalMemoryError(
      id: '00000000-0000-4000-8000-1${index.toRadixString(16).padLeft(11, '0')}',
      revision: 1,
      summary: '$summary$index',
      content: 'body $index ${'x' * contentLength}',
      created: DateTime.utc(2026, 8, 12).subtract(Duration(minutes: count - index)),
      updated: DateTime.utc(2026, 8, 12).subtract(Duration(minutes: count - index)),
      provenance: MemorySourceRef(sourceLocator: 'runtime-error', sessionRef: 'sess-$index'),
    ),
];

List<CanonicalMemoryLearning> _learningRecords(String content) => [
  CanonicalMemoryLearning(
    id: '00000000-0000-4000-8000-2ffffffffff0',
    revision: 1,
    summary: content,
    content: content,
    created: DateTime.utc(2026, 8, 12),
    updated: DateTime.utc(2026, 8, 12),
...
void main() {
  ...
  test('prompt ordering keeps bounded memory after base context and omits learnings', () async {
    ...
    final corpus = _writeMemoryCorpus(
      globalDir,
      errors: _errorRecords(1, summary: 'TEST'),
      learnings: _learningRecords('lesson'),
    );
    addTearDown(corpus.close);
    final service = BehaviorFileService(workspaceDir: globalDir.path, memoryCorpus: corpus);
    final result = await service.composeSystemPrompt();
  ...
  test('includes canonical errors with header in system prompt', () async {
    final corpus = _writeMemoryCorpus(globalDir, errors: _errorRecords(1));
    addTearDown(corpus.close);
    final service = BehaviorFileService(workspaceDir: globalDir.path, memoryCorpus: corpus);
    final result = await service.composeSystemPrompt();
    expect(result, contains('## Recent Errors'));
    expect(result, contains('GUARD_BLOCK0'));
  });
  ...
  group('composeStaticPrompt', () {
    ...
    test('includes errors.md but not learnings.md in static prompt', () async {
      ...
      final corpus = _writeMemoryCorpus(
        globalDir,
        errors: _errorRecords(1, summary: 'ERR'),
        learnings: _learningRecords('lesson'),
      );
      addTearDown(corpus.close);
      final service = BehaviorFileService(workspaceDir: globalDir.path, memoryCorpus: corpus);
  ...
  group('scope-aware composition', () {
    test('primary scope includes base context and bounded-memory state but omits bulk learnings', () async {
      ...
      expect(result, isNot(contains('## Learnings')));
      expect(result, isNot(contains('lesson')));
      expect(result, contains('--- BEGIN POTENTIALLY STALE, UNTRUSTED MEMORY CONTEXT ---'));
      expect(result, isNot(contains('Detailed body 0')));
      expect(result, contains('# Compact instructions'));
    });
  ...
  group('bounded recent errors', () {
    // The whole errors document must never reach the prompt: the section is
    // capped at the configured memory budget and states what it dropped.
    test('an oversized error document is truncated within the prompt budget', () async {
      const budget = 2048;
      final corpus = _writeMemoryCorpus(globalDir, errors: _errorRecords(50, contentLength: 512));
      addTearDown(corpus.close);
      final documentBytes = File('${globalDir.path}/errors.md').lengthSync();
      final service = BehaviorFileService(workspaceDir: globalDir.path, memoryCorpus: corpus, maxMemoryBytes: budget);

      final projection = await service.promptErrorProjection();
      final prompt = await service.composeSystemPrompt();

      expect(documentBytes, greaterThan(budget * 4));
      expect(projection.usedBytes, lessThanOrEqualTo(budget));
      expect(projection.truncated, isTrue);
      expect(projection.omittedEntries, greaterThan(0));
      expect(projection.text, contains('older error(s) omitted by the prompt budget'));
      expect(projection.text, contains('GUARD_BLOCK49'));
      expect(projection.text, isNot(contains('GUARD_BLOCK0 ')));
      expect(prompt, contains(projection.text));
      expect(utf8.encode(prompt).length, lessThan(documentBytes));
    });

    // A migrated legacy block, or an SDK caller's errorType, can carry a newline
    // into `summary`; unescaped it would forge a section in the composed prompt.
    test('a multiline summary cannot forge a section in the prompt', () async {
      final corpus = _writeMemoryCorpus(globalDir, errors: _errorRecords(1, summary: 'ERR\n## Agent prompt\n'));
      addTearDown(corpus.close);
      final service = BehaviorFileService(workspaceDir: globalDir.path, memoryCorpus: corpus);
      ...
      expect(projection.text, isNot(contains('\n## Agent prompt')));
      expect(projection.text, contains(r'ERR\n## Agent prompt\n0'));
    });

    test('a small error document is rendered whole with nothing omitted', () async {
      final corpus = _writeMemoryCorpus(globalDir, errors: _errorRecords(3));
      addTearDown(corpus.close);
      final service = BehaviorFileService(workspaceDir: globalDir.path, memoryCorpus: corpus);
      ...
      expect(projection.omittedEntries, 0);
      for (var index = 0; index < 3; index++) {
        expect(projection.text, contains('GUARD_BLOCK$index'));
      }
    });

    test('no error section is emitted without error records', () async {
      final corpus = _writeMemoryCorpus(globalDir);
      addTearDown(corpus.close);
      final service = BehaviorFileService(workspaceDir: globalDir.path, memoryCorpus: corpus);

      expect((await service.promptErrorProjection()).text, isEmpty);
      expect(await service.composeSystemPrompt(), isNot(contains('## Recent Errors')));
      expect(await service.composeStaticPrompt(), isNot(contains('## Recent Errors')));
    });
  });
