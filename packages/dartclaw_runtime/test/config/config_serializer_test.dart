...
void main() {
  ...
  setUpAll(ensureGitHubWebhookConfigRegistered);
  ...
  group('ConfigSerializer.toJson', () {
    test('default config produces correct nested camelCase JSON', () {
      ...
      expect(json.containsKey('workerTimeout'), isFalse);
      ...
      expect(json.containsKey('advisor'), isFalse);
    ...
    test('default config serializes context fields with correct defaults', () {
      ...
      expect(context['maxResultBytes'], 50 * 1024);
    ...
    test('custom context values serialize to camelCase JSON', () {
      ...
      expect(context['maxResultBytes'], 64 * 1024);
          ...
          channelConfigs: {
            'google_chat': _googleChatChannelConfig(
              const GoogleChatConfig(
                ...
                dmAllowlist: [GroupEntry(id: 'spaces/AAA/users/1')],
...
''';
    ...
    test('governance turn limits serialize nested budgets', () {
      final config = DartclawConfig.load(
        configPath: 'dartclaw.yaml',
        fileReader: (path) {
          if (path == 'dartclaw.yaml') {
...
''';
      ...
      final json = serializer.toJson(config, runtime: runtime);
      final turnLimits = ((json['governance'] as Map<String, dynamic>)['turnLimits']) as Map<String, dynamic>;

      expect(turnLimits['stallAction'], 'cancel');
      expect(turnLimits['stallTimeout'], anyOf(45, '45s', const Duration(seconds: 45)));
      expect(turnLimits['turnTimeout'], anyOf(900, '900s', const Duration(seconds: 900)));
    });
  });
  ...
  group('ConfigSerializer.metaJson', () {
    ...
    test('selected metadata entries expose constraints and flags', () {
      ...
      final cases = [
        ...
        (path: 'scheduling.heartbeat.interval_minutes', expected: {'mutable': 'restart', 'min': 1, 'max': 1440}),
        (
          path: 'workspace.git_sync.interval_minutes',
          expected: {'type': 'int', 'mutable': 'restart', 'min': 1, 'max': 1440},
        ),
    ...
    test('every field entry carries a non-blank description', () {
      final meta = serializer.metaJson();
      for (final entry in meta.entries) {
        final description = (entry.value as Map<String, dynamic>)['description'];
        expect(description, isA<String>(), reason: entry.key);
        expect((description as String).trim(), isNotEmpty, reason: entry.key);
      }
      expect((meta['port'] as Map<String, dynamic>)['description'], ConfigMeta.fields['port']!.description);
    });

    test('every serialized leaf under a registered section is itself registered', () {
      // Guards the partly-registered section that neither a per-key nor a
      // per-section check catches: alerts.targets/routes were emitted here for
      // releases while ConfigMeta knew only the three alerts scalars.
      const emittedButUnregistered = {
        // The top-level mirror of the deprecated memory_max_bytes alias. The
        // field itself is registered as memory.max_bytes.
        'memoryMaxBytes',
      };

      final runtime = RuntimeConfig(heartbeatEnabled: false, gitSyncEnabled: false, gitSyncPushEnabled: false);
      final json = serializer.toJson(const DartclawConfig.defaults(), runtime: runtime);
      ...
      // An enclosing entry shape resolves a leaf only when it names it — a
      // shape that merely exists would make the whole section unfailable.
      Map<String, EntryFieldMeta> entryFields(ConfigEntryShape? shape) => switch (shape) {
        ObjectEntry(:final fields) => {
          ...fields,
          for (final field in fields.entries)
            ...entryFields(field.value.entry).map((key, value) => MapEntry('${field.key}.$key', value)),
        },
        ValueEntry() || OpaqueEntry() || null => const {},
      };

      bool resolves(String jsonKey) {
        if (ConfigMeta.byJsonKey.containsKey(jsonKey)) return true;
        // A section serialized as null (an absent optional block) walks as a
        // leaf; it is answerable when its own leaves are registered.
        if (ConfigMeta.byJsonKey.keys.any((key) => key.startsWith('$jsonKey.'))) return true;
        final segments = jsonKey.split('.');
        for (var length = segments.length - 1; length > 0; length--) {
          final meta = ConfigMeta.byJsonKey[segments.take(length).join('.')];
          ...
          return switch (shape) {
            // An opaque shape names no field, so it answers for the entry
            // itself and for no descendant.
            OpaqueEntry() => relative.isEmpty,
            ValueEntry() => relative.isEmpty,
            ObjectEntry() => relative.isEmpty || entryFields(shape).containsKey(relative),
          };
        }
        return false;
      }

      expect(resolves('channels.whatsapp.notAKey'), isFalse);
      expect(resolves('channels.discord.enabled'), isFalse);

      final unregistered = leaves.where((leaf) => !emittedButUnregistered.contains(leaf) && !resolves(leaf)).toSet();
      expect(unregistered, isEmpty, reason: 'serialized leaves absent from ConfigMeta: $unregistered');
    });
