...
void main() {
  ...
  group('ContainerManager', () {
    test('isRuntimeAvailable returns true on zero exit code', () async {
      ...
      expect(await manager.isRuntimeAvailable(), isTrue);
    });

    test('isRuntimeAvailable returns false on non-zero exit code', () async {
      final manager = _manager(run: (executable, arguments) async => ProcessResult(1, 1, '', 'no docker'));

      expect(await manager.isRuntimeAvailable(), isFalse);
    });

    test('isRuntimeAvailable returns false on exception', () async {
    ...
    test('no host mount or Docker argument can be supplied from config', () async {
      // The keys that once claimed to carry them are gone: the parser reports
      // them like any other key it does not read, and the argument vector is
      // the fixed profile template with nothing config-sourced in it.
      final warns = <String>[];
      ...
      final create = calls.firstWhere((call) => call[1] == 'create');
      expect(create, isNot(contains('--privileged')));
      expect(create.where((arg) => arg.contains('/host')), isEmpty);
      expect(
        create,
        containsAll([
          '--network',
      ...
      // Every -v value the vector carries is one this manager was constructed
      // with — the bridge binary and the generated-state dir — and none of them
      // came out of the config.
      final mounts = [
        for (var i = 0; i < create.length - 1; i++)
          if (create[i] == '-v') create[i + 1],
      ];
      expect(mounts, [
        '/tmp/dartclaw-bridge:/opt/dartclaw/dartclaw-bridge:ro',
