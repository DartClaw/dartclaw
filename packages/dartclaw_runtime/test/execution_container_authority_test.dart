...
void main() {
  ...
  group('container authorities stay dedicated to one owner', () {
    ...
    // ADR-058: the two teardown paths that do not go through a lease release
    // publish the same single terminal event. Without these, replacing either
    // call's `quarantined:` argument with a literal `false` leaves the whole
    // package suite green, and an unconfirmed teardown goes back to reading as
    // a clean one on the path a logical-agent container actually takes.
    test('a session-continuity reset whose destroy throws reports the runner quarantined, not disposed', () async {
      final fixture = _Fixture(capacities: const {'claude': 1}, destroyThrows: true);
      addTearDown(fixture.dispose);

      final lease = await fixture.acquire(
        'logical-session',
        const ExecutionPolicy.container('workspace'),
        surface: ExecutionSurface.logicalAgent,
        logicalAgentId: 'researcher',
      ...
      expect(fixture.terminalEvents.map((event) => (event.kind, event.runnerId)), [
        (ExecutionEventKind.quarantined, runnerId),
      ], reason: 'one terminal event, naming the runner, because the container may still be alive');
      expect(fixture.coordinator.snapshot.providers['claude']!.quarantined, 1);
    });

    test('coordinator shutdown reports a cached worker whose destroy throws as quarantined', () async {
      final fixture = _Fixture(capacities: const {'claude': 1}, destroyThrows: true);
      ...
      expect(fixture.terminalEvents.map((event) => (event.kind, event.runnerId)), [
        (ExecutionEventKind.quarantined, runnerId),
      ], reason: 'shutdown withholds no slot, but it must not report an unconfirmed teardown as a clean one');
    });
...
class _Fixture {
  new({required Map<String, int> capacities, bool destroyThrows = false}) {
    ...
    _events = coordinator.events.listen((event) {
      if (event.kind == ExecutionEventKind.disposed || event.kind == ExecutionEventKind.quarantined) {
  ...
  /// The terminal teardown events ADR-058 allows: exactly one per teardown.
  final List<ExecutionEvent> terminalEvents = [];
  late final StreamSubscription<ExecutionEvent> _events;
