...
import 'acp_section_fixture.dart';
...
void main() {
  ...
  group('S01 unchanged configurations preserve the current boundary', () {
    test('containers enabled: every omitted setting resolves to container execution', () {
      ...
      expect(resolver.resolveForTask(), const ExecutionPolicy.container('workspace'));
      expect(resolver.resolveForTask(securityProfile: 'restricted'), const ExecutionPolicy.container('restricted'));
      expect(resolver.deploymentDefault, const ExecutionPolicy.container('workspace'));
    });

    // The posture itself is corrected at the downgrade site (`service_wiring`'s
    // `_correctPostureIfDowngraded`), so the resolver never sees "enabled with
    // no profile" for a *defaulted* subject. An operator who asked for
    // container execution still meets the refusal.
    test('an explicit container selection is still refused when no profile was registered', () {
      ...
      expect(
        ...
        throwsA(isA<ExecutionPolicyException>()),
        reason: 'the fallback covers an inferred posture, never an operator who asked for container execution',
      );
    });
    ...
    test('containers disabled: neutral-profile contexts resolve to host with no profile', () {
      ...
      expect(resolver.resolveForTask(), const ExecutionPolicy.host());
    ...
    test('containers disabled: a container-only default profile fails closed rather than degrading to host', () {
      ...
      expect(
        () => resolver.resolveForTask(securityProfile: 'restricted'),
        throwsA(isA<ExecutionPolicyException>().having((error) => error.message, 'message', contains('restricted'))),
      );
    });
    ...
    test('an explicit host selection legitimately drops the mode-conditional profile default', () {
      ...
      expect(
        resolverFor(containersEnabled: false, taskExecution: ExecutionMode.host).resolveForTask(),
  ...
  group('S02 explicit agent and task-lane choices coexist', () {
    ...
    final resolver = resolverFor(
      ...
      taskExecution: ExecutionMode.host,
    );
    ...
    test('the task-lane override does not infer placement from the security profile', () {
      expect(resolver.resolveForTask(), const ExecutionPolicy.host());
      expect(() => resolver.resolveForTask(securityProfile: 'restricted'), throwsA(isA<ExecutionPolicyException>()));
    });
  ...
  group('provider-declared container profiles resolve through the shared resolver', () {
    ExecutionPolicyResolver acpResolver(AcpContainerProfile profile, {ExecutionMode? primary}) {
      ...
      return ExecutionPolicyResolver(
        ...
        declaredContainerProfileFor: (providerId) => acpDeclaredContainerProfileFor(config, providerId),
      );
    }
    ...
    test('a composer-supplied registration declares a profile for a provider the config never names', () {
      final resolver = ExecutionPolicyResolver(
        ...
        availableContainerProfiles: workspaceAndRestricted,
        declaredContainerProfileFor: (providerId) => providerId == 'acme' ? 'restricted' : null,
      );

      expect(resolver.resolveForPrimary(providerId: 'acme'), const ExecutionPolicy.container('restricted'));
      expect(
        resolver.resolveForPrimary(providerId: 'claude'),
        const ExecutionPolicy.container('workspace'),
        reason: 'a provider no registration owns keeps the neutral profile',
      );
    });

    test('a lookup that owns nothing resolves exactly as no lookup does', () {
      // The empty-registrar default is not a null argument: the composition
      // root always passes its lookup, and with no registration it answers null
      // for every provider. That shape must produce what no lookup produces.
      final config = DartclawConfig.defaults().copyWith(container: const ContainerConfig(enabled: true));
      final ownsNothing = ExecutionPolicyResolver(
        config: config,
        availableContainerProfiles: workspaceAndRestricted,
        declaredContainerProfileFor: (_) => null,
      );
      final noLookup = ExecutionPolicyResolver(config: config, availableContainerProfiles: workspaceAndRestricted);

      for (final providerId in const ['claude', 'codex', 'unregistered']) {
        expect(
          ownsNothing.resolveForPrimary(providerId: providerId),
          noLookup.resolveForPrimary(providerId: providerId),
          reason: providerId,
        );
      }
    });
  ...
  group('S06 startup warnings name each explicit weakening once', () {
    test('every explicitly overridden path is named exactly once', () {
      ...
      expect(warnings.where((warning) => warning.contains('tasks.execution')), hasLength(1));
  ...
  group('S01 startup warns about contexts that will fail closed at first dispatch', () {
    test('an unrunnable task lane emits one warning', () {
      final warnings = resolverFor(
        containersEnabled: false,
        taskExecution: ExecutionMode.container,
      ).failClosedWarnings();

      expect(warnings, hasLength(1));
      expect(warnings.single, contains('task lane'));
    });

    test('a containers-disabled deployment names each unrunnable agent', () {
      final warnings = resolverFor(
        containersEnabled: false,
  ...
  group('S05 startup names each unavailable provider/mode combination once', () {
    ...
    test('an ACP provider under a container policy names each mode once with path and remediation', () {
      ...
      // The primary lane, both agents, and every task declaration reach the same two
      // resolved policies; the operator sees each combination once.
      expect(warnings.where((warning) => warning.contains('container/workspace')), hasLength(1));
      expect(warnings.where((warning) => warning.contains('container/restricted')), hasLength(1));
    ...
    test('a provider whose container execution is mediated warns about nothing', () {
      final mediatedOnly = ProviderExecutionInventory.of(providerIds: const ['claude'], registrarProviderIds: const {});
    ...
    test('host execution makes the ACP execution combinations available again', () {
      expect(compatibilityWarnings(resolverFor(containersEnabled: false), 'goose'), isEmpty);
    });

    test('contexts that already fail policy resolution stay with the fail-closed warnings', () {
      // The restricted agent has no host equivalent here, so it is unrunnable
      // for every provider — a compatibility warning would name the wrong cause.
      final resolver = resolverFor(containersEnabled: false, agents: const [ordinaryAgent, searchAgent]);

      expect(compatibilityWarnings(resolver, 'goose'), isEmpty);
    ...
    test('compatibility is checked after resolution and never substitutes a policy', () {
      ...
      expect(compatibilityWarnings(resolver, 'goose'), isNotEmpty);
      expect(
