...
import 'package:dartclaw_runtime/src/server.dart'
    show ServerChannelDeps, ServerCoreDeps, ServerObservabilityDeps, ServerTurnDeps;
  ...
  DartclawServer buildServer({GuardChain? guardChain, ChannelManager? channelManager, EventBus? eventBus}) {
    return composeServer(
      core: ServerCoreDeps(
        ...
        // Composition refuses a guard chain with no audit sink; these cases are
        // about guard registration, not the audit trail, so any sink will do.
        auditLogger: guardChain == null ? null : GuardAuditLogger(),
      ),
      turn: ServerTurnDeps(
      ...
      channels: ServerChannelDeps(channelManager: channelManager),
