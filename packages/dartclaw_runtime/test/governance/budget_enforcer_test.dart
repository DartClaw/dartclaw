...
void main() {
  ...
  group('BudgetEnforcer.check — enforcement', () {
    ...
    test('at 80% → warn once, then allow (no repeat)', () async {
      ...
      expect(first.outcome, BudgetOutcome.warning);
      expect(first.warningIsNew, isTrue);

      final second = await enforcer.check(now: t0);
      expect(second.outcome, BudgetOutcome.warning);
    ...
    test('at 100% with block action → block', () async {
      ...
      expect(enforcer.action, BudgetAction.block);
    ...
    test('at 100% with warn action → warn (first time), then allow', () async {
      ...
      final first = await enforcer.check(now: t0);
      expect(first.outcome, BudgetOutcome.exceeded);
      expect(first.warningIsNew, isTrue);

      final second = await enforcer.check(now: t0);
      expect(second.outcome, BudgetOutcome.exceeded);
      expect(second.warningIsNew, isFalse);
    });
    ...
    test('warning resets on day rollover', () async {
      ...
      expect(day1.outcome, BudgetOutcome.warning);
    ...
    test('config change (higher dailyTokens) → previously blocked turns now allowed', () async {
      ...
      expect(blocked.outcome, BudgetOutcome.exceeded);
      ...
      expect(allowed.outcome, BudgetOutcome.under);
    });
  });

  // ---------------------------------------------------------------------------
  // BudgetEnforcer.check — error posture (the daily guardrail never fails open)
  // ---------------------------------------------------------------------------

  group('BudgetEnforcer.check — error posture', () {
    test('a failing usage read surfaces to the caller instead of reporting under budget', () async {
      final tracker = _ThrowingUsageTracker(dataDir: tempDir.path, kv: kvService);
      final enforcer = BudgetEnforcer(usageTracker: tracker, config: const BudgetConfig(dailyTokens: 1000));

      // The failure the daily scope must NOT suppress.
      await expectLater(tracker.dailySummaryForDate('usage_daily:2026-03-15'), throwsA(isA<StateError>()));

      await expectLater(enforcer.check(now: DateTime(2026, 3, 15, 12, 0)), throwsA(isA<StateError>()));
    });

    test('a disabled budget reads no usage and resolves no timezone', () async {
      final logs = <LogRecord>[];
      final sub = Logger.root.onRecord.listen(logs.add);
      addTearDown(sub.cancel);
      ...
      final enforcer = BudgetEnforcer(
        usageTracker: tracker,
        config: const BudgetConfig(timezone: 'Not/AReal_Zone'), // dailyTokens: 0
      );

      final result = await enforcer.check(now: DateTime(2026, 3, 15, 12, 0));

      expect(result.outcome, BudgetOutcome.under);
      expect(logs.any((r) => r.message.contains('Unrecognized timezone')), isFalse);
    });
  });
...
/// A [UsageTracker] whose daily read always fails, standing in for a broken
/// or unreadable KV store.
class _ThrowingUsageTracker extends UsageTracker {
  new({required super.dataDir, required KvService super.kv});

  @override
  Future<Map<String, dynamic>?> dailySummaryForDate(String dateKey) async {
    throw StateError('usage store unavailable');
  }
}
