...
void main() {
  ...
  setUp(() async {
    ...
    // A page grown by four ingestions whose merges each called the material
    // unrelated: the supplement-stacked shape a lived-in wiki still holds, and
    // the one a single-window snippet reports wrongly.
    await wiki.writePage(
    ...
    for (var batch = 2; batch <= 4; batch++) {
      await wiki.writePage(
        ...
        merge: const WikiPageMerge(mode: WikiMergeMode.supplement),
      );
    }
  ...
  group('wiki lint over the same corpus', () {
    ...
    // Growth is the merge contract's problem now, not a debt category, so a page
    // carrying three supplements raises nothing on its own.
    test('a supplement-grown page raises no finding of its own', () async {
      final report = await lintWikiPages(wiki, kg: kg);

      expect(report.summary(), isNot(contains('consolidation')));
      expect(report.provenanceInconsistencies, isNot(contains(startsWith('kestrel-runbook.md'))));
    });
