...
import 'package:dartclaw_core/dartclaw_core.dart'
    show CodexHarness, DeltaEvent, SubscriptionCredentialStore, TurnResult, containerCodexExecutable;
...
///
/// Codex reaches the upstream through the auth-clean home its harness worker
/// builds, so this suite asserts the generated configuration as
...
void main() {
  ...
  Future<_MediatedCodex> assembleMediatedCodex({bool subscription = false}) async {
    ...
    // The worker deletes the generated home when the turn ends, so the
  ...
  test('a containerized codex turn writes into the mounted workspace through its auth-clean home', () async {
    ...
    // 1. The turn completed and delivered the provider response.
    expect(result.stopReason, 'completed', reason: 'the mediated codex turn did not complete');
    expect(fixture.assistantText, isNotEmpty, reason: 'the mediated codex turn returned no assistant response');

    // 2. The generated home pointed the CLI at this authority's bridge.
    expect(
    ...
    expect(
      proof.existsSync(),
      isTrue,
      reason: 'the container-side write never reached the host workspace; assistant=${fixture.assistantText}',
    );
  ...
  test('a containerized codex turn on a stored subscription completes credential-free', () async {
    ...
    // 1. The turn really ran through the harness and through the mount.
    expect(result.stopReason, 'completed', reason: 'the mediated codex turn did not complete');
    expect(fixture.assistantText, isNotEmpty, reason: 'the mediated codex turn returned no assistant response');
    final proof = File(p.join(fixture.workspace.path, _proofFileName));
    expect(
    ...
    // 3. The generated home is under this authority's own state mount
    //    and points at its bridge. Read from the mid-turn snapshot because the
    //    worker deletes the home when the turn ends — and asserted as the exact
    ...
    // state mount — not a shared or host-side home the worker reached out to.
  ...
  test('the container surface is the same under subscription auth as under an API key', () async {
    ...
    final apiKeyResult = await apiKey.run('Reply with a short greeting.');
    expect(apiKeyResult.stopReason, 'completed');
    expect(apiKey.assistantText, isNotEmpty);
    final subscription = await assembleMediatedCodex(subscription: true);
    final subscriptionResult = await subscription.run('Reply with a short greeting.');
    expect(subscriptionResult.stopReason, 'completed');
    expect(subscription.assistantText, isNotEmpty);
...
final class _MediatedCodex {
  ...
  /// The `config.toml` the harness worker generated for this turn.
  ...
  MapEntry<String, String> get _generatedConfigEntry => generatedStateSnapshot.entries.firstWhere(
    ...
    orElse: () => throw StateError('the harness worker generated no config.toml: ${generatedStateSnapshot.keys}'),
  );

  Future<TurnResult> run(String prompt, {Map<String, String>? providerEnvironment}) async {
    ...
    assistantText = '';
    final eventSub = harness.events.listen((event) {
      if (event case DeltaEvent(text: final chunk)) {
        assistantText += chunk;
      }
    });
    try {
      return await harness.turn(
        sessionId: 'mediated-codex',
        messages: [
