...
import 'package:dartclaw_runtime/dartclaw_runtime.dart'
    show ProjectGitRunner, ProjectServiceImpl, WorkflowStartPreconditionException;
...
/// Creates a fake [ProjectGitRunner] that returns a predetermined result.
ProjectGitRunner _fakeGitRunner({int exitCode = 0, String stderr = '', String stdout = ''}) =>
    RecordingGitRunner(defaultResult: ProcessResult(0, exitCode, stdout, stderr)).runForRecord;
...
void main() {
  ...
  group('initialize', () {
    ...
    test('seeds config-defined localPath projects without cloning', () async {
      ...
      final svc = makeService(projectConfig: config, gitRunner: git.runForRecord);
      await svc.initialize();

      final project = await svc.get('cfg-project');
      expect(git.recordedArguments, isEmpty);
      expect(project, isNotNull);
      expect(project!.configDefined, isTrue);
  ...
  group('create', () {
    ...
    test('localPath projects are created ready without cloning', () async {
      final git = RecordingGitRunner();
      final svc = makeService(gitRunner: git.runForRecord);
      await svc.initialize();
  ...
  group('resolveWorkflowBaseRef', () {
    ...
    test('resolves local-path projects from symbolic HEAD when defaultBranch is empty', () async {
      final git = RecordingGitRunner(
        responder: (call) => call.arguments.join(' ') == 'symbolic-ref --quiet --short HEAD'
            ? ProcessResult(0, 0, 'feature/live\n', '')
            : null,
      );
      final svc = makeService(gitRunner: git.runForRecord);
      await svc.initialize();
      ...
      expect(git.recordedArguments, hasLength(1));
      expect(git.recordedArguments.single, ['symbolic-ref', '--quiet', '--short', 'HEAD']);
    });
  });
  ...
  group('ensureFresh', () {
    ...
    test('fetches when outside cooldown (no lastFetchAt)', () async {
      ...
      expect(git.recordedArguments, anyElement(predicate<List<String>>((c) => c.isNotEmpty && c[0] == 'fetch')));
    });

    test('skips fetch within cooldown window (lastFetchAt recent)', () async {
      final git = RecordingGitRunner();
      final svc = makeService(projectConfig: const ProjectConfig(fetchCooldownMinutes: 5), gitRunner: git.runForRecord);
      await svc.initialize();
      ...
      expect(git.recordedArguments, isNot(anyElement(predicate<List<String>>((c) => c.isNotEmpty && c[0] == 'fetch'))));
    });
    ...
    test('fetch for external project includes branch name', () async {
      ...
      final fetchCall = git.recordedArguments.firstWhere((c) => c.isNotEmpty && c[0] == 'fetch', orElse: () => []);
    ...
    test('rejects option-shaped fetch refs before invoking git', () async {
      ...
      expect(git.recordedArguments.where((args) => args.isNotEmpty && args.first == 'fetch'), isEmpty);
    });
