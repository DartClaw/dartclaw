...
/// The tools this story registers on the guarded dispatch seam.
const _taskMcpTools = {'task_create', 'task_review', 'task_list', 'review_list', 'task_bind', 'task_unbind'};

/// The orchestration and content tools. A reviewed widening: they carry
/// canonical entries so the containerized lane reaches exactly what the host
/// lane of the same kind reaches, rather than losing capability to the default
/// posture.
const _orchestrationMcpTools = {
  'workflow_run',
  'workflow_list',
  'schedule_upsert',
  'schedule_list',
...
void main() {
  ...
  test('a restricted task in a container is granted the host tools its own policy allows', () async {
    ...
    final grant = security!.grants.singleWhere((entry) => entry.taskId == 'restricted-task');
  ...
  test('S03 container workflow worker mounts artifacts and translates its spawn paths', () async {
    ...
    final lease = await harnessWiring!.executions.acquire(
      const ExecutionRequest(
        ...
        sessionId: 'workflow-container',
        taskId: 'workflow-task',
        artifactsDir: '/host/artifacts',
        spawnEnvironment: {
          'DARTCLAW_STEP_ARTIFACTS_DIR': '/host/artifacts',
          'ANDTHEN_REPORT_PATH': '/host/artifacts/report.md',
          'ANTHROPIC_API_KEY': '/host/unmounted',
    ...
    final grant = security!.grants.singleWhere((entry) => entry.taskId == 'workflow-task');
    expect(grant.artifactsDir, '/host/artifacts');
    final environment = recordedConfigs.last.environment;
    expect(recordedConfigs.last.containerEnvironment, {
      'DARTCLAW_STEP_ARTIFACTS_DIR': containerArtifactsPath,
      'ANDTHEN_REPORT_PATH': '$containerArtifactsPath/report.md',
    });
    expect(environment['ANTHROPIC_API_KEY'], 'anthropic-key');
    expect(environment['CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'], '1');
    expect(environment['DARTCLAW_STEP_ARTIFACTS_DIR'], containerArtifactsPath);
    expect(environment['ANDTHEN_REPORT_PATH'], '$containerArtifactsPath/report.md');
  });

  test('S03 container workflow worker refuses an unmounted absolute spawn path', () async {
    await wireAll();
    ...
    await expectLater(
      harnessWiring!.executions.acquire(
        const ExecutionRequest(
          ...
          providerId: 'claude',
          policy: ExecutionPolicy.container('restricted'),
          sessionId: 'workflow-unmounted',
          taskId: 'workflow-unmounted-task',
          artifactsDir: '/host/artifacts',
          spawnEnvironment: {'OPERATOR_REPORT_DIR': '/host/unmounted'},
        ),
      ),
      throwsA(
        isA<StateError>().having(
          (error) => error.message,
          'message',
          allOf(contains('OPERATOR_REPORT_DIR'), contains('/host/unmounted'), contains('not mounted')),
        ),
      ),
    );
  });
  ...
  group('the container bridge reaches the mapped tools, at parity with the host', () {
    /// Drives the real bridge policy over [grant] and returns the tools it served.
    Future<Set<String>> servedOverBridge(Set<String> grant, {required Map<String, CanonicalTool> canonicals}) async {
      final called = <String>[];
      ...
      for (final name in {
        ...
        registry.registerTool(RecordingMcpTool(name, called));
      }
      final surface = McpBridgeSurface(
        handler: () => registry,
            ...
            body: Stream.value(
              utf8.encode(
                jsonEncode({
                  ...
                  'id': 1,
                  'method': 'tools/call',
                  'params': {'name': name, 'arguments': const <String, Object?>{}},
                }),
              ),
            ),
          ),
    ...
    test('a containerized primary is granted the whole host set, and serves every one', () async {
      config = config.copyWith(
        agent: const AgentConfig(provider: 'claude', execution: ExecutionMode.container),
      );
      ...
      expect(
        grant.allowedMcpTools,
        isNot(anyOf(contains('sessions_spawn'), contains('sessions_send'))),
        reason: 'the deliberate session-spawning exclusion is unchanged by the widening',
      );

      final served = await servedOverBridge(grant.allowedMcpTools, canonicals: harnessWiring!.ownMcpToolCanonicals);
      expect(served, {
        ..._taskMcpTools,
        ..._orchestrationMcpTools,
    ...
    test('a posture resolved to container by detection grants the same set, with no execution mode written', () async {
      // The default-posture path: nothing selects container execution, the
      // startup probe did. Turning isolation on by default must cost the
      // primary agent none of the tools it reaches on the host.
      config = config.copyWith(container: const ContainerConfig().resolved(enabled: true));

      await wireAll();

      expect(config.agent.execution, isNull, reason: 'the posture, not an execution selection, placed this agent');
      final grant = security!.grants.singleWhere((entry) => entry.sessionId == 'primary');
      expect(grant.allowedMcpTools, {
        'web_search',
        'web_fetch',
    ...
    test('no lane that is capability-free today gains a mapped tool', () async {
      await wireAll();

      final containerizedTask = await harnessWiring!.executions.acquire(
        ExecutionRequest(
          surface: ExecutionSurface.task,
          providerId: 'claude',
          policy: const ExecutionPolicy.container('workspace'),
      ...
      addTearDown(() async => containerizedTask?.release());
      final mcpCallOnlyTask = await harnessWiring!.executions.acquire(
        ExecutionRequest(
          surface: ExecutionSurface.task,
          providerId: 'claude',
          policy: const ExecutionPolicy.container('workspace'),
          sessionId: 'mcp-call-session',
          admission: ExecutionAdmission.failFast,
          taskId: 'mcp-call-task',
          allowedTools: const ['mcp_call'],
        ),
      );
      addTearDown(() async => mcpCallOnlyTask?.release());

      final lanes = {
        'no tool policy': security!.grants.singleWhere((entry) => entry.taskId == 'no-policy-task').allowedMcpTools,
        'mcp_call only': security!.grants.singleWhere((entry) => entry.taskId == 'mcp-call-task').allowedMcpTools,
        'workflow step allowing only mcp_call': harnessWiring!.workflowBridgedMcpTools(const ['mcp_call']),
      };
    ...
    test('nothing outside the mapped tools becomes reachable over the bridge', () async {
      config = config.copyWith(
        agent: const AgentConfig(provider: 'claude', execution: ExecutionMode.container),
      );
