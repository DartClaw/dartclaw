...
import 'package:dartclaw_runtime/dartclaw_runtime.dart'
    show DartclawServer, ExecutionAdmission, ExecutionRequest, ExecutionSurface, WorkerCreationException;
...
///
/// Harness construction is asynchronous, so a fixed delay can lose the race
...
void main() {
  ...
  test('primary runner keeps interactive prompt while spawned worker gets lean task prompt', () async {
    ...
    expect(primaryPrompt, contains('## Recent Errors'));
    ...
    expect(taskPrompt, isNot(contains('## Recent Errors')));
  ...
  test('S02 host workflow worker overlays step variables without displacing provider environment', () async {
    await wireStorageAndSecurity();
    final factory = fakeFactory(['claude'], onCreate: (_, factoryConfig) => recordedConfigs.add(factoryConfig));
    await wireHarness(factory);
    ...
    final lease = await harnessWiring!.executions.acquire(
      const ExecutionRequest(
        ...
        sessionId: 'workflow-env',
        artifactsDir: '/tmp/workflow-env',
        spawnEnvironment: {
          'DARTCLAW_STEP_ARTIFACTS_DIR': '/tmp/workflow-env',
          'ANDTHEN_REPORT_PATH': '/tmp/workflow-env/report.md',
          'ANTHROPIC_API_KEY': 'step-owned-key',
    ...
    expect(environment['DARTCLAW_STEP_ARTIFACTS_DIR'], '/tmp/workflow-env');
    expect(environment['ANDTHEN_REPORT_PATH'], '/tmp/workflow-env/report.md');
  });

  test('a workflow step carries its declared policy and its artifacts root to the spawn', () async {
    await wireStorageAndSecurity();
    final factory = fakeFactory(['claude'], onCreate: (_, factoryConfig) => recordedConfigs.add(factoryConfig));
    await wireHarness(factory);
    ...
    final lease = await harnessWiring!.executions.acquire(
      const ExecutionRequest(
        ...
        providerId: 'claude',
        policy: ExecutionPolicy.host(),
        sessionId: 'workflow-declared',
        allowedTools: ['shell', 'file_write'],
        artifactsDir: '/tmp/workflow-declared',
        spawnEnvironment: {'DARTCLAW_STEP_ARTIFACTS_DIR': '/tmp/workflow-declared'},
      ),
    );
    addTearDown(() async => lease?.release());
  ...
  test('a workflow step declaring no tools gets the fixed set, never the operator user settings', () async {
    ...
    final lease = await harnessWiring!.executions.acquire(
      const ExecutionRequest(
        ...
        providerId: 'claude',
        policy: ExecutionPolicy.host(),
        sessionId: 'workflow-undeclared',
        artifactsDir: '/tmp/workflow-undeclared',
        spawnEnvironment: {'DARTCLAW_STEP_ARTIFACTS_DIR': '/tmp/workflow-undeclared'},
      ),
    );
    addTearDown(() async => lease?.release());
  ...
  test('lazy worker ACP decisions use that worker identity and active tool policy', () async {
    ...
    // The decision an ACP registration derives is a pure function of the
    // runner's own chain, so the per-runner isolation is read off the recorded
    // chains rather than off a pre-bound callback.
    final readOnlyChain = recordedConfigs[1].guardChain!;
    final shellChain = recordedConfigs[2].guardChain!;
    const shellRequest = AcpPermissionRequest(operation: 'shell', params: {'command': 'pwd'}, agentId: 'shell-worker');

    expect((await acpPermissionDecision(readOnlyChain, shellRequest)).granted, isFalse);
    expect((await acpPermissionDecision(shellChain, shellRequest)).granted, isTrue);
    final identityDenied = await acpPermissionDecision(
      shellChain,
  ...
  test('guards hot-reload keeps the primary tool filter and picks up rebuilt base guards', () async {
    ...
    final commandGuardBefore = primaryChain.guards.whereType<CommandGuard>().single;
    ...
    expect(primaryChain.guards.whereType<CommandGuard>().single, isNot(same(commandGuardBefore)));
  ...
  test('configured Goose and Vibe ACP agents register without unknown-provider fallback', () async {
    ...
    expect(
      harnessWiring!.providerStatusEntries['goose']!.options['registration_validation_result'],
      containsPair('securityClassification', 'host_only'),
    );
    expect(harnessWiring!.providerStatusEntries['goose']!.options['registration_validation_owned'], isTrue);
    expect(
      harnessWiring!.providerStatusEntries['vibe']!.options['registration_validation_result'],
      containsPair('securityClassification', 'host_only'),
    );
  });
  ...
  test('guarded ACP agent without runtime probe evidence fails before registration', () async {
    ...
    expect(factory.supports('vibe'), isFalse);
  });
  ...
  test('a host-only ACP registration composes the inventory and is refused a container policy', () async {
    ...
    expect(inventory.supports, contains('goose'));
