...
import 'package:dartclaw_runtime/dartclaw_runtime.dart' show DartclawRuntime, HeadlessRuntimeStaging;
...
Never unexpectedExit(int code) {
  throw StateError('Unexpected exit($code) during headless runtime test');
}
...
ProcessResult runGitSync(String workingDirectory, List<String> args) {
...
final class HeadlessRuntimeFixture {
  new(this.tempDir);
  ...
  Future<HeadlessRuntimeStaging> stage(
    ...
    final resolvedEnvironment = environment ?? {'HOME': p.join(tempDir.path, 'fake-home')};
  ...
  /// Stages and completes for execution, the shape a caller that does not gate
  /// auth itself ends up with.
  Future<DartclawRuntime> runtime(
    ...
    final wired = await staging.completeForExecution(providers ?? {config.agent.provider});
    if (autoDispose) {
      addTearDown(wired.shutdown);
    }
    return wired;
  }
  ...
  Directory seedGitRepo(String name, {String readme = 'hello\n'}) {
    ...
    runGitSync(repoDir.path, ['init', '-b', 'main']);
    runGitSync(repoDir.path, ['config', 'user.name', 'Test User']);
    runGitSync(repoDir.path, ['config', 'user.email', 'test@example.com']);
    File(p.join(repoDir.path, 'README.md')).writeAsStringSync(readme);
    runGitSync(repoDir.path, ['add', 'README.md']);
    runGitSync(repoDir.path, ['commit', '-m', 'initial']);
    return repoDir;
  }
}
