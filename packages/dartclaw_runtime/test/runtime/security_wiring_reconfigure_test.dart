...
void main() {
  ...
  group('SecurityWiring.reconfigure()', () {
    ...
    test('invalid config does not partially mutate the live CommandGuard', () async {
      final wiring = buildWiring(_baseConfig);
      await wiring.wire(agentDefs: []);

      final originalCommandGuard = wiring.guardChain!.guards.whereType<CommandGuard>().first;
      final originalPatterns = originalCommandGuard.config.destructivePatterns.length;

      const badSecurity = SecurityConfig(
        guards: GuardConfig(enabled: true, failOpen: false),
        guardsYaml: {
          'command': {
            'extra_blocked_patterns': [r'secret\s+backdoor'],
          },
          'network': {
      ...
      final currentCommandGuard = wiring.guardChain!.guards.whereType<CommandGuard>().first;
      expect(currentCommandGuard, same(originalCommandGuard));
      expect(currentCommandGuard.config.destructivePatterns.length, equals(originalPatterns));
      expect(
        currentCommandGuard.config.destructivePatterns.any((entry) => entry.pattern == r'secret\s+backdoor'),
        isFalse,
      );
    });
