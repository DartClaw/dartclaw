...
void main() {
  ...
  group('security reload seam — valid reload', () {
    ...
    test('ConfigNotifier.reload with changed security config activates new CommandGuard extra patterns', () async {
      final configNotifier = ConfigNotifier(_baseConfig);
      final wiring = _buildRegisteredWiring(dataDir: dataDir, eventBus: eventBus, configNotifier: configNotifier);
      await wiring.wire(agentDefs: []);
      ...
      final originalChain = wiring.guardChain!;
      final beforeVerdict = await originalChain.evaluateBeforeToolCall('shell', {
        'command': 'deploy --secret-backdoor',
      }, sessionId: 'test-session');
      expect(beforeVerdict, isA<GuardPass>(), reason: 'Custom pattern should not block before reload');

      // Add an extra command pattern via security reload.
      const updatedSecurity = SecurityConfig(
        guards: GuardConfig(enabled: true, failOpen: false),
        guardsYaml: {
          'command': {
            'extra_blocked_patterns': [r'--secret-backdoor'],
          },
        },
      );
      const updatedConfig = DartclawConfig(security: updatedSecurity);
      ...
      final afterVerdict = await wiring.guardChain!.evaluateBeforeToolCall('shell', {
        'command': 'deploy --secret-backdoor',
      }, sessionId: 'test-session');
      expect(afterVerdict, isA<GuardBlock>(), reason: 'New extra pattern should block after reload');
    });
