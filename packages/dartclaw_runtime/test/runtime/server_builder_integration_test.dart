...
import 'package:dartclaw_runtime/src/runtime/service_wiring.dart' show PostMcpStartupHook;
...
Future<void> _disposeRuntime(DartclawRuntime result, LogService logService, {bool disposeExtras = true}) async {
  await result.server!.shutdown();
  if (disposeExtras) {
    await result.shutdownExtras();
  }
  result.scheduleService?.stop();
  result.requireResetService.dispose();
...
/// The tools this story registers, with the classification the seam reads.
const _taskMcpToolAccess = {
  'task_create': McpToolAccess.write,
  'task_review': McpToolAccess.write,
  'task_list': McpToolAccess.read,
  'review_list': McpToolAccess.read,
  'task_bind': McpToolAccess.write,
  'task_unbind': McpToolAccess.write,
};
...
void main() {
  ...
  setUpAll(() async {
    ...
    _templatesDirPath = await resolveTemplatesDir();
    initTemplates(_templatesDirPath);
  });

  setUp(() {
    tempDir = Directory.systemTemp.createTempSync('dartclaw_runtime_builder_integration_');
  ...
  Future<DartclawRuntime> Function() buildWiring(
  ...
  test('DartclawRuntime registers the task, review and binding tools with their access classifications', () async {
    final config = _baseConfig(tempDir);

    final result = await buildWiring(config)();
    addTearDown(() => _disposeRuntime(result, logService));

    final server = result.server!;
    expect(await _mcpToolNames(server), containsAll(_taskMcpToolAccess.keys));
    for (final MapEntry(key: tool, value: access) in _taskMcpToolAccess.entries) {
      expect(server.mcpHandler.toolAccess[tool], access, reason: '$tool must carry its read/write classification');
    }
    ...
    final raw = await server.mcpHandler.handleRequest(
      _mcpRequest(
        ...
        params: {
          'name': 'task_create',
          'arguments': {'title': 'Wired', 'description': 'Created through the composition root'},
        },
      ),
    );
    final callResult = (jsonDecode(raw!) as Map<String, dynamic>)['result'] as Map<String, dynamic>;
    expect(callResult['isError'], isNull);
  ...
  test('DartclawRuntime registers surfaced outbound MCP tools on the live MCP handler', () async {
    ...
    final result = await wiring();
    addTearDown(() => _disposeRuntime(result, logService, disposeExtras: false));

    final toolNames = await _mcpToolNames(result.server!);
    expect(toolNames, contains('mcp__acme__lookup'));
    expect(toolNames, isNot(contains('mcp__acme__delete_all')));

    // A third-party adapter is a write whatever the upstream server claims —
    // the host cannot verify it.
    expect(result.server!.mcpHandler.toolAccess['mcp__acme__lookup'], McpToolAccess.write);
  ...
  test('DartclawRuntime preserves outbound MCP startup errors when cleanup close fails', () async {
    final transport = _FakeOutboundTransport(
      tools: const [
        {'name': 'delete_all'},
      ],
  ...
  test('DartclawRuntime keeps direct outbound policy when startup listing fails', () async {
  ...
  test('DartclawRuntime does not connect direct-only outbound MCP servers during startup', () async {
    var factoryCalls = 0;
    final transport = _FakeOutboundTransport(
      tools: const [
        {'name': 'delete_all'},
  ...
  test('DartclawRuntime fails when surfaced outbound MCP tool is not exposed', () async {
    final transport = _FakeOutboundTransport(
      tools: const [
        {'name': 'delete_all'},
      ],
  ...
  test('DartclawRuntime closes the outbound MCP pool when later startup fails', () async {
    final transport = _FakeOutboundTransport(
      tools: const [
        {'name': 'lookup'},
      ],
  ...
  test('DartclawRuntime returns handled errors for live denied outbound MCP calls', () async {
    final transport = _FakeOutboundTransport(
      tools: const [
        {'name': 'lookup'},
        {'name': 'allowed'},
  ...
  test('DartclawRuntime fails when an outbound MCP adapter name collides', () async {
    final transport = _FakeOutboundTransport(
      tools: const [
        {'name': 'lookup'},
      ],
  ...
  test('DartclawRuntime does not connect disabled or uncredentialed outbound MCP servers', () async {
    const yaml = '''
mcp_servers:
  disabled:
...
''';
  ...
  test('DartclawRuntime closes shared lifecycle services during shutdown extras', () async {
    final transport = _FakeOutboundTransport(
      tools: const [
        {'name': 'lookup'},
      ],
    ...
    expect(transport.closeCount, 1);
    expect(
      () => result.requireSelfImprovement.appendLearning(text: 'rejected after shared corpus shutdown'),
      throwsStateError,
    );
  });

  test('DartclawRuntime wires AlertRouter into the production EventBus', () async {
    final config = _baseConfig(tempDir).copyWith(
  ...
  test('DartclawRuntime loads built-in skills from source tree without materializing project copies', () async {
  ...
  test('DartclawRuntime rejects missing local refs for local-path workflow starts', () async {
  ...
  test('DartclawRuntime drops legacy session_cost entries at boot and logs the cleanup count', () async {
    ...
    expect(
      records.any(
        (record) =>
            record.loggerName == 'DartclawRuntime' &&
  ...
  test('DartclawRuntime runs the AndThen skills bootstrap before wire() returns', () async {
