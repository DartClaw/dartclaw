...
void main() {
  ...
  Future<Map<String, String>> probeEnvironment({
    ...
    registrarOverlay: acpAgent == null
        ? null
  ...
  test('an ACP registration under a codex-named id is not gated on the Codex store', () async {
    // The store gate refuses on behalf of a first-party `providers.<id>.auth`
    // selection. An ACP registration has none — it presents only the key its own
    ...
    expect(environment.containsKey('CODEX_HOME'), isFalse);
    expect(environment.containsKey('OPENAI_API_KEY'), isFalse);
    expect(environment['VENDOR_API_KEY'], 'vendor-key');
  });
