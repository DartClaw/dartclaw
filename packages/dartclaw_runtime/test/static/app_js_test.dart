...
void main() {
  late String baseDir;
  late String componentsCssPath;
  late String designSystemCssPath;
  late String iconsCssPath;
  late String tokensCssPath;
  late List<File> controllerSources;

  setUpAll(() async {
    baseDir = await resolveStaticDir();
    componentsCssPath = p.join(baseDir, 'app.css');
    designSystemCssPath = await resolveDesignSystemCss('components.css');
    iconsCssPath = await resolveDesignSystemCss('icons.css');
    tokensCssPath = await resolveDesignSystemCss('tokens.css');
    controllerSources = Directory(p.join(baseDir, 'controllers'))
        .listSync()
        .whereType<File>()
  ...
  group('legacy static asset removal', () {
    ...
    test('app.js is no longer loaded as the shell owner', () async {
      expect(File('$baseDir/app.js').existsSync(), isFalse);
      final layoutSource = File(p.join(await resolveTemplatesDir(), 'layout.dart')).readAsStringSync();
    ...
    test('scheduling controller owns only cron previews and row confirmation', () {
      ...
      expect(source, contains('updateJobCronPreview(event)'));
      expect(source, contains('updateTaskCronPreview(event)'));
      expect(source, contains('confirmDelete(event)'));
      expect(source, contains('cancelDelete(event)'));
      expect(source, isNot(contains('submitJobForm')));
      expect(source, isNot(contains('toggleScheduledTask')));
    });

    test('memory controller owns only file browsing behavior', () {
      ...
      expect(source, isNot(contains('/api/memory/prune')));
      expect(source, isNot(contains('htmx.ajax')));
      expect(source, isNot(contains('confirmPrune')));
    });
    ...
    test('workflow lifecycle events reconcile the server-rendered detail page', () {
      ...
      expect(source, contains("htmx.ajax('GET', '/workflows/' + encodeURIComponent(runId) + qs"));
    ...
    test('shared dialog controller owns only the dialog lifecycle', () {
      final source = File('$baseDir/controllers/dc_dialog_controller.js').readAsStringSync();
      expect(source, contains('showModal()'));
      expect(source, contains('data-dialog-close'));
      // The server owns every project mutation and re-renders the surface; a
      // controller that still assembles requests would be a second authority.
      expect(source, isNot(contains('fetch(')));
      expect(source, isNot(contains('JSON.stringify')));
      expect(source, isNot(contains('/api/projects')));
      expect(source, isNot(contains('dataset.project')));
      expect(source, isNot(contains('initCustomSelects')));
      expect(source, isNot(contains('htmx.ajax')));
      expect(source, isNot(contains('showToast')));
    });
    ...
    test('task status filtering is server rendered', () {
      final source = File('$baseDir/controllers/dc_tasks_controller.js').readAsStringSync();
      expect(source, isNot(contains('applyTaskFilters')));
      expect(source, isNot(contains('data-task-filter')));
    });
    ...
    test('tasks controller handles turn wait state and early cancel', () {
      ...
      expect(source, contains("body: '{\"reason\":\"operator_cancel\"}'"));
    ...
    test('feedback controllers use canonical loaders and progress primitives', () {
      ...
      expect(workflowSource, isNot(contains('loadingEl')));
      expect(workflowSource, isNot(contains("querySelector('.meter-fill')")));
      expect(workflowSource, isNot(contains('percentage.textContent')));
      expect(workflowSource, isNot(contains('progressPercent')));
    });

    test('workflow dialog feedback is server rendered', () {
      final workflowSource = File('$baseDir/controllers/dc_workflows_controller.js').readAsStringSync();
      expect(workflowSource, isNot(contains('fetchWorkflowDefinitions')));
      expect(workflowSource, isNot(contains('renderWorkflowCard')));
    ...
    test('server-rendered forms leave workflow action hierarchy canonical', () {
      ...
      expect(settingsSource, isNot(contains('document.createElement')));
      expect(settingsSource, isNot(contains('className =')));
      expect(workflowSource, isNot(contains('class="form-label')));
      expect(workflowSource, isNot(contains("const controlId = 'workflow-var-' + index")));
      expect(workflowSource, isNot(contains('name="wf-var-')));
    ...
    test('settings use full-width panes and a single-row responsive tab strip', () {
      ...
      // The controller no longer knows what a config field is: values, tiers
      // and the section note are all server-rendered from the registry.
      expect(settingsSource, isNot(contains('mutable')));
      expect(settingsSource, isNot(contains('Changes reload without a server restart.')));
    ...
    test('composer input keeps only the reference palette path', () {
      ...
      expect(chatSource, contains('maybeOpenReferencePalette()'));
      expect(chatSource, contains('renderReferencePalette()'));
      expect(chatSource, contains('selectReference(index)'));
      expect(chatSource, contains("prefix.startsWith('@')"));
      expect(chatSource, isNot(contains("prefix.startsWith('/')")));
      expect(chatSource, isNot(contains("'/commands'")));
    });
  ...
  group('native dialog eradication', () {
    ...
    test('the scheduled-task delete confirms in-row before it deletes', () {
      ...
      expect(source, contains('confirmDelete(event)'));
      expect(source, contains("confirm.setAttribute('hx-post', url)"));
      expect(source, contains('htmx.process(confirm)'));
      expect(source, contains('cancelDelete(event)'));
      expect(source, isNot(contains('fetch(')));
    });

    test('restart failures surface as toasts and guard testing is the only JSON interaction', () {
      final shellSource = File('$baseDir/controllers/dc_shell_controller.js').readAsStringSync();
      ...
      final settingsSource = File('$baseDir/controllers/dc_settings_controller.js').readAsStringSync();
      expect('/api/'.allMatches(settingsSource).length, 1);
      expect(settingsSource, contains("fetch('/api/config/guards/test'"));
      expect(settingsSource, isNot(contains('.innerHTML')));
      expect(settingsSource, isNot(contains('guardEditorState')));
    });

    test('server-rendered allowlist removals use the shared confirmation path', () {
      final template = File(p.normalize(p.join(baseDir, '..', 'templates', 'channel_detail.html'))).readAsStringSync();
      expect('hx-confirm="'.allMatches(template).length, 2);
      expect(template, contains('hx-confirm="Remove this allowlist entry?"'));
      expect(template, contains('hx-confirm="Remove this allowed group?"'));
    });
  });
  ...
  group('shell failure, feedback and loading contracts', () {
    ...
    test('in-flight work has a visible treatment in the shared layer', () async {
      final layout = File(p.join(await resolveTemplatesDir(), 'layout.html')).readAsStringSync();
      final appCss = File(componentsCssPath).readAsStringSync();
  ...
  group('settings dirty state is wired to the paths that need it', () {
    ...
    test('the dirty computation and the tab guard are one traversal', () {
      ...
      expect(
        ...
        contains('applyFormDirtyState(form, formChanges(form).length > 0);'),
      );
      ...
      // The baseline is the control's own server-rendered default, so there is
      // no client-side config to cache, invalidate or fetch.
      expect(_jsFunction(source, 'function controlChanged'), contains('input.defaultValue'));
      expect(_jsFunction(source, 'function controlChanged'), contains('input.defaultChecked'));
      expect(source, isNot(contains('settingsInitialConfig')));
      expect(source, isNot(contains("fetch('/api/config')")));
    });

    test('the flag is recomputed on every edit and settled once on connect', () {
      // Without these call sites the flag is written once and never again:
      // typing would not arm Save, and a discarded edit would keep reporting.
      final listeners = _jsFunction(source, 'function attachSettingsListeners');
      expect(RegExp(r'updateFormDirtyState\(form\);').allMatches(listeners), hasLength(4));
      expect(listeners, contains("content.addEventListener('input'"));
      expect(listeners, contains("content.addEventListener('change'"));
      expect(listeners, contains("content.addEventListener('reset'"));
      // The section posts through HTMX, so Save is held inert for the flight —
      ...
      expect(listeners, contains("content.addEventListener('change', clearFieldError);"));
      expect(_jsFunction(source, 'function clearFieldError'), contains("removeAttribute('aria-invalid')"));

      final init = _jsFunction(source, 'function initSettingsForm');
      expect(init, contains(".querySelectorAll('.settings-form').forEach(updateFormDirtyState);"));
    });
    ...
    test('the operator tab-click path consults dirty state before hiding a card', () {
      ...
      // path feeds it the live grid rather than bypassing it, and that a
      // form-less tab is not special-cased at the call site.
      final request = _jsFunction(source, 'function requestSettingsTab');
      expect(request, contains("const leaving = dirtyFormsLeaving(document.querySelector('.settings-grid'), tabId);"));
    ...
    test('confirm restores the fields from the rendered defaults and re-clears the flag', () {
      final request = _jsFunction(source, 'function requestSettingsTab');
      final gate = request.indexOf('if (!confirmed) return;');
      final restore = request.indexOf('leaving.forEach(discardFormEdits);');
      ...
      // Discard is the native reset, so no client-side value cache exists to
      // go stale, and it re-baselines so one discarded edit cannot warn twice.
      final discard = _jsFunction(source, 'function discardFormEdits');
      expect(discard, contains('form.reset();'));
      expect(discard, contains('updateFormDirtyState(form);'));
    });
...
const select = {
  ...
  dispatchEvent(event) {
    ...
    if (event.bubbles) bubbledChanges += 1;
  },
};
