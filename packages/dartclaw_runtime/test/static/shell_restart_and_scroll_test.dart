...
/// live in `shared.js` so the shell controller and an out-of-band slot
/// replacement cannot disagree. The state machine is proven end to end —
/// dormant, pending, updated, dismissed, replaced-while-dismissed, replaced
/// while live, cleared, pending again. A settings save now delivers the banner
/// as a server-rendered `hx-swap-oob` fragment rather than a client refresh;
/// that the response carries it is proved in `web/settings_form_test.dart`, and
/// what the client does with the replacement is proved here.
///
...
void main() {
  Future<void> runHarness(String harness, List<String> scripts) async {
    final assets = await Future.wait(scripts.map(controllerAsset));
    await expectNodeHarness(harness, assets.map((asset) => asset.absolute.uri.toString()).toList());
  }
...
// A settings save swaps this node in out of band, already carrying its fields.
// Nothing was dismissed, so the replacement must surface as the server sent it.
fields.textContent = 'agent.model';
banner.setAttribute('hidden', '');
banner.setAttribute('inert', '');
syncRestartBannerAfterSwap();
assert(visible(), 'an out-of-band replacement naming a pending field must be visible');
''';
