...
void main() {
  group('PrCreator', () {
    ...
    test('includes GitHub validation error reasons in the failure details', () async {
      final creator = PrCreator(
        credentials: const CredentialsConfig(
          entries: {'github-main': CredentialEntry.githubToken(token: 'ghp_test', repository: 'u/my-app')},
        ),
        apiRunner: (method, uri, {required headers, body}) async => (
          ...
          body:
              ...
              '"message":"No commits between main and dartclaw/task-1"}]}',
        ),
      );
      ...
      final failed = result as PrCreationFailed;
      expect(failed.details, 'Validation Failed: No commits between main and dartclaw/task-1');
    });
    ...
    test('sends the JSON body over a real HTTP connection', () async {
      // Regression: the shipped path (no apiRunner) used to set `request.encoding` after
      // `content-type`, which IOSink refuses, so every real publish failed before reaching GitHub.
      final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      addTearDown(() => server.close(force: true));
      final seen = <({String method, String path, String? contentType, String body})>[];
      server.listen((request) async {
        ...
        request.response
          ...
          ..write('{"html_url":"https://github.com/u/my-app/pull/7","number":7}');
      ...
      expect(result, isA<PrCreated>(), reason: '$result');
      expect((result as PrCreated).url, 'https://github.com/u/my-app/pull/7');
      expect(seen, hasLength(1));
      expect(seen.single.method, 'POST');
      expect(seen.single.path, '/repos/u/my-app/pulls');
      expect(seen.single.contentType, startsWith('application/json'));
      final payload = jsonDecode(seen.single.body) as Map<String, dynamic>;
      expect(payload['head'], 'dartclaw/task-1');
    });
