...
GitRunner _fakeRunner({int exitCode = 0, String stdout = '', String stderr = ''}) =>
    RecordingGitRunner(defaultResult: ProcessResult(0, exitCode, stdout, stderr)).run;

RecordingGitRunner _recordingRunner({int exitCode = 0, String stderr = ''}) =>
    RecordingGitRunner(defaultResult: ProcessResult(0, exitCode, '', stderr));
...
void main() {
  group('RemotePushService', () {
    ...
    test('calls git push with correct args', () async {
      final git = _recordingRunner();
      final service = RemotePushService(processRunner: git.run);
      ...
      expect(git.calls.single.arguments, ['push', 'origin', 'dartclaw/task-1']);
      expect(git.calls.single.workingDirectory, '/data/my-app');
      // Push needs insteadOf/proxy/sshCommand from user or system git config.
      expect(git.calls.single.noSystemConfig, isFalse);
    });
    ...
    test('local-path projects push without remote.origin.url override', () async {
      ...
      expect(git.calls, hasLength(1));
      expect(git.calls.single.arguments, ['push', 'origin', 'dartclaw/task-1']);
      expect(git.calls.single.arguments, isNot(contains('-c')));
    });
    ...
    test('GitHub token credentials normalize SSH remotes to HTTPS and disable prompts', () async {
      ...
      expect(git.calls, hasLength(1));
      expect(
        git.calls.single.arguments,
        containsAll(['-c', 'remote.origin.url=https://github.com/acme/repo.git', 'push', 'origin', 'dartclaw/task-1']),
      );
      expect(git.calls.single.environment['GIT_TERMINAL_PROMPT'], '0');
      expect(git.calls.single.environment['GCM_INTERACTIVE'], 'never');
      expect(git.calls.single.environment['GIT_CONFIG_COUNT'], '1');
      expect(git.calls.single.environment['GIT_CONFIG_KEY_0'], 'credential.helper');
      expect(git.calls.single.environment['GIT_CONFIG_VALUE_0'], '');
      expect(git.calls.single.environment['GIT_ASKPASS'], isNotNull);
    });
