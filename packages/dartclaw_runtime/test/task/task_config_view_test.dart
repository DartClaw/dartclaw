...
import 'package:dartclaw_runtime/src/task/task_config_view.dart';
...
void main() {
  group('TaskConfigView', () {
    ...
    test('boolean and workflow readers handle default and valid values', () {
      ...
      expect(TaskConfigView(_task()).needsWorktree, isFalse);
      expect(TaskConfigView(_task(configJson: {'needsWorktree': true})).needsWorktree, isTrue);
      expect(TaskConfigView(_task(configJson: {'needsWorktree': false})).needsWorktree, isFalse);
      expect(TaskConfigView(_task()).hasNeedsWorktreeDeclaration, isFalse);
      expect(TaskConfigView(_task(configJson: {'needsWorktree': false})).hasNeedsWorktreeDeclaration, isTrue);
      expect(
        TaskConfigView(_task(workflowStepExecution: _workflowStepExecution(), configJson: {'needsWorktree': true}))
            .needsWorktree,
        isTrue,
      );
    });
    ...
    test('reviewMode and postCompletionStatus validate known modes', () {
      ...
      expect(
        TaskConfigView(_task(configJson: {'reviewMode': 'worktree-only', 'needsWorktree': false})).postCompletionStatus,
        TaskStatus.accepted,
      );
      expect(
        TaskConfigView(_task(configJson: {'reviewMode': 'worktree-only', 'needsWorktree': true})).postCompletionStatus,
        TaskStatus.review,
      );
      expect(TaskConfigView(_task(configJson: {'reviewMode': 'retired-mode'})).postCompletionStatus, TaskStatus.review);
    });

    test('artifactExtensions is an optional closed list', () {
      expect(TaskConfigView(_task()).artifactExtensions, isNull);
      expect(TaskConfigView(_task(configJson: {'artifactExtensions': '.md'})).artifactExtensions, isEmpty);
      expect(
        TaskConfigView(
          _task(
            configJson: {
              'artifactExtensions': [' .MD ', '.csv'],
            },
          ),
        ).artifactExtensions,
        {'.md', '.csv'},
