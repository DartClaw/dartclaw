...
void main() {
    ...
    final turns = TurnManager.fromCoordinator(
      ...
      coordinator: ExecutionCoordinator(
        ...
        createWorker: (request) async => TurnRunner(
          turnLimits: const TurnLimitsConfig.defaults(),
          harness: workflowWorker ?? FakeTaskWorker(),
          messages: messages,
          behavior: behavior,
          sessions: sessions,
          guardChain: ctx.harness.workflowGuardChain,
          taskToolFilterGuard: ctx.harness.workflowToolFilterGuard,
  ...
  test('invokes auto-accept callback with the task id after completion when provided', () async {
    ...
    await tasks.create(
      id: 'task-worktree-only-accepted',
      title: 'Non-worktree accepted task',
      description: 'Tasks without a worktree in worktree-only review mode should skip auto-accept.',
      autoStart: true,
    ...
    expect((await tasks.get('task-worktree-only-accepted'))!.status, TaskStatus.accepted);
  });
  ...
  test('marks queued tasks as failed when the agent turn crashes', () async {
    ...
    await tasks.create(id: 'task-3', title: 'Failing task', description: 'This should fail.', autoStart: true);
  ...
  test('completed workflow outcome preserves a concurrent task cancellation', () async {
    final workflowWorker = FakeTaskWorker()..responseText = 'Done.';
    var workflowCancellationCallbacks = 0;
    workflowWorker.beforeComplete = (_) async {
      workflowCancellationCallbacks++;
      await tasks.transition('task-workflow-cancelled', TaskStatus.cancelled);
    };
    worker.beforeComplete = (_) async => fail('workflow cancellation callback reached the primary worker');
    final cancellingExecutor = buildWorkflowCapacityExecutor(workflowWorker: workflowWorker);
    addTearDown(cancellingExecutor.stop);
    ...
    expect(workflowCancellationCallbacks, 1);
    expect(task?.status, TaskStatus.cancelled);
    expect(records.any((record) => record.message.contains('Cannot update terminal task')), isFalse);
    expect(
      records.any((record) => record.message.contains('Task execution failed for task-workflow-cancelled')),
      isFalse,
    );
  });

  // Assert the order tasks are *started* in, not that the second one stayed
  // queued: `_pollOnceInner` walks the whole queue and leases a worker per task,
  // so under load the first task can settle and free its worker before the loop
  // reaches the second, which then runs in the same poll. That is capacity
  // behaviour, not an ordering failure.
  test('processes queued tasks in FIFO order', () async {
    worker.responseText = 'ok';
    final started = <String>[];
    worker.onTurn = started.add;
    await tasks.create(
      id: 'task-old',
      title: 'Older',
      description: 'first',
    ...
    final older = (await tasks.get('task-old'))!;
    expect(older.status, TaskStatus.review);
    expect(started, isNotEmpty);
    expect(started.first, older.sessionId, reason: 'the older task must be started first');
  });
  ...
  test('breaks equal queue timestamps by task ID', () async {
    ...
    await tasks.create(id: 'task-b', title: 'Second by ID', description: 'second', autoStart: true, now: queuedAt);
    await tasks.create(id: 'task-a', title: 'First by ID', description: 'first', autoStart: true, now: queuedAt);

    await executor.pollOnce();
    await executor.drain();

    final first = (await tasks.get('task-a'))!;
    expect(first.status, TaskStatus.review);
    expect(started, isNotEmpty);
    expect(started.first, first.sessionId, reason: 'the lower task ID must break the timestamp tie');
  });
  ...
  test('serializes two tasks that continue the same durable session', () async {
    ...
    final secondRunner = TurnRunner(
      turnLimits: const TurnLimitsConfig.defaults(),
      harness: secondWorker,
      messages: messages,
      behavior: behavior,
      sessions: sessions,
    );
  ...
  test('provider-less workflow acquires a default-provider worker while running one-shot turns', () async {
    ...
    final poolTurns = TurnManager.fromCoordinator(
      turnLimits: const TurnLimitsConfig.defaults(),
  ...
  test('dispatches multiple queued tasks concurrently when multiple runners are idle', () async {
    ...
    await tasks.create(id: 'task-pool-a', title: 'Pool A', description: 'Should run in parallel.', autoStart: true);
    await tasks.create(
      id: 'task-pool-b',
      title: 'Pool B',
  ...
  test('concurrent shared workflow dispatch uses one worktree create call', () async {
    ...
    await tasks.create(
      ...
      configJson: const {'_baseRef': 'dartclaw/workflow/runconcurrent/integration', 'needsWorktree': true},
    );
    await seedWorkflowExecution(
  ...
  group('prompt scope selection', () {
    ...
    setUp(() {
      ...
      scopeExecutor = ctx.harness.buildWorkflowExecutor(
        ...
        projectService: fakeProjectServiceFor(readyProject()),
        workflowRunRepository: workflowRuns,
        workflowStepExecutionRepository: workflowStepExecutions,
      );
    });
    ...
    test('regular task gets task scope', () async {
      worker.responseText = 'Done.';
      await tasks.create(id: 'task-scope-regular', title: 'Scope test', description: 'Regular task.', autoStart: true);
    ...
    test('workflow workspace override keeps task scope and behavior path', () async {
      ...
      expect(capturing.lastDirectory, workflowWorkspaceDir);
    });

    test('workflow workspace override is preserved for workflow tasks', () async {
    ...
    test('project-backed workflow task runs in the project directory', () async {
      worker.responseText = 'Done.';
      ...
      if (existingExecution == null) {
        await agentExecutions.create(
          const AgentExecution(id: 'ae-task-scope-project', provider: 'claude', workspaceDir: workflowWorkspaceDir),
        );
      } else {
