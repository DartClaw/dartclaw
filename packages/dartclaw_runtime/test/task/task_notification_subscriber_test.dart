...
void main() {
  ...
  test('task-shaped channel text follows normal routing and creates no task notification', () async {
    ...
    expect(queue.enqueued.single.message.text, 'task: fix the login bug');
  ...
  test('sends review notification for channel-originated tasks', () async {
    ...
    expect(
      ...
      "Task '${task.title}' needs review. Review it in the web UI, or tell me here to accept or reject it.",
    );
  });

  // Removing the host-side accept/reject grammar removed the only call to
  // action a channel without an interactive card had: the notification named
  // work and left the reader nowhere to go. Neither route named here is a
  // grammar — the host parses no reply, and "tell me" reaches the agent.
  test('a review-ready notification links the task when the deployment declares a base URL', () async {
    await subscriber.dispose();
    subscriber = TaskNotificationSubscriber(
      tasks: tasks,
      channelManager: channelManager,
      baseUrl: 'https://dartclaw.example/',
    );
    subscriber.subscribe(eventBus);
    ...
    expect(
      channel.sentMessages.single.$2.text,
      "Task 'Linked' needs review. Review it at https://dartclaw.example/tasks/task-linked, "
      'or tell me here to accept or reject it.',
    );
  });
  ...
  test('keeps merge wording for accepted worktree-backed tasks', () async {
    final task = await tasks.create(
      ...
      description: 'Task',
      autoStart: true,
      configJson: {'needsWorktree': true, 'origin': channelOriginJson()},
    );
