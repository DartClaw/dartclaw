...
import 'package:dartclaw_runtime/src/templates/audit_table.dart';
...
void main() {
  ...
  group('guard audit table', () {
    test('adopts the canonical table, chip filters and shared empty state', () {
      ...
      // 7 guard chips (All + the six emitting guard families) + 3 verdict chips.
      expect(RegExp('aria-pressed=').allMatches(populated), hasLength(10));

      // A chip that matches no emitted guard name is a filter that can only
      // ever render an empty table. The chain-guard names are read off real
      // guard instances so a rename cannot leave a chip silently dead; the
      // three literals are the guard labels written straight into AuditEntry
      // by the MCP egress, KG-write, and permission-denied paths.
      final emittedGuardNames = <String>[
        for (final guard in <Guard>[
          CommandGuard(),
          FileGuard(),
          NetworkGuard(),
          ContentGuard(scan: ContentScan(classifier: FakeContentClassifier())),
          TaskToolFilterGuard(),
          ToolPolicyGuard(cascade: ToolPolicyCascade()),
          EgressGuard(allowlist: const {}),
        ])
          guard.name,
        'EgressGuard',
        'KgWriteGuard',
      ...
      final chipFilters = RegExp(r'guard=([a-z-]+)').allMatches(populated).map((m) => m.group(1)!).toSet();
      expect(chipFilters, isNotEmpty);
      expect(chipFilters, isNot(contains('input')));
      for (final filter in chipFilters) {
        expect(
          emittedGuardNames.any((name) => name.toLowerCase().contains(filter)),
          isTrue,
          reason: 'chip "$filter" matches no emitted guard name',
        );
      }
