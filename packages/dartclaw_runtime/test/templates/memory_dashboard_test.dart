import 'package:dartclaw_runtime/src/templates/memory_dashboard.dart';
...
void main() {
  ...
  group('memory page structure', () {
    ...
    test('the pruner action is a real destructive control', () {
      final html = render();
      expect(html, contains('hx-post="/memory/prune"'));
      expect(html, contains('hx-confirm="Prune memory now? Archived entries remain available in the memory archive."'));
      expect(html, contains('hx-target="#memory-content" hx-select="#memory-inner" hx-swap="innerHTML"'));
      expect(html, contains('hx-disabled-elt="this"'));
      expect(html, isNot(contains('data-action="click->dc-memory#confirmPrune"')));
    });
  ...
  group('operator-visible memory lifecycle', () {
    ...
    // The Errors card must read the corpus projection, like Learnings: the
    // workspace file count is legacy evidence that lags the canonical corpus.
    test('the Errors card shows the corpus-derived count against its cap', () {
      final status = sampleStatus(errorsCount: 99)
        ..['collection'] = {'state': 'available', 'revision': 42, 'errorEntryCount': 4, 'learningEntryCount': 2};
      ...
      expect(html, contains('<span class="metric-number">4</span>'));
      expect(html, isNot(contains('<span class="metric-number">99</span>')));
      expect(html, contains('/ 50'));
    });

    test('the Errors card renders unknown when the collection is unavailable', () {
      final status = sampleStatus(errorsCount: 3)..['collection'] = {'state': 'unknown'};
      ...
      expect(html, isNot(contains('<span class="metric-number">3</span>')));
      expect(html, isNot(contains('<span class="metric-number">0</span>')));
      expect(html, contains('>unknown</span>'));
    });

    test('renders independent healthy collection and degraded index states', () {
      final status = sampleStatus()
        ..addAll({
          'collection': {
            'state': 'available',
    ...
    // S06: the retired lifecycle must leave no card an operator could still act on.
    test('the dashboard renders no curation card', () {
      final html = memoryDashboardTemplate(
        status: sampleStatus(),
        sidebarData: emptySidebarData(),
        navItems: emptyNavItems,
      ...
      expect(html, isNot(contains('Semantic curation')));
      expect(html, isNot(contains('dc-memory#curateMemory')));
      expect(html, isNot(contains('Curate now')));
      expect(html, contains('Canonical collection'));
    });

    test('only an in-flight index state uses the running badge', () {
      String render(String index) => memoryDashboardTemplate(
        status: sampleStatus()
          ..addAll({
            'index': {'state': index},
          }),
      ...
      expect(RegExp('status-badge-running').allMatches(render('rebuilding')), hasLength(2));
      for (final settled in ['healthy', 'degraded', 'unknown']) {
        expect(RegExp('status-badge-running').allMatches(render(settled)), isEmpty, reason: settled);
      }
    });
