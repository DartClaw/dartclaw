...
import 'package:dartclaw_runtime/src/templates/chat.dart' show richInputHtmlFromMetadataMap;
import 'package:dartclaw_runtime/src/templates/components.dart';
import 'package:dartclaw_runtime/src/templates/error_page.dart';
import 'package:dartclaw_runtime/src/templates/layout.dart';
import 'package:dartclaw_runtime/src/templates/loader.dart' as server;
import 'package:dartclaw_runtime/src/templates/login.dart';
import 'package:dartclaw_runtime/src/version.dart';
...
Map<String, dynamic> _settingsContext(Map<String, dynamic> overrides) => {
  ...
  'sidebar': '',
  'topbar': '',
  // The strip and the editable cards are projections of the section map; the
  // page template only places what Dart shaped.
  'tabs': [
    {'label': 'Scheduling', 'href': '#scheduling', 'controlId': 'tab-scheduling', 'panelIds': 'panel-scheduling'},
  ],
  'formPanels': [
    {
      'elementId': 'panel-scheduling',
      'tab': 'scheduling',
      'tabControlId': 'tab-scheduling',
      'title': 'Scheduling',
      'formHtml': '<form class="settings-form" data-section="scheduling"></form>',
      'linkHref': '/scheduling',
...
void main() {
  ...
  group('sidebar.html', () {
    ...
    test('renders empty, provider, navigation, and action states', () async {
      ...
      _expectAll(providers, [
        ...
        // The archive list ships collapsed; the disclosure control reveals it.
        'class="sidebar-archive-list" hidden',
      ]);
  ...
  group('status pages', () {
    test('health dashboard renders metrics and escapes version', () async {
      ...
      _expectAll(html, ['Healthy', '3d 14h 22m', '0.3.0', 'idle', '12', '2.4 MB']);
  ...
  group('settings.html', () {
    test('renders cards and channel configure links', () async {
      ...
      // The editable card is the shared fragment, placed verbatim under a
      // header the section map named.
      expect(html, contains('<form class="settings-form" data-section="scheduling"></form>'));
      expect(html, contains('id="panel-scheduling"'));
      expect(html, contains('aria-controls="panel-scheduling"'));
      expect(html, contains('View Schedule'));
      _expectNone(html, [
        '<style',
        'summary-stat',
        'summary-value',
  ...
  group('chat.html', () {
    ...
    test('chat area and send response render HTMX/SSE wiring', () async {
      ...
      _expectAll(area, [
        ...
        'hx-swap="beforeend"',
        'name="attachments"',
        'data-dc-chat-target="contextTray"',
        'data-dc-chat-target="attachmentsInput"',
  ...
  group('shell scaffolding contracts', () {
    setUp(() async => server.initTemplates(await resolveTemplatesDir()));
