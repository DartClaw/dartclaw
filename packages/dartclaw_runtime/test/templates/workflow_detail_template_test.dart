...
import 'package:dartclaw_runtime/src/templates/workflow_detail.dart';
...
void main() {
  ...
  group('workflowDetailPageTemplate', () {
    ...
    test('renders exactly the persisted step cards without a null fragment prototype', () {
      final html = workflowDetailPageTemplate(
        sidebarData: emptySidebar,
        navItems: const [],
        run: makeRun(),
        steps: makeSteps(count: 3),
        contextEntries: const [],
        loopInfo: const [],
      );
      final count = RegExp(r'class="pipeline-step ').allMatches(html).length;
      expect(count, 3);
      expect(html, isNot(contains('workflow-step-null')));
      expect(html, isNot(contains('/steps/null')));
    ...
    test('full and live progress fragments share one presentation for every run shape', () {
      final cases = [
        (status: WorkflowRunStatus.running, steps: <Map<String, dynamic>>[]),
        (status: WorkflowRunStatus.running, steps: makeSteps(count: 2, completedCount: 1)),
        (status: WorkflowRunStatus.failed, steps: makeSteps(count: 2, completedCount: 1)),
        (status: WorkflowRunStatus.completed, steps: makeSteps(count: 2, completedCount: 2)),
      ];

      for (final testCase in cases) {
        final run = makeRun(status: testCase.status.name);
        final full = workflowDetailPageTemplate(
          sidebarData: emptySidebar,
          navItems: const [],
          run: run,
        ...
        final presentation = workflowProgressPresentation(
    ...
    test('progress bar counts skipped steps as progressed', () {
      final steps = [
        ...
        {'index': 2, 'id': 'step-2', 'name': 'Step 3', 'status': 'skipped', 'parallel': false, 'taskId': null},
        {'index': 3, 'id': 'step-3', 'name': 'Step 4', 'status': 'pending', 'parallel': false, 'taskId': null},
      ];
    ...
    test('loop step shows iteration badge when loopInfo matches', () {
      final steps = [
        {'index': 0, 'id': 'review', 'name': 'Review', 'status': 'running', 'parallel': false, 'taskId': null},
      ];
      final loopInfo = [
        {
          'loopId': 'review-loop',
    ...
    test('pipeline class and non-colour cue match step status', () {
      final html = workflowDetailPageTemplate(
        ...
        run: makeRun(),
        steps: [
          {'index': 0, 'id': 'step-0', 'name': 'Done', 'status': 'completed', 'parallel': false, 'taskId': 'task-0'},
          {
            'index': 1,
            'id': 'step-1',
            'name': 'Interrupted',
    ...
    test('lazy step details expose terminal error and retry states', () {
      final html = workflowDetailPageTemplate(
        ...
        run: makeRun(),
        steps: [
          {'index': 0, 'id': 'step-0', 'name': 'Research', 'status': 'running', 'parallel': false, 'taskId': 'task-0'},
        ],
        contextEntries: const [],
        loopInfo: const [],
      );
...
String _progressFragment(String html) {
  final start = html.indexOf('<div id="workflow-progress"');
  expect(start, greaterThanOrEqualTo(0));
  final tags = RegExp(r'<div\b|</div>').allMatches(html, start);
  var depth = 0;
  for (final tag in tags) {
    if (tag.group(0) == '<div') {
      ...
      depth--;
      if (depth == 0) return html.substring(start, tag.end);
    }
  }
  throw StateError('Unclosed workflow progress fragment');
}
