...
void main() {
  ...
  test('ACP session title metadata does not rename the main workspace', () async {
    ...
    unawaited(() async {
      await worker.turnInvoked;
      worker.completeSuccess(
        const TurnResult(stopReason: 'end_turn', inputTokens: 1, outputTokens: 1, sessionTitle: 'Plan cleanup'),
      );
    }());
  ...
  test('ACP stop_reason cancelled resolves to TurnStatus.cancelled', () async {
    ...
    unawaited(() async {
      await worker.turnInvoked;
      worker.completeSuccess(const TurnResult(stopReason: 'cancelled', inputTokens: 1));
    }());
