...
TurnResult _turnResult({int inputTokens = 0, int outputTokens = 0}) =>
    TurnResult(inputTokens: inputTokens, outputTokens: outputTokens);
...
void main() {
  ...
  test('a turn reporting no usage leaves the last context reading standing', () async {
    final worker = _CompactionCapabilityHarness(supportsPreCompactHook: false);
    addTearDown(worker.dispose);
    final contextMonitor = ContextMonitor(reserveTokens: 20000);
    final runner = _buildRunner(
    ...
    unawaited(() async {
      await worker.turnInvoked;
      worker.emit(SystemInitEvent(contextWindow: 200000));
      worker.completeSuccess(_turnResult(inputTokens: 100000, outputTokens: 1));
    }());
    final measured = await runner.startTurn(session.id, [
      {'role': 'user', 'content': 'measured'},
    ]);
    await runner.waitForOutcome(session.id, measured);
    expect(contextMonitor.lastContextTokens, 100000);

    unawaited(() async {
      await worker.turnInvoked;
      worker.completeSuccess(_turnResult());
    }());
    final unmeasured = await runner.startTurn(session.id, [
      {'role': 'user', 'content': 'unmeasured'},
    ]);
    await runner.waitForOutcome(session.id, unmeasured);

    // Codex normalizes a fully-cached turn to zero fresh input tokens and reports
    // none at all on its error arm; zeroing the monitor on either would claim 0%
    // context use and suppress the flush the real reading would have triggered.
    expect(
      contextMonitor.lastContextTokens,
      100000,
      reason: 'a turn without a usage report must not overwrite the last measured context size',
    );
  });
