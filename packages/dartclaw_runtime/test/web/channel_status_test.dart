...
void main() {
  ...
  group('ChannelStatus.presentation', () {
    ...
    test('the presentation getter has no wildcard or default arm', () async {
      final source = File(await resolveServerPackagePath('lib', 'src', 'web', 'channel_status.dart'))
          .readAsStringSync();
  ...
  group('emitted class names exist in the canonical stylesheet', () {
    late String css;

    setUpAll(() async {
      css = File(await resolveDesignSystemCss('components.css')).readAsStringSync();
    });
  ...
  group('channel detail consumes the record', () {
    ...
    test('channel_detail.dart makes no status decision of its own', () async {
      final source = File(await resolveServerPackagePath('lib', 'src', 'templates', 'channel_detail.dart'))
          .readAsStringSync();
