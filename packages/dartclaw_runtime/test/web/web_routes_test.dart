...
void main() {
  ...
  group('GET /knowledge/wiki/<source>', () {
    ...
    test('renders the document inside the app shell instead of raw text/plain', () async {
      File('${tempDir.path}/workspace/wiki/onboarding.md')
    ...
    test('escapes hostile markdown instead of emitting it as markup', () async {
      File('${tempDir.path}/workspace/wiki/evil.md')
        ..parent.createSync(recursive: true)
    ...
    test('rejects a valid prefix with a non-md locator', () async {
      File('${tempDir.path}/workspace/wiki/notes.txt')
        ..parent.createSync(recursive: true)
    ...
    test('rejects wiki symlinks that resolve outside the wiki root', () async {
      File('${tempDir.path}/secret.md').writeAsStringSync('outside wiki');
      Link('${tempDir.path}/workspace/wiki/linked.md')
        ..parent.createSync(recursive: true)
    ...
    test('rejects a FileSystemException on read', () async {
      final locked = File('${tempDir.path}/workspace/wiki/locked.md')
        ..parent.createSync(recursive: true)
  ...
  group('GET /health-dashboard/audit', () {
    ...
    Handler auditHandler({int entryCount = 30}) {
      ...
      return webRoutes(sessions, messages, kvService: kvService, dataDir: dataDir.path, pageRegistry: registry).call;
    }
  ...
  group('login routes', () {
    ...
    test('POST /login refuses an oversized body before validating the token', () async {
      final handler = webRoutes(
        sessions,
        messages,
        tokenService: TokenService(token: 'a' * 64),
  ...
  // -------------------------------------------------------------------------
  group('page-declared routes', () {
    Handler handlerFor(DashboardPage page) {
      final registry = PageRegistry()..register(page);
      return webRoutes(sessions, messages, kvService: kvService, pageRegistry: registry).call;
    }

    test('a declared POST sub-route is served through the page handler with its path parameter', () async {
      final page = _DeclaringStubPage();
      final res = await handlerFor(page)(Request('POST', Uri.parse('http://localhost/stub/abc-1/act')));

      expect(res.statusCode, equals(200));
      expect(await res.readAsString(), equals('acted on abc-1'));
      expect(page.seenMethods, equals(['POST']));
    });

    test('the page GET keeps working alongside its declared routes', () async {
      final res = await handlerFor(_DeclaringStubPage())(Request('GET', Uri.parse('http://localhost/stub')));

      expect(res.statusCode, equals(200));
      expect(await res.readAsString(), equals('page'));
    });

    test('a throw inside a declared route answers exactly as a throwing page GET does', () async {
      final handler = handlerFor(_ThrowingStubPage());

      final fromGet = await handler(Request('GET', Uri.parse('http://localhost/stub')));
      final fromDeclared = await handler(Request('POST', Uri.parse('http://localhost/stub/abc-1/act')));

      final getBody = await fromGet.readAsString();
      expect(fromDeclared.statusCode, equals(fromGet.statusCode));
      expect(fromDeclared.headers['content-type'], equals(fromGet.headers['content-type']));
      expect(await fromDeclared.readAsString(), equals(getBody));
      expect(getBody, contains('Failed to load Stub: Exception: boom'));
    });
  });
}

class _DeclaringStubPage extends DashboardPage {
  final seenMethods = <String>[];

  @override
  String get route => '/stub';
  ...
  @override
  List<PageRouteDeclaration> get declaredRoutes => const [(method: 'POST', path: '/stub/<id>/act')];

  @override
  Future<Response> handler(Request request, PageContext context) async {
    final id = request.params['id'];
    if (id == null) return Response.ok('page');
    seenMethods.add(request.method);
    return Response.ok('acted on $id');
  }
}

class _ThrowingStubPage extends DashboardPage {
  @override
  String get route => '/stub';
  ...
  @override
  Future<Response> handler(Request request, PageContext context) async => throw Exception('boom');
}
