...
void main() {
  Future<void> verifyFailedRecoveryRelease({required bool terminationConfirmed}) async {
    ...
    // ADR-058: a teardown publishes one terminal event, and which one names whether the slot was withheld.
    final terminalKind = terminationConfirmed ? ExecutionEventKind.disposed : ExecutionEventKind.quarantined;
    final terminal = coordinator.events.firstWhere(
      (event) => event.kind == terminalKind && identical(event.runner, failedRunner),
    );
