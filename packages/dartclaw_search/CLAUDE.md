...
- The native provider imports `NativeLlamaBackend` from exactly pinned `llamadart 0.8.22` to forward its initialization
  timeout. This approved import exception must be requalified on dependency upgrades and removed when the public API
  exposes the worker deadline. The outer provider timeout remains the whole-initialization bound.
