...
applying fixed weighted reciprocal-rank fusion, then returns the bounded ranking after authenticating its current source
content. `VectorSynchronizer` reuses exact content/model matches and publishes only vectors that still match the current
source.
