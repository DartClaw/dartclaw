...
## Architecture
- **Guard chain** — `Guard` (interface), `GuardChain` (sequential evaluator: first-block-wins, 5s `.timeout()`, fail-closed default), `GuardContext` (canonical tool name + args + raw provider name, active session, and delegated-agent identity), `GuardVerdict` (sealed `Pass` / `Warn` / `Block`), `GuardVerdictCallback` (the seam upstream uses to translate verdicts to events).
