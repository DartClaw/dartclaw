...
class KnowledgeInboxService {
  ...
  Future<KnowledgeInboxRunReport> runOnce({bool requireStable = true, String jobId = 'knowledge-inbox'}) async {
    ...
    for (final file in entries) {
      ...
      // Only the extraction is retried. It is the nondeterministic step and it
      // writes nothing, so repeating it is free; every step after it is durable,
      // and replaying those is what duplicated memory findings, KG facts, and
      // supplement sections.
      String? text;
      ...
      while (attempt <= retryAttempts) {
        ...
        try {
          ...
          extraction = await _runExtractionTurn(file, text, jobId: jobId);
          break;
        } catch (e) {
      ...
      try {
        outcome = await _processFile(file, text: text!, extraction: extraction, jobId: jobId);
      } catch (e) {
        quarantined.add(await _quarantine(file, name: name, attempts: attempt, error: e));
        continue;
      }
  ...
  /// Commits one accepted [extraction] and returns the contradictions, wiki
  /// collision, and coverage the run report surfaces for [file].
  ///
  ...
  Future<_FileOutcome> _processFile(
    ...
    final wikiSlug = WikiPageStore.pageSlug(extraction.wikiSlug ?? title);
    ...
    return (
      ...
      collision: write.outcome == WikiPageOutcome.created
          ? null
          : KnowledgeInboxWikiMerge(file: name, slug: wikiSlug, supplements: write.supplements, outcome: write.outcome),
      coverage: KnowledgeInboxCoverage(
        file: name,
        sourceBytes: utf8.encode(text).length,
        synthesizedBytes: utf8.encode(wikiBody).length,
        declaredDrops: extraction.droppedTopics,
      ),
    );
  }
...
- dropped_topics MUST name every source topic you left out and stay empty when you left out nothing.
...
class KnowledgeExtraction {
  ...
  /// Source topics the extraction turn reports as deliberately left out.
  final List<String> droppedTopics;
  ...
  factory fromAssistantText(String text) {
    ...
    return KnowledgeExtraction(
      ...
      // operator's channel and SSE stream, so it is bounded here rather than at
      // each surface that renders it.
      droppedTopics: [
        for (final topic in _stringList(payload['dropped_topics']).take(20))
          if (normalizeWhitespace(topic) case final clean when clean.isNotEmpty)
...
class KnowledgeInboxRunReport {
  ...
  /// One entry per processed file, whether or not it declared a gap.
  ...
  /// Files whose extraction declared it left source material out.
  List<KnowledgeInboxCoverage> get declaredGaps =>
      coverage.where((item) => item.declaredDrops.isNotEmpty).toList(growable: false);
  ...
  String get summary {
    ...
    final details = <String>[
      ...
      if (declaredGaps.isNotEmpty)
        'declared drops: ${declaredGaps.map((item) => "${item.file}: ${item.declaredDrops.join(", ")}").join("; ")}',
    ...
    final counts =
        ...
        'wiki-merges=${wikiMerges.length} declared-gaps=${declaredGaps.length}';
...
/// A wiki slug collision surfaced during ingestion.
///
/// [outcome] is [WikiPageOutcome.supplemented] when the stored page kept its
/// content and gained the new synthesis as a supplement section, so the page now
/// mixes sources and wants operator consolidation, or
/// [WikiPageOutcome.refreshed] when the page already carried the synthesis and
/// nothing was written – which the operator has to be able to tell apart from
/// no collision at all.
class KnowledgeInboxWikiMerge {
  final String file;
  final String slug;

  /// Supplement sections the page carries after the write.
  final int supplements;
  final WikiPageOutcome outcome;

  const new({required this.file, required this.slug, required this.supplements, required this.outcome});

  String get detail =>
      outcome == WikiPageOutcome.supplemented ? '(supplement $supplements)' : '(refresh, no new content)';
}

/// How much of one source survived into the wiki page the run stored.
///
/// [declaredDrops] is the extraction turn's own account of what it left out. The
/// turn runs read-only in a single pass and cannot re-read the source to diff
/// its output, so an empty list is the absence of a declaration, not evidence
/// that nothing was lost. [sourceBytes] against [synthesizedBytes] is the
/// objective half: the model cannot talk around it. Both are UTF-8 byte counts
/// of the source and of the wiki body alone, so the ratio is the same comparison
/// an operator makes by hand between `processed/` and `wiki/`. A ratio above
/// 100% is normal for a short source that synthesizes into a longer page.
class KnowledgeInboxCoverage {
  final String file;
  final int sourceBytes;
  final int synthesizedBytes;
  final List<String> declaredDrops;

  const new({
    required this.file,
    required this.sourceBytes,
    required this.synthesizedBytes,
    this.declaredDrops = const [],
  });
