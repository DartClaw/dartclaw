...
class MaintenanceAction {
  ...
  /// 'stale', 'count_cap', 'cron_retention', 'disk_budget', or 'artifact_retention'.
  final String reason;
...
class MaintenanceReport {
  ...
  final int artifactDiskReclaimedBytes;
  ...
  const MaintenanceReport({
    ...
    this.artifactsDeleted = 0,
    this.artifactDiskReclaimedBytes = 0,
...
class SessionMaintenanceService {
  ...
  Future<MaintenanceReport> run({MaintenanceMode? modeOverride}) async {
    ...
    final artifactResult = await _cleanTaskArtifacts(isEnforce);
    artifactsDeleted += artifactResult.deletedArtifacts;
    artifactDiskReclaimed += artifactResult.reclaimedBytes;
    ...
    return MaintenanceReport(
      ...
      artifactDiskReclaimedBytes: artifactDiskReclaimed,
  ...
  Future<_ArtifactResult> _cleanTaskArtifacts(bool isEnforce) async {
    final tasks = taskService;
    if (artifactRetentionDays <= 0 || tasks == null) return _ArtifactResult.empty();
    if (dataDir == null || dataDir!.isEmpty) {
      return const _ArtifactResult(warnings: ['Artifact retention skipped: dataDir is not configured']);
    }

    final cutoff = DateTime.now().subtract(Duration(days: artifactRetentionDays));
    ...
    for (final task in allTasks) {
      ...
      final artifactsDir = Directory(p.join(dataDir!, 'tasks', task.id, 'artifacts'));
      final artifactBytes = _calculateDiskUsage(artifactsDir.path);

      try {
        final artifacts = await tasks.listArtifacts(task.id);
        if (artifacts.isEmpty && !artifactsDir.existsSync()) continue;

        if (isEnforce) {
          var deletedForTask = 0;
          for (final artifact in artifacts) {
            await tasks.deleteArtifact(artifact.id);
            deletedForTask++;
          }
          ...
          deletedArtifacts += deletedForTask;
          reclaimedBytes += artifactBytes;
        } else {
          reclaimedBytes += artifactBytes;
        }

        actions.add(
          MaintenanceAction(sessionId: task.id, actionType: 'delete', reason: 'artifact_retention', applied: isEnforce),
        );
      } catch (e) {
        warnings.add('Failed to clean artifacts for task ${task.id}: $e');
        actions.add(
          MaintenanceAction(sessionId: task.id, actionType: 'delete', reason: 'artifact_retention', applied: false),
        );
      }
    }
