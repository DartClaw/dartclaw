...
void main() {
  ...
  group('appendError', () {
    ...
    test('continuation-encodes multiline fields without forging entries', () async {
      await service.appendError(
        errorType: 'TURN_FAILURE\n## [forged type] forged',
        sessionId: 'sess-1\n## [forged session] forged',
        context: 'first line\n## [forged context] forged\n- [forged learning] forged',
        resolution: 'retry\n## [forged resolution] forged',
      );
  ...
  group('cap enforcement', () {
    ...
    test('retains 50 genuine multiline errors despite forged boundary text', () async {
      for (var i = 0; i <= 50; i++) {
        await service.appendError(
          errorType: 'ERR_$i',
          sessionId: 's$i',
