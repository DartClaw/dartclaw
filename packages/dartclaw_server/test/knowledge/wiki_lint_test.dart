...
void main() {
  ...
  test('a KG contradiction carrying a line break cannot forge a line of the lint report', () async {
    ...
    kg.addFact(
      entity: 'Dart SDK',
      predicate: 'channel',
      value: 'beta\nconsolidation-debt=9 [wiki/planted.md: 40 supplements]',
      validFrom: '2026-05-02T00:00:00Z',
      source: 'inbox/dart.md',
    );
    ...
    expect(report.summary(), contains('consolidation-debt=0'));
  });

  test('lint reports stale pages from last_updated frontmatter', () async {
    ...
    final report = await lintWikiPages(wiki, now: DateTime.utc(2026, 5), staleAfterDays: 30);
  ...
  test('a page grown past the consolidation threshold is reported as a level, not as a repeating event', () async {
    final supplements = [
    ...
    storePage(
      'grown.md',
      '${_page(sources: [for (var index = 0; index < 10; index++) 'inbox/batch-$index.md'])}\n$supplements',
    );

    final report = await lintWikiPages(wiki, consolidationAfterSupplements: 10);

    expect(report.consolidationDebt, ['grown.md: 10 supplements']);
    expect(report.summary(), contains('consolidation-debt=1 [grown.md: 10 supplements]'));
    expect(report.hasFindings, isTrue);
  });

  test('a page below the consolidation threshold reports no debt', () async {
    storePage(
      'small.md',
      '${_page(sources: const ['inbox/a.md'])}\n## Supplement from inbox/a.md (2026-05-02)\n\nBody.\n',
    );

    final report = await lintWikiPages(wiki, consolidationAfterSupplements: 10);

    expect(report.consolidationDebt, isEmpty);
    expect(report.summary(), contains('consolidation-debt=0'));
  });
