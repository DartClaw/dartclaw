...
aliases: ["deep-work"]''',
    ...
    test('each supplement raises the count the run report carries', () async {
      storePage('growing', _page(sources: const ['inbox/first.md'], body: 'Prior synthesis body.'));

      final second = await write('growing', body: 'Second body.', sources: const ['inbox/second.md']);
      final third = await write('growing', body: 'Third body.', sources: const ['inbox/third.md']);

      expect(second.supplements, 1);
      expect(third.supplements, 2);
    });
  });
  ...
  group('a write that contributes nothing changes nothing', () {
    // The page already carries every character of the incoming body, so there is
    // no contribution to record. Rewriting the authorship fields for it is the
    // over-claim; refusing to write at all is also what makes a retried
    // ingestion a no-op.
    test('a repeat of the same source leaves the stored page byte-identical', () async {
      ...
      final result = await write('stable', body: 'Shared body text.');

      expect(result.outcome, WikiPageOutcome.refreshed);
      expect(result.supplements, 0);
    ...
    test('a new source whose body the page already carries is recorded without moving any authorship field', () async {
      final file = storePage('accounted', _page(provenance: 'human-authored', body: 'Shared body text.'));

      final result = await write('accounted', body: 'Shared body text.', lastUpdatedBy: 'cron:knowledge-inbox');
    ...
    test('a body the page carries above a later supplement still counts as carried', () async {
      final file = storePage(
        'buried',
        '${_page(sources: const ['inbox/first.md'], body: 'Prior synthesis body.')}\n'
            '## Supplement from inbox/second.md (2026-05-02)\n\nLater body.\n',
      );
      final before = file.readAsStringSync();

      final result = await write('buried', body: 'Prior synthesis body.', sources: const ['inbox/first.md']);

      expect(result.outcome, WikiPageOutcome.refreshed);
      expect(result.supplements, 1, reason: 'a refresh still reports the level the page is at');
      expect(file.readAsStringSync(), before);
    });
  });
