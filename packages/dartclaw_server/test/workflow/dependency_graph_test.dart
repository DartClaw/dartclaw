import 'package:dartclaw_server/src/workflow/dependency_graph.dart';
...
void main() {
  group('DependencyGraph', () {
    ...
    test('linear chain: s01→s02→s03 — only s01 ready initially', () {
      final items = [
        {'id': 's01', 'name': 'Story 1'},
        {'id': 's02', 'name': 'Story 2', 'dependencies': ['s01']},
        {'id': 's03', 'name': 'Story 3', 'dependencies': ['s02']},
      ];
      final graph = DependencyGraph(items);
      graph.validate();
    ...
    test('diamond dependency: s01→s02, s01→s03, s02&s03→s04', () {
      final items = [
        {'id': 's01', 'name': 'Story 1'},
        {'id': 's02', 'name': 'Story 2', 'dependencies': ['s01']},
        {'id': 's03', 'name': 'Story 3', 'dependencies': ['s01']},
        {'id': 's04', 'name': 'Story 4', 'dependencies': ['s02', 's03']},
      ];
      final graph = DependencyGraph(items);
    ...
    test('circular dependency (s01→s02→s01) throws ArgumentError', () {
      final items = [
        {'id': 's01', 'name': 'Story 1', 'dependencies': ['s02']},
        {'id': 's02', 'name': 'Story 2', 'dependencies': ['s01']},
      ];
      final graph = DependencyGraph(items);
    ...
    test('self-referencing dependency (s01→s01) throws ArgumentError', () {
      final items = [
        {'id': 's01', 'name': 'Story 1', 'dependencies': ['s01']},
      ];
      final graph = DependencyGraph(items);
      expect(
        () => graph.validate(),
    ...
    test('circular dependency error message contains cycle path', () {
      final items = [
        {'id': 's01', 'name': 'Story 1', 'dependencies': ['s03']},
        {'id': 's02', 'name': 'Story 2', 'dependencies': ['s01']},
        {'id': 's03', 'name': 'Story 3', 'dependencies': ['s02']},
      ];
      final graph = DependencyGraph(items);
    ...
    test('dependency referencing non-existent item ID — item treated as independent', () {
      final items = [
        {'id': 's01', 'name': 'Story 1', 'dependencies': ['s99']}, // s99 doesn't exist
        {'id': 's02', 'name': 'Story 2'},
      ];
      final graph = DependencyGraph(items);
    ...
    test('getReady grows as completed set grows', () {
      final items = [
        {'id': 's01'},
        {'id': 's02', 'dependencies': ['s01']},
        {'id': 's03', 'dependencies': ['s02']},
      ];
      final graph = DependencyGraph(items);
