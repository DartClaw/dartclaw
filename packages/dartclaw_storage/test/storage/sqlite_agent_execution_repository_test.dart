  ...
  group('SqliteTaskRepository migration', () {
    test('migrates an S31-style tasks database by moving runtime columns onto AgentExecution', () async {
      ...
      try {
        ...
        final columns = migratedDb.select('PRAGMA table_info(tasks)').map((row) => row['name'] as String).toSet();
        ...
        final aeRow = migratedDb.select('SELECT * FROM agent_executions WHERE id = ?', [
        ...
        SqliteTaskRepository(migratedDb);
        final secondPass = migratedDb.select('SELECT agent_execution_id FROM tasks WHERE id = ?', ['task-1']).single;
