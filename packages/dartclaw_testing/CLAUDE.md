...
- `FakeAgentHarness` records the authoritative prompt/model/effort turn inputs. Its `supportsNoWorkTools` and `supportsOutputSchemaConstraint` options default to `false`; tests opt in to the first only when their controlled protocol represents complete interception, and to the second to build a Codex-shaped worker whose provider constrains the reply without typed readback. `FakeTurnManager` must mirror prompt scope and `systemPromptOverride` exactly so cross-package routing tests can assert the contract.
