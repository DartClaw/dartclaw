...
Map<String, dynamic> _v8Envelope({String event = 'message', String? deviceId, required Map<String, dynamic> payload}) =>
    {'event': event, 'device_id': ?deviceId, 'payload': payload};
...
void main() {
  ...
  group('WhatsAppChannel', () {
    ...
    test('handleWebhook parses group message (v8 envelope)', () {
      channel.handleWebhook(
        _v8Envelope(
          payload: {'from': '123@s.whatsapp.net', 'body': 'group msg', 'chat_id': 'grp@g.us', 'from_name': 'Bob'},
        ),
      );
      expect(channelManager.received, hasLength(1));
    ...
    test('handleWebhook ignores non-message events', () {
      channel.handleWebhook(
        _v8Envelope(
          event: 'reaction',
          payload: {'from': '123@s.whatsapp.net', 'body': 'some reaction', 'chat_id': '123@s.whatsapp.net'},
        ),
      );
    ...
    test('handleWebhook drops message with missing body', () {
      channel.handleWebhook(_v8Envelope(payload: {'from': '123@s.whatsapp.net', 'chat_id': '123@s.whatsapp.net'}));
      expect(channelManager.received, isEmpty);
    });

    test('handleWebhook drops message with missing from', () {
      channel.handleWebhook(_v8Envelope(payload: {'body': 'text', 'chat_id': '123@s.whatsapp.net'}));
      expect(channelManager.received, isEmpty);
    });
    ...
    test('handleWebhook respects group disabled policy', () {
      ...
      disabledChannel.handleWebhook(
        _v8Envelope(payload: {'from': '123@s.whatsapp.net', 'body': 'group msg', 'chat_id': 'grp@g.us'}),
      );
      expect(channelManager.received, isEmpty);
    });
    ...
    test('handleWebhook respects DM access control', () {
      ...
      restrictedChannel.handleWebhook(
        _v8Envelope(payload: {'from': '123@s.whatsapp.net', 'body': 'Hello', 'chat_id': '123@s.whatsapp.net'}),
      );
      expect(channelManager.received, isEmpty);
    });
    ...
    test('v8 mentionedJids are empty (removed in v8)', () {
      channel.handleWebhook(
        _v8Envelope(payload: {'from': '123@s.whatsapp.net', 'body': 'Hello @bot', 'chat_id': 'grp@g.us'}),
      );
    ...
    test('pairing mode: known sender (in allowlist) message forwarded normally', () {
      final dmAccess = DmAccessController(mode: DmAccessMode.pairing, allowlist: {'known@s.whatsapp.net'});
      final pairingChannel = WhatsAppChannel(
        gowa: gowa,
        config: WhatsAppConfig(enabled: true),
        dmAccess: dmAccess,
      ...
      pairingChannel.handleWebhook(
        _v8Envelope(payload: {'from': 'known@s.whatsapp.net', 'body': 'Hello', 'chat_id': 'known@s.whatsapp.net'}),
      );
    ...
    test('allowlist mode: unknown sender drops without createPairing', () {
      ...
      allowlistChannel.handleWebhook(
        _v8Envelope(payload: {'from': 'unknown@s.whatsapp.net', 'body': 'Hello', 'chat_id': 'unknown@s.whatsapp.net'}),
      );
