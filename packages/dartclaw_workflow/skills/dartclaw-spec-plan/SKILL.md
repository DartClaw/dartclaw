...
1. Resolve `PLAN_SOURCE` per the **Resolve Plan-Bundle Input** procedure in `../references/github-artifact-roundtrip.md`. Incompatible typed artifacts → **STOP** and exit with the correct downstream skill.
2. Read `PLAN_DIR/plan.md`. If missing, **STOP** and report that a valid plan artifact is required upstream (typically produced by `dartclaw-plan`).
...
**Sub-agent 4: External Research** _(only if stories reference external APIs/libraries needing documentation lookup)_ — For each external resource: look up current docs (use a documentation-lookup or research-oriented sub-agent), identify relevant patterns and known gotchas. Output: consolidated reference with one section per resource.
...
- References: FIS template (`../dartclaw-spec/templates/fis-template.md`), authoring guidelines (`../references/fis-authoring-guidelines.md`), technical research (`{PLAN_DIR}/technical-research.md`)
...
If `PLAN_SOURCE_MODE = github-artifact`, apply the **Plan-Bundle Continuation Sync** from `../references/github-artifact-roundtrip.md` now.
