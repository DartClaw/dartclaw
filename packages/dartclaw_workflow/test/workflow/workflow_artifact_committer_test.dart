...
void main() {
  group('workflow_artifact_committer', () {
      ...
      expect(
        workflowHasArtifactProducer(
          const WorkflowDefinition(
            name: 'wf',
            description: 'test',
            steps: [WorkflowStep(id: 'plan', name: 'Plan', outputs: {'plan': OutputConfig(format: OutputFormat.path)})],
          ),
        ),
        isTrue,
      );
    ...
    test('commits path output when artifact commit is enabled', () async {
      ...
      await maybeCommitStepArtifacts(
      ...
      final log = await _git(repoDir.path, ['log', '--oneline']);
      expect(log.stdout, contains('chore(workflow): artifacts for run run-1'));
    });
  });
}
...
Future<ProcessResult> _git(String workingDirectory, List<String> args) async {
