...
void main() {
  group('WorkflowDefinitionResolver', () {
    ...
    test('applies stepDefaults turn timeout to resolved agent steps', () {
      const def = WorkflowDefinition(
        name: 'timeout-default-demo',
        description: 'test',
        stepDefaults: [StepConfigDefault(match: 'review*', turnTimeoutSeconds: 900)],
        steps: [
          WorkflowStep(id: 'review-code', name: 'Review Code', prompts: ['Review']),
          WorkflowStep(id: 'review-fast', name: 'Review Fast', prompts: ['Review'], turnTimeoutSeconds: 120),
        ],
      );

      final authoredYaml = resolver.emitYaml(def);
      expect(authoredYaml, contains('turn_timeout: 900'));
      final authored = parser.parse(authoredYaml, sourcePath: 'authored:timeout-default-demo');
      expect(authored.stepDefaults!.single.turnTimeoutSeconds, 900);

      final resolved = resolver.resolve(def);
      expect(resolved.stepDefaults, isNull);
      expect(resolved.steps.firstWhere((s) => s.id == 'review-code').turnTimeoutSeconds, 900);
      expect(resolved.steps.firstWhere((s) => s.id == 'review-fast').turnTimeoutSeconds, 120);

      final reparsed = parser.parse(resolver.emitYaml(resolved), sourcePath: 'resolved:timeout-default-demo');
      expect(reparsed.steps.firstWhere((s) => s.id == 'review-code').turnTimeoutSeconds, 900);
      expect(reparsed.steps.firstWhere((s) => s.id == 'review-fast').turnTimeoutSeconds, 120);
    });

    test('round-trips a duration-string stepDefaults turn timeout through unresolved YAML', () {
      final authored = parser.parse('''
name: timeout-default-demo
description: test
stepDefaults:
  - match: "review*"
...
''');
      ...
      final emitted = resolver.emitYaml(authored);
      final reparsed = parser.parse(emitted, sourcePath: 'reparsed:timeout-default-demo');

      expect(reparsed.stepDefaults!.single.turnTimeoutSeconds, 5400);
    });
    ...
    test('emitYaml preserves each path output\'s declared selector through reparse', () async {
      final def = await parser.parseFile(builtInWorkflowPath('plan-and-implement.yaml'));
      final resolved = resolver.resolve(def);
      final reparsed = parser.parse(resolver.emitYaml(resolved), sourcePath: 'resolved:plan-and-implement.yaml');

      // Three `format: path` outputs on one step whose only discriminator is the
      // declared selector: dropping it from the emit would collapse them onto the
      // same file while every structural round-trip assertion still passed.
      final emitted = reparsed.steps.firstWhere((s) => s.id == 'discover-plan-state').outputs!;
      final declared = resolved.steps.firstWhere((s) => s.id == 'discover-plan-state').outputs!;
      expect(declared.keys, containsAll(['prd', 'plan', 'technical_research']));
      for (final key in declared.keys) {
        final source = declared[key]!.resolverOverride;
        if (source is! FileSystemOutput) continue;
        final target = emitted[key]!.resolverOverride;
        expect(target, isA<FileSystemOutput>(), reason: 'output "$key" lost its filesystem resolver on round-trip');
        expect((target as FileSystemOutput).pathPattern, source.pathPattern);
        expect(target.preferPatterns, source.preferPatterns);
      }
    });
    ...
    test('top-level loop emits inline loop YAML that reparses cleanly', () {
      const def = WorkflowDefinition(
        ...
        loops: [
          WorkflowLoop(id: 'review-loop', steps: ['review'], exitGate: 'review.status == done', maxIterations: 3),
        ],
      );
      ...
      expect(reparsed.loops.single.exitGate, 'review.status == done');
      expect(validator.validate(reparsed).errors, isEmpty);
    });
...
''';
      ...
      expect(
        report.errors.any((e) => e.type == WorkflowValidationErrorType.invalidLoopPolicy && e.loopId == 'story-loop'),
        true,
      );
    });
  });
